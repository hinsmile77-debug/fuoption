...
def check_matrix_coverage(
    *,
    ...
    """
    buildable = buildable_structures(config)
    cells: list[CellCoverage] = []
    for (direction, iv_state), structures in matrix.matrix_cells().items():
        cells.append(
            CellCoverage(
                direction=direction,
                iv_state=iv_state,
    ...
    return MatrixCoverage(
        cells=tuple(cells),
        buildable=buildable,
        unbuildable=frozenset(matrix.ALL_STRUCTURES) - buildable,
        hits=hits,
    )
