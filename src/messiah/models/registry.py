...
#: **키는 있고, 그 값이 "그 시절엔 안 적었다"** (2026-08-24 F-18).
#:
#: `THRESHOLD_SOURCE_UNKNOWN`과 무엇이 다른가:
#:
#:   unknown            키가 **없다**. 왜 없는지는 이 값만으로 알 수 없다 —
#:                      옛 번들이라 없는 것일 수도, 새 번들인데 기입이 실패한 것일 수도 있다.
#:   unrecorded_pre_f6  키가 **있고**, 사람이 사후에 확인한 결과 *"학습 산출물이 남아
#:                      있지 않아 소급 기입이 불가능하다"* 는 사실이 적혀 있다.
#:
#: 둘을 안 가르면 「옛 번들이라 없다」와 「새 번들인데 기입이 실패했다」가 같은 문자열로
#: 접힌다(2026-08-21 G-11이 이름 붙인 「미측정의 표현형」 문제와 같은 형태).
#:
#: **상수를 늘리되 판정 분기는 늘리지 않는다.** 읽는 쪽에서는 둘 다 「최적화가 아니다」로
#: 똑같이 취급된다 — 이 값이 답하는 것은 판정이 아니라 **"왜 답이 없는지"** 다.
#: 이 값이 적힌 번들의 임계가 왜 그 값인지는 **재학습만이 답할 수 있고**, 그 사실 자체를
#: 기록으로 남기는 것이 이 상수의 목적이다(항목을 영원히 「확인 필요」로 두지 않는다).
THRESHOLD_SOURCE_UNRECORDED = "unrecorded_pre_f6"
