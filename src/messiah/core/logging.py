...
TAG_LEVELS: dict[str, int] = {
    ...
    # 완성봉 확정 → 피처 발행까지의 지연 분포, **시간대 축 포함** (2026-08-20 F-E).
    ...
    # **유예 여유가 음수인 세션** (2026-08-26 F-66 · 1-11).
    #
    # 위 두 태그는 2026-08-24 F-21 이후 **발행 예산**(봉 도착 → 발행, 대기 제외)을 잰다.
    # 유예(봉 확정 → 발행, 대기 포함)를 넘긴 회차는 그 축에 안 걸린다 — 2026-08-26에
    # 유예 초과 14건 · 최악 여유 −3,596ms 였는데 경고가 **0건**이었던 이유가 그것이다.
    # 세션 마감 절차에서 **하루 최대 1줄**. 게이트가 아니라 경보다(R18 대상 아님).
    "PublishGraceBreached": logging.WARNING,
    # 퇴화한 피처의 상수가 **야간 갭 때문인가** (2026-08-20 F-G).
    #
    # 창 60은 60분이 아니라 60봉이다. 10m에서 60봉 = 600분 ≈ 1.46 거래일이라 한 세션
    # (410분 ≈ 41봉)보다 길고, 그래서 argmax가 세션 중에 창을 벗어날 수 없다. 그 argmax가
    # 하필 전일 종가 → 당일 시가를 잇는 세션 경계 봉이고, 야간 갭은 장중 최대의 1.85배다.
    # 이 태그가 없던 동안 등록부는 같은 사실을 「수정이 듣지 않았다 · 3회 재발」이라고만
    # 말했다 — 원인이 익명이면 세 번을 세도 네 번째가 온다.
    "SessionBoundaryInflation": logging.WARNING,
    ...
    # 장중 청산 — 손절·시간배리어 (2026-09-22 F-119). **WARNING이다** — 사람이 안 시켰는데
    # 포지션이 나가는 경로는 KillSwitch·CB·EOD와 같은 무게로 보여야 한다. 이제 그 경로가
    # 넷이 됐고, 넷 다 같은 높이에 선다.
    "PositionExitLiquidating": logging.WARNING,
    # 이 포지션의 손절선과 시간배리어가 **얼마로 잡혔나** — 포지션 하나당 한 번.
    ...
    "PositionExitFailed": logging.ERROR,
    ...
    # 부수 태스크(화면 표시 전용 등)가 예외로 중단됐다 — `core/supervise.run_isolated()`가
    # 예외 전파만 막고 사실은 여기 남긴다. ERROR인 이유: 2026-09-03에 바로 이 자리에서
    # 막혔어야 할 예외가 gather를 타고 올라가 G2 세션 전체를 내렸고(판단 공백 하루), 그때
    # 프로세스는 **죽어서** 최소한 시끄럽기라도 했다. 이제 프로세스가 사니 그 시끄러움을
    # 이 한 줄이 대신 져야 한다 — 조용해지는 방향의 예외는 금지계명 12번이 막는 그것이다.
    "IsolatedTaskCrashed": logging.ERROR,
