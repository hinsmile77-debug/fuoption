...
마흐디(선행 옵션 프로젝트) mahdi/broker/rest_client.py에서 2026-07-21 이식
(KISSettings → KISCredentials로 설정 소스만 교체, 로직은 동일).
...
#
# 2026-07-20 재실측: 2건/초로도 부족함을 확인 — 콜→풋 순서로 호출하는데, 콜은 거의 항상 성공하고
# 풋만 계속 500이 되는 정확한 교대 패턴이 확인됨 — 매 쌍의 두 번째 호출(0.5초 뒤)만 계속 걸리는
# 이 패턴은 KIS 모의투자의 실제 한도가 2건/초가 아니라 1건/초에 더 가깝다는 강한 정황이다 —
# 1건/초(1.0초 간격)로 상향한다.
...
class _RateLimiter:
    ...
    고정 간격 대신 적응형이다 — KIS 모의투자의 실제 초당 호출 한도는 문서화돼 있지 않고
    실측값이 이미 한 번(2건/초 → 1건/초) 바뀐 적이 있다. 앞으로도 계좌/시간대별로 실제 한도가
    달라질 가능성을 고려해, 레이트리밋(500 + KIS 에러코드 EGW00201)이 감지되면 다음 호출부터
    간격을 즉시 넓히고(record_rate_limit_hit), 그 넓어진 간격에서 성공이 충분히 이어지면 서서히
    기준 간격(min_interval)까지만 되돌린다(record_success) — 기준 간격 밑으로는 절대 안 내려가고,
    ...
    def record_rate_limit_hit(self) -> None:
        ...
        with self._lock:
            ...
            self._current_interval = min(
                max(self._current_interval, self._min_interval) * self._BACKOFF_MULTIPLIER,
                max_interval,
            )
        ...
        with self._lock:
            ...
            if self._consecutive_successes >= self._RECOVERY_SUCCESS_THRESHOLD:
                ...
                self._current_interval = max(
                    self._current_interval * self._RECOVERY_FACTOR, self._min_interval
                )


def _is_kis_rate_limit_error(exc: httpx.HTTPStatusError) -> bool:
    """
    계산: KIS가 초당 거래건수 초과 시 돌려주는 특정 에러코드(EGW00201)인지 확인한다(마흐디
         2026-07-20 US10Y 조회 500 응답 바디에서 {"msg_cd":"EGW00201","msg1":"초당 거래건수를
         초과하였습니다"} 실측). 이 코드일 때만 백오프를 키운다 — 그 외 500(계좌 미승인, 존재하지
         않는 종목 등)은 페이싱과 무관한 원인이라 무분별하게 전체 호출을 느리게 만들면 안 된다.
...
class KISRestClient:
    ...
    def get_quote(
        self, symbol: str, market_div_code: str = tr_codes.FID_MRKT_DIV_INDEX_OPTION
    ) -> dict:
        """
        ...
        해석: 이 엔드포인트는 종목 1건 시세만 반환한다 — 모의투자에는 체인 전체를 한 번에
             반환하는 REST가 없으므로, 여러 행사가를 조회하려면 종목코드 마스터파일 기준으로
             이 호출을 반복해야 한다(아직 미구현 — KIS 종목코드 마스터파일 연동 필요).
    ...
    def get_investor_flow(self, market_code: str, sector_code: str) -> dict:
        ...
        계산: "모의 TR_ID/Domain: 모의투자 미지원"으로 문서화되어 있지만, 계좌 무관 공개
             ...
             성공한다 — 시세 WS와 같은 이유로 실전 도메인을 고정 사용한다.
    ...
    def get_overseas_future_price(self, srs_cd: str) -> dict:
        ...
        계산: PATH_OVERSEAS_FUTUREOPTION_PRICE GET 호출. 이 엔드포인트는 계좌 파라미터가 없어
             ...
             있어야 한다(CBOE(VX)·HKEx(CNH)는 모의계좌로 바로 성공, CME/CBOT(ZN 등)는
             "EGW00552: CBOT SUB거래소 신청 계좌가 아닙니다"로 거부됨 — 코드가 아니라 계좌 설정
             문제이므로 호출측이 이 에러를 구분해 재시도하지 말아야 한다).
    ...
    def get_overseas_daily_chartprice(
        ...
        해외선물옵션 계좌 신청 없이도 얻을 수 있는 일봉 대체 경로.
        ...
        해석: 같은 API 계열의 분봉 엔드포인트(inquire-time-indexchartprice)는 I(국채) 구분을
             "ERROR INVALID FID_COND_MRKT_DIV_CODE"로 거부해 분봉 미지원이 확정됐다 — 이 함수
             (일봉)만이 US10Y를 계좌 제약 없이 얻는 유일한 경로다.
        실패 조건: 4xx/5xx면 httpx.HTTPStatusError 그대로 전파.
        """
    ...
    def submit_order(
        ...
        실패 조건: 4xx/5xx면 httpx.HTTPStatusError 전파 — 상위 Order State Machine이
                  REJECTED로 기록.
        """
