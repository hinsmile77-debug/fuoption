...
마흐디(선행 옵션 프로젝트) mahdi/broker/token_daemon.py에서 2026-07-21 이식
(KISSettings → KISCredentials로 설정 소스만 교체, 로직은 동일).

여러 프로세스가 동시에 토큰을 발급받으면 KIS가 직전 토큰을 무효화할 수 있어(레이트리밋),
프로세스 전역에서 TokenDaemon 인스턴스 하나만 공유해 캐싱해야 한다 (NEXT_TODO W3~5 참고 —
Redis 캐시로 프로세스 간 공유하는 상위 래퍼는 별도 TODO).
"""
