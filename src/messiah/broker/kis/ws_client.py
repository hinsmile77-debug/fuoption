...
마흐디(선행 옵션 프로젝트) mahdi/broker/ws_client.py에서 2026-07-21 이식
(KISSettings → KISCredentials로 설정 소스만 교체, 로직은 동일).

세션당 구독 슬롯이 제한적이므로(KIS 공지 기준 최대 약 41건), 슬롯 롤링 관리는 상위 Data Layer가
담당하고 이 클래스는 순수 WS 송수신·구독 등록/해제만 책임진다. 연결 객체(WSConnection)는
주입받아, 실제 네트워크 없이도 단위 테스트가 가능하게 한다.
"""
...
class ApprovalKeyIssuer:
    ...
    def __init__(self, creds: KISCredentials, client: httpx.Client | None = None) -> None:
        self._creds = creds
...
class KISWebSocketClient:
    ...
    async def subscribe(self, sub: Subscription) -> None:
        """
        입력: 구독할 TR ID/종목코드.
        계산: 활성 구독 집합에 추가하고 등록(tr_type=1) 메시지 송신.
              이미 활성 구독이면 아무 것도 안 함.
        실패 조건: 활성 구독 수가 MAX_SUBSCRIPTIONS에 도달하면 ValueError — 호출측
                  (상위 Data Layer)이 롤링으로 기존 구독을 먼저 해제해야 한다.
        """
