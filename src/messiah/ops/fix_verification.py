...
def _scorable_days_until(
    ...
    ## 기한 **뒤**의 위반은 기한 **전**의 채점 가능일을 지우지 못한다 (2026-09-04 F-92)

    이 함수가 답하는 질문은 「기한까지 잴 날이 있었나」 하나다. 그런데 기산점을 마지막
    위반일로 잡으면, 기한이 지난 **뒤에** 새로 난 위반이 기산점을 기한 너머로 밀어
    구간이 통째로 비고, 그 빈 구간이 「잴 날이 0일이었다」로 나간다. 시제가 어긋난
    것이지 사실이 아니다 — 그날들은 실제로 있었고 실제로 채점됐다.

    2026-09-04 실측이 정확히 그 형태였다:

        08-20~08-28  리포트 7일 전부 `abnormal_exits: 0` — 채점됐고 통과했다
        08-28        판정 「7거래일 연속 기준 충족」   ← 기한 당일에 이미 통과 상태였다
        09-03        `abnormal_exits: 1`(장중 사망) — **기한이 지난 뒤의** 새 위반
        09-04        기산점이 09-03으로 밀려 (09-03, 08-28] = 0일
                     → 「채점 가능일이 0일뿐이었다 — 잴 날이 없었다. 기한 재조정 필요」

    그 문장은 두 번 틀렸다. 잴 날은 7일 있었고, 처방도 「기한 재조정」이 아니라 09-03
    재발을 보는 것이다. **그 재발이 이 문구에 가려졌다** — 이 등록부가 막으려던 실패
    그 자체다. 그래서 기산점이 기한을 넘으면 `scored_after`로 되돌린다.

    되돌린 뒤 판정은 `기한 초과`(⑤)로 간다 — 「아직 1/3일」은 참이고, 재발이 사람 앞에
    올라오는 자리도 그대로다. **판정을 무르게 하지 않는다**: `기한 불가`와 `기한 초과`는
    둘 다 `needs_attention`이고 로그 태그도 둘 다 WARNING이다.
    """
    deadline = effective_deadline(item, report_days)
    if deadline is None:
