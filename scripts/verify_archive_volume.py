...
def compare_day(
    ...
    """(비율, 공통 분 수, 아카이브 합, 공식 합, **미수집 분 수**) — 공통 분이 없으면 비율 None.
    ...
    **공통 분만** 더한다. 거래소 분봉과 수집 분봉은 장전 구간 포함 여부가 다를 수 있고,
    ...
    그래서 다섯 번째 값으로 `공식에는 있는데 아카이브에 없는 분 수`를 돌려준다. 반대
    방향(아카이브에만 있는 분)은 장전 구간 차이라 정상이므로 세지 않는다.
    """
    common = sorted(set(archived_volumes) & set(official_volumes))
    missing = len(set(official_volumes) - set(archived_volumes))
    if not common:
        return None, 0, 0, 0, missing
    mine = sum(archived_volumes[m] for m in common)
    theirs = sum(official_volumes[m] for m in common)
    return (mine / theirs if theirs else None), len(common), mine, theirs, missing
...
def main() -> int:
    ...
    for symbol, day in targets:
        ...
        ratio, common, mine_sum, theirs_sum, missing = compare_day(mine, theirs)
        if ratio is None:
        ...
        if missing > MISSING_MINUTES_LIMIT:
            mark = f"** 미수집 {missing}분 **"
        print(
            f"  {symbol} {day}  비율 {ratio:.3f}  "
            f"(공통 {common}분 · 공식 {len(theirs)}분 · "
            f"아카이브 {mine_sum:,} / 공식 {theirs_sum:,})  {mark}"
        )
        results.append((symbol, day, ratio, common, mine_sum, theirs_sum, missing))
        if ratio < WARN_RATIO or missing > MISSING_MINUTES_LIMIT:
    ...
    for symbol, day, ratio, common, mine_sum, theirs_sum, missing in results:
            ...
            json.dumps(
                {
                    ...
                    "ok": (
                        ratio is not None
                        and ratio >= WARN_RATIO
                        and missing <= MISSING_MINUTES_LIMIT
                    ),
                },
                ensure_ascii=False,
                indent=2,
