...
## L2 피처 인프라 재설계 F0 + MS 수집 개시 F2 ([MW0601], 2026-08-04)

피처 재설계 조사에서 나온 것: 설계(Ver 1.4)의 기저 152개 중 **모델에 실제 도달하는 것은
44개**였다(PX 30 + VL 14). FL 9개는 코드가 있는데 `FeatureEngine` 생성처 7곳 전부가
사이드카를 안 넘겨 **한 번도 도달한 적이 없었고**, MS/OP/RG/EV는 미착수였다.

카테고리를 늘리기 전에 두 가지를 먼저 세웠다 — 벡터 모양의 정본(F0-1)과 품질 관문(F0-3).

### F0 — 피처 인프라

| 기능 | 구현 | 단위테스트 | 비고 |
|---|---|---|---|
| `features/spec.py` — `feature_set` → 카테고리 → 정확한 피처 이름 | ✅ | ✅ 2026-08-04 | 이름 목록을 손으로 안 적는다(계산기 모듈을 **호출 시점에** 참조). `FEATURE_SETS`: `v2026.07`=PX+VL(121) · `v2026.08-fl`=+FL(130) |
| `features/sidecar.py` — `DailySidecar` Protocol | ✅ | ✅ 2026-08-04 | "요청일보다 **엄격히 이전**만 준다"는 `FlowHistory` 계약을 OP·RG가 다시 발명하지 않게 이름 붙임. `flow_as_of` → `as_of` 통일 |
| `FeatureEngine` 스펙 구동 전환 | ✅ | ✅ 2026-08-04 | `if self._flow is not None` 제거. 스펙이 요구하는 사이드카가 없으면(또는 안 쓰는 걸 넣으면) **생성 시점 ValueError** — 종전에는 조용히 9개가 사라진 벡터가 같은 이름을 달고 나갔다 |
| `InstanceConfig.feature_set` 검증기 | ✅ | ✅ 2026-08-04 | `universe`와 같은 패턴 — 오타가 조용히 기저 벡터로 떨어지지 않게 기동 시점 거부 |
| 윈도우 예산 테스트 스펙 구동화 | ✅ | ✅ 2026-08-04 | 종전에는 PX/VL 두 모듈을 손으로 나열 — 신규 카테고리가 검사에서 통째로 빠질 수 있었다 |
| `features/gate.py` — Ver 1.4 §3 ①단독 ②중복 ③생존 | ✅ | ✅ 2026-08-04 (27건) | Ver 1.4가 처음부터 요구했으나 코드로 존재한 적 없던 관문. Spearman IC + **겹침 보정 강제** + 크기·유의성·효과크기 3중 요구 |
| `scripts/run_feature_gate.py` | ✅ | ✅ 2026-08-04 (실 아카이브) | 아래 실측 참고 |

**관문 첫 실행 실측** (연속물 63,508봉, 2025-12-12~2026-08-04, 겹침 3봉 보정):

| Horizon | 표본 | active | retired | 통과율 |
|---|---|---|---|---|
| 5m | 12,684 | 7 | 114 | 5.8% |
| 15m | 4,216 | 12 | 109 | 9.9% |
| 30m | 2,099 | 17 | 104 | 14.0% |

**첫 실행에서 결함 하나를 잡았다**: `px_macd_h_5`가 **프로덕션에서 항상 정확히 0**이었다.
`window=5` → `5//3=1` → `_ema_series(x, 1)`은 k=2/(1+1)=1이라 EMA가 항등이 되고, 히스토그램
`macd[-1] − signal[-1]`이 0이 된다. `px_ema_cross_60`(2026-08-04)과 같은 종류인데 **그건
NaN이라 `nan_ratio`에 흔적이 남았고 이건 값을 내므로 무결성 리포트에 아무 흔적도 없었다** —
검출 수단이 하나뿐이면 그 수단이 못 보는 결함은 영원히 안 보인다. 시그널 기간 하한을 2로
고정(`_MIN_SIGNAL_PERIOD`), 회귀 테스트 추가.

### F2 — MS(마이크로구조) 수집 개시

**호가는 이미 매 틱 도착하고 있었다.** H0IFCNT0은 50필드고 idx34~37이 매도호가1/매수호가1/
잔량인데(2026-07-22 캡처로 교차검증된 위치) 파서가 4필드만 읽고 46개를 버렸다. MS 30개가
"데이터 없음"이었던 이유는 데이터가 아니라 스키마였다.

| 기능 | 구현 | 단위테스트 | 실측 | 비고 |
|---|---|---|---|---|
| `Tick` L1 호가 4필드 + `raw_fields` | ✅ | ✅ 2026-08-04 | — | 확정 위치만 이름을 붙이고 나머지 46필드는 원본 문자열로 보존(R8/L16 — `Docs/KIS_RAW_FIELD_RANGES.md` 참고) |
| `normalizer` 호가 파싱 + `side_hint` quote rule | ✅ | ✅ 2026-08-04 (7건) | ✅ 실캡처 프레임 | 종전 `side_hint`는 항상 0. 실캡처 프레임(체결가=최우선매도호가)에서 +1 산출 확인. **알려진 근사**: Lee-Ready는 체결 직전 호가를 쓰지만 여기는 동시 스냅샷 |
| `data/tick_archiver.py` | ✅ | ✅ 2026-08-04 (12건) | ✅ 5만틱 부하 | 시간대 조각 + 500행 버퍼 flush. 5만틱 7.3초 · **하루 0.3MB**(원시필드가 반복이라 압축률이 높다) |
| `run_l1_daily.py` 결선 | ✅ | — | 2026-08-05 08:35 첫 실운영 | **아카이버를 수집기보다 먼저** gather에 넣는다 — 순서가 뒤집히면 첫 틱이 버스에서 증발한다(수급에서 7개월을 날린 그 순서) |

### 알려진 갭 (F0·F2, 2026-08-04)

- **생존 검정(③)은 실행된 적이 없다.** G1 창이 하나뿐이라 3창 요구를 못 채운다. 관문은 이
  경우 **아무도 탈락시키지 않는다**(데이터 부족을 피처 결함으로 오역하지 않기 위해) — 즉 지금
  통과한 7/12/17개는 "여러 국면에서 살아남았다"가 아니라 "한 국면에서 유의했다"까지만
  증명된 것이다. 생존자 대부분이 변동성 크기 계열(vl_atr·vl_rv·vl_semi·px_bb_width)이고
  IC 부호가 전부 음수인데, 표본 구간이 8개월 단일 상승장이라 **국면 특수적일 가능성이 높다.**
  하락/횡보 국면이 생긴 뒤 재실행할 것.
- **`side_hint`의 동시 스냅샷 근사**: 같은 프레임의 호가는 체결 **후** 갱신된 값일 수 있어
  일부 체결의 방향이 반대로 잡힌다. 편향 방향이 한쪽으로 쏠리는지는 **측정 대상**이며,
  MS 피처를 실제로 학습에 넣기 전(F6)에 재검토한다.
- **틱 프레임의 미결제약정·이론가·총잔량·체결강도는 위치 미확정**: 값의 모양으로 추정만 했고
  공식 문서 대조·타 종목 재현을 안 했다. 보존은 시작했으므로 매핑 확정은 소급 적용된다.
- **MS 피처 자체는 아직 하나도 없다**: F2는 **수집만**이다. 계산기(F6)는 데이터가 3개월쯤
  쌓인 뒤 — 지금 붙이면 학습 이력이 163거래일에서 0일로 리셋된다.
- **L5/L10 호가 깊이 8개는 영구 제외 후보**: 체결 프레임에는 최우선 1단계만 있다. 별도
  호가 WS(선물용 TR 상수조차 아직 없음) 구독이 필요하고, 현재 소비처가 없다.

## L2 피처 F1 — EV(이벤트·시간·만기) ([MW0601], 2026-08-04)

**EV는 이력 전체에 소급 계산되는 유일한 카테고리다** — 전부 시각·달력 함수라 과거 봉의
타임스탬프만 있으면 값이 나온다. MS/OP처럼 "오늘부터 모아서 3개월 뒤"가 아니라 지금 있는
163거래일에 그대로 붙어 즉시 A/B가 된다. 그래서 F1이 됐다.

동기: 121개가 전부 OHLCV 파생이라 모델이 **"장 마감 10분 전"과 "개장 직후"를 구분할 수단이
하나도 없었다**.

| 기능 | 구현 | 단위테스트 | 비고 |
|---|---|---|---|
| `features/ev_core.py` — 기저 12개 / **16컬럼** | ✅ | ✅ 2026-08-04 (32건) | 요일이 one-hot 5컬럼(Ver 1.4 원문). 확정 시각(`bar_confirm_time`) 기준 — 봉 시작이 아니다 |
| `EventCalendar` 만기·거래일 질의 | ✅ | ✅ 2026-08-04 | `monthly_expiry`/`is_monthly_expiry`/`is_quadruple_witching`/`next_monthly_expiry`/`next_weekly_expiry`/`trading_days_until`/캘린더 간격 2종 |
| **만기 규칙 중복 제거** | ✅ | ✅ 2026-08-04 | `data/backfill.monthly_expiry`(휴장 보정 있음, 7개 월물 실측 검증)와 `is_expiry_day()` 인라인 판정(`8<=day<=14`, 보정 없음)이 **두 벌**이었다 — 정본을 `core/event_calendar.py`로 합치고 backfill은 재수출. EV가 세 번째 사본을 만들기 직전이었다 |
| `features/sidecar.py` — 관측/참조 2종 구분 + `build()` | ✅ | ✅ 2026-08-04 | 캘린더는 **참조** 사이드카라 미래 조회가 정당하다(내일 휴장은 몇 달 전 공표된 사실). 관측 사이드카(FL)의 "엄격히 이전" 계약을 여기 강요하면 `ev_dte_fut`가 계산 불가가 된다 |
| `spec` EV 등록 + `v2026.08-ev`(137) · `v2026.08-fl-ev`(146) | ✅ | ✅ 2026-08-04 | |
| `trainer`·`run_feature_gate`에 `sidecars` 결선 | ✅ | ✅ 2026-08-04 | |

### 관문 실측 — v2026.07(121) vs v2026.08-ev(137)
...
통과한 EV 피처는 **시각 축 하나뿐**이다:

| Horizon | 피처 | IC | t | 판정 |
|---|---|---|---|---|
| 5m | `ev_tod_cos` | +0.0214 | +1.39 | retired (유의성 미달) |
| 15m | `ev_tod_cos` | +0.0665 | +2.50 | **active** |
| 30m | `ev_tod_cos` | +0.0844 | +2.24 | **active** |
| 30m | `ev_close_remain` | −0.0838 | −2.22 | **active** |

Horizon이 길수록 시각 축의 IC가 커진다(0.021 → 0.067 → 0.084) — 긴 Horizon일수록 일중
계절성에 더 노출된다는 해석과 방향이 맞는다. 나머지 EV 전부(요일 5개·만기 D-day 3종·
만기플래그·롤오버·연휴인접·점심)는 이 표본에서 **측정 가능한 신호가 없다**.

**관문 ②가 예고된 중복을 실제로 잡아냈다**: 30m에서 `ev_open_elapsed`·`ev_tod_sin`이
`ev_close_remain`과 |ρ|>0.9로 탈락했다. `ev_core` 모듈 docstring이 "세션 길이가 상수라
정확히 상보적"이라고 미리 적어 둔 그 쌍이고, 내 판단으로 지우는 대신 측정으로 떨어뜨렸다.
`ev_dte_fut`·`ev_dte_opt_m`도 전 Horizon에서 IC가 소수점 4자리까지 동일해(둘 다 둘째 목요일
만기) 예측한 항등이 확인됐다 — 다만 둘 다 ①에서 먼저 탈락해 ②까지 가지 않았다.

### 실측으로 정한 상수

`ev_lunch_flag`의 창은 **추측이 아니라 측정값**이다(연속물 63,508봉 = 163거래일). 정규장
분당 평균 345계약 대비 10분 버킷: 11:00 1.01x → 11:30 0.84x → **12:20 0.65x(최저)** →
13:50 0.78x → 14:00 0.95x. 창을 11:30~14:00으로 잡았다.

### 알려진 갭 (F1, 2026-08-04)

- **`ev_econ_prox`·`ev_econ_grade`·`ev_overnight_gap_risk` 3개는 미구현**: 경제지표 발표
  일정 피드(FOMC·CPI·고용)가 이 프로젝트에 없다. **NaN으로 자리를 채우지 않았다** —
  `px_ema_cross_60`이 그렇게 죽은 채로 학습됐다. `ev_core.EXCLUDED_FEATURES`에 상수로
  남겨 "빠뜨린 것"과 "일부러 뺀 것"을 구분한다.
- **`ROLLOVER_TRADING_DAYS=5`는 미실측 초기값**. 측정을 시도했으나 불가능했다: 백필이
  날짜마다 **근월물만** 저장해 두 월물이 겹치는 날이 만기일 하루뿐이다(그날 차월물 거래량
  비중 36~64% — 만기일엔 이미 이전이 끝나 있다는 것만 보인다). **측정 가능한 갭이다** —
  KIS 분봉 API가 만기물도 주므로 각 만기 직전 10거래일치 차월물을 추가 백필하면 된다.
- **위클리 만기 판정은 요일 규칙 근사**: 그 요일이 휴장이면 KRX가 앞당기는지 미루는지,
  정규월물 만기 목요일에 목위클리가 별도 상장되는지 둘 다 symbol_master 미대조다
  (현재 구현은 그날도 위클리로 세어 `ev_dte_opt_w`가 0을 낸다).
- **휴장일 데이터가 2025~2026뿐**: 2027년으로 넘어가면 `is_trading_day`가 예외를 던지고
  `ev_dte_*`가 통째로 NaN이 된다. 조용히 틀린 D-day보다 낫도록 **일부러** 그렇게 뒀지만,
  연말 전에 `configs/krx_holidays.yaml`을 갱신해야 한다.
- **프로덕션 `feature_set`은 아직 `v2026.07`이다**: EV를 켜려면 그 피처로 모델을 다시
  학습해야 하고(R11 장중 배포 금지), 이번 산출물은 **측정**까지다.
- **생존 검정 미실행은 그대로**(F0 기존 갭): 위 t 2.2~2.5는 8개월 단일 상승장 한 국면의
  값이다. 30m 유효표본이 겹침 보정 후 ~700이라 임계 바로 위이기도 하다.
