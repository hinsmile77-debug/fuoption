...
def test_approval_key_issuer_returns_key_from_response():
    ...
    creds = KISCredentials(app_key="key", app_secret="secret", is_mock=True)
    issuer = ApprovalKeyIssuer(creds, client=httpx.Client(transport=httpx.MockTransport(handler)))
