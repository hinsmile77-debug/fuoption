...
import pytest
from messiah.broker.kis.order_state_machine import (
...
def _new_order(state: OrderState = OrderState.PENDING) -> Order:
    return Order(
        ...
        intended_px=350.0,
        qty=10,
        timestamp=datetime(2026, 7, 5, 9, 5, tzinfo=timezone.utc),
        state=state,
    )
...
def test_pending_to_cancelled_and_rejected_allowed():
    assert (
        OrderStateMachine(_new_order()).transition(OrderState.CANCELLED).state
        == OrderState.CANCELLED
    )
    assert (
        OrderStateMachine(_new_order()).transition(OrderState.REJECTED).state == OrderState.REJECTED
    )
