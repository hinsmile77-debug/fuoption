...
def test_submit_order_includes_required_fields_kis_would_otherwise_reject():
    # ORD_PRCS_DVSN_CD와 ORD_DVSN_CD는 "선물옵션 주문" 문서 기준 Required=Y —
    # 누락 시 KIS가 거부한다.
    captured = []
...
def test_get_investor_flow_always_uses_real_domain_even_for_mock_account():
    ...
    result = client.get_investor_flow(
        tr_codes.FID_MRKT_DIV_DERIVATIVES, tr_codes.FID_INVESTOR_FLOW_FUTURES
    )
...
def test_uses_real_domain_when_not_mock():
    captured = {}
...
def test_requests_are_paced_to_respect_shared_rate_limit():
    # 마흐디 2026-07-08 실측: 옵션체인/수급 폴링 루프가 동시에 REST를 쏘면 KIS가 500을 대량
    # 반환함 — min_request_interval이 실제로 호출 사이를 벌리는지 검증한다.
...
# --- _is_kis_rate_limit_error (적응형 레이트리미터) -----------------------------


def _http_error_with_body(
    status_code: int, json_body: dict | None = None, content: bytes | None = None
):
...
def test_is_kis_rate_limit_error_true_for_egw00201():
    exc = _http_error_with_body(
        500, {"rt_cd": "1", "msg_cd": "EGW00201", "msg1": "초당 거래건수를 초과하였습니다"}
    )
...
def test_is_kis_rate_limit_error_false_for_other_500():
    ...
    exc = _http_error_with_body(
        500, {"rt_cd": "1", "msg_cd": "EGW00552", "msg1": "CBOT SUB거래소 신청 계좌가 아닙니다."}
    )
