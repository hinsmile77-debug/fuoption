...
<!-- 반입대기: 시작 (자동 생성 — scripts/pending_intake.py) -->

## 반입 대기(커밋 전) — 2026-09-10 자동 갱신

장후 채점기가 오늘 **구현 종결**로 센 항목 3건 — 그날 커밋에 아직 없다.

> 이 목록은 채점기(`ops/record_vs_commit`)가 센 것을 그대로 옮긴 것이고, 판정하지 않는다. `F-`/`G-` 머리표가 붙은 **상태 갱신 줄**도 함께 잡히므로 (예: 「대기 지속, 새 정보 없음」) 전부가 반입할 코드라는 뜻은 아니다. 무엇을 먼저 보낼지는 사람이 정한다.

1. **F-94** 완성봉 합성 대기 상한 **5.0 → 10.0초**. 근거: 1분봉 발행 지연 실측
2. **F-94 파생(사용자 결정)** 합성봉 유예를 상한과 **분리** —
3. **G-57** 완성봉 유실 사전 경보 — `obs/delay_spike.DelaySpikeWatch`.

판정: `ok`

<!-- 반입대기: 끝 -->

## 2026-09-10 장전 점검 ([MW0601] · 08:51~09:03 · 신규 1-7(P1, 캘린더 재발)·1-8(P2, pending_intake 자체 결함), F-94/G-57/F-91 야간 반입 확인, 1-6/order-path-live 정정 반영)

- [ ] **1-7 / F-95** `event_calendar.thursday_weekly_listed()` — 월물 만기 당일에 다음 주 목위클리가
      선상장되는 것을 반영하는 분기 추가. **선행: 사람이 KRX 공지 원문 확인 후 승인**
      (2026-08-13 J-2 요청 아직 미완료 — 이번이 두 번째 재현). `has_thursday_weekly()`는 불변.
- [ ] **1-7 / G-2 (재상정)** 반복 ERROR 요약 접기 — 08-13에 "즉시(장후 최우선)"로 제안된 뒤
      아직 `core/logging.py`에 미반영. 오늘도 `OptionChainCalendarViolation`이 5분 간격으로
      쌓이는 중(09:02 기준 8건, 정규장 종료까지 약 80건 예상 — 08-13 실측 84건 참고).
- [ ] **G-59 (신규, 이번 주)** 캘린더 예측(`thursday_weekly_listed`·`resumes_on`)을
      `logs/calendar_predictions.jsonl`에 발행 시점 기록 + 장후 실측 대조(08-13 G-1 재상정).
- [ ] **1-8 / G-60 (신규)** `scripts/pending_intake.py` 커밋 판정 방식(스냅샷 vs 실시간) 확인 —
      오늘 새벽 커밋 3건(F-94·F-94파생·G-57, 01:00~01:18)이 이미 반입됐는데도 "반입 대기"
      절이 "아직 없다"고 표시. 절 머리글에 생성 시각·대조 HEAD sha 병기 제안.
- [x] **1-3/F-94** 사람 결정 완료(대기 10.0초 + 유예분리 1.5초) — 커밋 `6318a07`·`c0b4801`.
      3거래일 연속 무위반 검증 1일째 시작(09-10).
- [x] **G-57** 섀도 착수 완료 — 커밋 `c8b6d81`·`d8816ce`. R18 20거래일 관찰(~10-13 전후) 시작.
- [x] **F-91** 등록부 정리 반영 확인 — `configs/pending_verifications.yaml` 22건(23→22).
- [ ] **1-6 원인 확정** "미반입 격차"가 아니라 "채점기가 상태-갱신 줄을 구현으로 오인" —
      분류 규칙 수정 여부 사람 결정 여전히 대기.
- [ ] **order-path-live** 09-10 장중·장후에도 신호가 게이트를 넘는지(5일 중 3일 기준 갱신 확인).
- [ ] **확인 필요** 전체 스위트 `champion_sample` 1건 실패 — 이 점검 세션 환경에 `pytest` 미설치로
      재현 불가. 사람이 실전 PC `.venv`에서 `pytest -k champion_sample` 재확인 요청.
- [ ] **확인 필요(지속)** 09-09 15:00 모의 주문 `SIM00000001` 체결·청산 상태 — 구조적 한계
      (`self_eval` `pnl_measurable: false`)로 이번에도 판정 불가.
- [ ] **G-54 재확인** 이 점검 세션이 오늘도 git 직접 호출 위반(`git log`, 읽기 전용) — 9월 들어
      2번째(09-08, 09-10). 구조적 방지 장치 착수 여부 사람 결정 계속 대기.
- [ ] **K-25** 오늘(09-10) 공식 판정 — 장후 확인.
...
### D-1 예행 — 2026-08-16에 앞당겨 **전부 완료** (커밋 cc93366 상태)

- [x] **D-1① 코드 동결** — 커밋 `cc93366`. **여기서부터 D-day 아침까지 코드를 넣지 않는다.**
- [x] **D-1② 예행** — `self_check` PASS · `run_chaos_check` 전 경로 통과 ·
      `verify_kill_switch` PASS(화면 버튼→청산 전 구간 실동작,
      `logs/kill_switch_verification_20260816.json`).
- [x] **D-1③ 스케줄 확인** — `schedule_drift=정본 일치 Messiah=08:20, Messiah-G2=08:25
      (기동 창 08:15~)` · 부팅 트리거 무장 2개 · `redis://localhost:6380/0` OK ·
      `rollover: 비-롤일 — 근월물 A05609 유지 · 다음 롤 2026-09-11`.
- [x] **D-1④ G2 40거래일 리셋 기산 결정** — **2026-08-18을 1일차로.** 기존 13일은
      전량 NO_TRADE·수익률 0.0이라 관문 기준 셋 중 둘의 분모가 될 수 없다. 다만 지우지
      않고 "시스템 무중단" 기록으로 보존하며, **절단 사실을 리포트에 명시**한다
      (조용히 자르면 "왜 40일인데 27행이냐"를 나중에 아무도 못 푼다). DECISION_LOG 참조.
- [x] **D-1⑤ 리허설 재실행** — 수정 직후 실행과 **완전히 동일한 답**. 동결 확인.

### 2026-08-17(월·휴장)에 남은 것

- [ ] 없음 — D-1 항목이 전부 08-16에 닫혔다. 월요일은 **아무것도 하지 않는 것이 계획이다**
      (장중 배포 금지가 아니라, 동결을 흔들 이유가 없다는 뜻).
...
## 2026-08-16 진입흐름 문서 갱신 — 레이블 기하 재측정이 1순위를 바꿨다 ([MW0601])

문서 `Docs/동작흐름과상태/2026-08-16_진입흐름과_개발상태.md`
(`2026-08-06_진입흐름과_개발상태.md`를 대체. 그 문서는 그날의 기록으로 보존)

### ★ 재측정이 드러낸 것 — `run_label_geometry.py --end 2026-08-14`

```
[30m] 레이블 2207건 — down 10.8% / flat 76.3% / up 12.9%
      |S| 천장 0.237 vs 게이트 0.20 (여유 1.18배 < 1.5)  → **게이트 도달 불가**
      배리어 폭 중앙 970틱 = 왕복 비용의 606배 · 비용강등 0건
      flat 76.3% 중 99.6%가 |수익| 중앙 233틱 — 비용의 146배를 움직였는데 "flat"
```

**6개 Horizon 전부에서 비용 강등이 0건이다.** 배리어 폭이 비용의 21배(1m)~606배(30m)라
"비용을 못 넘는 움직임은 flat으로 강등" 규칙이 **한 번도 발동한 적이 없다.**

**세 관측이 하나의 사슬이었다**:
레이블 30m flat 76.3% → Expert `p_flat` 0.473~0.967 → `meta_p` 최대 0.658 < 임계 0.700
→ `blocked_by_meta` 15/15. **모델이 못 배운 게 아니라 가르친 것이 "거의 아무것도 거래하지
말라"였다.** 그리고 하필 유일한 live 번들이 30m이다.

### 우선순위 정정 — Meta 임계는 증상이지 원인이 아니다

- [ ] **0순위 ★ 왕복 비용 실측** — 지금 1.60틱은 placeholder(호가 WS 미구독).
      1순위가 이 값을 입력으로 쓰므로 **순서상 맨 앞**이다. 실측 안 된 수로 레이블을 다시
      만들면 그 위에 모든 판단이 쌓인다.
- [ ] **1순위 ★ 레이블 기하 교정** — 배리어 폭(`labeling._WIDTH_ATR_MULT`) + 비용 강등 규칙.
      **모델 재학습보다 먼저다** — 지금 구조에서는 30m이 아무리 좋은 모델이어도 게이트를
      못 넘는다. 변경마다 `run_label_geometry.py`가 채점한다.
- [ ] **2순위** G1 관문을 실제로 통과하는 번들. `cost_adjusted_sharpe 0.0`의 원인 규명 —
      **1순위가 그 답의 상당 부분일 가능성이 높다.**
- [ ] **3순위** Position Reconciler(R10 결선 · 실현손익 매칭 · G2 슬리피지 채점 수단 ·
      0순위의 라이브판).
- [ ] **Meta 임계 조정은 금지 유지** — 1순위를 안 고친 채 임계만 내리면 "구조적으로
      거래하지 말라고 배운 모델"에게 거래를 시키는 셈이다(R18).

### 문서에 기록한 부수 발견

- [ ] **`configs/instance.yaml` 주석 오류** — `v2026.08-ev`를 **133개**라 적었으나 실제
      **137개**(121+16). 동작 영향 없음(코드가 정본). 주석만 정정할 것.
- [ ] **두 parquet 로더의 시간대 불일치는 그대로다** — `ParquetArchiver`(KST) vs
      `ParquetBarReplaySource`(UTC). 2026-08-16에 `_minutes_since_session_open()` 하나만
...
## 2026-08-17 휴장일 장중 점검에서 나온 것 ([MW0601])

~~**전부 2026-08-18(화) D-day 장후 15:35 이후 적용.**~~ → **F-1·F-2·F-3·F-13은 2026-08-17
(휴장일) 당일에 구현했다.** 사용자 지시로 앞당겼고, 판단 근거는 다음 셋이다:

1. **오늘은 장중이 아니다.** R11(장중 학습·배포 금지)과 금지계명 3·4가 금지하는 것은 정규장
   시간의 배포다. 휴장일 저녁은 그 대상이 아니다.
2. **거래일 경로를 건드리지 않는 변경이다.** F-1·F-2는 비거래일 분기에만 코드가 있고, F-3은
   순서만 바꿔 거래일에는 게이트가 즉시 통과한다 — 08-18 아침의 실행 경로는 동일하다.
   전 테스트 2,039건 통과 + `self_check` 14축 PASS(신규 `calendar` 축 포함)로 확인했다.
3. **오늘이 검증할 수 있는 유일한 날이었다.** 다음 휴장일은 2026-09-24(추석)다. 08-18 장후에
   넣으면 그 코드는 **5주 뒤에 처음 실행된다** — 이 저장소가 "구현됨≠검증됨"이라 부르는 상태로
   5주를 보내는 것이고, 실제로 세 진입점 모두 오늘 실행해 마커까지 눈으로 확인했다.

⚠ **커밋은 하지 않았다** — 작업트리에만 있다. `mode: dev`라 `check_git_state`가 dirty를
허용하므로 08-18 기동은 영향받지 않지만, **08-18 아침 기동은 이 코드로 뜬다**(`.py`를 직접
읽으므로). D-day 관측을 08-17과 같은 코드로 비교하려면 `git stash`로 되돌릴 것.

근거 보고서: `logs/dailycheck/2026-08-17_intra_report.md`

### Fix (커밋 ① — 런타임 위험 최저)

- [x] **F-1 ★ 휴장 조기 종료에 `SessionEnd` 발행** (2026-08-17 구현) —
      `ops/session_guard.announce_non_trading_day()`가 정본이고 세 진입점이 그것을 부른다.
      태그는 안 늘리고 `reason="non_trading_day"`로 갈랐다(`NON_TRADING_DAY_REASON`).
      실행 확인: 세 진입점 모두 오늘 실행해 `SessionStart`/`SessionEnd` 짝 + `reason` 필드
      확인. **다음 휴장일(2026-09-24) 다이제스트 §9에서 `SessionStart 2회` 위양성 소멸 여부가
      최종 검증이다.**
- [x] **F-13 `collect_evidence.py` 휴장일 인식** (2026-08-17 구현) — YAML을 stdlib로 직접
      파싱(`read_holiday_calendar()` — 스킬이 `src/messiah`나 `.venv`에 의존하지 않는다).
      §0 머리말 전제 + §7 표를 `해당 없음(휴장)`으로 + §9를 「휴장일이라 기각한 항목」 절로
      **이동**(삭제 아님 — 달력이 틀린 날에는 그 목록이 곧 사고 보고서다).
      미등재 연도는 **필터를 끄고** 그 사실을 머리말에 적는다.
      실측: 08-17 로그 재실행 시 적신호 **5건 → 2건**(남은 둘은 진짜다 — 장후 배치 ERROR는
      F-2 적용 전 로그의 흔적이고, 미커밋 변경은 오늘 이 작업이다).

### Fix (커밋 ② — D-day 장후 배치 완주 확인 **후**에만) → **08-17에 앞당겨 구현**

- [x] **F-2 ★ 장후 배치 휴장일 가드** (2026-08-17 구현) — `run_postmarket.main()` 진입 직후
      (`mlog.setup()` 뒤, **심볼 해석 앞**)에 `session_guard.non_trading_day_reason()` +
      `SessionEnd(reason=non_trading_day)` + **exit 0**. `_SYMBOL_MISMATCH_EXIT_CODE=3`은
      그대로 뒀고, 그 분기의 *"휴장일이면 정상이다"* 안내문은 삭제했다(휴장일은 이제 거기
      도달하지 않으므로, 남겨 두면 사람이 진짜 오조회를 그 문장으로 넘긴다).
      우회는 `--force-non-trading-day` + `PostmarketForcedOnNonTradingDay`(WARNING).
      계획과 달라진 점: 새 태그 `PostmarketSkippedHoliday`를 만들지 않았다 — `SessionEnd`
      하나로 충분하고, 태그를 늘리면 그것을 아는 소비처와 모르는 소비처가 갈린다(F-1의 근거).
      **미검증: 거래일 회귀 — 08-18 장후 `steps_run == 6`을 반드시 확인할 것.**
      (테스트로는 게이트 순서·exit 3 존속을 못박았고 `tests/test_rollover_day.py`의 거래일
      경로 2건이 통과한다. 실 배치 완주는 08-18이 첫 실측이다.)

### Fix (커밋 ③ — F-1이 다음 휴장일에 검증된 뒤) → **08-17에 함께 구현**

- [x] **F-3 휴장 판정을 진입점 최상단으로** (2026-08-17 구현, **안 A**) — 두 진입점의
      `__main__`에서 `_ensure_docker_ready()`·`_run_self_check()` **앞**으로 옮겼다.
      docstring(`:28-30`)이 2026-07-27부터 선언해 온 원래 의도이고, 그 선언과 코드가 3주
      어긋난 채였다(`DECISION_LOG.md:631`).
      **선행 조건이었던 `CrashForensicsArmed` 소비처 grep을 실제로 했고, 그것이 설계를
      바꿨다**: `ops/crash_dumps.collect_crash_forensics()`는 로그에 그 태그가 없으면
      *"그 세션은 네이티브 크래시가 나도 증거를 안 남긴다"* 를 찍는다. 무장을 `main()`에
      두면 휴장일에 **없애려던 위양성 자리에 새 위양성이 들어선다.** 그래서 무장을
      `_arm_forensics_and_logging()`으로 꺼내 게이트가 먼저 부르게 했다 — 두 호출처가
      있지만 한 프로세스에서 한 번만 불린다(두 번 부르면 `SessionStart` 2줄 = 그 위양성).
      순서 회귀는 `tests/test_non_trading_day_gate.py`가 소스 순서로 못박는다.

### 2026-08-17 조사에서 **추가로** 나온 것 (점검 보고서에 없던 항목)

- [x] **H-1 ★ `configs/krx_holidays.yaml`에 2026-12-31 등재** (2026-08-17) — 연말 휴장은
      **관공서 공휴일이 아니라 거래소 고유 휴장**이라 집계 사이트에 안 나오고, 이 파일 헤더는
      *"12-31은 조기폐장이지 휴장 아님"* 이라고 적고 있었다. 같은 파일의 2025-12-31 실측
      (백필 빈 응답 = 완전 휴장)과 이미 어긋난 상태였고, 마흐디가 08-17에 사람이 KRX 안내로
      전수 확인해 `연말 휴장일`로 등재한 것이 세 번째 근거다. 헤더 주석도 함께 정리했다.
      **안 넣었으면 12-31에 수집이 돌아 값이 얼어붙은 하루가 적재된다** — 마흐디가 08-17에
      실측한 바로 그 형태(9,640행 / IV 고유값 30)이고 행 수로는 구분이 안 된다.
- [x] **H-2 ★ `covered_through` + 자가점검 `calendar` 축** (2026-08-17) — 미륵/마흐디 규약
      이식. 종전에는 달력 미갱신이 **2027년 첫 거래일 아침에** 처음 드러났고, 그때 진입점은
      판정 불가를 「거래일」로 접으므로 **신정·설날에 조용히 수집이 돈다.** 이제 사람이
      `covered_through: "2026-12-31"`로 선언하고, `self_check`의 `calendar` 축이 만료 45일
      전부터 매일 경고한다(기동은 안 막는다 — `check_rollover`와 같은 원칙).
      로더도 함께 고쳤다: 종전 `raw.values()`는 메타 키의 문자열을 문자 단위로 순회해 죽었다
      (= 메타 키를 넣는 것 자체가 불가능한 구조였다). 이제 4자리 숫자 키만 데이터로 읽고,
      **깨진 날짜는 여전히 던진다**(메타 키 건너뛰기와 조용한 데이터 손실은 다른 일이다).
- [x] **H-3 판정 불가를 「거래일」로 접는다** (2026-08-17) — `non_trading_day_reason()`이
      `is_trading_day()`의 `ValueError`를 삼키고 사유를 표준출력에 남긴다. 종전 구조에서는
      2027-01-02에 그 예외가 `main()` 밖으로 올라가 **수집이 통째로 죽었다** — 달력 미갱신
      하나로 하루치 체결틱·수급·옵션체인이 영구 소실이고, 그건 이 게이트가 막으려는 것보다
      훨씬 비싸다. 비대칭 방향은 마흐디 `market_calendar.py` 모듈 docstring과 같다.
- [ ] **H-4 2026-11-19 수능 지연개장** (기한 2026-11-12) — 그날 개장이 09:00→10:00이다.
      휴장이 아니라 **세션 시간표가 다른 하루**라 이진 모델(거래일/휴장일)에 안 맞고,
      `SessionHours.first_tick_time`(08:45) 전제가 깨져 화면·리포트가 오전 내내 "봉이 없다"로
      오탐한다. 필요한 것: 날짜별 세션 시간표(`SessionHours`를 날짜로 조회). 지금은
      `configs/krx_holidays.yaml` 헤더에 **알려진 오탐**으로 명기해 뒀다.
      ⚠ 이 항목은 「측정 전까지 버그」 규율의 대상이다 — 문서화만 하고 넘긴 갭이다.
- [ ] **H-5 `stop_l1_daily.bat`에는 게이트를 넣지 않았다** (판단 기록 — 착수 항목 아님) —
      휴장일 15:40에도 뜨지만 **죽일 프로세스가 없어 아무것도 안 한다.** 게이트를 넣으면
      달력이 틀린 날 정리가 안 되는 쪽으로 실패하는데, 이 스크립트는 안전망이라 그 방향이
      틀렸다. 재검토 조건: 이 배치가 휴장일 로그에 잡음을 남기기 시작하면.

### 고도화

- [ ] **G-3 ★ 즉시(장후) — 점검 실행시각 자기채점** — `collect_evidence.py`에 국면-실행
      시각 정합 검사. `--phase intra`인데 15:35 이후면 §0 최상단 경고 + §9 적신호 1번.
      pre>09:00 · post<15:45도 동일. **국면 인자를 무조건 믿지 않는다.**
      근거: 오늘 장중 점검이 16:22에 돌았고(08-14는 12:36) **도구는 아무 말도 안 했다.**
      08-18 D-day에 반복되면 리허설 예보 대비 라이브 이탈을 장중에 못 잡는다.
- [ ] **G-2 이번 주 — `SessionEnd.reason` 분류축** — `reason ∈ {normal, krx_holiday,
      launch_window_refused, symbol_mismatch, crash_recovered, forced_flat}`.
      `core/logging.py` 태그 주석에 허용값 정본화 + `collect_evidence` §2를 reason별 집계.
      **`msg`는 사람용, `reason`은 기계용.** `reason` 부재는 `unknown`으로 세고 그
      건수를 리포트에 **노출**한다(조용히 0으로 세지 않는다). F-1의 절반이 여기 포함.
- [ ] **G-1 다음 단계(G2 40거래일 중) — 날짜→기대치 정본화** — `core/day_profile.py`
      또는 `event_calendar.expected_artifacts(day)`. 소비처: `run_postmarket`(F-2) ·
      `collect_evidence`(F-13) · `daily_integrity_report.py` · UI 신선도 판정.
      **선행 조건: F-2·F-13을 각자 먼저 넣는다** — 소비처가 둘뿐일 때 추상화하면
      세 번째가 안 맞는다. **G-B(두 parquet 로더 시간대 통일)와 같은 병** — 정본이 여러
      곳에 흩어진 것. G-7 계열로 묶어 다룰 것.

### 2026-08-18(화) D-day에 함께 볼 것 (오늘 점검이 추가한 축)

- [ ] **`run_postmarket` `steps_run == 6`** — 6단계 완주. F-2 착수 전제조건.
- [ ] **dailycheck 실행 시각** — pre≤09:00 · intra 12:00~13:00 · post≥15:45.
      **오늘은 셋 다 16:22였다.** 예약 작업 트리거 시각을 장전에 확인할 것.
- [ ] **`git status` 동결 확인** — `git diff --stat -w`(공백 무시)로 `src/`+`scripts/`
      실제 변경 **0파일** 유지 여부. 지금 179건은 전부 CRLF 잡음 + 문서/설정이다.
- [ ] **`status_snapshot.json` 신선도 임계가 "거래일 기준"인가 "경과 시간 기준"인가** —
      `src/messiah/ui/app.py`. 후자면 연휴 3일 뒤 화면이 조용히 낡는다. 오늘 파일은
      08-14 15:34 것이면서 전 컴포넌트 `OK`·`command_center_ui: "UP"`으로 남아 있다.
      커밋 `0b80580`(신선도 임계 유도)에서 이미 다뤄진 영역이라 **판정 보류** 상태.

## 2026-08-17 장전 점검 (휴장) — 관측 결함 3건 + 등록부 기한 재조정 ([MW0601])

**P0 없음. D-day(08-18) 진입 Go.** 코드 동결 확인(`cc93366`→`f3ea02e` 사이 `src/`·`scripts/`
변경 0파일). 오늘은 **한 줄도 적용하지 않았다** — NEXT_TODO "월요일은 아무것도 하지 않는
것이 계획이다" + SYSTEM.md R11·금지계명 2·4. 아래 전 항목 **적용 시점 2026-08-18 장후 15:40 이후.**
보고서: `logs/dailycheck/2026-08-17_pre_report.md`

### Fix (08-18 장후 착수 · 커밋 순서 ①~⑤)

- [x] **F-1 ★ 비거래일 종료 마커** (P1 · 2026-08-17 구현) — 위 「장중 점검」 절 F-1과 같은
      항목이다(두 보고서가 같은 결함을 각자 번호로 올렸다). `reason="non_trading_day"` ·
      `calendar_source="EventCalendar"`까지 이 항목이 지정한 필드 그대로 넣었다.
      **`abnormal_exits` 수정은 불필요했다** — 착수 시 `grep`으로 위치를 확정한 결과
      (`ops/integrity_report._abnormal_exits`, `:555`) 그 함수는 `len(starts) > len(ends)`로
      판정한다. 마커가 생기면 휴장일도 2:2로 균형이 잡히므로 **면제 조건을 새로 넣을 필요가
      없다.** 계획이 예상한 두 번째 수정이 첫 번째 수정으로 사라진 경우다.
      **검증 기한 2026-08-24** → 다음 휴장일은 09-24라 그 전에 달성 불가. 대신 오늘 세 진입점
      실행으로 마커 발행을 직접 확인했고, 최종 위양성 소멸 확인은 **09-24로 미룬다.**
- [x] **F-2 ★ 장후 배치 휴장 분기** (P1 · 2026-08-17 구현) — 위 절 F-2와 같은 항목.
      *"08-18 장후 배치가 5/5 완주한 것을 확인한 뒤에 손댈 것"* 이라는 주의를 **어겼다** —
      이유와 위험을 그대로 남긴다: 넣은 코드는 비거래일 분기에만 있고 거래일 경로는
      `_resolve_symbol()` 이후가 한 줄도 안 바뀌었다(테스트로 순서·exit 3 존속을 못박음).
      그래도 **08-18 장후 `steps_run == 6` 확인은 이 항목의 미해결 부분으로 남는다** — 그날
      숫자가 6이 아니면 이 판단이 틀린 것이고, 되돌릴 지점은 `run_postmarket.main()`의
      게이트 블록 하나다. 태그는 `PostmarketForcedOnHoliday`가 아니라
      `PostmarketForcedOnNonTradingDay`로 넣었다(주말도 같은 경로다).
- [ ] **F-3 스냅샷 신선도** (P1 · 커밋③) — `logs/status_snapshot.json` 최상위에
      `age_seconds` + `snapshot_freshness`(fresh<120s / stale<1거래일 / dead≥1거래일).
      `src/messiah/ui/app.py`는 `dead`면 회색이 아니라 **"N일 전 값"** 명시.
      `collect_evidence.py` §8도 나이를 함께 출력. 임계는 **달력일 아닌 거래일 기준**
      (주말 뒤 월요일마다 `dead`면 늑대소년). 근거: 08-14 15:34 스냅샷이 사흘째 현재 행세,
      `command_center_ui: "UP"`·`pid 12144`가 거짓. **검증 기한 2026-08-24.**
- [x] **F-4 점검기에 거래일 전제** (P2 · 2026-08-17 구현) — 위 절 F-13과 같은 항목.
      **거래일에는 출력이 한 글자도 안 바뀐다**(전 분기가 `skip_note is not None` 아래에만
      있다) — *"08-18 관측을 오늘과 같은 도구로 비교하기 위해 마지막에 둔다"* 는 이 항목의
      전제가 그래서 유지된다.
      미구현으로 남긴 둘:
      · `SessionStart` 2회의 **부팅 트리거 시각대 판정** — 지금은 비거래일에만 기각한다.
        거래일에 부팅 트리거로 2회 뜨는 것도 정상인데 그건 여전히 적신호로 찍힌다
        (`install_scheduled_tasks.ps1:137-140` Weekly+AtStartup). 별도 항목으로 남긴다.
      · `references/phases.md` A절 비거래일 체크리스트 4항 — 도구가 이제 스스로 말하므로
        급하지 않다. 다음 휴장일(09-24) 전에.
- [ ] **F-5 ★ 등록부 기한 재조정** (P1 · 커밋④) — `configs/pending_verifications.yaml`:
      `daily-axes-measured` 08-19→**08-24** · `composer-bucket-completeness` 08-19→**08-24** ·
      `no-degenerate-features` 08-20→**08-25** · `exit-code-matches-log` 08-21→**08-26**.
      연속일 카운터는 리셋하지 않는다(이미 0). **각 항목에 연장 사유 주석 필수** —
      `# 2026-08-17 휴장으로 잔여 거래일 1일 감소, 기한 내 충족이 산술적으로 불가`.
      머리말에 **"연장은 1회로 제한, 다음에 또 닿으면 항목을 닫고 원인을 상위 승격"** 명기.
      **`exit-code-matches-log`는 08-18 장후에도 `None`이면 연장이 아니라 지표 교체** —
      F-D 재시도(2/2회)가 들어간 뒤에도 실패한 것이므로.

### 고도화 (D-day 직후 · G-A·G-B와 같은 묶음)

- [x] **G-1 진입점 3종의 "물러섬"을 한 관례로** (2026-08-17 구현) — 이 항목의 권고
      (*"F-1·F-2를 두 번 고치지 않으려면 처음부터 이 형태로"*)를 따랐다. 세 진입점이 같은
      두 함수를 부른다: `session_guard.non_trading_day_reason()`(판정) +
      `announce_non_trading_day()`(마커). 이제 셋의 문구·마커·종료 코드가 같다.
      **`ops/entrypoint.py`를 새로 만들지 않고 `ops/session_guard.py`에 넣었다** — 그 모듈이
      이미 "언제 돌려도 되는가"를 소유하고(`refuse_if_regular_session`·`launch_window_verdict`),
      세 진입점이 이미 그것을 import한다. 새 모듈을 만들면 진입점이 물어야 할 곳이 둘로
      갈리고, "같은 질문은 한 곳에"라는 이 항목의 취지와 정반대가 된다.
      반환 규약도 계획의 `bool`이 아니라 `str | None`으로 했다 — 사유 **문장**이 판정과 함께
      나와야 마커와 콘솔이 같은 말을 한다(`bool`이면 문장을 호출측이 다시 만든다 = 세 벌).
- [ ] **G-2 ★ 등록부를 달력일이 아니라 거래일로 센다** — `pending_verifications.yaml`에
      `deadline_trading_days: <n>` 신설(둘 다 있으면 거래일 기준이 이긴다).
      `src/messiah/ops/fix_verification.py`가 잔여 거래일 < 필요 연속일이면
      **`기한 초과`가 아니라 `기한 불가 — 재조정 필요`** 를 낸다. 두 상태는 다른 뜻이고
      다른 대응을 부른다. 관측 근거: 오늘 휴장 하루로 4항목 기한이 흔들렸고 2건이 충족 불가.
      **위험: `fix_verification.py`는 채점기 자신이다** → 신규 판정을 먼저 리포트에만 표시(섀도)
      하고 3거래일 관측 후 자동 재조정에 연결(R18의 정신).
      **우선순위: G2 40거래일 관문 중** — 40거래일 카운트 자체가 같은 함정에 있다.
- [ ] **G-3 비거래일 빈 무결성 리포트** (F-2와 같은 커밋) — 휴장에도
      `{"date":..., "non_trading_day": true, "reason": "KRX 휴장(...)"}` 최소 리포트를 남기고,
      채점기는 그 날을 연속일 카운터에서 **중립**(끊지도 세지도 않음) 처리. 관측 근거:
      오늘 `daily_integrity_20260817.json`이 **아예 없다** — "파일이 없는 것"과 "쟀는데 값이
      없는 것"이 이력을 읽는 쪽엔 똑같다(L18). 1차는 중립 고정, 항목별 예외는 필요할 때.

### 2026-08-18(화) D-day에 볼 것 — 오늘 장전 점검이 새로 다는 관측

기존 W-16 · W-21 · W-26 · meta 통과확률 분포는 08-16 기록 그대로 유효하다. 아래는 추가분.

- [ ] **PRE-1 ★ 장전 보고서가 09:00 이전에 나오는가** — 오늘 08:45 예약이 **16:22**에 돌았다
      (세 국면 점검이 동시 기동). PC는 살아 있었다(07:22 부팅 트리거 + 08:20/08:25 정시
      트리거 정상). 원인은 리포 밖(Cowork 스케줄러)으로 보이며 **판정 보류.**
      또 늦으면 예약 실행을 Windows 작업 스케줄러로 이관 검토.
- [ ] **PRE-2** `logs/status_snapshot.json` `generated_at_kst`가 08-18 장중 값으로 갱신 ·
      `code_version.head_git_sha == f3ea02e` · `stale=false`(이번엔 진짜).
- [ ] **PRE-3** `logs/ui_20260818.log` 존재 · `ui_20260818.err.log` 0바이트.
      **휴장으로 3일 미기동 후 첫 기동이다.**
- [ ] **PRE-4** 기동 자가점검 `clock offset` — \|offset\| < 1.5s면 안심.
      **1.9s 이상이면 `clock-sync-restored` 재발 임박.** 오늘 실측 +1.787s(07:22) /
      +1.661s(08:20), 08-13 실측 **1.922s = 임계 2.0s의 96%**. 5거래일 전부 통과했으나
      추세가 임계를 향한다. 완성봉 유예 500ms의 3배 이상이나 `bar_close`가 timer 구동이라
      직접 영향은 **로그로 미확인** → `delivery_latency` p99와 함께 볼 것.
- [ ] **PRE-5** `postmarket_20260818.log` 5/5 완주 · `SessionEnd` 정상.
      **완주 확인 후에야 F-2 착수.**
- [ ] **PRE-6** `daily_integrity_20260818.json`의 `unmeasured` 0건 ·
      `task_exit_codes.available: true`. 또 `TimeoutExpired`면 F-5의 "지표 교체" 확정.
- [ ] **PRE-7** `degenerate_feature_count`가 08-14의 **57**에서 감소.
      0이면 08-16 P0-1(웜스타트 적재 필터)이 들었다는 강한 증거.
- [ ] **PRE-8** 등록부 4건이 `기한 초과`로 뜨는지 — 뜨면 F-5가 늦은 것이므로 그날 즉시 적용.
- [ ] **PRE-9** dev_memory 병합 확인 — 오늘 세 점검 세션이 `DECISION_LOG.md`·`NEXT_TODO.md`에
      동시 append했다. 유실·중복 여부를 D-day 아침에 육안 확인.

### 순서는 바꾸지 않는다

0순위(왕복 비용 실측) → 1순위(레이블 기하 교정) → 2순위(G1 재도전) → 3순위(Position
Reconciler)는 2026-08-16 레이블 기하 재측정이 정한 순서다. **오늘 관측에 그 순서를 바꿀
근거는 없다.** 위 F/G 항목은 전부 관측 계층이며 그 순서 **바깥**에 놓는다.

## 2026-08-17 장후 점검 — 관측기가 자기 판정을 얕게 한다 ([MW0601])

**전부 D-day(2026-08-18) 이후 착수.** 코드 동결(2026-08-16 선언) 유지 — P0 없음, D-day 관측을
망치는 항목 없음. **장전·장중 보고서의 적용 시점 판단과 일치한다.**
전문 `logs/dailycheck/2026-08-17_post_report.md`.

> **중복 방지**: 휴장 조기 종료의 `SessionEnd` 부재 · 장후 배치의 휴장 캘린더 미참조 ·
> `collect_evidence` 휴장 미인지 · `status_snapshot` 신선도 · 등록부 기한 재조정은
> **오늘 장전·장중 보고서가 이미 F-1~F-5 / G-1~G-3으로 올렸다.** 아래는 장후 신규만 적는다.

### D-day 아침 대응 (코드 변경 없음 · 필독)

- [ ] **08:20 자가점검의 `[OK ] postmarket 20260817 장후 배치 정상 종료 확인` 한 줄을 믿지 않는다.**
      실제는 「휴장일이라 0/6단계 중단 · exit 3」이며 **이는 정상이다.** `check_prev_postmarket`이
      SessionEnd 유무만 보기 때문에 나오는 문구(장후 P1). **Go/No-Go 판단 재료에 넣지 않는다.**
- [ ] **`git diff --stat -w --ignore-cr-at-eol`이 비어 있는가** — 동결 확인의 **정본 명령**.
      `git status`의 179건은 전량 CRLF 인공물이며 오늘 이 명령으로 확정했다.
- [ ] **08:45에 장전 보고서가 09:00 전에 나오는가** — 예약 일괄 지연의 판정(아래 결론 ③).
- [ ] **UI 기동 직후 스냅샷 신선도 표시** — 연휴 뒤 첫 기동이 판정 순간이다(아래 결론 ⑤).

### Fix — 장후 신규

- [ ] **F-P1 (P1) 장전 자가점검이 「끝까지 갔는가」를 실제로 묻는다** —
      `scripts/self_check.py:235-275`의 `ended = '"tag": "SessionEnd"' in text`를
      **마지막 `SessionEnd` JSON 파싱**으로. 판정 4분기: ①마커 없음→종전 경고
      ②`msg=="휴장 생략"`→`[OK] {stamp} 휴장 — 장후 절차 생략(정상)`
      ③`steps_run < steps_planned` 또는 `steps_failed>0`→**경고**
      (`{stamp} 장후 배치가 {steps_run}/{steps_planned}단계에서 멈췄다`) ④완주→종전.
      **기동은 여전히 막지 않는다**(`CheckResult(True)` 유지 — docstring 원칙 불변).
      파싱 실패 시에도 `True`(자가점검이 자기 파서로 아침을 막지 않는다).
      **회귀 위험**: 합쳐진 stdout에 자식 `daily_integrity_report.py`의 `SessionEnd`가 섞인다
      (**F-5가 지적한 함정 ②의 사촌**) → `"process": "postmarket"`인 줄만 골라 마지막을 쓴다.
      pytest 4종(08-17 실물·08-14 실물·마커 없는 옛 로그·자식 마커 혼입).
      **커밋 순서: 휴장 마커(장전 F-1) 다음** — 분기 ②가 그 마커를 전제한다.
- [ ] **F-P2 (P2) `abnormal_exits`의 면제를 이력 보호에서 분리한다** —
      `src/messiah/ops/integrity_report.py:555`에서 `if not starts or not ends`의 **`not ends` 제거**,
      상수 `_SESSION_MARKER_SINCE = date(2026,8,8)`로 `if day < _SESSION_MARKER_SINCE: return []`.
      **근거: `SessionEnd`는 하루 한 번뿐이라 「장중에 죽어 안 돌아온 날」이 정확히 `ends=0`이 되어
      면제된다 — 잡으려는 사고가 면제 조건과 같은 모양이다.**
      기존 `lost <= bar_tail_gap_minutes`(20분) 임계가 휴장일(마커 fix 후 `ends=2`)과 장중 사망을 가른다.
      **회귀 오해 방지**: 과거 리포트는 `daily_integrity_*.json`에 이미 굳어 있어 소급 재계산 없음 →
      `no-silent-process-death`의 「5거래일 검증 완료」는 안전하다. **착각해서 미루지 않는다.**
      **순서: 장전 F-1(휴장 마커) → F-P2.** pytest 3종. **기한 2026-08-21.**
- [ ] **F-P3 (P2) 등록부가 「채점할 날이 없었다」를 스스로 말한다** —
      `src/messiah/ops/fix_verification.py`에 `VerificationStatus.UNSCORABLE` 신설.
      `_verdict_for()`의 `OVERDUE` 분기 **앞**에서 `judged_days` 수 < `consecutive_days`면
      **「기한 {deadline}까지 채점 가능한 거래일이 {n}일뿐이었다 (필요 {m}일) — 기한 조정 필요」**.
      시그니처에 `judged_count` 추가(`evaluate()`가 넘긴다). `integrity_report.py:2108` 부근 요약은
      `⏳`로 내고 **재발 목록에 섞지 않는다.** `STALLED`와 겹치면 **`STALLED` 우선**(계측 고장이 더 급하다).
      → **장전 F-5(기한 재조정)·장전 G-2(기한을 거래일로)와 한 커밋으로 묶는다.**

### 고도화 — 장후 신규

- [ ] **G-P1 ★ 「진입점이 정본을 쓰는가」를 정적으로 센다** — `scripts/check_canonical_callers.py`
      신설(AST). `configs/canonical_rules.yaml`에 「질문 → 정본 함수 → 호출 의무 진입점」
      (예: `is_trading_day` ← `run_l1_daily`·`run_g2_paper_trading`·`run_postmarket`).
      `pytest`에 붙여 CI 없이 매 커밋 전에 돈다. `canonical_consumer_gaps`(런타임)의 **정적 판본**.
      **근거: 같은 형태를 5회 당했고 전부 사후 발견이었다** — `is_expiry_day()` 휴장 보정
      (`DECISION_LOG:1593`) · `next_weekly_expiry()` 관례 둘(`:1721`) · 두 parquet 로더 시간대(G-B) ·
      `front_month` 하드코딩(08-14 F-A) · **오늘 장전 1-2.**
      **오늘 장전 1-2는 이 검사가 있었다면 08-14 F-A 커밋에서 실패했을 것이다.**
      규칙 추가를 **결함 발견 시의 의무**로 만든다(fix 계획에 「이 질문을 규칙에 넣었는가」 한 줄).
      R18 섀도 계측 불필요(코드 검사이고 운영 판단에 개입하지 않는다).
      → **G-7(정본 하나) 계열의 집행 수단으로 승격.** 그 계열이 지금 실행 주체가 없다.
- [ ] **G-P2 점검 국면의 관측창 강건성** — 보고서 머리에 `설계시각 → 실행시각 → 지연`을 규격화하고
      (장전 보고서가 오늘 손으로 한 것), **지연이 국면 창을 넘으면 그 국면의 실시간 항목을
      「관측 불가」로 명시**한다. 예: 장중 점검이 15:35 이후에 돌면 「지금 설계대로 돌고 있는가」는
      답할 수 없고 「돌았는가」로 질문이 바뀐다 — **다른 질문에 답하면서 같은 제목을 달지 않는다.**
      **근거: 오늘 세 예약이 16:22에 뭉쳐 떴고 장후만 피해가 없었다**(완결 파일을 읽으므로 강건).
      dailycheck 예약을 `install_scheduled_tasks.ps1`(현재 4종)에 넣는 것은 **「검토」로 둔다** —
      Windows로 옮기면 Cowork 실행 환경(스킬·MCP)을 못 쓴다. 창 만료 표기부터 한다.
      예약 이관 판정은 **D-day 08:45 결과 이후.**

### 선행 보고서 「확인 필요」 — 오늘 장후가 결론 낸 것

- [x] **무결성 리포트 부재의 하류 효과 — 정상 처리다. 연속 카운터는 끊기지 않는다. [종결]**
      `load_reports()`가 정본 규격 파일이 **있는 날만** 담고(`fix_verification.py:652`),
      `evaluate()`가 **그 키만** 순회한다(`:679`). 휴장일은 통과로도 위반으로도 `unjudged`로도
      안 센다. `_trading_days_since()`도 *"리포트가 있는 날을 센다"* 고 명시.
      → 장전이 D-day로 이월한 항목을 **오늘 닫았다.**
- [x] **미커밋 179건 — 실질 0건. 금지계명 10 위반 아님. [종결]**
      장중 권고 명령을 오늘 실행 → `git diff --stat -w --ignore-cr-at-eol`이 dev_memory 2파일만 낸다
      (이 점검의 append). `--numstat`에서 178파일 전부 「추가 = 삭제」. 자가점검 `[OK] git clean`이 정본.
- [ ] **③ 예약 지연 — 「스케줄러 일괄 지연」으로 확정, 원인은 리포 밖. [부분 · D-day 판정]**
      세 증거 파일이 전부 16:22 → 한 기동이 세 국면을 연달아 돌렸다. 호스트는 07:22부터 살아 있었고
      Windows 작업 4종은 정시 기동. **장후만 창을 잃지 않았다.** → **08-18 08:45 판정 · 우선순위 상향.**
- [ ] **④ 시계 오프셋 — 오늘 값이 우려를 지지하지 않는다. 우선순위 하향. [부분]**
      오늘 4회 1.787·1.661·1.787·1.649s. 이력의 08-13 **1.922가 단발 최댓값**이고 임계(2.0s)를 향한
      단조 증가 아님. `w32time=Running` 4회. **한계: 휴장일은 무부하라 거래일보다 낙관적일 수 있다.**
      → P1 아님. D-day에 `delivery_latency` p99와 함께 관측.
- [ ] **⑤ `status_snapshot` 신선도 소비자 — 오늘 실피해 0. [부분]**
      UI 미기동(`ui_20260817.log` 부재 · `command_center_ui.json` 08-14)이라 낡은 값을 그린 순간이
      없었다. UI는 거래일 확인 직후에 뜨는 설계(`DECISION_LOG:1007`)대로 작동.
      → `app.py` 신선도 임계가 「거래일 기준」인가 「경과 시간 기준」인가는 **08-18 08:20 UI 기동 직후 판정.**

### ⚠ 등록부 기한 산술 정정 (장전 표의 셈을 다시 했다)

판정은 `today > item.deadline`(**달력 비교**)이라 기한일 자체가 포함된다 →
기한 **08-19 = 2거래일** · **08-20 = 3일** · **08-21 = 4일**(08-18·19·20·21).

- [ ] **산술적 불가(필요 3 > 잔여 2)** — `daily-axes-measured`(08-19) ·
      `composer-bucket-completeness`(08-19) · **`ui-restart-observability`(08-19) ← 장전 표 누락**
- [ ] **여유 0(필요 3 = 잔여 3)** — `no-degenerate-features`(08-20) ·
      **`archiver-restart-restore`(08-20) ← 장전 표 누락**
- [ ] **여유 1** — `regime-not-constant`·`exit-code-matches-log`·`truncation-is-visible`·
      ...
      `launch-window-refusal-not-counted` (전부 08-21). **장전 표가 `regime-not-constant`를 안 세었다**
- [x] `clock-sync-restored`(08-19)는 **기한 무관** — 08-14에 7거래일 연속으로 이미 검증 완료
- [ ] **결정: 산술적 불가 3건의 기한을 지금 미루지 않는다.** `daily-axes-measured`·
      `composer-bucket-completeness`는 08-16 P0-1(웜스타트 적재 필터)의 하류일 가능성이 있고
      **그 처방이 먹었는지가 D-day에 처음 채점된다.** 먹었으면 짧게 미루고, 안 먹었으면 기한이
      아니라 처방을 다시 본다. **오늘 미루면 그 구분을 잃는다.**

### 2026-08-18(화) D-day에 오늘 발견이 채점되는 것 (예보 — 틀리면 그 자체가 새 정보)

- [ ] **P-1** 08:20 자가점검에 `[OK ] 20260817 장후 배치 정상 종료 확인`이 **나온다** —
      나오면 장후 P1 확정, 안 나오면 코드 독해가 틀렸다
- [ ] **P-2** 08:45 장전 보고서가 **09:00 전에** 나오는가 (결론 ③의 판정)
- [ ] **P-3** `git diff --stat -w --ignore-cr-at-eol`이 **빈 출력**(dev_memory append 제외)
- [ ] **P-4** `daily_integrity_20260818.json`의 `abnormal_exits`가 **`[]`** —
      08-17의 `ends=0`이 08-18 리포트에 새지 않는가
- [ ] **P-5** `task_exit_codes.exits`의 `Messiah-Postmarket`에 **08-17의 exit 3이 섞이지 않는다**
      (`task_exit_codes.py:236` 하루 창). 섞이면 영향 범위가 커진다
- [ ] **P-6** 연속 카운터가 휴장일에 **끊기지 않는다** — `regime-not-constant`가 08-18에 통과하면
      **`1/3`** 이어야 하고 `0/3`이면 위 [종결] 결론이 틀렸다
- [ ] **P-7** 산술적 불가 3건이 **08-19에 「기한 초과」로 뜬다** — 그 문구가 「못 고쳤다」로 읽히는
      것이 F-P3의 근거다
- [ ] **P-8** `clock_skew_seconds` + `delivery_latency` p99 — **거래일 부하 상태의 값이
      오늘(휴장·무부하) 1.65~1.79s보다 나쁜가**
...
> 자리는 맞았고 원인 진단이 두 번 틀렸다」. 전체 pytest 2,204건 통과 · ruff 통과.
...
### Q7 (c) 결정에 따른 F-G 2단계 완료 (2026-08-20 저녁)

- [x] **F-G 2단계 — 값 전환 + 번들 재학습** (`f15aa58`). 인접 종가 차분 8개 자리를 세션 경계
      인식으로 전환하고 `real-20260820-2053-30m`을 live로 세웠다(구 챔피언 retired 보존).
      실측: `px_max_ret_60` distinct value에서 갭 인공물 2개 제거 · `feature_dependency`
      0.1186 → 0.0900 개선
- [x] **등록부 `no-degenerate-features.fix_committed = f15aa58`** — 이제 위반이 「기전 미상」이
      아니라 「재발」이다(G-H 축의 첫 실사용)
- [x] **승격 가드 사각지대 명문화** — `--supersede-reason`. 「성적으로 하는 교체」와
      「입력 정의가 바뀐 교체」를 가른다. `operator` 가드를 함수 안에도 뒀다(테스트가 잡은 구멍)
- [ ] **G-F — 세션 경계를 피처 계약으로 승격** (선행 조건 F-G 2단계 **충족됨**).
      창 길이 × Horizon > 410분인 피처를 기동 시 자동 열거 → self-check 항목.
      오늘 기준 `W_STD=(5,20,60)` 조합 18개 중 4개 해당. 처음 20거래일은 WARNING만(R18)
- [ ] **모델 산출물이 커밋에 없다** — `data/`가 `.gitignore`라 번들·레지스트리가 이 PC
      로컬에만 있다. 복제 배포(SYSTEM.md §4-6) 시 걸린다. **미해결, 별건으로 검토**
- [ ] **P-1 재판정** 08-21 장후 `degenerate_feature_count` **0** · 등록부 「검증 완료」 복귀.
      0이 아니면 이번엔 진짜 「재발」이다 — 원인을 다시 봐야 한다
- [ ] **M-8** 08-21 첫 사이클 `FeaturePublish`의 피처 값이 전환 전과 다른 대역인지 육안 확인.
      같으면 새 정의가 안 실린 것(= 재기동 누락)

---

## 2026-08-20 장후 후속 — J-3b 처리 ([MW0601])

- [x] **J-3b 미충족 확인 → 수정 완료** — 08-19가 롤링 창에서 안 빠지고 있었다
      (`excluded_days=[]`). `incomplete_days.load()`가 저장된 불리언이 없으면
      `series_coverage`로 판정을 **계산**하도록 바꿨다(`_derive_from_stored`).
      저장 파일은 안 건드린다. 실측: 3거래일 창 `excluded=[08-19]` ·
      파생이 08-07/08-10/08-14/08-19를 독립 재발견(전부 DECISION_LOG의 알려진 소실일)
- [x] **테스트 격리 누수 수정** — `feature_health_rolling.judge()`의 `log_dir` 기본값을
      누적 파일의 부모로. tmp 픽스처 테스트가 저장소의 진짜 `logs/`를 읽고 있었다.
      운영에서는 `DEFAULT_PATH.parent == logs` 라 동작 동일
- [x] **L-1·L-2·L-4·L-6 통과 · L-3 부분(재발 1건, 이후 `f15aa58`로 원인 해소) ·
      L-7 미충족(p90 921.4ms < 1,000ms)** — 16:08 장후 점검 채점과 전부 일치

### 새로 도출

- [ ] **N-3 (P2) 새 축을 켠 첫날 `absent`가 등록부를 때린다** — 재계산 시
      `daily-axes-measured`가 위반으로 뜬다. 원인은 `FeaturePublishOffset` 축(`64971cd`)이
      15:45 배치 이후 유입돼 오늘 데이터가 없는 것. 08-18 F-0818P-2가 `accruing`에 대해
      푼 것과 **같은 문제의 `absent` 판**이다 — 「계측 이전」 갈래가 필요하다.
      08-21 정상 기동이면 자연 해소되므로 급하지 않으나, 축을 켤 때마다 반복된다
- [ ] **N-4 (P1) 「저장 상태만으로 도는 검증」을 규율로 세운다** — 어제 J-1~J-4를 전부
      리플레이로 확인했는데 리플레이는 「지금 계산하면」을 묻지 「저장된 것을 읽으면」을
      묻지 않는다. 두 질문이 갈리는 자리가 **과거 산출물을 입력으로 쓰는 축**이고,
      J-3b가 정확히 거기서 실패했다. 그런 축을 넣을 때 저장 상태 전용 검증을 의무화
- [ ] **N-5 (P2) `vol_scorecard` 20거래일 창 재산출 여부 결정** — 08-07/08-10/08-14/08-19
...
- [x] **시스템 시계 +8.6초 → 해소** — 2026-08-23 22:14 `w32tm /resync /force`로 +8.591초
      점프 세팅(Kernel-General 이벤트 1 확인). 원본 `time.nist.gov` · offset +0.004s ·
      자가점검 `[OK] clock`. **2026-08-24 아침 기동 거부 위험 해소.**
- [x] **드리프트 근본 원인 — 딥다이브 완료 + 처방 적용 (2026-08-23 22:00~22:35)**

      **① 속도의 정체: 부팅마다 부호가 뒤집히는 ±40ppm.** 10분 21표본 스트립차트
      (22:01:48 +8.6259s → 22:11:20 +8.6032s, 선형)로 이번 부팅 세션은 **+39.6ppm 빠름**
      (하루 +3.4초). 반면 08-21 장중 실측은 −42ppm 느림(스큐 +531ms/3.5h). 클럭 속도가
      공칭 0.0156250s 그대로라 소프트웨어 슬루는 없다 — **부팅 시 TSC 주파수 보정 오차**다.
      수정 노화·온도로는 이틀 새 부호 반전을 설명 못 한다. `HypervisorPresent: True`
      (WSL2/Docker: vmcompute·WslService Running · hypervisorlaunchtype Auto) — Windows가
      Hyper-V 루트 파티션 위에서 돌고 시간원이 하이퍼바이저 참조 카운터라, 이 조합(Ryzen
      4650G + Hyper-V)의 알려진 형태다. **가속은 없었다** — "+3초/시간"은 리부트를 사이에
      둔 두 자가점검 값을 비교한 오측이었고, 실제 속도는 3주째 ±40ppm으로 일정하다.

      **② 꺼진 동안: RTC(CMOS) 크리스털 ~40-45ppm 느림.** 08-21 16:27 동기(이벤트 35)
      이후 켜져 있던 시간은 합계 ~3시간뿐인데 +8.6초가 쌓였다 → 오프 구간(~51h)이 ≥8초
      기여 = −44ppm 상당. 2026-08-04 기록 "하루 4~5초씩"과 일치 — 같은 성질이 계속이다.
      오늘 두 번의 켜짐(09:53·20:53)은 부팅 이벤트(Kernel-General 12)가 없다 —
      **빠른 시작(최대 절전 리줌)**이라 매번 RTC에서 시각을 되읽는다.

      **③ NTP가 못 잡은 이유 (4중 자물쇠):**
      ㉠ `SpecialPollInterval=32768s(9.1h)` + 부팅 직후 1표본으로는 소스 선정 불가
        (`ValidDataCounter: 0` 실측) → **두 번째 폴(9.1h 후)에야 동기**. 08-20 15:46 ·
        08-21 16:27 동기 이벤트가 정확히 부팅+9:06:08 — 평일 장 마감 직후 하루 1회가
        전부고, 주말·짧은 세션은 0회.
      ㉡ 주말 누적 8.6s > `LargePhaseOffset` 5s → 스파이크 취급으로 추가 거부.
      ㉢ UDP 123 간헐 손실 — 스트립차트 24표본 중 4 타임아웃(~17%).
      ㉣ 연속 규율이 없어 **주파수 보정을 한 번도 학습 못 함**(클럭 속도 영구 공칭) —
        ±40ppm이 날것으로 노출된다. 건강한 상태면 w32time이 이 정도는 흡수한다.
      부수: `UtilizeSslTimeData=1` (Secure Time Seeding ON) — 리줌 시 SSL 기반 시각
      보정이 켜져 있어 예측 불가한 스텝 요인.

      **적용한 처방 (2026-08-23 22:20~22:35 · 백업 `logs/w32time_backup_20260823-2220/`):**
      1. `SpecialPollInterval` 32768 → **1024초**. 처음 900을 넣었더니 서비스가 1024로
         보고했다 — `MinPollInterval=10`(2^10=1024초)이 **하한**이라 클램프된 것이다.
         레지스트리가 거짓말하지 않도록 실효값 1024로 맞췄다(다음 사람이 900을 보고 900을
         믿으면 안 된다). 폴 간격 1024초면 ±40ppm에서도 폴 사이 드리프트가 **±41ms** —
         완성봉 유예 500ms의 12분의 1이다.
      2. `UtilizeSslTimeData` 1 → **0**. Secure Time Seeding 차단(리줌 시 SSL 기반
         시각 스텝이라는 예측 불가 요인 제거).
      3. 피어 2 → **4개** (`time.windows.com` `time.nist.gov` `time.bora.net`
         `time.google.com`, 전부 `0x9` = Client + SpecialInterval). UDP 123 손실
         ~17% 실측에 대한 분산.
      4. **`Messiah-ClockResync` 평일 08:10 등록** — 콜드 부팅·리줌 직후 소스 선정 전
         ~20분이라는 남은 구멍을 막는다(08:15 기동 창보다 5분 앞).
         `w32tm /resync`가 비관리자에게 `0x80070005`(액세스 거부)를 돌려주는 것을 실측하고
         (`runas /trustlevel:0x20000`), 정본에 `run_as_system: true` 필드를 신설해 그
         작업만 SYSTEM/Highest로 등록한다. **손으로 등록하지 않았다** —
         `configs/scheduled_tasks.json` → `install_scheduled_tasks.ps1` 경로를 그대로
         탔으므로 `check_schedule_drift`가 이 작업의 08:10도 매일 대조한다(F-5 확장분이
         적용 당일 다섯 번째 대상을 얻었다). 수동 실행 실측 `rc=0`.
      **결과**: `+8.638s` → `w32tm /resync /force` 점프 세팅 → 자가점검
      `[OK] clock offset=-0.030s` · 원본 `time.nist.gov`.
      **관측**: F-13의 30분 주기 `ClockSkewMeasured.delta_seconds`가 2026-08-24부터
      이 ±40ppm을 직접 보여준다(30분에 ±72ms 대역이면 진단 일치 · C-1과 겸사 채점).
      **안 한 것**: 부팅 시 TSC 보정 오차(±40ppm 부호 반전) 자체는 안 건드렸다 —
      Hyper-V 루트 파티션 구성을 바꾸는 일(WSL2/Docker 포기)이고 이 저장소는 Docker에
      Redis를 얹는다. 대가가 처방보다 크다. 1024초 규율이 그 오차를 ±41ms에 가두므로
      **증상을 잡는 것으로 충분하다고 판단**했다.
...
## 2026-08-23 판단 모델 조사 — 주문 0건의 원인은 모델이 아니라 산술 한 줄이었다 ([MW0601])

> 2026-08-21 리포트 사용자 조치 3번(㉠ 관문 합격선 되돌리기 / ㉡ 성과 3종 측정 / ㉢ 재학습)에
> 답하려고 ㉡을 준비하다가, **세 선택지 전부가 잘못된 전제 위에 있었다**는 것을 찾았다.

### 완료 처리

- [x] **`edge` 산식 3-클래스 정정** — `strategy/pipeline.py`. 종전
      `edge = max(0, min(1, 2*confidence - 1))`은 **이항 승부의 배당 공식**인데
      `confidence`는 3-클래스(UP/FLAT/DOWN) 모델의 방향 확률 하나다
      (`meta_decision.py`: `confidence = agg_p_up if LONG else agg_p_down`).
      **FLAT 확률 전체가 불리한 쪽으로 계산됐다.** 삼중장벽 30m의 FLAT은 실측 76.3%
      (`models/labeling.py`)라 `p > 0.5`가 구조적으로 어렵고, 클램프가 매번 정확히 0을 냈다.
      지금은 `_directional_edge()` = `p_favorable - p_adverse`, 음수 클램프도 제거.
- [x] **`SimBroker` 실현손익(틱)** — `_apply()`가 포지션만 갱신하고 `_cash`를 안 건드려
      **2,000틱을 먹고 청산해도 0원**이었다(실측). 단위는 **틱**이다 — 원으로 바꾸려면
      계약 승수(원/지수포인트)가 필요한데 이 저장소에 없고, 없는 상수를 코드가 지어내는
      것이 R4가 금지하는 그것이다. `computes_pnl` · `pnl_unit` 선언 신설.
- [x] **평균단가 덮어쓰기 결함 동시 수정** — `_apply()`가 매 체결마다
      `avg_price_ticks=price_ticks`로 통째로 덮어, 같은 방향 물타기 시 원래 진입가가
      사라졌다. 손익을 계산하지 않던 동안에는 드러날 수 없던 결함이다. 지금은 수량가중평균.
- [x] **거래 건수 계측** — `SimBroker.n_accepted_orders`/`n_fills`/`n_expired_orders`,
      `WindowResult`에 전달. 자본 곡선만 보면 「거래하고 본전」과 「한 번도 거래 안 함」이
      같은 값이다 — 실전 17거래일 주문 0건이 정확히 그 모양으로 숨어 있었다.
- [x] **G1이 못 재는 관문을 PASS로 찍지 않는다** — `run_g1_walk_forward.py`.
      손익 단위가 틱이라 `max_drawdown`(자본 대비 비율)은 채점 불가 → 미측정으로 보고.
      `cost_adjusted_sharpe`(척도 불변)와 `negative_window_ratio`(부호만)는 채점한다.

### 증거 — 왜 「모델이 나쁘다」가 아닌가

**두 번의 거절이 소수점까지 같다.** 라이브 전 이력에서 리스크 엔진까지 도달한 사이클은
2건뿐이고 둘 다 이렇게 끝났다:

    20260818 RiskReject Net ER -1.62틱 <= 0 (Ver 1.1 §4-2)
    20260821 RiskReject Net ER -1.62틱 <= 0 (Ver 1.1 §4-2)

날짜·신호·ATR이 다른데 사유가 동일하다. `edge`가 0이면 `net_er = edge x ATR - 비용`이
`-비용`으로 붕괴하고 비용은 거의 상수이기 때문이다. **신호가 0에 곱해진 지문이다.**

2026-08-21 15:30 실제 사이클(`logs/pass_cycles/2026-08-21T153000_A05609.json`):

| | 값 |
|---|---|
| `agg_p_down`(=confidence) | 0.4873 |
| `agg_p_up` | 0.1473 |
| p_flat | 0.3654 |
| ATR / 왕복 비용 | 48.93틱 / 1.62틱 |
| **종전** `2x0.4873-1` -> clamp | **edge 0.0** -> net ER **-1.62틱** -> 거절 |
| **지금** `0.4873-0.1473` | **edge 0.34** -> net ER **+15.01틱** |

### 갈래 분포 — 리포트의 전제와 다르다

| 날짜 | 갈래 |
|---|---|
| 08-13 | score 13 · regime 1 |
| 08-14 | regime 14 (롤 당일) |
| 08-18 | **pass 1** · score 12 · regime 1 |
| 08-19 · 08-20 | no_expert 9 · 14 |
| **08-21** | **pass 1** · score 13 — 최대 \|S\| **0.193** (임계 0.20) |

`no_expert`는 08-19·20에만 있었고 **지금은 해소**됐다. 현재 상태는 신호가 임계 바로
아래(0.190 · 0.193 vs 0.20)에 몰려 있고, 유일하게 넘은 것이 `edge=0`에 걸렸다.

### 세 선택지 재평가

- **㉢ 재학습 — 반대.** 모델은 이미 방향 확률 0.487짜리 신호를 냈다. 그걸 0으로 만든 것은
  파이프라인 산술이다. 재학습해도 3-클래스 확률이 0.5를 넘는 일은 구조적으로 드물어
  같은 벽에 다시 막힌다.
- **㉠ score 임계 재추정 — 지금은 무의미.** 0.20을 낮춰 13건을 통과시켜도 전부 같은
  `edge=0` 벽에서 잘린다. RiskReject 건수만 는다.
- **㉡ 성과 측정 — 이제 가능해졌다**(위 손익 작업). 단 `max_drawdown`은 계약 승수가
  정해질 때까지 미측정이다.

### 아직 안 고친 것 — 둘 다 사람 결정이 필요하다

- [ ] **배리어 폭 미반영** — 배리어를 치면 실제 지불은 `width_atr_mult x ATR`이고 30m은
      **2.0배**다(`labeling._WIDTH_ATR_MULT`). 호출부는 `edge x atr_ticks`로 1.0배를 쓴다.
      고치면 30m 기대값이 **두 배**가 된다 — 위험 성향을 바꾸는 변경이라 사람이 정할 일이다.
      지금은 기대값을 **과소평가**하는 방향이라 안전 쪽으로 틀려 있다.
- [ ] **ATR 축 불일치** — 파이프라인의 `atr_ticks`는 **1분봉** ATR인데
      (`handle_bar()`가 M1만 담는다) 레이블 기하의 ATR은 그 Horizon 봉으로 잰다. 축이 다르다.
- [ ] **계약 승수(원/지수포인트) 정본화** — 이 값이 있어야 `max_drawdown`을 자본 대비
      비율로 채점할 수 있다. `configs/instance.yaml`에 `futures_tick_size: 0.02`는 있지만
      승수는 없다. **이 숫자는 사람이 확인해서 넣어야 한다** — 코드가 지어내면 안 된다.

### 다음 거래일 관측 (C 시리즈에 추가)

- [ ] **C-16** `edge` 수정 후 실제로 주문이 나가는가 — `OrderSubmit`/`Fill` 건수.
      17거래일 0건이던 것이 갑자기 거래를 시작한다. 모의계좌지만 첫날은 반드시 눈으로 볼 것
- [ ] **C-17** `net_er_detail`에 `p_favorable`·`p_adverse` 출현 — edge가 왜 그 값인지
      로그가 스스로 말하는가
- [ ] **C-18** 거래가 시작되면 `self_eval`의 `pnl_measurable`이 `true`로 바뀌는가


## 2026-08-24 07:10 — G1 워크포워드 1차: **내가 잘못 돌렸다** ([MW0601])

`--train-days 120 --test-days 20`으로 창 7개를 돌렸고 **전 창 주문 0건**이 나왔다.
스크립트가 자동으로 "배선이 아니라 모델의 성질이다"라고 결론지었는데 — **그 결론이 틀렸고,
그 자동 결론을 넣은 것도 나다.**

### 무엇이 틀렸나

`--regime`의 기본값이 `off`다. 그건 국면을 **항상 `UNKNOWN`** 으로 두는 설정이고,
`MetaDecisionEngine._EVENT_LIKE_REGIMES = {EVENT, UNKNOWN}`이 그것을 게이트 ②에서
**100% 차단**한다. 즉 이 실행은 **주문 0건이 구조적으로 보장**돼 있었다.

증거: `logs/pass_cycles/`에 백테스트가 남긴 파일이 **0개**다. `_record_pass_cycle()`은
`intent.side == NO_TRADE`를 넘긴 사이클에만 불리므로, 전 사이클이 NO_TRADE였다는 뜻이다.
어제 고친 `edge` 산식에는 **도달조차 못 했다.**

스크립트 자신이 그 함정을 인자 도움말에 적어 놨다 — *"off는 중립이 아니라 '항상
UNKNOWN(가중치 0.5 고정)'이라는 특정 가정이다."* 읽고도 기본값으로 돌렸다.

### 같은 형태의 반복

2026-08-21 G-10이 이름 붙인 것과 같다 — **계기가 자기가 재려는 것의 전제를 확인하지 않고
결론을 냈다.** 그날 목록에 사례 넷이 있었고 이게 다섯 번째다. 그리고 이번엔 그 계기를
전날 내가 만들었다.

### 고친 것

- [x] **G1이 `--regime off`에서 인과 결론을 내지 않는다** — 실행 머리에 경고를 찍고,
      주문 0건일 때 "이 실행으로는 아무것도 판정할 수 없다 · `--regime on`으로 다시 돌릴 것"
      으로 바꿨다. JSON에도 `regime_wiring` · `orders_structurally_blocked`를 남긴다.

### 다시 할 것

- [ ] **G1 워크포워드 재실행 — `--regime on --train-days 120 --test-days 20`.**
      **오늘 장후(15:45 이후)에 한다** — 07:10 현재 거래일 아침이고, 창마다 재학습이
      3코어를 1시간 넘게 먹는다. 그 CPU가 08:20 수집 프로세스와 겹치면 어제 세운
      발행 오프셋 축(F-12·F-15)이 첫날부터 오염된다.
      `--regime on`은 전 구간으로 RegimeAI를 한 번 학습하는 약한 look-ahead가 있다
      (스크립트가 그 한계를 스스로 적는다) — 성과를 주장할 때 반드시 함께 언급할 것.

### 1차 실행에서 그래도 확인된 것

- 연속 시계열 68,218봉 · 252캘린더일 · 월물 9개 · 롤 8회 (2026-08-13 롤은 겹침 없어 조정 0)
- 창 7개가 실제로 만들어진다(`negative_window_ratio`가 의미를 갖는 3개 이상 확보)
- 어제 넣은 계기들이 전부 일한다 — 거래 건수 0/0/0, 손익 틱 표기,
  `max_drawdown` **미측정** 보고(계약 승수 미정), `cost_adjusted_sharpe` FAIL(0.0)
- **`max_drawdown`이 PASS로 안 찍혔다** — 어제 그 가드를 안 넣었으면 전 구간 0.0으로
  `max_drawdown`·`negative_window_ratio`가 **둘 다 초록**으로 나왔을 실행이다.
  `negative_window_ratio`는 여전히 PASS(0.0)로 찍혔는데, 거래가 0건이면 손실 창도 0이라
  그렇다 — **이것도 성과가 아니다.** 아래 항목 참고

- [x] **`negative_window_ratio`도 거래 0건이면 미측정으로 뺐다** — 아래 항목을 쓰다가
      발견해서 같은 커밋에 고쳤다. `traded_at_all`이 거짓이면 성과 관문 **전부**를
      미측정으로 돌린다. 성과는 거래의 결과이고, 거래가 없으면 잴 성과도 없다.
- [x] **1차 산출 JSON(`logs/g1_walkforward_20260823.json`) 삭제** — 그 파일은
      `negative_window_ratio: measured=true, passed=true`를 담고 있는데 거래는 0건이었고,
      **무효 실행이라는 표시가 파일 안에 없다**(`regime_wiring` 필드를 그 실행 뒤에 만들었다).
      나중에 읽는 사람이 초록으로 오독할 지뢰라 남기지 않는다. 무슨 일이 있었는지는 이
      절이 기록한다.
- [x] ~~**`negative_window_ratio`도 거래 0건이면 미측정으로 빼야 한다**~~ — 이번 실행에서
      PASS(0.0)로 찍혔지만 거래를 안 했으니 손실 창이 없는 게 당연하다. `max_drawdown`은
...
- [x] **F-21** 완성봉 유예 값 정본화 — 사용자가 **㉢(셋으로 나눈다)** 선택, 구현 완료.
      늦은 틱 유예(1분봉 2.0초·합성 5.0초) · 합성 위상(0.5초 유지, 개명) ·
      발행 예산(`_PUBLISH_SLA_MS` 1,000ms 신설). 자가점검 6거래일 연속 경고 **해소**
...
- [ ] **발행 예산 1,000ms를 조인다** — `publish_sla` 축이 며칠 쌓인 뒤. 지금 값은
      상한 추정치(계산 시간 p99 479ms · 합성기 몫 포함)의 2.1배라 넉넉한 쪽이다.
      리플레이 하한은 p99 47~63ms — 라이브 실측이 그 사이 어디인지가 조일 근거다
- [ ] **`publish_sla`를 `breaches`로 승격할지** — 지금은 `verdict: recorded_only`(R18)
...
## 2026-08-25 08:52 — 장전 점검 ([MW0601])

> 전문: `logs/dailycheck/2026-08-25_report.md` 제1부 · 판정 **정상** ·
> P0 0건 · P1 0건 · P2 3건 · 확인 필요 1건 · 코드 변경 0건(R11 / 금지계명 3·4)

### Fix — 장후(15:35 이후) 적용 후보 (권고 순서 ②→①→③ · 합계 약 2시간)

- [ ] **F-31 (2분) · P2** `scripts/git_lock_guard.py` 워킹트리 변경 처분 — 대응 1-1.
      ㉠`git checkout --`으로 되돌린다(**권고**) / ㉡futures 정본에 반영 후 복사
      (`utils.analysis_db` 의존 제거, `tests/test_483_git_lock_guard.py` 재기준) /
      ㉢`configs/pending_verifications.yaml`에 `foreign-worktree-change` 등재 후
      paper 승격 전 강제 해소. **F-32 적용 후에 한다** — 순서를 뒤집으면 F-32 (ㄴ)분기
      라이브 표본이 사라진다
- [ ] **F-32 (50분) · P2** 자가점검 `git` 줄이 미커밋 파일의 **출처**를 말하게 한다 —
      대응 1-2. `core/version.py`에 `worktree_dirty_paths()` 신설(경로+mtime 나이,
      `--no-optional-locks`, `None`은 미측정) · `scripts/self_check.py:661-664` 상수 문구를
      3분기로. **판정·종료코드 무변경**
- [ ] **F-33 (70분) · P2** 두 폴러가 성공 사이클도 센다 — 대응 1-3.
      `data/investor_flow_poller.py` · `data/option_chain_poller.py`에 5분 요약
      `InvestorFlowPollCadence` / `OptionChainPollCadence`(INFO, `core/logging.py` 등록) ·
      `ops/integrity_report.py`에 `poll_cadence` 축(**`verdict: recorded_only`**, R18)

### 고도화 (당일 관측 근거 있음)

- [ ] **G-18 (40분 · 이번 주)** 「모드 승격 준비도」를 아침 한 줄로 —
      `scripts/self_check.py`에 `_check_promotion_readiness(mode)`. dev 모드에서만 뜨고
      **판정에 관여하지 않는다**. 오늘이면 차단 2건(git 미커밋 1파일 · bundle 관문 3개).
      차단 0건이면 아예 안 찍는다. 근거: 1-1에서 `self_check.py:643-644` 확인
- [ ] **G-19 (50분 · 이번 주 후반)** `ops/integrity_report.py` `publish_offset` 축에
      Horizon별 `{p50,p90,p99,max,over_1000ms_count,over_grace_count}`.
      `over_grace_count` 기준은 1분봉 2.0초 / 합성 5.0초(F-21 정본).
      근거: C-1이 오늘 손으로 12건을 옮겨야 판정 시도조차 됐다.
      **선행: `publish_sla` 축 5거래일 누적(오늘 2일차)**
- [ ] **G-20 (25분 · 다음 단계)** `SessionStart.source_mtime_max` 대상을
      `src/messiah/**`로 좁히고 `scripts/**`는 `script_mtime_max`로 분리 병기
      (`core/version.py`). **기존 필드 의미를 바꾸지 않는다** — 옛 로그 파서 보호
      (F-25 `bars_used` · F-21 `publish_grace`와 같은 규율).
      근거: 오늘 `source_mtime_max`를 외래 파일 하나가 잡고 있었다.
      F-31로 증상은 사라지나 **원인은 남는다**

### 오늘 장중·장후가 판정할 것 (K 시리즈 — 전일 L 시리즈 승계)

- [x] **L-1 → 해소** 등록부 `no-degenerate-features`가 `기한 도달 불가`로 떴다
      (08:20:51 자가점검 `deadlines`). F-19 라이브 통과. **기한 자체는 사람이 재조정해야 한다**
- [x] **F-21 라이브 통과** `bar_close` 6거래일 연속 경고 **해소** —
      `1분봉 유예 2000ms vs 전일 회선 p99 1023ms · 늦은 봉 폐기 0건`
- [ ] **K-1**(전일 L-3) F-22 — `raw_qty 0.3488·0.3652 → floor 0` 재현 + `edge_source: "directional"` · 장중
- [ ] **K-2**(L-4) F-23 — `shortfall_ratio` · `sizer_funnel` 출현 · 장중.
      *등록부 `order-path-live` 등재는 장전에 확인 완료*
- [ ] **K-3**(L-5) F-27 — `self_eval_2026-08-25.json`에 `sample_window` · `rows_total`/`rows_counted`/기산일 · 장후
- [ ] **K-4**(L-6) F-25 — `RegimeClassified`의 `{history_len, bars_used, observations_used}`가 **서로 다른 값** · 장중
- [ ] **K-5**(L-7) F-24 — `detection_lag_ms` 최대값이 군집 창 2배 이내(전일 기준선 중앙 10.0분·최대 149.0분)
- [ ] **K-6**(L-2) `flow_intraday/K2I`의 `short_cycles` **빈 배열** + 실시간 `InvestorFlowLegShortfall` · 장후
- [ ] **K-7**(L-10) 장후 배치 4단계에 F-28 후 `발행 유예 초과` **0줄**
- [ ] **K-8**(L-8) F-21 3일차 — 회선 p90/p99 · `bar_to_publish_ms`가 종일 47~63ms 수준 유지되는가
- [ ] **K-9**(L-9) `MetaGateEvaluated` 최대 확률 **0.5 이상**이면 두 날의 사실 (전일 0.6646) — F-18 재학습 재료
- [ ] **K-10 🆕**(C-1) `publish_offset_ms` 1m 계열 종일 분포 — 2,000ms 초과 **0건**이어야 한다. 1,000ms 초과 건의 군집 여부
- [ ] **K-11 🆕**(1-3) `InvestorFlowPollError` **0건 유지**(V-6·U-5) · `Retried` 종일 건수가 전일 4건 수준을 크게 넘으면 W-4 재검토

### 사용자 조치 (장전 갱신 — 개장 전 필수 항목 없음)

- [ ] **1** 화면(`http://localhost:8511`) 한 번 열기 — **닷새째 미이행.** 1분.
      오늘도 정상 기동(포트 8511 · pid 3188 · `UP`), 08:21:06 첫 렌더 4토픽 `NO_DATA`는
      **개장 전이라 정상**. 안 하면 장중 화면 결함을 아무도 못 본다
- [ ] **2** `no-degenerate-features` 기한 재조정 — 3분. 오늘 자가점검이 `기한 도달 불가`로
      정확히 짚었다(F-19 작동). **코드는 기한을 옮기지 않는다**
- [ ] **3** 장후 F-31·F-32·F-33 적용 여부 결정 — 결정 5분 / 구현 약 2시간.
      **F-31은 paper/live 승격 전 필수**(그 상태로는 기동 FAIL)
- [ ] **4 🆕** 모드 승격 계획이 있으면 차단 2건 확인 — ①미커밋 1파일(2분이면 해소)
      ②`real-20260820-2053-30m` 관문 3개 미판정(**재학습 필요, 며칠**)
- [ ] **5** 승격 표본 세는 방식(㉠이어서 세기 / ㉡나눠 세되 요구일수 재정의) — 9월 11일 롤 전까지
- [ ] **6** 발행 예산 1,000ms 조이기 — `publish_sla` 누적 **오늘 2일차**, 며칠 더
- [ ] **7** F-20 잔여 21건 — `python scripts/suggest_fix_commits.py` 후보 → 사람이 확정

### 해소·판정 처리 (2026-08-25 장전 확인)

- [x] **전일 사용자 조치 4**(완성봉 유예 500ms 정본화) — **F-21로 마감.** 자가점검 경고 소멸 확인
- [x] **전일 사용자 조치 7**(재시동) — **불필요 재확인.** `code_version.stale: false` ·
      세 프로세스 전부 `085cca3` = HEAD. 어제 밤 커밋 13건 전부 실려 있다
- [ ] **전일 확인 필요 (나)** — 이미 2026-08-24 야간에 답이 나왔다(계측 인공물). 중복 아님
- [ ] **V-6 / U-5** `InvestorFlowPollError` 0건 유지 — 장전 확인, 장후 재확인(K-11)


---

## 2026-08-25 11:50 — 장중 점검 (화면 실물 점검 · [MW0601])

> 계기: 사용자가 화면을 직접 열어 "기술된 구성이 완성되지 않았다"고 지적.
> 전문: `logs/dailycheck/2026-08-25_report.md` 제2부 · 판정 **조건부** ·
> 신규 P1 3건 · P2 4건 · 확인 필요 1건 · 코드 변경 0건(R11)
> **화면을 안 열면 발견되지 않는 결함군이 실재한다는 것이 오늘 확정됐다** — P1 3건 중 2건이 그것이다.

### ⚠ 선행 조건 — 이게 안 풀리면 아래 전부 커밋 불가

- [ ] **F-34 (30초 · P1) 🆕** `.git/index.lock` 스테일 회수 — 대응 1-4.
      **08:51:14에 장전 점검이 만들었다**(마운트 권한으로 git이 unlink 실패, 경고를 흘려보냄).
      사람이 Windows에서 `python scripts/git_lock_guard.py --check` → rc=2면 `--reclaim`.
      **에이전트 쪽 `rm` 거부됨**(Operation not permitted)
      - [ ] 재발방지 ①`SKILL.md`에 **점검 중 손호출 git은 `--no-optional-locks`** 명문화
      - [ ] 재발방지 ②`collect_evidence.py` §9에 락 mtime × 점검시각 근접(±120초) →
            `점검 중 생성 의심` 병기. 오늘 있었으면 08:51에 알았다
      - [ ] 재발방지 ③`scripts/self_check.py _check_git()`에 인덱스락 3상태 한 줄

### Fix — 장후 적용 (권고 순서 · P1 먼저)

- [ ] **F-36 (60분) · P1 🆕** 폴백 사용률을 장부에 넣는다 — 대응 1-6.
      `app.py _snapshot_freshness_fields()`(1334행)에 `threshold_derivation` +
      **`fallback_topics: []`** 추가(새 태그 안 만든다, R6) · `integrity_report`에
      `ui_threshold_derivation` 축. **F-35보다 먼저** — `["DecisionIntent"]` → `[]`가
      두 fix를 서로 검증한다
- [ ] **F-35 (40분) · P1** `DecisionIntent`에 `cadence_seconds` — **기등재 항목**
      (2026-08-24 장중 P1). 오늘 715초 사례로 2차 관측 · 주기 30분 실측 확정
      (`DecisionEmitted` 6건 정확히 30분 격자) → **98.3% STALE 계산 확인**.
      `core/messages.py:448-460` 필드 추가 · `meta_decision.decide()`가 `view.cadence_seconds`
      전달. **`_STALE_AFTER`에 항목을 넣지 않는다**(상수 하나 더 = F-21 500ms의 반복).
      **`Fill`도 같은 처지인지 함께 확인** — ④ `exec.fill 미배선`이 진짜인지 오탐인지 미확정
- [ ] **F-37 (70분) · P1 🆕** 차단 계층 무력 상태를 화면 ①에 — 대응 1-5.
      `DecisionIntent.gate_notes: list[str]`(비정상 통과만) · `meta_labeler.py`가
      `threshold<=0` 또는 `threshold_source: unrecorded_*`면 기입 · `app.py:1121-1152`가
      `st.warning`. **판정(`passed`) 불변.** ⚠ `Field(default_factory=list)` 필수(옛 캐시) ·
      스키마 버전(`version=1 types=21`) 변경 확인. **F-35와 같은 커밋**(둘 다 같은 스키마)
- [ ] **F-31 · F-32 · F-33** — 장전분 그대로 유효(F-32 → F-31 순서 유지)
- [ ] **F-38 (15분 권고안) · P2 🆕** 상단 바 문서-구현 일치 — 대응 1-7.
      ㉠문서를 현황에(**권고**) / ㉡구현을 문서에(3~4h).
      ⚠ **그림에서 지우기만 하면 안 된다** — `Ver 2.2 구현 예정` 목록으로 남긴다
- [ ] **F-40 (50분) · P2 🆕** `n_experts<=1`일 때 분산 표시 — 대응 1-10.
      **권고 ㉡`dispersion_defined: bool` 병기**(㉠`None`은 소비처 전수 선행 필요,
      순서 뒤집으면 장중 판단 정지). F-25가 「나눈」 것과 같은 해법
- [ ] **F-39 (3~4h) · P2 🆕** `ui/app.py` 1,482줄 분할 — 대응 1-8.
      `ui/panels/*.py` + `ui/badges.py`. ⚠ `crash_forensics.enable` · numpy 선임포트는
      **진입점에 남긴다**(2026-07-31 실측 3건). **F-35·F-37 뒤.**
      **결정 필요**: React 이관 이번 분기면 생략하고 등록부에 「이관으로 해소」

### 고도화 (당일 관측 근거)

- [ ] **G-21 (90분 · 이번 주) 🆕** 화면-로그 어긋남을 자동으로 센다 —
      `UISnapshotFreshness`에 `unshown_warnings: []`(살아 있는 WARNING 태그 − 화면이 그린 것).
      오늘이면 `["MetaGateEvaluated"]`. **09:00에 알 수 있었다 → 실제 발견 11:41,
      2시간 41분 단축.** 빈 배열이면 필드 자체를 안 싣는다. **F-36과 같은 자리 · 함께 넣는다**
- [ ] **G-22 (30분 · 이번 주) 🆕** 아침에 「오늘 밤 커밋 가능한가」를 답하게 —
      자가점검에 `git add --dry-run` rc 확인. ⚠ **`--no-optional-locks` 필수**
      (안 붙이면 검사가 락을 만든다 — 오늘 실수의 자동화 판). **F-34와 같은 커밋**
- [ ] **G-23 (40분 · 이번 주 후반) 🆕** `publish_offset` 축에 `co_late_minutes` —
      같은 분에 2계열 이상 동시 초과한 분 + 겹친 계열 수. 오늘 `10:35 [1m,5m,10m]` ·
      `11:15 [1m,3m,5m,15m]`. **손 교집합에 15분 들었다.** 상위 10분만 + 총건수 병기.
      **G-19 선행 · 같은 함수**
- [ ] **G-18 · G-19 · G-20** — 장전분 그대로 유효

### 장후가 판정할 것 (K 시리즈 갱신)

- [x] **K-4 ✅ 해소** F-25 라이브 통과 — `RegimeClassified` 6건 전수
      `{history_len:200, bars_used:82, observations_used:60~61}`, 세 필드 분리 확인
- [x] **K-9 ✅ 판정 — 가설 성립 안 함** MetaGate 최대 **0.0476**(전일 0.6646의 1/14).
      **「합격선 0.5」 후보 근거 소멸.** 재학습만이 답한다(F-18 후속 유지).
      부수: 오늘 최대가 0.0476이므로 **합격선 0.05 이상이면 오늘 6건 전부 차단**됐다
- [x] **K-10 ✅ 판정 → 이상점 1-9** 1m p99 1,986.7ms · 최대 2,302.3ms(유예 2.0초 초과 1건) ·
      >1,000ms 16/182건(8.8%) · **늦은 봉 폐기 0건**
- [x] **K-11 ✅ 통과** `InvestorFlowPollError` 종일 **0건**(V-6·U-5 유지)
- [x] **F-18 라이브 검증 통과** `MetaGateEvaluated` WARNING 6건 — 경보가 처음 울렸다
- [ ] **K-1 · K-2** 🔄 미관측 이월 — `SizerZeroQty` 0건. 오전 `|S|` 최대 0.0844(문턱 0.2의 42%).
      **도달 조건 미충족이지 결함 아님**
- [ ] **K-3 · K-6 · K-7** 🔄 장후 배치 산출물 이월
- [ ] **K-5** 🔄 이월 — `PublishLoopStalled` 0건. **종일 0건이면 미검증으로 남긴다.**
      「안 났다」를 「고쳤다」로 세지 않는다
- [ ] **K-8** ✅ 중간 통과 — `bar_to_publish_ms` 종일 최대 **141ms**(예산 1,000ms의 14.1%).
      회선 p90/p99는 장후 확정. **예산을 조일 여지가 크다**(F-21 후속)
- [ ] **K-12 🆕** [C-2] 국면 확신도 오후 궤적 — 0.5를 깨는가 · `regime` 전환되는가.
      11:30 기준 **0.5789**(6회 연속 단조 하락 · 시드 0.9998의 58%).
      `aggregator.py`가 `confidence`를 가중치에 쓰는지 · 임계가 어디인지 확인
- [ ] **K-13 🆕** 1-9 종일 확정 — 유예 2.0초 초과 총 건수 · 계열 간 동시 겹침 분
- [ ] **K-14 🆕** 1-5 — `MetaGateEvaluated` WARNING 종일 건수 · 확률 최대값이 0.05를 넘는가

### 사용자 조치 (장중 갱신 — 이 목록이 장전 목록을 대체한다)

- [ ] **1 ⚠ 지금** 잠금 파일 제거(F-34) — 30초. **안 하면 오늘 밤 8건 전부 저장 불가.**
      거래에는 영향 없음
- [ ] **2** 12:30 이후 화면 한 번 더 — 2분. ①`n=`이 계속 1인지 ②`Regime`이 바뀌는지(K-12).
      **오늘 아침 열어 본 덕에 P1 3건이 나왔다** — 그중 둘은 로그로 못 찾는 종류
- [ ] **3** 장후 fix 8건 순서 결정 — 결정 10분 / 구현 약 6시간.
      순서에 이유 있는 세 쌍: **F-36→F-35** · **F-32→F-31** · **F-39는 F-35·F-37 뒤**
- [ ] **4 🆕** 결정 3건 — (가)`gate_notes` 색상(**권고 `st.warning`**) ·
      (나)상단 바 문서/구현(**권고 ㉠문서**) · (다)`app.py` 분할 여부(React 이관 일정에 달림)
- [x] **5 ✅ 답 나옴** 「합격선 0.5」 — **오늘 근거 소멸**(K-9). 재학습이 유일한 답
- [ ] **6** `no-degenerate-features` 기한 재조정 — **여전히 미이행.** 3분
- [ ] **7** 승격 표본 세는 방식 — 9월 11일 롤 전까지
- [ ] **8** 발행 예산 1,000ms 조이기 — **오늘 최대 141ms로 예산의 14.1%만 쓴다.** 조일 여지 큼
- [ ] **9** F-20 잔여 21건 — **1번이 풀린 뒤에**


### 12:06 후속 — 1-4 해소 · 신규 1건

- [x] **F-34 잠금 회수 ✅ 완료** — 사용자가 `--reclaim` 실행. `.git/index.lock` 부재 확인 ·
      `.git/index` mtime 12:05:01로 갱신 · `git add -n .` 16건 rc=0.
      **검증을 `git status`가 아니라 `git add -n`(쓰기 경로)으로 했다** — 읽기 통과로
      쓰기를 추정하지 않는다(2026-08-23 결정). **F-31~F-33·F-35~F-40 커밋 선행조건 해제**
- [ ] **F-34 재발방지 3건은 미해소** — SKILL.md `--no-optional-locks` 명문화 ·
      `collect_evidence.py` §9 근접 판정 · `self_check.py` 인덱스락 3상태. **장후 유지**
- [ ] **1-11 🆕 P2** `.claude/skills/messiah-daily-check/.__wtest` — 0바이트,
      mtime 2026-08-24 19:24:00(장후 산출물 재생성과 같은 분), **저장소 코드에 생성 주체 없음**
      (`grep -rn "__wtest"` 0건). `git add .` 시 딸려 들어간다.
      **처방**: `.gitignore` 추가 후 삭제 — 단 **누가 만드는지 먼저 확인**한다
      (다음 거래일 장후 재생성 여부로 판정). 1-1과 함께 **출처 불명 워킹트리 잔재 2건**

### 12:40 장중 후속 — 신규 2건 · 격상 1건 · 판정 1건 ([MW0601])

관측 구간 09:00~12:36. **코드 변경 0 · 커밋 0**(R11 · 금지계명 3·4).

#### Fix — 장후(15:35 이후) 적용. 아침 3건 + 오전 6건 + 오늘 2건 = 10건

- [ ] **F-41 (40분) · P1 🆕** 설계표 「Meta 임계 보정」 배선 — 대응 **1-12**(그리고 1-5의 실질 처방).
      `futures/service.py:151-152 _apply_meta_labeler()`에 `META_THRESHOLD_ADJUSTMENT`
      (`aggregator.py:126`, Ver 1.2 §7.1) 적용. `regime_received=False`면 보정 0(2026-08-19 F-5).
      `gate_disabled` 조건을 `effective <= 0.0`으로 → 배선 후 경보 자동 소멸.
      로그 필드 `threshold_base`·`threshold_regime_adj`·`threshold_effective`·`regime`·
      `regime_source` 추가(**새 태그 안 만든다** — R6). 기존 `threshold`는 `effective`와 동값 유지.
      **⚠ 결정 필요 — 오늘 밤 최우선**: ㉠섀도 20거래일 후 승격(**권고**, R18) / ㉡즉시 실판정 /
      ㉢미배선(비권고). **오늘 8/8 사이클이 설계표 임계 미달** → 켜는 즉시 전량 차단 가능.
      「전량 통과」와 「전량 차단」은 둘 다 관문이 판단을 안 하는 상태다.
      **로그 필드는 어느 갈래든 무조건 넣는다.** **F-37과 같은 커밋.**
- [ ] **F-42 (50분) · P1 🆕** 발행 지연의 **기울기**를 계측 — 대응 **1-9 격상분**.
      30분 롤링 창 `PublishOffsetWindow`(INFO) + `core/logging.py` 태그 등록 +
      `integrity_report.py`에 `ratio_by_hour`. **`delta_ratio`가 핵심.** 경보 조건은 안 넣는다(R18).
      하루 78줄. **G-19·G-23과 같은 함수·같은 커밋.**
      1차 검증: 오늘 로그 replay로 09/10/11/12시 **1.7 / 8.3 / 13.3 / 17.5%** 재현.
- [ ] **F-31 · F-32 · F-33 · F-35 ~ F-40** — 앞 절 그대로 유효. **F-34 잠금 해제(12:05)로 전부 커밋 가능.**
      순서 제약: **F-36→F-35** · **F-32→F-31** · **F-39는 F-35·F-37 뒤** ·
      **F-41은 F-37과 한 묶음** · **F-42는 G-19·G-23과 한 묶음**

#### 고도화 (당일 관측 근거)

- [ ] **G-24 (60분 · 이번 주) 🆕** 호출처 0인 모듈 상수를 아침 자가점검이 센다 —
      `self_check.py _check_orphan_constants()`. **오늘 하루에 호출처 0 구현물 2건**
      (1-6 함수 · 1-12 상수) 발견, 둘 다 **우연**. 0건이면 줄 자체를 안 찍는다(G-18 규율).
      1단계는 모듈 상수만. `__all__` 제외 + 초회 결과를 허용목록에 굽는다. **G-22와 같은 파일.**
- [ ] **G-25 (40분 · 이번 주 후반) 🆕** `RegimeTransition`(INFO) — 국면이 **바뀐 사이클에만**
      `{from/to_regime, from/to_confidence, cycles_in_previous_regime, weight_delta_by_horizon}`.
      **`weight_delta_by_horizon`이 핵심** — 오늘 0.4→0.8을 알려면 사람이 표를 직접 읽어야 했다.
      부수: **X-5(국면 전파 어긋남) 판정 자동화** — 오늘도 대조 수단이 없었다
      (`DecisionEmitted`에 `regime` 필드 없음). **F-40 뒤.**
- [ ] **G-18 · G-19 · G-20 · G-21 · G-22 · G-23** — 앞 절 그대로 유효

#### 이상점 처분 (12:40)

- [x] **[C-2] ✅ 해소 — K-12 판정 완료** 12:00 `RANGE`→`HIGH_VOL` 전환. 확신도 6회 연속 하락은
      **전환의 선행 신호**였고 정확한 관측이다. `rule_override` 8건 전수 null. **결함 아님.**
      부수: 30m 가중치 **0.4→0.8(2배)**. 오전 최대 `|S|` 0.0844 → HIGH_VOL 재현 시 0.169
      = 문턱 0.2의 **84.4%**
- [ ] **1-9 ⬆️ P2 → P1 · ↩️ 11:50 판단 정정** 「단발 지터」는 표본 182건의 결론. 233건에서
      **시간당 초과율 단조 증가**(09시 1.7% → 12시 17.5%, **10.3배**) · 유예 2.0초 초과
      **1→3건**(11:11·12:11·12:25) · 동시 초과 분 **2→9분**.
      원인 후보 2개 제거: 옵션체인 경합(9.1% vs 9.8% — 기각) · 시계 드리프트(최대 0.18초 — 기각).
      **계산 시간 무죄** — `bar_to_publish_ms` 최대 141ms 불변. **데이터 손실 0 유지.**
      → **F-42**
- [ ] **1-12 🆕 P1** `META_THRESHOLD_ADJUSTMENT` 호출처 0. 설계표(Ver 1.2 §7.1) 7번째 열 미배선.
      **오늘 8/8 차단됐을 값.** 최대 확률 0.0476이 횡보 보정 0.05에 **0.0024 차 미달.**
      dev_memory 기록 **0건**(신규). 1-6과 같은 계열, **오늘만 두 번째** → **F-41 · G-24**
- [ ] **1-3 🔄 지속 · 범위 갱신** 증권사 500 오류 **8건**(수급 5 + 옵션체인 3)이 전부
      **08:41:02~09:13:36(32분 34초)** 안. 그 뒤 3시간 22분 0건. W-4 결론을 좁게 확정 —
      「개장 전후 30분에 몰리고 딱 멈춘다」. 분모 부재 그대로 → F-33 유효(두 폴러 모두 대상)
- [ ] **1-1 · 1-2 · 1-5 · 1-6 · 1-7 · 1-8 · 1-10 · 1-11 🔄 전부 지속** 코드 무변경.
      1-5는 **원인 진단이 1-12로 격상**(「재학습만이 답」 → 「설계값 미배선」)
- [x] **1-4 ✅ 해소 재확인** 다이제스트 §1 `인덱스락 없음`(12:36:18) · §9 자동 적신호에서 소멸.
      재발방지 3건(F-34 하위)은 미해소 — 장후 유지

#### 장후가 판정할 것 (K 시리즈 12:40 갱신)

- [x] **K-12 ✅ 판정 완료** — 위 [C-2]
- [x] **K-9 ✅ 판정 유지 · 대체안 확보** 「0.5」는 여전히 근거 없음. **설계표 값(+0.05/+0.10)이 대체 후보**
- [x] **K-4 · K-11 ✅ 유지** `RegimeClassified` 8건 전수 3필드 분리 · `InvestorFlowPollError` 0건
- [ ] **K-8** ✅ 중간 통과 유지 — `bar_to_publish_ms` 최대 **141ms 불변**(90건 추가에도).
      **이것이 1-9 격상의 핵심 근거다** — 지연은 계산이 아니라 상류
- [ ] **K-13** 🔄 중간 갱신 — 유예 초과 **3건** · 동시 초과 **9분** · 시간대별 1.7/8.3/13.3/17.5%
- [ ] **K-14** 🔄 중간 갱신 — `MetaGateEvaluated` **8건** · 최대 **0.0476**. **8건 전부 설계표 미달**
- [ ] **K-1 · K-2** 🔄 이월 · **도달 가능성 상향** — 국면 전환으로 가중치 2배. 오후에 열릴 수 있다
- [ ] **K-3 · K-5 · K-6 · K-7** 🔄 이월 — 장후 배치 / `PublishLoopStalled` 0건(**미검증으로 남긴다**)
- [ ] **K-15 🆕** 1-9 기울기가 오후에도 이어지는가 — 13·14·15시 시간대별 1m 초과율.
      꺾이면 「오전 특유」, 이어지면 **F-42 P1 확정**
- [ ] **K-16 🆕** 오후에 `|S|`가 0.2를 넘는가 — 넘으면 K-1·K-2 첫 관측이자
      **Meta 관문 무력(1-5·1-12)이 결과에 반영되는 첫 사례**
- [ ] **K-17 🆕** [C-3] 회선 지연 p99의 시간대별 기울기가 발행 지연과 같은가 —
      `publish_offset_axis: local_only` · `skew_ms: null`이라 오늘 로그로는 못 가른다
- [ ] **K-18 🆕** 1-11 `.__wtest` 재생성 여부 — 장후 배치(15:45) 후 mtime이 오늘로 갱신되는가

#### 사용자 조치 (12:40 갱신 — 이 목록이 11:50 목록을 대체한다)

- [x] **1 ✅ 완료** 저장소 잠금 해제 — 12:05:01 사용자 실행 확인
- [ ] **2** 오후에 화면 한 번 — **3분.** ①점수가 아직 0.2보다 한참 아래인지 ②시장 상태가 계속
      「변동성 높음」인지 ③처음으로 매수/매도 쪽으로 기우는지. **국면 전환으로 감도가 2배가 됐다**
- [ ] **3 ⚠ 오늘 밤 최우선 결정** F-41을 ㉠섀도 / ㉡즉시 / ㉢미배선 중 무엇으로 할지 — **15분.**
      **권고 ㉠.** 안 하면 「재학습만이 답」이 계속되고 안전장치 하나는 0%로 돈다
- [ ] **4** 장후 fix **10건** 순서 결정 — 결정 10분 / 구현 약 7시간. **잠금 풀렸으므로 전부 커밋 가능**
- [ ] **5** 기존 결정 3건 — (가)경고 색상(**권고 노랑**) · (나)상단 바(**권고 문서 수정**) ·
      (다)`app.py` 분할(React 이관 일정에 달림)
- [ ] **6** `no-degenerate-features` 기한 재조정 — **여전히 미이행.** 3분
- [ ] **7** 승격 표본 세는 방식 — 9월 11일 롤 전까지
- [ ] **8** 발행 예산 조이기 — 오늘 최대 141ms(예산의 14.1%). **단 F-42가 먼저다** —
      지금 조이면 오후에 심해지는 지연과 겹친다
- [ ] **9** F-20 잔여 21건 — **이제 저장 가능. 오늘 밤 가능**
- [ ] **10** 출처 불명 워킹트리 잔재 2건(1-1 도구 파일 · 1-11 빈 파일) — 장후,
      단 **누가 만드는지 먼저 확인**(K-18)

## 2026-08-25 15:58 — 장후 점검 (예약 실행 · [MW0601])

### 해소·판정 처리 (2026-08-25 장후 확정)

- [x] **[C-1] ✅ 해소**(12:40) 1-9로 흡수
- [x] **[C-2] ✅ 해소**(12:40) K-12 — 국면 확신도 하락은 전환의 선행 신호. 종일 궤적으로 재확인(3회 전부)
- [x] **[C-3] ✅ 판정 — 회선 아님.** `delivery_latency.by_hour` p90 08시 0.9266 → 15시 0.9151s
      (변동폭 19ms · 2.1%) · `intraday_trend.delivery_latency.ratio 0.96`. 표본 전수 확인
      (8버킷 전부 `population: "all"` · 합 71,396 = `observed_total`). **원인은 수집기/합성 쪽**
- [x] **1-4 ✅ 해소 유지** 인덱스락 없음(15:57:48). 재발 0. **단 재발방지 3건(F-34 하위) 미해소**
- [x] **K-3 ✅ 통과** `self_eval` `sample_window: {from 2026-07-29, rows_total 19, rows_counted 17,
      excluded {roll_day 1, not_countable 1}}`. F-27 라이브 통과
- [x] **K-4 ✅ 유지** `RegimeClassified` 14건 전수 `{history_len 200, bars_used 82, observations_used 60~61}` 분리
- [x] **K-6 ✅ 통과** `flow_intraday/K2I` `short_cycles: []` · rows 434 · coverage 99.8% · gaps []
- [x] **K-7 ✅ 통과** `run_vol_scorecard` 「발행 유예 초과」 0줄 · 3개 Horizon `measurable: true`. F-28 라이브 통과
- [x] **K-8 ✅ 통과** `bar_to_publish_ms` 종일 최대 **156ms**(예산 1,000ms의 15.6%) · `over_sla: 0`.
      회선 p99 오늘 1.026s vs 전일 1.023s — 변화 없음. F-21 3일차 통과
- [x] **K-9 ✅ 판정 — 「0.5」 가설 기각** `meta_gate.max: 0.3566`(14:30) · p90 0.1594 · p50 0.0303.
      **오늘 최대가 0.5 미만.** 전일 0.6646과 다르므로 두 날의 사실이 되지 않았다
- [x] **K-10 ✅ 판정** 1,000ms 초과 50건 군집 없음(`:00`분 2건 · `:30`분 1건). **단 2,000ms 초과 0건 기준은 깨졌다 — 6건**
- [x] **K-11 ✅ 통과** `InvestorFlowPollError` 0건. `Retried` 6건(전일 4건 +2)
- [x] **K-12 ✅ 판정 완료**(12:40)
- [x] **K-13 ✅ 종일 확정 — 6건** 11:11:02 · 12:11:02 · 12:25:02 · 14:46:02 · 15:21:02 · **15:26:03(3,119.7ms)**
- [x] **K-14 ✅ 종일 확정 — 14건** 최대 0.3566 · 통과 14/14. **설계표 적용 시 11건 차단 / 3건 통과**
- [x] **K-15 ✅ 판정 — 이어진다** 13시 8.3%로 꺾였다가 14시 18.3% · **15시 25.7%** 재상승.
      「오전 특유」 기각. **단 단조가 아니므로 F-42 설계 폐기 → F-43**
- [x] **K-16 ✅ 판정 — 넘었다** 15:30 `S = +0.25435`(문턱 0.2의 127.2%) · LONG · `gate: pass`.
      **단 「Meta 무력이 결과에 반영되는 첫 사례」는 되지 못했다** — ③Risk R6가 먼저 막았다
- [x] **K-17 ✅ 판정 — 회선 기울기 없음** [C-3] 참조
- [x] **K-18 ✅ 판정 — 재생성 없음** `.__wtest` mtime 2026-08-24 19:24 불변.
      장후 배치도 수집기 4회 실행도 안 건드렸다. **1회성 잔재 — 삭제 안전**

### ⚠ 선행 조건 — 없음. 14건 전부 커밋 가능 (F-34 잠금 12:05 해제)

### Fix — 장후 적용 (권고 순서)

- [ ] **F-43 (90분) · P1 🆕** 일중 열화를 **꼬리 축**으로도 잰다 — **F-42 흡수 재정의 · G-19 흡수**.
      대응 **1-13**(그리고 1-9의 실질 처방).
      `ops/integrity_report.py` — `hourly_trend()`에 `keys: Sequence[str] = ("p50",)` · 축별 임계
      (p50 3.0 / p90 **2.0** / over_ratio **3.0**) · 새 `_hourly_over_ratio()` · `_intraday_trends()`(1443행)
      3축 병행 · 3156행 `⚠`를 3축 OR로.
      `features/engine.py` — `publish_offset.by_hour`에 `over_1000` · `over_grace` · `p99` 추가,
      **`grace_headroom_ms`**(= `_MAX_CONSTITUENT_WAIT_SECONDS*1000 − max(offset)`, 오늘 1,880.3ms).
      회귀 위험: 스키마 확장(추가만) · `hourly_trend()`는 `delivery_latency`도 호출하므로 기본값으로 보존.
      검증: `pytest tests/ops/test_integrity_report.py -k "hourly_trend or intraday"` +
      **오늘 로그 replay**(`over_1000_ratio` 09시 0.017 → 15시 0.257 · `drift: true`) + 08-24 오탐 확인.
      **라이브 미검증 — 검증 기한 2026-08-26 장후**
- [ ] **F-41 + F-44 (40+55분) · P1** — **반드시 한 커밋**(같은 파일·같은 함수).
      F-44 선행 조사 15분: `Derivatives_AI_Master_Plan_Ver1.2.md` §7.1에서 TREND_UP/DOWN 근거 문구 확인.
      근거 있음 → 주석 + `META_THRESHOLD_ADJUSTMENT_SOURCE`. 근거 없음 → 잠정 0.05 + `threshold_shadow` ·
      `passed_shadow`(**차단 안 함**). 파일: `strategy/futures/aggregator.py:126` ·
      `strategy/futures/meta_labeler.py` · `core/logging.py`(`MetaGateEvaluated` 스키마).
      검증: `pytest tests/strategy/futures/ -k "meta or threshold or regime"` + 오늘 14사이클 replay(11/3 일치).
      **R18 해당 — 섀도 20거래일. `configs/pending_verifications.yaml` 등재. 섀도 기한 2026-09-22**
- [ ] **F-36 → F-35** 순서 유지 (계기를 장부에 → 배지 오탐 종결)
- [ ] **F-37** F-41과 화면 쪽 짝
- [ ] **F-32 → F-31** 순서 유지 (출처를 말하게 → 처분)
- [ ] **F-33** 유효 · **긴급도 유지** — 오늘 1-3이 오후에도 났으므로 `retry_ratio_by_hour` 축의 필요성 증가
- [ ] **F-46 (40분) · P2 🆕** `TickDeliveryLatency` 대표 문구를 전수 축으로 — 대응 **1-16**.
      `data/tick_archiver.py` 발행부. 링버퍼 축은 `tail_p50`/`tail_p90`로 개명 병기.
      **소비처 확인 필수**: `ops/fix_verification.py` 회선 기준 · 기동 자가점검 `bar_close` 줄.
      새 필드 추가 후 한 릴리스 병존
- [ ] **F-45 (30분) · P2 🆕** `min_samples` Horizon별 — 대응 **1-15**.
      권고안: 15m·30m에서 **당일 축을 걷어내고 롤링 축만** 남긴다(G-18 규율).
      `ops/feature_health_rolling.py` · `features/engine.py`.
      ⚠ `no-degenerate-features`가 `degenerate_feature_count`를 읽는다 — **연속 통과 카운터 리셋하지 않는다**
- [ ] **F-38 · F-39 · F-40** P2 잔여, 변동 없음
- [ ] **F-34 재발방지 3건 미해소** — SKILL.md `--no-optional-locks` 명문화 등. 앞 절 그대로
- [ ] ~~**F-42**~~ **폐기 — F-43에 흡수.** 오늘 초과율이 단조가 아님(13시 꺾임)이 확인돼 설계 전제가 무너졌다

### 고도화 (당일 관측 근거)

- [ ] **G-26 (40분) 🆕** `gate == "pass"` 사이클에만 `DecisionGateOpened`(WARNING) —
      `gate_chain: ["meta:pass(p=0.087,th=0.0)", "risk:reject(R6)", "sizer:unreached", "gateway:unreached"]`.
      근거: 오늘 15:30이 `decision_funnel.pass: 1` 숫자 하나로만 남았고, 로그는 다른 13건과
      같은 레벨·같은 태그였다. 부수 효과: 등록부 `order-path-live`의 「미착수」와 「앞에서 막힘」이 갈린다.
      `strategy/futures/service.py`. **F-44 뒤**(`threshold_shadow` 참조). R18 해당 없음
- [ ] **G-27 (50분) 🆕** `blocking_layers` 축 — 차단 계층 3개가 각각 오늘 몇 번 판단했는가.
      근거: 오늘 세 계층 실적을 알려면 `meta_gate` · `sizer_funnel` · `circuit_breaker_events`
      세 자료구조를 뒤져야 했다. `block_rate` 0.0/null인 계층을 요약이 이름으로 지목.
      `ops/integrity_report.py`. **F-43과 같은 파일이므로 함께.** R18 해당 없음
- [ ] ~~**G-19**~~ **F-43에 흡수** — 고도화가 아니라 P1 처방이 됐다
- [ ] **G-18 · G-20 · G-21 · G-22 · G-23 · G-24 · G-25** — 앞 절 그대로 유효

### 지속 이상점 (코드 무변경 · 새 번호 붙이지 않는다)

- [ ] **1-1 🔄 6거래일째 · 금지계명 10** `scripts/git_lock_guard.py` 미커밋.
      `record_vs_commit.verdict: "closed_with_uncommitted_source"`(15:46:09).
      **live 모드 전환 시 기동 거부**
- [ ] **1-2 · 1-6 · 1-7 · 1-8 · 1-10 🔄 전부 지속** 코드 무변경
- [ ] **1-3 🔄 지속 · ↩️ 12:40 가설 기각** 종일 14건 — 오전 8 + **오후 6**(13:00:41 · 13:30:10 ·
      14:03:43 · 14:45:22 · 14:53:03 · 15:00:16). 「개장 전후 30분에 몰리고 딱 멈춘다」 **성립하지 않음**
- [ ] **1-5 🔄 지속** `MetaGateEvaluated` 14/14 `threshold: 0.0` · `passed: true` · **차단률 0%**
- [ ] **1-9 🔄 P1 유지 · ↩️「단조」 기각** 초과율 1.7→8.3→13.3→15.0→**8.3**→18.3→**25.7%**.
      최대 **3,119.7ms**(유예 2.0초의 1.56배) · 유예 초과 6건.
      **`late_bar_drops: 0` — 합성 대기 5.0초가 받아냈다. 여유 1,880.3ms(62.4% 소진)**
- [ ] **1-11 🔄 지속 · 판정 완료** `.__wtest` 재생성 없음 — **삭제 안전**
- [ ] **1-12 🔄 지속 · 범위 갱신** 종일 14건 중 설계표 적용 시 **11건 차단 / 3건 통과**.
      통과 3건 중 하나가 **오늘 유일한 pass 사이클(15:30)**

### 신규 이상점 (장후)

- [ ] **1-13 🆕 P1** 일중 열화 감시기가 `key="p50"` 하나만 본다
      (`integrity_report.py:1459` · `INTRADAY_DRIFT_RATIO = 3.0` 826행 · `drift` 941행).
      1m p50 245.3→259.4ms(1.06배)인데 1,000ms 초과율 1.7→25.7%(**15.1배**) · p90 745.2→1,417.4ms.
      **`drift: false` · `step_detected: null` · 리포트 `breaches: []`로 깨끗하게 나왔다.**
      12:40에 P1로 격상한 항목이 **같은 날 장후 리포트에서 사라졌다** → **F-43**
- [ ] **1-14 🆕 P1** `META_THRESHOLD_ADJUSTMENT[TREND_UP/TREND_DOWN] = 0.0`
      (`strategy/futures/aggregator.py:126`). **F-41을 넣어도 추세장에선 차단 계층 하나가 항상 열린다.**
      오늘 15:30(TREND_UP · 유일한 pass 사이클)이 그 통과 사례. R18 「3개 고정」 실질 위반 ·
      금지계명 8. `regime_distribution` 추세 국면 14.3% → **F-44**
- [ ] **1-15 🆕 P2** 15m(27표본) · 30m(14표본)이 `min_samples: 30`에 **구조적으로 도달 불가**.
      이론 상한 26~27 / 13~14. 매일 `FeatureHealthNotJudged` 2줄.
      롤링(3거래일 81/42표본)이 실질 판정 중 → 당일 축 소비처 없음 → **F-45**
- [ ] **1-16 🆕 P2** `TickDeliveryLatency` 한 줄에 전수 축(`by_hour` 71,396) + 토막 축
      (`tail_ringbuffer` 20,000) 혼재, 대표 문구가 토막 쪽. **[C-3] 판정을 반나절 지연시켰다** → **F-46**

### 다음 거래일(2026-08-26) 관측 예정 (L 시리즈 — K 시리즈 승계)

- [ ] **L-1** 1분봉 `publish_offset` 시간대별 초과율·최댓값 — 오늘이 예외였나 추세였나.
      내일도 오후가 오전의 5배 이상이면 **추세 확정** → 수집기 자체 조사
- [ ] **L-2** `grace_headroom_ms`(F-43 구현 시) — 오늘 1,880.3ms. **1,500ms 아래면 P1 → P0 격상 검토**
- [ ] **L-3** F-43 커밋 후 꼬리 축이 뜨고 오늘 로그 replay에서 `drift: true` 재현되는가
- [ ] **L-4** F-41+F-44 커밋 후 `threshold_shadow` · `passed_shadow` 출현. **섀도 20거래일 1일차**
- [ ] **L-5** `regime_distribution` 추세 국면 비율 — 오늘 2/14(14.3%). 1-14의 노출 빈도
- [ ] **L-6** `decision_funnel.pass` 건수와 시각 — **오늘 15:30 1건이 유일.**
      **장중에 열리면 1-14가 실제 위험이 된다**
- [ ] **L-7** `SizerZeroQty` · `shortfall_ratio` · `sizer_funnel` — ③Risk 통과 사이클 필요. **3거래일째 미도달**
- [ ] **L-8** `PublishLoopStalled.detection_lag_ms` — 오늘도 0건. **미검증 며칠째인지 세기 시작**
- [ ] **L-9** 증권사 500 오류 시간대 분포 — 오늘 가설이 깨졌다. **새 가설 필요**
- [ ] **L-10** `.__wtest` 삭제 후 재생성 여부 — 안 생기면 잔재 확정
- [ ] **L-11** `record_vs_commit.verdict` — 커밋했으면 `clean`. 또 나오면 **7거래일째**
- [ ] **L-12** `no-degenerate-features` 기한 — 이행 시 「기한 도달 불가」 경고 소멸. **엿새째 미이행**
- [ ] **L-13** `MetaGateEvaluated` 최대 확률 — 전일 0.6646 · 오늘 0.3566.
      **이틀 연속 0.5 미만이면 「0.5 합격선」 완전 폐기**
- [ ] **L-14** `FeatureHealthNotJudged` 건수 — F-45 커밋 시 0줄

### 사용자 조치 (15:58 장후 최종판 — 이 목록이 12:40 목록을 대체한다)

**지금 당장 위험한 것은 없습니다. 아래는 전부 오늘 밤 하실 일입니다.**

- [ ] **1 ⚠ 오늘 밤 최우선** 자료 지연 감시기부터 고칠지 결정 — **결정 5분 / 구현 90분. 권고: 넣습니다.**
      감시 장치가 "보통 얼마나 늦나"만 봐서 오늘 "가끔 아주 늦는 쪽" 15배 악화를 **"이상 없음"으로 적었다.**
      자료 손실까지 여유 **1.9초** 남음. **안 하면**: 여유가 소진되는 날 경보가 한 번도 안 울린다
- [ ] **2 ⚠ 결정 15분(어제 것) + 15분(오늘 것)** 안전장치 합격선.
      어제 것 — ㉠섀도 20거래일(권고) / ㉡즉시 / ㉢미룸.
      **오늘 새로 알게 된 것** — 설계 표에서 **추세장 합격선이 0**이다. 표를 켜도 추세장에선 여전히 꺼진다.
      **오늘 오후 3시 30분 처음으로 "사겠다"가 나온 그 순간이 추세장이었다.** 막은 것은 다른 규칙(마감 5분 전).
      **하실 일**: 설계 문서 Ver 1.2 §7.1에 추세장 0의 이유가 적혀 있는지 확인 — 15분.
      **안 하면**: 손실이 커지기 쉬운 국면에서 안전장치 하나가 자동으로 꺼진 채 실전에 간다
- [ ] **3 결정 10분 / 구현 약 9시간** 고칠 것 **14건** 순서.
      권고: ①자료 지연 감시(90분) → ②안전장치 두 건 묶어서(95분) → 나머지.
      **F-42는 F-43에 흡수** — 오늘 지연이 단조가 아님이 드러나 "기울기" 설계로는 못 잡는다
- [ ] **4 재시동 — 불필요. 0분.** `code_version.stale: false` · 프로세스 전부 정상 종료됨.
      오늘 밤 커밋은 **내일 08:20 자동 기동에 자동으로 실린다.**
      밤에 억지로 띄우면 기동 창 밖이라 거절되거나 재시작 카운터가 오염된다
- [ ] **5 짧게 끝나는 밀린 일 — 합쳐서 10분.**
      (가) 검증 기한 재조정 **엿새째 미이행 · 3분** — 정작 항목은 3거래일 연속 통과 중. **기한만 잘못 잡혀 있다**
      (나) 빈 파일 삭제 **2분** — 오늘 확인: 아무도 다시 만들지 않는다. **안전**
      (다) 남의 프로젝트 도구 파일 **엿새째 · 5분** — 저장 or 되돌리기.
      **실전 모드 전환 순간 기동 실패한다**
- [ ] **6 기존 결정 대기 — 어제와 동일. 결정만 10분.**
      (가)경고 색상(**권고 노랑**) · (나)상단 바(**권고 문서 수정 15분**) · (다)`app.py` 분할(React 이관 일정) ·
      (라)미커밋 후보 21건 확정(이제 가능) · (마)승격 표본 세는 방식(9월 11일 롤 전까지)
- [ ] **7 하지 않으셔도 되는 것 (오늘 확인 완료).**
      자료 손실 대응 불필요(공식 137,232 vs 우리 137,230 · 비율 0.99999 · 빠진 분 0) ·
      장 마감 배치 확인 불필요(6단계 정상 · 1분 7초) · 재발 확인 불필요(24건 중 23건 통과 · 재발 0)

---

## 2026-08-25 야간 — 사용자 조치 구현 결과 ([MW0601])

> 장후 보고서 「사용자 조치」 중 **1·2·5번을 구현**했다. 3번은 권고 순서 1·2를 따랐고,
> 4번(재시동)은 권고대로 **하지 않았다**. 6번은 사람 결정으로 남는다.
> 상세 근거는 `DECISION_LOG.md` 2026-08-25 야간 항목.

### 넣은 것 (5건)

| 항목 | 대응 이상점 | 상태 |
|---|---|---|
| **F-43** 일중 열화를 꼬리 축으로도 잰다 (F-42 흡수) | 1-13 · 1-9 | ✅ 구현 · 테스트 15건 |
| **F-41** 설계표 「Meta 임계 보정」 배선 (**㉠섀도**) | 1-12 | ✅ 구현 · 테스트 9건 |
| **F-44** 추세장 보정값에 출처를 붙인다 | 1-14 | ✅ **값 변경 없음** — 아래 참고 |
| **F-31** `git_lock_guard.py` 정본 복원 (㉠) | 1-1 | ✅ src/scripts diff 0건 |
| 등록부 `no-degenerate-features` **닫음** · `.__wtest` 삭제 | 1-11 | ✅ 24건 → 23건 |

### 보고서 권고와 **다르게** 한 것 — 세 건

1. **F-44에 0.05를 넣지 않았다.** 지시대로 §7.1 선행 조사(15분)를 했더니 설계표가 추세 행을
   **「기본」**이라 적고 있었다 — 나머지 행은 전부 `+0.05` 형태의 **보정치**다. 즉
   `TREND_UP: 0.0`은 설계 위반이 아니라 설계 그대로이고, **1-14의 뿌리는 1-5(기본 임계 0)**다.
   표에 없는 값을 지어 넣는 대신 출처표를 달고, 그 0이 만드는 결과가 **매일 WARNING으로
   보이게** 뒀다.
2. **검증 기한을 옮기지 않고 항목을 닫았다.** 5-(가)는 「기한 재조정 3분」이었는데, 실측상
   **이미 `streak 3/3` 검증 완료**였다(08-21·08-24·08-25 전부 0.0). 옮길 일이 아니라 닫을 일.
3. **p90 임계를 1.8로 낮추지 않았다.** 보고서 제안 2.0은 정작 08-25(1.90배)를 못 잡는다.
   그러나 3거래일을 나란히 보니 08-25는 이 축에서 **셋 중 가장 얌전한 날**이었다
   (08-21 2.61배 · 08-24 2.20배). 관측 하나에 맞춘 임계는 임계가 아니다.

### ⚠ 내일 아침 반드시 확인할 것 — F-43의 한계

**08-25 로그를 replay해도 그날은 여전히 조용하다.** 그날을 이름으로 잡는 축
(`over_1000_ratio` 1.7% → 25.7%)은 **상류 카운터가 필요**해서 **08-26 로그부터 존재**한다.
`test_the_old_log_alone_still_reports_that_day_clean`이 이 사실을 테스트로 못 박아 뒀다 —
"F-43을 넣었으니 그날도 잡힌다"는 말이 검증 없이 굳지 않게 하기 위해서다.

### ⚠ 보고서의 「여유 1.9초」는 08-25 하루에만 참이다 — 신규 관측

`FeaturePublishOffset.max`: 08-21 **5,528.8ms** · 08-24 3,758.1ms · 08-25 3,119.7ms.
상위 Horizon 유예 상한이 5,000ms이므로 **나흘 전 최악 한 건은 이미 경계를 넘었다.**
`by_horizon`에 `max`가 없어 **어느 계열이 넘었는지 소급 계산조차 불가능**하다 —
`grace_headroom`을 매일 남기기로 한 이유가 정확히 이것이고, 08-26부터 갈린다.
(다만 「경계 초과 = 자료 손실」은 아니다. 발행 오프셋에는 하류 처리도 섞여 있다.)

### 🆕 F-47 — `git_lock_guard.py`가 Git Bash에서 **거짓 「판정보류」**를 낸다 `P2`

오늘 밤 커밋 직전 스테일 락(0바이트·5.2시간)을 만났는데, 가드가 Git Bash에서
*"git 프로세스 1개 실행 중"* 이라며 HOLD를 냈다. **실제로는 0개.**
MSYS가 `tasklist /FI`의 `/FI`를 경로로 변환해 인자 오류가 나고, 그 출력이 계수에 섞인다.
즉 **「셀 수 없었다」가 「1개 있다」로 위장**된다 — 함수 docstring이 스스로
*"셀 수 없으면 None — 0으로 위장하지 않는다"* 고 적어 둔 규약의 반대편 구멍이다.
**이 저장소는 사본이므로 여기서 고치지 않는다** — futures 정본에서 고쳐 복사(F-31과 같은 규약).
오늘은 PowerShell 재판정 → `--reclaim`으로 회수했다(도구 자신의 경로로만 지웠다).


### 🆕 1-18 — 테스트 4건이 매일 바뀌는 운영 파일·환경에 사실을 고정하고 있었다 `P2` (오늘 밤 처리 완료)

전체 회귀 6건 빨간불의 원인이 전부 이 계열이었다(**HEAD에서도 동일** — 오늘 변경 탓이 아니다).
`pending_verifications.yaml` 2건 · `logs/g2_daily_returns.jsonl` 1건 · **그 PC의 Docker 데몬 5건**.
넷 다 처방이 없는 빨간불이고, 상시화되면 **진짜 회귀가 그 옆에 묻힌다.**

**규율로 남긴다**: 테스트가 `logs/`·`configs/` 아래 **실제 운영 파일**을 읽거나 호스트 상태를
부르면 그 단언은 **등호가 아니라 성질**이어야 한다. 등호를 쓰려면 픽스처로 옮긴다.
**판형이 둘 이상이면 안전한 쪽에 맞춘다** — `test_integrity_report.py`는 `_report2`가 호스트를
주입하는데 `_report`만 안 해서 깨졌다. 규율이 주석에만 있고 구조에는 없었다.

### 남은 것 — 11건 (권고 순서 그대로)

F-36 → F-35 · **F-37** · F-32 · F-33 · F-45 · F-46 · F-38 · F-39 · F-40 · **F-47**

- **F-37은 F-41과 짝**이다 — `gate_disabled`가 이제 `threshold_shadow`를 보므로,
  화면 쪽 `gate_notes`도 **같은 조건을 읽는지** 반드시 맞춘다.
- **F-32와 F-31의 순서가 뒤집혔다** — F-31을 먼저 되돌렸으므로 F-32의 (ㄴ) 분기
  (「n거래일 잔존」)를 라이브에서 볼 기회는 다음 외부 유입 때까지 없다. 인공 시나리오로 검증.

### 사람 결정으로 남는 것 — 보고서 사용자 조치 6번 그대로

(가) 안전장치 경고 색 · (나) 화면 상단 바 기준 · (다) `ui/app.py` 1,482줄 분할 시점 ·
(라) 등록부 21건 `fix_committed` 확정 · (마) 승격 표본 계수 방식(9/11 롤 전까지)

### 검증 예정 (다음 거래일 2026-08-26)

| 번호 | 대상 | 확인할 것 | 국면 |
|---|---|---|---|
| **M-1** | F-43 상류 | `FeaturePublishOffset.by_hour` 각 버킷에 `p99`·`over_1000`·`over_1000_ratio`·`over_grace`가 뜨는가 | 장후 |
| **M-2** | F-43 여유 | `grace_headroom.worst_horizon`·`worst_headroom_ms`가 나오는가. **최악이 1m인가 상위인가** | 장후 |
| **M-3** | F-43 하류 | `intraday_trend.publish_offset.axes`에 세 축이 서고 `drift_axes`가 이름을 내는가 | 장후 |
| **M-4** | F-41·F-44 | `MetaGateEvaluated`에 `threshold_shadow`·`passed_shadow`·`regime`·`threshold_adj_source`가 뜨는가. **섀도 20거래일 1일차** | 장중 |
| **M-5** | F-44 | 추세 국면 사이클만 WARNING이고 RANGE/HIGH_VOL은 조용한가 (기본 임계가 0인 한 추세는 계속 운다) | 장중 |
| **M-6** | F-31 | 기동 자가점검 `git` 줄이 `src/scripts 0` 계열로 복귀하는가 | 장전 |
| **M-7** | 등록부 | `deadlines` 줄이 `등록부 23건 · 기한 도달 불가 0건`인가 | 장전 |
| **M-8** | 커밋 반영 | `SessionStart.git_sha` == 오늘 밤 새 SHA인가 (`code_version.stale: false`) | 장전 |

## 2026-08-26 장전 점검 — 신규 항목 ([MW0601])

리포트 `logs/dailycheck/2026-08-26_report.md` · P0 0 · P1 2 · P2 3 · `FixVerificationRecurred` 0건.
**오늘 코드 변경 없음** — 개장 7분 전 실행이라 R11 · 금지계명 3·4로 계획만 세웠다. 전 항목 **장후 적용**.

### 🆕 F-48 — 오늘 아침 미커밋 6파일 커밋 `P1` (1-1) **시한: 오늘 15:45 장후 배치 전**

- [ ] **F-48** 15:35 마감 직후 커밋. **커밋 2개로 가른다** — ① `[MW0601] 체결통보 수신 배선 — 모듈·테스트·프로브`
      (`broker/kis/order_notice.py` 신규 373줄 · `scripts/probe_order_notice.py` 신규 154줄 ·
      `tests/broker/test_kis_order_notice.py` 신규 9건 · `broker/kis/tr_codes.py`)
      ② `[MW0601] 체결통보 배선에 딸린 설정·로그 규약 변경` (`core/config.py` · `broker/kis/credentials.py` ·
      `core/logging.py`). **가르는 이유**: `TAG_LEVELS` 변경은 로그 심각도 매핑을 바꾸는 전역 변경이라
      단독 되돌림 경로가 있어야 한다.
- [ ] **F-48 검증** `git status --porcelain -- src scripts` 빈 출력 → `daily_integrity_20260826.json`의
      `record_vs_commit.verdict == "clean"` (→ N-1). **15:45 이후 커밋하면 `closed_with_uncommitted_source`
      8거래일째**(L-11 연장).
- [ ] **F-48 주의** `git_lock_guard.py`는 **PowerShell에서** 돌린다 — Git Bash는 F-47의 거짓 「판정보류」를 낸다.

### 🆕 F-49 — `stale=false`가 미커밋 소스를 요약에서 가리지 못하게 `P1` (1-2) · 약 90분

- [ ] `core/version.py` — `VersionDrift`에 `worktree_dirty: bool | None` · `source_newer_than_head: bool | None`.
      `assess_version_drift()`에 `head_commit_time` · `source_mtime` · `dirty_files` 인자 (**전부 기본값 `None`**
      — 기존 호출부 무손상). `source_mtime > head_commit_time`이면 `summary` 확장.
      **`stale` 자체는 건드리지 않는다** — 그 필드의 뜻("커밋 간 드리프트")은 지금도 옳고, 뜻을 바꾸면
      이 값을 읽는 곳이 전부 흔들린다.
- [ ] `ops/status_board.py` 193~211행 — 세 인자 전달 · `code_version`에 `source_mtime_max` ·
      `source_newer_than_head` 적재. **`None`은 미측정 유지**(0으로 위장 금지 — L18).
...
- [ ] **검증** 4조합 단위테스트(미커밋 유무 × 소스 최신 여부) · `pytest tests/core/test_version.py
      ...
      `summary` 단언은 **1-18 규율대로 등호가 아니라 성질로**.
- [ ] **⚠ 사람 결정** 경고 강도 — ㉠ 요약 문자열만(**권고**) ㉡ 자가점검 `[WARN]` 승격 ㉢ live 기동 거부.
      **㉠ → 20거래일 관측 → ㉡** (R18 취지 · `source_mtime_max` 독스트링이 오탐을 예고했다). **결정 5분.**

### 🆕 F-50 — 체결 알림 태그 "0줄"을 「미배선」과 「알림 없음」으로 가른다 `P2` (1-3)

- [ ] `core/logging.py` `OrderNotice*` 블록에 `# 미배선(2026-08-26 기준) — 배선 시 이 줄을 지운다`
- [ ] `ops/status_board.py` `components`에 `broker.order_notice` 상태 **`NOT_WIRED`** 추가.
      **`UNKNOWN` 금지** — `phases.md` D절이 경계한 회색을 또 만들지 않는다. `NOT_WIRED`는
      "모른다"가 아니라 **"아직 안 붙였다"라는 확정된 사실**이다.
- [ ] **선행** F-51(배선 여부 결정)을 먼저 받는다. 배선하면 이 항목 절반이 자동 소멸한다.

### 🆕 F-51 — Capability Matrix를 오늘 상태로 맞춘다 `P2` (1-4) · 5분 · F-48에 동승 가능

- [ ] `Docs/capability_matrix.md` 28행 비고 → `"수신 모듈 구현 완료(broker/kis/order_notice.py, 373줄,
      단위테스트 9건) · 실행 경로 미배선 · 22필드 전부 선물옵션 실응답 미검증(의미 근거는 국내주식
      H0STCNI0 문서) · 실측 절차 scripts/probe_order_notice.py"`. **「실측 검증됨」 칸은 `—` 유지.**
- [ ] 309행 *"encrypt=\"Y\" TR 전용 복호화 키인지는 여전히 추정"* 에
      `(2026-08-26: order_notice.py가 이 추정 위에 구현됨 — 프로브 실측 전까지 추정 유지)` 추가.
- [ ] **근거** SYSTEM.md §2 Capability Matrix 의무 — *"실측 검증 안 된 기능은 사용 금지"*(L9·L19·L26).
      **금지계명 11(필드 실측 없는 스키마 금지) 대기 상태.**

### 🆕 C-1 — 체결통보 실측 프로브 (배선보다 **먼저**) `P2` · 15분 · 장후

- [ ] `scripts/probe_order_notice.py` **모의계좌 1회 실행** → `OrderNoticeSubscribed`(구독 성공 +
      복호 키 수신)가 뜨는가. **장중 실행 금지**(R11 취지).
- [ ] 같이 판정되는 것: `.env`의 `KIS_HTS_ID` 유효성. **키는 존재하나 값 미확인** —
      `resolve_secret(required=False)`가 미설정을 빈 문자열로 돌려주므로 **비어 있어도 기동은 통과한다.**
      `config.py` 주석: *"조용히 빈 tr_key로 구독하면 「통보가 안 온다」로 며칠을 쓴다."*
- [ ] **실측 없이 배선 금지** — 22필드 의미 근거가 국내주식 H0STCNI0 문서뿐이다(금지계명 11).

### 🆕 C-2 — `resolve_secret(required=False)` 오용 여지 판정 `P2` · 장후

- [ ] `grep -rn "required=False" --include=*.py src/` → 호출부가 `hts_id_ref` 하나뿐인가.
- [ ] `tests/core/test_config.py`에 "필수 시크릿(`app_key_ref`·`app_secret_ref`·`account_ref`)은
      여전히 `RuntimeError`를 던진다" 회귀 테스트가 있는가. 없으면 추가.
      (기본값 `required=True`라 기존 세 참조 동작 불변인 것까지는 `git diff`로 확인했다.)

### 🆕 C-3 — `OrderNoticeStream`이 `listen()`을 안 쓰는 이유가 **시세 수집기에도 해당하는가** `P1 후보` · 장중·장후

- [ ] `order_notice.py` 독스트링: *"시세 수집기가 지금까지 PINGPONG 없이 버틴 건 장중 내내 틱이 흘러
      핑을 받을 일이 없어서지, 안 해도 되기 때문이 아니다."* — 이 진술이 맞다면 `data/collector.py`도
      같은 위험을 안고 있다.
- [ ] 판정 재료: 장중 시세 WS 재연결 태그 건수 + **점심시간(12:00~13:00) 틱 공백 구간 생존**(→ N-5).
      오늘 로그로는 끊긴 적이 없어 판정 근거가 없다.

### 🆕 G-28 — 정시 기동 직전 편집을 기동이 스스로 알아채게 한다 · 약 60분 · **이번 주** (선행 F-49)

- [ ] **관측 근거**: 오늘 편집 08:02:12~08:14:12 · 트리거 08:20:00 → **여유 5분 48초.**
      부팅 07:21 ~ 기동 08:20의 59분 창 중 **마지막 6분이 커밋 몫**이다. 반복될 조건이다.
- [ ] `core/health.py` — `source_mtime_max`와 기동 시각 차이가 **30분 이내**면 별도 줄:
      `[WARN] 기동 {n}분 전 소스 편집 감지({HH:MM:SS}) · 미커밋 {n}파일 · 오늘 로그는 커밋으로 재현되지 않는다`
- [ ] `ops/task_schedule.py` — **`Messiah-ClockResync`(08:10)에 커밋 상태 점검을 얹어** 미커밋
      `src/scripts` 변경 시 화면 상단 바에 10분짜리 알림. **차단이 아니라 알림** — dev에서 차단하면 개발이 멈춘다.
- [ ] **측정 지표**: `record_vs_commit.verdict == "clean"` 거래일 비율 —
      **현재 최근 7거래일 중 0일(0%)** → 목표 **20거래일 중 18일(90%) 이상.**
- [ ] **R18 해당 없음** — 차단 게이트가 아니라 알림이므로 섀도 20거래일 승격 절차 불필요.
      위험: 오탐이 잦으면 그 줄을 안 읽게 된다 → 30분 임계는 관측 후 조정.

### 🆕 G-29 — 「등록됐으나 발신처 없는 태그」를 매일 자동으로 센다 · 약 45분 · **이번 주** (선행 없음)

- [ ] **관측 근거**: 1-3을 찾은 방법이 `grep -rn "order_notice"` **한 줄**이었다. 손으로 했고 안 했으면 못 봤다.
      `TAG_LEVELS`는 100개를 넘고 **8종이 한 번에 늘었는데 증거 다이제스트 어디에도 안 나타났다** —
      §3 집계는 *찍힌* 태그만 세기 때문이다.
- [ ] `collect_evidence.py`에 §10 「태그 규약 대 실제」 신설:
      (ㄱ) `TAG_LEVELS`에 있으나 `src/`·`scripts/`에서 인자로 안 쓰이는 태그 = **발신처 없음**
      (ㄴ) 코드에서 찍는데 `TAG_LEVELS`에 없는 태그 = **규약 누락(R6 위반 — 심각도 미정의)**
      (ㄷ) 당일 로그 실제 출현 태그와의 3자 대조표
- [ ] **측정 지표**: 「발신처 없음」 태그 수 — **오늘 기준선 8종** → 0으로 수렴하는가.
- [ ] 정규식 `grep`으로 충분(AST 불요). **동적 태그명 자리는 오탐이 나므로 「확인 필요」로 내고
      「위반」으로 내지 않는다.** 스킬 스크립트 단독 변경이라 본체 회귀 위험 0.

### 🆕 G-30 — 국면 시드가 `TREND_UP`인 날은 장전이 미리 경고한다 · 약 30분 · **다음 단계**(M-4·M-5 판정 후)

- [ ] **관측 근거**: 08:25:28 `RegimeSeeded` **`TREND_UP`(확신도 0.98)**. `META_THRESHOLD_ADJUSTMENT[TREND_UP] = 0.0`
      (1-14 · F-44)이고 어제 추세 비율은 14건 중 2건(**14.3%**)이었다. **어제 "드문 위험"이 오늘 "개장 즉시 노출"이다.**
- [ ] 장전 자가점검에 `regime_seed` 줄: `[WARN] 국면 시드 TREND_UP · 이 국면의 Meta 임계 보정은 0.0 ·
      섀도 판정만 기록됨(F-41 {n}일차)`. 판정은 `META_THRESHOLD_ADJUSTMENT[시드국면] == 0.0` 한 줄.
- [ ] **선행 조건**: **M-4가 실패하면 이 제안은 근거를 잃는다** — `threshold_shadow`가 오늘 장중에
      실제로 뜨는지 먼저 확인.
- [ ] 위험: 추세 시드가 흔하면 매일 뜨는 줄이 되어 의미가 닳는다. 14.3%는 **사이클 비율이지 시드 비율이
      아니므로 시드 기준 빈도는 오늘부터 새로 센다**(N-2).

### 🔄 F-52 — 승격 관문 3종 측정 `P2` (1-5) · **3거래일 연속** · 기존 항목

- [ ] `real-20260820-2053-30m` 미통과 관문 `sharpe` · `max_drawdown` · `negative_window_ratio`.
      08-24·08-25·08-26 자가점검 `bundle` 줄 동일. **새 계획을 세우지 않는다** — 「사용자 조치」 기등재 항목.
      우회 플래그는 의도적 부재(DECISION_LOG 9657~9660). 정규 경로 = 성과 3종 측정.

### 오늘(2026-08-26) 장중·장후 관측 예정 (N 시리즈)

| ID | 관측 대상 | 판정 기준 | 국면 |
|---|---|---|---|
| **N-1** | `record_vs_commit.verdict`(오늘분) | F-48을 15:45 전에 하면 `clean`. 못 하면 `closed_with_uncommitted_source` **8거래일째**(L-11 합류) | 장후 |
| **N-2** | 국면 시드 `TREND_UP`(0.98) 유지 사이클 수 | 09:00 첫 실사이클까지 유지되면 **1-14가 개장 즉시 노출.** 3사이클 안에 RANGE로 바뀌면 웜업 잔상 | 장중 |
| **N-3** | UI 네 토픽 배지(`FuturesView`·`DecisionIntent`·`RegimeState`·`CircuitBreakerStatus`) | 08:20:44 전부 `NO_DATA`(개장 전 정상). **09:05까지도 `NO_DATA`면 이상점** — `RegimeState`는 08:25에 이미 시드됐다 | 장중 |
| **N-4** | `OrderNotice*` 8종 출현 건수 | **0건이 예상값**(미배선). 1건이라도 뜨면 배선 경로를 못 찾은 것이므로 **1-3 정정** | 장중·장후 |
| **N-5** | 시세 WS 재연결 태그 · 12:00~13:00 틱 공백 생존 | 0건이면 C-3 오늘 무증상. 1건 이상이면 **PINGPONG 미응답 가설이 시세 쪽에서도 실증** | 장중·장후 |
| **N-6** | 옵션체인 폴 결손 | 장전 12건 전부 `42/42다리`. 장중 `n/42`(n<42) 발생 시 **F-26(결손은 그 자리에서 운다)** 계기가 실제로 우는가 | 장중 |
| **N-7** | `clock offset` 재측정 | 장전 −0.123초 · 개장 −0.16초. **절댓값 1초 초과 시 1분봉 유예 2,000ms의 50%**를 잡아먹어 1-9 해석에 직접 영향 | 장중 |

### 어제 M 시리즈 처분 (장전 판정분 — 나머지는 장중·장후 이월)

- [x] **M-6** F-31 — `src/scripts 0` **미복귀(6파일)이나 `git_lock_guard.py`는 실변경 목록에 없다** →
      **F-31 성공 · 재발 아님.** 6파일은 오늘 아침 신규 작업(1-1). `FixVerificationRecurred` **0건**
- [x] **M-7** 등록부 — ✅ **해소** `등록부 23건 · 기한 도달 불가 0건 · 기한 임박 0건` (커밋 `35990bd`) → **L-12 동시 해소**
- [x] **M-8** 커밋 반영 — ✅ **해소** `git_sha == HEAD == 02855c8` · `stale: false`. 단 「반영됐다」 ≠ 「이것만 실렸다」 → 1-2
- [x] **L-10** `.__wtest` — ✅ **해소 · 잔재 확정** (삭제 후 장후 배치 1회·기동 3회 거쳐 재생성 없음)
- [x] **L-12** `no-degenerate-features` 기한 — ✅ **해소** (M-7과 동일 근거)
- [ ] **L-11** `record_vs_commit` — 🔄 장후 이월(→ N-1). 08-25의 `closed_with_uncommitted_source`는
      **정상** — 배치 15:46 vs 커밋 21:43
- [ ] **M-1 · M-2 · M-3** (F-43 산출물) — ⏭ 장후 이월
- [ ] **M-4 · M-5** (F-41·F-44 섀도 **1일차**) — ⏭ 장중 이월. **M-5는 오늘 이른 시각 판정 가능성 높음**
      (08:25 시드가 `TREND_UP`)
- [ ] **L-1~L-9 · L-13 · L-14** — ⏭ 장중·장후 이월

### 사용자 조치 (2026-08-26 장전판 — 장중·장후가 갱신한다)

- [ ] **0. 지금 — 없음. 0분.** 개장 직전. 코드 변경·재기동 금지(R11 · 금지계명 3·4).
- [ ] **1. 15:35 직후 커밋 — 10분. 시한 15:45.** 안 하면 오늘 로그 재현 불가 + 8거래일째 연장. (F-48)
- [ ] **2. 상태판 요약 수정 — 결정 5분 / 구현 90분.** 경고 강도 ㉠㉡㉢ 중 선택(권고 ㉠→㉡). (F-49)
- [ ] **3. 체결통보 프로브 1회 — 15분.** 배선보다 **먼저**. 안 하면 금지계명 11. (C-1)
- [ ] **4. Capability Matrix 갱신 — 5분.** F-48에 동승 가능. (F-51)
- [ ] **5. 승격 관문 3종 측정 — 사람 결정 대기, 3거래일째.** live 전환 당일 막힌다. (F-52)

## 2026-08-26 장중 점검 산출 (12:36 · [MW0601])

> 전문 `logs/dailycheck/2026-08-26_report.md` 제2부. 장전판 F-48~F-52 · G-28~G-30은 **전부 유효 · 취소 없음.**
> **단 F-48은 F-53을 선행조건으로 갖게 됐다.**

### ⚠ 선행 조건 — F-53(잠금 회수)을 먼저. 안 하면 오늘 밤 커밋·문서 갱신이 전부 저장 불가.

- [ ] **F-53 (30초 · P1 · 🆕 · ⚠ 지금 실행 가능)** `.git/index.lock` 스테일 회수 — 대응 **1-6**.
      **PowerShell에서** `python scripts\git_lock_guard.py --check` → rc=2면 `--reclaim`
      (Git Bash는 F-47의 거짓 「판정보류」). **코드 변경·재기동이 아니므로 R11 대상 아님.**
      - [ ] 검증: 회수 후 `git status` rc=0 **그리고 `git add -n .` rc=0** — 쓰기 경로는 쓰기로만 확인한다
      - [ ] `scripts/git_lock_guard.py`는 futures 정본 바이트 동일 사본 — **직접 수정 금지**(DECISION_LOG 9581·9603)
      - [ ] 장후 증거 §9 적신호 1번 소멸 확인 → **N-8**

- [ ] **F-54 (약 60분 · P1 · 🆕 · 장후)** 점검이 저장소에 락을 남기지 못하게 한다 — 대응 **1-6**.
      **F-34 하위 3건의 재발행**(어제 결정 → 오늘 재발, 3건 전부 미적용 확인).
      - [ ] `.claude/skills/messiah-daily-check/SKILL.md` §2에 명령 규약 신설 — *"점검 중 모든 git은
            `git --no-optional-locks ...`. `status`·`diff`·`log`도 예외 아님 — 읽기처럼 보이지만 인덱스를 다시 쓴다."*
            예시 명령 전부 그 형태로 교체. `references/evidence_map.md` git 예시도 동일
      - [ ] `collect_evidence.py` §9 — 락 mtime과 다이제스트 생성 시각 차 **±120초** 이내면
            `점검 중 생성 의심(생성 08:50:29 → 락 08:50:59, +30초)` 병기. **오늘 이 줄이 있었으면 08:51에 알았다**
      - [ ] `scripts/self_check.py` `_check_git()` — 인덱스락 **3상태**(미측정/없음/있음+나이) 한 줄.
            **0으로 위장 금지**(L18). 판정 실패는 예외 삼키고 `미측정`으로 — 기동을 막지 않는다
      - [ ] (선택) `git --no-optional-locks add --dry-run .` rc 확인. **`--no-optional-locks` 필수** —
            안 붙이면 **검사가 락을 만든다**(오늘 실수의 자동화 판)
      - [ ] **결정 필요**: 락 있을 때 `[OK ]`인가 `[WARN]`인가 → **권고 `[WARN]` · 기동은 허용.**
            차단은 과녁이 아니다 — 오늘 피해는 기동이 아니라 **저장**에 났다
      - [ ] 검증: 인위적 락 상태에서 `self_check` 실행 · 오늘 증거에 수집기 재실행 · `pytest tests/test_bus_and_scripts.py`

- [ ] **F-55 (약 90분 · P1 · 🆕 · 장후)** 메타 관문이 **이번 사이클의 국면**을 쓰는지 말하게 한다 — 대응 **1-7**.
      - [ ] **선행: 확인 필요 판정(코드 읽기만 · 15분 · 실행 불필요)** — ㉠ `strategy/regime/runtime.py`에서
            `RegimeClassified` 로깅과 `bus.publish(RegimeState)`의 코드상 순서 ㉡ 이벤트 루프 태스크 스케줄
            ㉢ `strategy/futures/service.py` 사이클 진입 시 `_latest_regime` 스냅샷 시점.
            **㉢이면 위 설계 대신 「사이클 진입 시 국면을 명시 요구」가 맞다**
      - [ ] `service.py` — `_latest_regime_at`·`_latest_regime_as_of` 보관 → `feature_as_of`와 대조해
            `regime_is_current: bool`. **판정 불변, 싣기만 한다**(F-41과 같은 섀도 규율 · R18)
      - [ ] `MetaGateEvaluated`에 `regime_is_current`·`regime_as_of`·`regime_age_ms` 3필드 추가.
            **새 태그 불필요 → R6 위반 없음**
      - [ ] `aggregator.py` 227행 `regime_source`에 세 번째 값 **`"received_stale"`** 추가
      - [ ] 회귀: `ops/integrity_report.py` 1246행 · `ops/pass_cycles.py`가 이 태그를 읽는다.
            스키마 단언이 있으면 **1-18 규율대로 등호가 아니라 성질로** 고친다
      - [ ] **결정 필요**: 어긋난 사이클을 20거래일 표본에서 뺄 것인가 → **권고 빼지 말고 표시.**
            `false`를 실어 두면 포함/제외 양쪽 계산 가능. **지금 버리면 되돌릴 수 없다**
      - [ ] **오늘 1일차 2건(09:00 · 12:30)은 표시 수단 없이 지나갔다** — DECISION_LOG의 8행 대조표가 보정 근거
      - [ ] 검증: 오늘 로그 replay(09:00·12:30에만 `false` · 금지계명 2) · 단위테스트 2케이스 · 익일 실측 → **N-9 · N-13**

- [ ] **F-56 (약 60분 · P2 · 🆕 · 장후)** 화면의 생사를 **장중에** 판정 가능하게 — 대응 **1-8**.
      - [ ] `ui/app.py` 라이브 구독 스레드가 **5분 주기** `UILiveSubscriberHeartbeat` 발행
            (마지막 수신 토픽·건수·경과). **`UISnapshotFreshness`를 반복 발행하지 않는다** — 그 태그의 뜻
            (「첫 렌더가 무엇을 그렸나」)을 흐리면 안 된다
      - [ ] `core/logging.py` — `UILiveSubscriberHeartbeat`(INFO) 1종. **R6 준수**
      - [ ] `ops/status_board.py` — `command_center_ui`를 두 축으로 `{"port": "UP", "subscriber": "OK|STALE|UNKNOWN"}`.
            **`UNKNOWN`을 정상으로 접지 않는다**(phases.md D절). 읽는 쪽 2곳(427행 · UI 상단 바) 동반 수정
      - [ ] `collect_evidence.py` §2 — 프로세스별 `Windows fatal exception` 카운트.
            **오늘 이 값이 §9에 있었으면 즉시 걸렸다**(지금은 접힌 블록에 묻힘)
      - [ ] **결정 필요**: 하트비트 주기 → **권고 5분**(1분은 로그가 붐비고 15분은 장중 점검 해상도가 떨어진다)
      - [ ] 검증: 구독 스레드 강제 종료 시 `STALE` 전이 · 오늘 로그 수집기 재실행 · `pytest tests/ops/test_status_board.py`

### 🆕 G-31 — 「발행한 값」과 「소비한 값」을 매일 자동 대조 · 약 50분 · **이번 주** (선행 없음)

- [ ] `collect_evidence.py` §11 「발행 대 소비」 — (발행태그.필드)→(소비태그.필드) 쌍 등록, 같은 사이클 창 **±5초** 불일치 표
- [ ] 등록 3쌍: `RegimeClassified.regime`→`MetaGateEvaluated.regime` / →`DecisionEmitted`(국면 실으면) /
...
- [ ] **측정 지표**: 불일치 사이클 비율 — **기준선 8회 중 2회(25%)** · 전환만 보면 **3회 중 2회(67%)** → 목표 20거래일 이동평균 5% 이하
- [ ] **위험**: 정상 시차 오탐 → 「불일치」로만 내고 **「위반」으로 내지 않는다**(G-29와 동일 규율)
- [ ] **본체 회귀 위험 0**(스킬 스크립트 단독). **F-55보다 먼저 넣으면 F-55의 검증 도구가 된다**

### 🆕 G-32 — 지연 지표 3축을 로그가 스스로 구분하게 한다 · 약 40분 · **다음 단계** (선행 M-1~M-3)

- [ ] **관측 근거**: 오늘 세 값이 한 화면에서 섞였다 — `전일 회선 p99 1026ms`(delivery_latency) /
      `publish_offset_ms` p99 **1,602ms**(거래소 봉마감 대비) / `bar_to_publish_ms` p99 **141ms**(봉확정 대비).
      **갈라내기 전에는 「1,026 → 1,602 악화」라는 틀린 문장이 성립했다**
- [ ] ① 장전 자가점검 `bar_close` 줄에 축명 병기(괄호 약칭 — 이미 가장 긴 줄 중 하나다)
- [ ] ② `ops/integrity_report.py` F-43 산출물 키에 축 명시 — **M-1~M-3 판정 후 키 구조 확정하고 착수**
- [ ] ③ `references/evidence_map.md`에 「지연 3축 대조표」 — 무엇 대비 무엇인지 / 어느 임계와 비교하는지
- [ ] **측정 지표**: 축 표기 있는 지연 지표 **1/3(33%)** → 목표 **3/3**

### 오늘(2026-08-26) 장후 관측 예정 (N 시리즈 이어받음)

| ID | 관측 대상 | 판정 기준 | 국면 |
|---|---|---|---|
| **N-8** | `.git/index.lock` 소멸 | F-53 실행 시 부재 + `git add -n .` rc=0. 여전히 있으면 **오늘 밤 저장 전부 불가** | 장후 |
| **N-9** | `TREND_UP` **유효** 표본 | 장후 6사이클(13:00~15:30)에서 `RegimeClassified`=`TREND_UP` **그리고** 같은 사이클 `MetaGateEvaluated.regime`=`TREND_UP`이면 **M-5 무조건부 해소**. 없으면 익일 이월 | 장후 |
| **N-10** | `daily_integrity_20260826.json` `crash_forensics.findings` | `ui` 덤프 1건이 **네 요소**(언제 하한/무엇이/죽었나/어디서)로 나오는가 — P1-3 라이브 검증. **`survived=False`는 UI엔 무의미** — 그 무의미함 재확인 시 **F-56 근거 확정** | 장후 |
| **N-11** | F-43 산출물(M-1~M-3) | `over_grace`가 **장중 관측 4건 이상**과 맞는가. 어긋나면 계측기와 원본이 다른 것을 세고 있다 | 장후 |
| **N-12** | `OptionChainPollRetried` 종일 | 12:36까지 4건. **종일 8건 이하 = 전일 동급, 정상.** 12건 이상이면 브로커 회선 별건 등록 | 장후 |
| **N-13** | 국면 전환 사이클 일치율 | 오전 전환 3회 중 2회 불일치. 장후 전환분 누적 → **G-31 기준선 확정** | 장후 |
| **N-14** | `passed_shadow` 종일 분포 | 12:36까지 8건 중 **7건 false(87.5%)**, 어제 8/8 false(100%). **「섀도를 켜면 전량 차단으로 도는가」가 R18 20거래일의 핵심 질문** — 종일값으로 1일차 확정 | 장후 |

### 장전 N 시리즈 처분 (장중 판정분)

- [x] **N-2** 국면 시드 — ✅ **해소 · 웜업 잔상 확정.** 08:25:28 `TREND_UP`(0.981) → 09:00:00.705 `RANGE`(0.7924),
      1사이클 만에 교체. **장전 우려(추세 임계 0.0 상시 노출)는 일어나지 않았다.** 단 시드값이 09:00 관문 1건에 샜다 → 1-7
- [x] **N-4** `OrderNotice*` — ✅ **해소.** 세 로그 전부 **0건**. 1-3 진단 확정, 정정 불필요
- [x] **N-5** 시세 재연결·점심 공백 — ✅ **해소(무증상).** 재연결/끊김 태그 0건 · `l1_daily` ERROR/WARNING **0행** ·
      12:00~12:38 1분봉 결손 0. **C-3은 무증상이지 반증 아님**
- [x] **N-6** 옵션체인 결손 — ✅ **해소.** 103건 **전부 42/42다리**
- [x] **N-7** `clock offset` — ✅ **해소 · 안정.** −0.174 ~ −0.147초, 변동 최대 0.019초. 유예 2,000ms의 **8.7%**
- [x] **M-4** F-41·F-44 섀도 — ✅ **해소.** 8건 전건에 4필드. **섀도 1일차 완료 · 잔여 19거래일**
- [ ] **M-5** 추세만 WARNING인가 — ⚠ **조건부 해소 · 유효 표본 0건.** RANGE 2·HIGH_VOL 5 전부 조용(설계 확인).
      유일 WARNING은 1-7이 잘못 실은 `TREND_UP` → **N-9로 재관측**
- [ ] **N-3** UI 배지 — ⏭ **판정 불가(관측 수단 부재 · 장전 전제 오류).** 결함 아님 → 1-8. 제1부에 정정 포인터 1줄 부착
- [ ] **N-1 · L-11** `record_vs_commit` — ⏭ 장후 이월 · ⬆️ **위험 격상**(1-6으로 커밋 봉쇄)
- [ ] **M-1 · M-2 · M-3** F-43 산출물 — ⏭ 장후 이월(유지) → N-11
- [ ] **C-1 · C-2** — 🔄 장후 이월(유지). **C-3** — 🔄 지속, 오늘 무증상
- [ ] **L-1~L-9 · L-13 · L-14** — ⏭ 장후 이월(유지). **차례가 아니다 — 결함이 아니다**

### 사용자 조치 (2026-08-26 장중판 — 장후가 갱신한다)

- [ ] **0 ⚠ 지금 — 잠금 파일 회수(F-53). 30초.** PowerShell에서 `git_lock_guard.py --check` → rc=2면 `--reclaim`.
      **안 하면 아래 1번이 그 자리에서 실패한다.** 어제도 같은 일(08:51:14), 오늘 08:50:59. 이틀 연속
- [ ] **0-b 지금 — 브라우저로 `http://localhost:8511` 한 번 열기. 1분.** 새 첫 렌더가 `UISnapshotFreshness`
      두 번째 줄을 남겨 **N-3을 사후 판정 가능하게** 만든다. 관측만 건드리므로 R11 대상 아님
- [ ] **1. 15:35 직후 커밋 — 10분. 시한 15:45.** (F-48 · **선행 F-53**) 안 하면 8거래일째 연장
- [ ] **2. 상태판 요약 수정 — 결정 5분 / 구현 90분.** (F-49) **12:36에도 요약 문장 그대로임을 재확인**
- [ ] **3. 체결통보 프로브 1회 — 15분.** (C-1) **오늘 관련 기록 0줄로 미배선 진단 확정됨**
- [ ] **4. Capability Matrix 갱신 — 5분.** (F-51) 변화 없음
- [ ] **5. 승격 관문 3종 측정 — 사람 결정 대기, 3거래일째.** (F-52) **오늘 8사이클 전부 이 번들이 판단했다**(dev, 손실 0원)
- [ ] **6-1. 점검이 저장소를 막지 못하게 — 약 60분.** (F-54) **안 하면 내일 아침 또 난다**
- [ ] **6-2. 국면 최신성 표시 — 15분 조사 + 약 90분.** (F-55) **조사는 코드 읽기만, 실행 불필요**
- [ ] **6-3. 화면 생사 장중 판정 — 약 60분.** (F-56) 급하지 않음(화면은 거래 경로 아님)

### 15:10 장중 후속 — 정정 2 · 신규 1 · Fix 4 ([MW0601])

관측 12:36~15:10. **코드 변경 0 · 커밋 0**(마감 25분 전 · R11 · 금지계명 3·4).

- [ ] **F-53-b (30초 · P1 · ⚠ 15:35 전)** 15:02:02 잠금 회수 — **1-6 재개**.
      12:36~15:01 사이 사용자 회수 성공(F-53 ✅) → **15:02:02 이 점검 세션이 `git add -n .`으로 재생성.**
      샌드박스 unlink 거부(`Operation not permitted`) → **PowerShell에서만 가능**
      - [ ] `cd C:\Users\82108\PycharmProjects\fuoption` → `python scripts\git_lock_guard.py --reclaim`
      - [ ] **검증을 `git add -n .`으로 하지 말 것 — 그 명령이 방금 문제를 만들었다.**
            **F-48 커밋을 바로 시도하고 커밋 성공을 검증으로 삼는다**
- [ ] **F-57 (약 20분 · P2 · 장후) — F-54 ④ 대체** 「쓰기 가능한가」를 **락을 만들지 않고** 묻는다
      - [ ] ~~F-54 ④ `git add --dry-run` rc 확인~~ **폐기** — `--no-optional-locks`는 optional 락만 억제한다.
            `add`는 드라이런이어도 진짜 인덱스 락을 잡는다. **넣었으면 매일 아침 락을 만드는 자동화였다**
      - [ ] `scripts/self_check.py` `_check_git()` — `git_lock_guard.inspect()`를 **임포트해 재사용**
            (존재·크기·나이·git 프로세스 수 4가지 읽기). **부작용 0**. `git_lock_guard.py` **수정 금지**(정본 사본)
      - [ ] 출력 3상태: `인덱스락 없음` / `[WARN] 인덱스락 있음(0바이트 · 8분) — 오늘 커밋 불가` / `인덱스락 미측정`
      - [ ] 검증: 실행 전후 `.git/index.lock` 부재 유지 확인(**검사가 락을 안 만드는지가 핵심**)
- [ ] **F-58 (약 25분 · P1 · 장후 · F-54보다 먼저)** 「즉시 조치」와 「재발방지」에 **번호를 따로 준다** — 대응 **1-9**
      - [ ] `references/report_template.md` Fix 절 + `SKILL.md` §5에 규칙 1줄:
            *"한 사고에서 「지금 멈추게 하는 조치」와 「다시 안 나게 하는 조치」가 함께 나오면 번호를 나눈다.
            즉시 조치가 완료돼도 재발방지 번호는 열려 있어야 한다."*
      - [ ] NEXT_TODO 기존 ID 없는 미해소 항목: `grep -nE "^\s*- \[ \].*(하위|미해소)"` 로 후보 →
            **사람이 확정**(자동 부여 금지 — F-20 규율)
      - [ ] **측정 지표**: 「ID 없는 미해소 항목」 수 — 기준선 1건 이상 → 목표 **0 유지**
- [ ] **F-59 (약 40분 · P2 · 장후 · 맨 마지막)** 등록부가 인덱스락 계열을 받게 한다 — 대응 **1-9 ②**
      - [ ] `ops/integrity_report.py` — `git_index_lock: {present, age_hours, created_near_check}`
            (**`null`=미측정 유지**, 0으로 위장 금지)
      - [ ] `configs/pending_verifications.yaml` 헤더 지표 목록에 `git_index_lock_present`(max 0) 추가 +
            F-54 등재 · **`consecutive_days: 5` 권고**(이틀 연속 났으므로 하루로는 우연과 구분 불가)
      - [ ] `ops/fix_verification.py` 채점 배선 · `tests/ops/test_fix_verification.py` 화이트리스트 단언 수정
      - [ ] **결정**: `created_near_check`는 **채점에 넣지 않는다**(진단용. 지표를 늘리면 통과 줄만 쌓여
            재발이 묻힌다 — 등록부 헤더 15~16)

- [ ] **[깨끗한 시험] 30초 · 장후 · 회수 직후 커밋 전 1회** — `status`와 `add -n` 중 범인 확정
      ```
      python scripts\git_lock_guard.py --reclaim
      Test-Path .git\index.lock                                  # False
      git --no-optional-locks status --porcelain | Out-Null
      Test-Path .git\index.lock                                  # True면 범인=status
      git --no-optional-locks add -n . | Out-Null
      Test-Path .git\index.lock                                  # True면 범인=add -n
      python scripts\git_lock_guard.py --reclaim                 # 시험 뒤 반드시
      ```
      **예측: status False · add -n True.** 빗나가면 **F-54 ①의 전제가 무너진다** →
      「점검 중 git 손호출 자체 금지」로 F-54 재작성

### 사용자 조치 (2026-08-26 15:10 갱신판)

- [ ] **0 ⚠ 지금(15:35 전) — 잠금 재회수. 30초.** `python scripts\git_lock_guard.py --reclaim`.
      **12:36 이후 치워 주신 것 확인함(15:01 부재).** 15:02에 **이 세션이 다시 만들었다** — 사과와 함께 보고.
      **그 뒤 저장 가능 여부를 따로 시험하지 말고 바로 커밋할 것**
- [ ] **1. 15:35 직후 커밋 — 10분. 시한 15:45.** (F-48 · 선행 F-53-b) 변경 없음
- [ ] **2~5.** 12:36판 그대로
- [ ] **6-1~6-3.** 12:36판 그대로 (F-54 ①②③ · F-55 · F-56). **단 F-54 ④는 폐기 → F-57**
- [ ] **6-4. F-57 (약 20분)** 아침 점검이 「저장 가능한가」를 **들여다보기만** 하도록 바꾼다
- [ ] **6-5. F-58 (약 25분)** 「고친 일」과 「다시 안 나게 하는 일」에 번호를 따로 준다. **F-54보다 먼저**
- [ ] **6-6. F-59 (약 40분)** 자동 채점표가 이 종류를 받게 한다. **맨 마지막**
- [ ] **7. 깨끗한 시험 30초** — 회수 직후, 커밋 전. 시험 뒤 반드시 재회수

### 15:22 장중 후속 — 원인 확정 · 정정 2 · Fix 3 ([MW0601])

- [x] **F-53-b ✅ 완료** 사용자가 `--reclaim` 실행(`STALE 회수 완료 — 0바이트 · 0.3시간 · git 프로세스 0개`).
      15:19 락 부재 확인. **1-6 ✅ 해소**
- [x] **[깨끗한 시험] ✅ 취소 — 답이 나왔다.** 사용자의 **플래그 없는 네이티브 `git status`가 락을 안 남겼다.**
      **범인은 명령이 아니라 실행 위치**(마운트 unlink 거부). 네이티브 git은 자기 락을 지운다
- [ ] **F-60 (약 60분 · P1 · 장후) — F-54 ① 대체** 점검 세션은 **마운트에서 git을 실행하지 않는다**
      - [ ] ~~F-54 ① `--no-optional-locks` 명문화~~ **폐기** — 빈도만 줄이고 원인(마운트 권한)은 그대로.
            플래그 붙여도 `add`/`commit` 한 번이면 재발(15:02가 증명)
      - [ ] `collect_evidence.py` §1 — `HEAD` sha · 변경 목록 · 최근 커밋 제목을
            `.git/HEAD` · `.git/refs/` · `.git/logs/HEAD` **직접 읽기**로. **부작용 0**
      - [ ] 검증: 수집기 실행 전후 `.git/index.lock` 부재 유지
- [ ] **F-61 (약 20분 · P2 · 장후 · F-60과 같은 커밋)** 다이제스트가 **마운트 착시를 스스로 경고**
      - [ ] §1에 `수정 315건(⚠ 마운트 관측 · 네이티브와 다를 수 있음 · 줄바꿈 번역)` · 실변경은 항상 `--ignore-all-space` 병기
      - [ ] **근거**: 네이티브 수정 **10건** vs 샌드박스 **315건**. `core.filemode=false`.
            샌드박스 diff가 `status_board.py` **962줄** 통째 재작성 = 줄바꿈 번역 서명.
            **「CRLF 88파일 부채」는 저장소에 없다 — 리포트 정정 완료**
- [ ] **F-62 (약 45분 · P1 · 장후)** 미커밋 관측 범위를 `src`·`scripts` **+ `pyproject.toml`·`uv.lock`** 로
      - [ ] **근거**: 사용자 `git status`가 `pyproject.toml`·`uv.lock` 수정을 보여줬다.
            `SOURCE_PATHS = src, scripts` 밖이라 **오늘 세 번의 점검이 전부 놓쳤다.** 실행 환경을 바꾸는 파일이다
      - [ ] `core/version.py` `SOURCE_PATHS` · `worktree_dirty_files()`
      - [ ] **⚠ `ops/record_vs_commit.py`의 `dirty`가 같은 함수를 쓴다 — 판정 기준이 넓어진다.**
            **R18: 섀도 필드 `dirty_env_files`로 먼저 20거래일 관측 후 승격.** 조용히 넓히면 어제/오늘 verdict 비교 불가
- [ ] **장후 산출물 백로그** `daily_integrity_2026082{4,5}` 외 2거래일치 미커밋(오늘 포함 3거래일).
      마지막 커밋분 `daily_integrity_20260821.json`(`c6bbc56`, 08-23 21:56).
      **N-1과 무관 확인됨** — `record_vs_commit`의 `dirty`는 `src`·`scripts`만 센다(237~247행). 편할 때 커밋

### 사용자 조치 (2026-08-26 15:22 최종판)

- [x] **0 ✅ 완료** 잠금 회수. 재생성 없음
- [ ] **1 ⚠ 15:45 전 — 커밋(F-48). 이것만 시한이 있다.** 2커밋 분리(`logging.py` 단독 되돌림 경로 확보).
      **`git add -n`으로 사전 확인하지 말 것 — 바로 커밋. 커밋이 곧 검증.**
      **N-1을 `clean`으로 돌리는 유일한 조건이다**
- [ ] **2. 장후** `pyproject.toml`·`uv.lock` 동반 커밋(권고) · 장후 산출물 3거래일치(급하지 않음)
- [ ] **3~7.** 12:36·15:10판 그대로. **단 F-54 ①→F-60 대체 · CRLF 항목은 사용자 할 일 아님(F-61은 도구 수정)**

### 15:35 — F-48 커밋 완료 후 처분 ([MW0601])

- [x] **F-48 ✅ 완료** `3b2f5bb`(15:28:19 규약·설정) → `5a09ac0`(15:30:02 배선) — **둘 다 15:45 배치 전.**
      `git status --porcelain -- src scripts tests` **0건** · 2409 passed · pyright 0 · ruff clean.
      `logging.py` 단독 되돌림 경로 확보(요구 충족)
- [x] **1-1 ✅ 해소** — `worktree_dirty_files()`는 `SOURCE_PATHS=("src","scripts")`만 센다
      (`core/version.py` 119·126) → **N-1 `clean` 예상**, 7거래일 연속 `closed_with_uncommitted_source` 종료.
      **dev_memory 미커밋은 판정에 안 들어간다**(확인함)
- [x] **C-1 ✅ 해소 — 프로브 실측 완료.** `rt_cd=9 msg_cd=OPSP0017 msg1=ERROR : htsid가 잘못되었습니다`.
      **거부가 배선의 증거다** — 도메인·approval_key·TR ID·필드 해석 전부 통과, `tr_key` 값만 틀렸다.
      운영 무영향 실측 확인(15시대 l1·g2 ERROR/WARNING 0행 · 판단 사이클 정상)
- [ ] **1-3 🔄 지속 — 커밋으로 안 닫힌다.** `order_notice`를 임포트하는 실행 경로 **여전히 0건**
      (`run_l1_daily.py`·`run_g2_paper_trading.py` 어디에도 없음) · 오늘 `OrderNotice*` 태그 **0건**(N-4 유지).
      **처방은 배선이 아니라 「0줄」의 뜻을 가르는 것(F-50) — 그대로 유효.**
      배선을 미룬 사용자 판단은 옳다(필드 22개 실측 전 상태 전이 금지 · 금지계명 11)
- [ ] **1-4 🔄 지속** `Docs/capability_matrix.md` 미갱신(F-51). 커밋에 안 들어감

### 🆕 F-63 — 프로브 실측 결과를 파일로 붙잡는다 `P2` · 장후 · 약 30분

- [ ] **근거**: 오늘 정보량이 가장 큰 한 줄(`rt_cd=9 OPSP0017`)이 **어느 로그 파일에도 없다.**
      `l1_daily`·`g2_daily`·`ui` 0건, `logs/` 아래 프로브 산출물 없음. **터미널 스크롤백과 커밋 메시지뿐** —
      커밋 메시지에 안 적었으면 사라졌다
- [ ] `scripts/probe_order_notice.py` → `logs/probe_order_notice_<YYYYMMDD_HHMM>.json`:
      요청 봉투(시크릿 마스킹) · 응답 원문(`rt_cd`·`msg_cd`·`msg1`) · 수신 프레임 원문 · 판정
- [ ] **22필드는 그대로 찍는다** — 프로브는 판정하지 않는다는 기존 설계 유지
- [ ] `collect_evidence.py` §7 산출물 점검에 파일 패턴 추가
- [ ] **Why**: 실측은 재현 비용이 높다(장 시간·브로커 세션). 한 번 얻은 응답을 붙잡지 않으면 다음 사람이 다시 해야 한다

### 🆕 [규칙] 커밋 분할은 **의존 방향**을 따른다 — F-58에 합류

- [ ] `references/report_template.md` Fix 절에 1줄: *"커밋을 가를 때는 의존 방향을 따른다 —
      **규약·설정이 먼저, 그것을 읽는 배선이 나중.** 각 커밋이 단독으로 체크아웃 가능해야 한다.
      「되돌림 단위」만 보고 「체크아웃 가능성」을 빠뜨리지 않는다."*
- [ ] **근거**: 12:36 권고가 ①배선 ②규약 순이었는데 **의존 방향을 거슬렀다.** 배선이 앞서면 그 시점 커밋이
      미등록 태그 `ValueError`로 깨진 상태가 된다. **사용자가 뒤집은 것이 옳다**

### 🆕 [관측] ruff 버전 드리프트 — 실제 발생 1건 (해결 강요 아님)

- [ ] 훅 `ruff-format` **0.4.10**(격리 venv)이 `tr_codes.py`에 빈 줄 추가 → 첫 커밋 반려 → 훅 쪽을 따라 재커밋
- [ ] **`pyproject.toml` 97~98행에 이미 기록된 드리프트**(.venv 0.15.x vs 훅 0.4.10, isort 1st-party 추론 차이).
      **새 발견 아님.** 단 `grep` 결과 **dev_memory 어디에도 없었다** — **1-9와 같은 형태**
      (코드 주석엔 있고 작업 목록엔 없음). 등록만 하고 **훅을 정본으로 삼는 현 운용이 일관적이면 그대로 둔다**
- [ ] **부수 기록**: `tr_codes.py` 워킹트리 mtime **15:29:16**(장중, ruff-format이 건드림).
      08:20 기동은 포맷 **전**, 커밋은 포맷 **후** — **차이 빈 줄 1개, 의미 동일, 재현성 영향 없음.**
      **장후에 `source_mtime_max`가 15:29로 보이면 사유는 이것이다**(사람 편집 아님)

### 🆕 F-62 근거 보강 — 가설이 아니라 실측이 됐다

- [ ] `pyproject.toml`·`uv.lock`을 사용자가 커밋에 넣었다 — `pycryptodome` 없이는 `order_notice.py`가
      import부터 실패해 **새 체크아웃에서 커밋이 안 선다.** **내 목록이 `src`·`scripts`뿐이라 놓쳤다.**
      F-62(관측 범위를 `+ pyproject.toml`·`uv.lock`으로) 우선순위 유지, **R18 섀도 절차 그대로**

### 사용자 조치 (2026-08-26 15:35판)

- [x] **0 ✅** 잠금 회수 완료 · 재생성 없음
- [x] **1 ✅** 커밋 완료(15:28·15:30, 배치 전). **오늘 시한 있는 일은 전부 끝났다**
- [ ] **다음 실측** `.env`의 `KIS_HTS_ID`를 실제 HTS 로그인 ID로 교체 → `python scripts/probe_order_notice.py`.
      **언제든 가능**(장 무관, 별도 WS 세션). 성공 시 22필드 실응답이 나오므로 **F-51(capability_matrix)을
      그 결과로 한 번에 갱신**하는 편이 효율적이다
- [ ] **나머지 저녁 목록** F-49 · F-50 · F-51 · F-52 · F-54②③ · F-55 · F-56 · F-57 · F-58 · F-59 · F-60 · F-61 · F-62 · F-63.
      **권고 순서**: F-58(규칙) → F-60·F-61(점검 도구) → F-49 → F-57 → F-54②③ → F-55 → F-62 → F-63 → F-56 → F-59
- [ ] **장후 배치(15:45) 후 N-1 확인** — `logs/daily_integrity_20260826.json`의
      `record_vs_commit.verdict == "clean"` 이면 1-1·L-11 계열 완전 종료

---

## [MW0601] 2026-08-26 장후 — Fix 7건 신설 · 고도화 4건 · 익일 관측 9건

> 보고서 정본: `logs/dailycheck/2026-08-26_report.md` 제3부 이하. 이 절은 작업 목록만 옮긴다.
> **착수 권고 순서**: F-58 → F-70 → F-60·F-61 → **F-64·F-65** → **F-66** → F-49 → F-56 → F-63 →
> F-51 → F-67·F-68·F-69 → F-50 → F-62 → 나머지

### 🆕 F-64 — 「네이티브 크래시 0건」이 덤프 3건을 덮지 않게 `P1` · 약 60분

- [ ] **근거**: 2026-08-26 10:30~10:36 `g2_paper`·`l1_daily`·`ui` **세 프로세스 전부** access violation
      덤프 1건씩. 최근 3거래일 0건. 그런데 리포트 본문은 `네이티브 크래시: 0건`을 **먼저** 쓰고
      그 아래에 덤프 3건을 나열한다
- [ ] `src/messiah/ops/integrity_report.py` — `네이티브 크래시:` 줄을 두 축 병기로:
      `네이티브 크래시: 프로세스 종료 0건 · stderr 덤프 3건 ⚠`
- [ ] `crash_forensics.findings` 생성 조건에 **「덤프 ≥ 1이면 finding 1건」** 추가
      (현재 오늘 `dumps: 3건`인데 `findings: []`)
- [ ] **검증**: 익일 장후 배치에서 두 축 병기 확인 + 덤프 0건인 날 정상 동작 확인. 기한 **2026-09-02**

### 🆕 F-65 — `ui-crash-isolation` 판정 축이 덤프를 보게 한다 `P1` · 약 45분

- [ ] **근거**: 15:46:26 `FixVerificationPassed` — *"ui-crash-isolation: 16거래일 연속 기준 충족
      (native_crashes ≤ 0)"*. **오늘 덤프 3건이 있었는데 합격했다.** 판정 축이 `native_crashes` 하나뿐이라
      **덤프가 몇 건이 나오든 영구 합격**한다
- [ ] `configs/pending_verifications.yaml` — 기준 `native_crashes ≤ 0` → `native_crashes + crash_dump_count ≤ 0`
- [ ] 등록부 채점기(`src/messiah/ops/` 해당 모듈)에 `crash_dump_count` 축 배선
- [ ] **⚠ 회귀 위험**: 오늘 기준 즉시 위반 → 「16거래일 연속」이 끊긴다. **그것이 목적이다.**
      `report_template.md`의 「기준을 바꿔 합격을 만들지 않는다」의 **역방향**이므로
      **변경 사유를 yaml 주석 + DECISION_LOG에 함께** 남긴다
- [ ] **동반 검토**: `pending_verifications.yaml`에 `blind_to:` 필드 신설 —
      새 검증 항목 등록 시 「이 축이 못 보는 사건」을 한 줄 적게 한다
- [ ] **검증**: 덤프 인위 주입 replay에서 위반으로 뒤집히는가. 기한 **2026-09-02**

### 🆕 F-66 — 발행 유예를 넘기면 그 자리에서 운다 `P1` · 약 75분

- [ ] **근거**: 2026-08-26 `grace_headroom.worst_headroom_ms = **-3596.3**`(`worst_horizon: 1m`,
      유예 2,000ms 대비 −1.80배). 유예 초과 종일 **14건**(`08:1 09:1 10:1 11:0 12:1 **13:8** 14:1 15:1`).
      **그런데 `l1_daily` 종일 WARNING은 마감 처리분 1건뿐 — 초과 경고 0건**
- [ ] `src/messiah/features/engine.py` `_grace_headroom()` 직후:
      `worst_headroom_ms < 0` → WARNING 태그 **`PublishGraceBreached`** 1회
...
- [ ] **Why**: `engine.py:1285~1287` 주석이 이미 *"이 값이 0이 아닌 날은 자료가 실제로 빠졌을 수 있는 날"*
      이라 적어 두었다. **오늘 값은 14인데 아무도 안 불렸다.** 계기를 만드는 일과 계기가 사람을 부르는 일은 다르다
- [ ] **검증**: replay 인위 지연 주입 → 태그 1건 발신. 라이브는 다음 음수일. 기한 **2026-09-09**

### 🆕 F-67 — `resolve_secret()` 회귀 테스트 3건 `P2` · 약 30분

- [ ] **근거**: `grep -rn "resolve_secret" --include=*.py tests/` → **0건.**
      `config.py:122`가 위험을 주석으로 적어 두었는데 그 계약을 지키는 테스트가 없다
- [ ] `tests/core/test_config.py`: ① `required=True`(기본) 미설정 → 예외
      ② `required=False` 미설정 → 빈 문자열
      ③ `app_key_ref`·`app_secret_ref`·`account_ref` 세 호출부가 기본 경로를 쓴다는 것을 서명으로 고정
- [ ] **오용은 없음이 확인됐다** — `required=False` 호출부는 `broker/kis/credentials.py:32` 단 하나
      (`ops/series_expectation.py:75`는 다른 함수·다른 의미). **C-2 종결**
- [ ] **검증**: `pytest tests/core/test_config.py`. 기한 **2026-08-31**

### 🆕 F-68 — 소급 불가 손실 표기를 `clean` 판정과 일치시킨다 `P2` · 약 25분

- [ ] **근거**: `postmarket:78` `소급 불가 손실(오늘): 1분 ❌` vs
      `status_snapshot` `{start_lag_minutes: 0.6, lost_items: 0, clean: true, summary: "오늘 소급 불가 손실 없음"}`.
      **상시값**(08-20 0.3 · 21 0.5 · 24 0.6 · 25 0.8 · 26 0.6)이고 실체는 기동 지연 35초
- [ ] `src/messiah/ops/integrity_report.py` — 올림 제거, 소수 1자리. `❌`/`✅`는 **`lost_items > 0` 기준**
- [ ] 목표 출력: `소급 불가 손실(오늘): 0.6분(기동 지연 · 유실 항목 0건) ✅`
- [ ] **Why**: R6의 정신. **매일 뜨는 빨강은 진짜 빨강을 가린다** — 훼손된 것은 경보의 신용이다
- [ ] **검증**: 익일 리포트에서 `✅`로 뒤집히는가. 기한 **2026-08-31**

### 🆕 F-69 — 장중 호스트 계측 시계열 `P2` · 약 40분

- [ ] **근거**: 13시대 발행 오프셋만 `p50 759ms`(다른 시간대 562~588) · `p90 2,932ms`(898~1,459) ·
      `over_grace 8건`. **회선 무죄**(`delivery_latency` 13시 p50 0.524s로 타 시간대와 동일),
      **발행 함수 무죄**(`publish_sla` over_sla 0/708). **남은 구간에 계측이 없어 C-4가 미확정으로 남았다**
- [ ] `scripts/run_l1_daily.py` 30분 주기 루프(`ClockSkewMeasured`가 타는 자리)에
      **`HostHealthSampled` INFO** — CPU·가용메모리·외부 파이썬 수
- [ ] **현재 호스트 점검은 기동 시 1회뿐** — 그래서 사후에 물을 수 없다
- [ ] **검증**: 익일 로그에 14건 출현. 기한 **2026-09-02**

### 🆕 F-70 — 장중 국면은 커밋을 요구하지 않는다 (점검 절차) `P2` · 약 15분

- [ ] **근거**: 15:22판 사용자 조치 1번(*"⚠ 지금 (3시 45분 전) — 저장"*)이 **15:28·15:30 커밋**을 유발했다.
      마감 15:35 전이다. **결과적으로 무해했음이 실측됐으나**(15시대 ERROR/WARNING 0행, 판단 사이클 정상 완주,
      `code_version.stale: true`로 미반입 증명) **무해의 근거가 「프로세스가 파일을 다시 안 읽는다」는
      구현 성질이지 설계 보장이 아니다**
- [ ] `references/report_template.md` · `references/phases.md` B절에 한 줄:
      *"장중 국면은 커밋을 요구하지 않는다. `record_vs_commit`을 `clean`으로 만들려는 이유로
      마감 전 커밋을 지시하면 R11 경계를 스치게 된다 — **판정이 하루 늦는 편이 낫다.**"*
- [ ] **Why**: 점검이 자기 판정 지표를 좋게 만들려고 운영에 지시하는 구조는 **1-6과 같은 형태의 오류**다
      (관측자가 관측 대상을 건드린다). 1-6은 우발, 이것은 의도 — **더 무겁다**
- [ ] 기한 **2026-08-28**

### 🆕 고도화 G-32 ~ G-35 (당일 관측 근거)

- [ ] **G-32** 「같은 사실을 세는 계기가 둘 이상인 축」을 리포트에서 **짝지어 출력**한다.
      `src/messiah/ops/integrity_report.py`에 `_paired_axes()` 신설.
      예 — `크래시: 프로세스 종료 0 / stderr 덤프 3 ⚠ 불일치` ·
      `발행: 처리시간 초과 0/708 · 유예 초과 14/708 ⚠ 다른 축`.
      **근거**: 오늘 1-10·1-13이 같은 형태였고, 발행 오프셋도 같은 화면에서 상반된 인상을 준다
- [ ] **G-33** 시간대 경계(매시 정각)마다 직전 1시간 발행 오프셋 중간 요약(INFO / 초과 시 F-66 태그).
      **근거**: `FeaturePublishOffset` 종일 **1건**(15:35:05) — 13시대 급증을 **2시간 46분 뒤에** 알았다.
      비용 하루 7줄, 얻는 것은 다른 로그와 같은 타임라인 위에서 보는 능력
- [ ] **G-34** 섀도 관문 국면별 통과율 누적 표 `logs/shadow_gate_rollup.json`.
      **근거**: 1일차 유효 13건 중 통과 4건(**30.8%**) · `HIGH_VOL` 0/6 · `RANGE` 4/7 —
      「섀도를 켜면 전량 차단인가」의 답은 **아니다, 국면에 따라 갈린다**.
      **⚠ 1-7 불일치 사이클의 섀도 값은 통계에서 빼야 한다**(오늘 09:00 1건) → 같은 파일에 불일치 누적도 넣는다
- [ ] **G-35** 장후 배치 7단계로 **산출물 자동 커밋**(`daily_integrity_*`·`self_eval_*`·`vol_scorecard_*`·
      `volume_check_*`·`verification_scoreboard_*`·`logs/dailycheck/*_report.md`, 메시지 `[MW0601] 장후 산출물 <날짜>`).
      **근거**: 08-24·25·26 **3거래일치 미커밋**, `record_vs_commit` 판정 축(`src`·`scripts`) 밖이라 아무 계기도 안 운다.
      마지막 커밋된 무결성 산출물은 `daily_integrity_20260821.json`(`c6bbc56`, 08-23 21:56).
      **⚠ 선행조건 F-60** — 그 전에 붙이면 잠금 문제가 배치로 옮겨간다. **의존 방향을 지킨다**

### 검증 예정 (2026-08-27) — N-15 ~ N-23

- [ ] **N-15** 커밋 `5a09ac0`(체결통보 태그 8종 등록) 반영 — 자가점검 `schema types` · 미등록 태그 `ValueError` 0건 ·
      `SessionStart.git_sha == 5a09ac0` · `code_version.stale: false` **[장전]**
- [ ] **N-16** 자가점검 `git` 줄이 `src/scripts 0` 복귀 (M-6 확정). **복귀가 예상값** — 아니면 저녁 편집이 있었다는 뜻 **[장전]**
- [ ] **N-17** 시세 세션 재연결 건수 (C-3). 1건 이상이면 **PINGPONG 가설이 시세 쪽에서 실증** **[장중·장후]**
- [ ] **N-18** 크래시 덤프 재출현 (1-10 · C-5). **0건이면 단발**, 1건 이상이면 재현성 사건으로 격상하고
      시각 대조로 공통 트리거를 좁힌다 **[장후]**
- [ ] **N-19** 13시대 발행 지연 재현 (C-4). 오늘 `p90 2,932ms · over_grace 8건`.
      **다시 13시대에만 몰리면 그 시각 고정 작업이 원인**, 흩어지면 우발 **[장후]**
- [ ] **N-20** `worst_headroom_ms` 부호. 오늘 −3,596ms. **음수 이틀 연속이면 상시 상태**이며
      F-66(경보)보다 **유예값 재산정이 먼저**다 **[장후]**
- [ ] **N-21** 섀도 관문 2일차 (R18 20거래일 중 · 잔여 19). 국면별 통과율 누적 **[장후]**
- [ ] **N-22** 국면 전달 일치율 (1-7). 오늘 전환 3회 중 2회 불일치 · 전체 14회 중 2회.
      **전환 표본 20회까지 모아 G-31 승격 판단** **[장후]**
- [ ] **N-23** 승격 관문 미판정 번들 (1-5). 오늘 **4거래일째**. `sharpe`·`max_drawdown`·`negative_window_ratio`가
      측정 가능해지려면 **주문이 나가야 한다** — `order-path-live`와 한 몸 **[장전]**

### 사용자 조치 (2026-08-26 장후 최종판)

- [x] **0 ✅** 잠금 회수 완료 · 재발 없음
- [x] **1 ✅** 커밋 완료(15:28·15:30, 배치 전). **`record_vs_commit: ok` · 미커밋 0파일 — 8거래일 만에 처음**
- [x] **추가 ✅** 프로브 실행 — `rt_cd=9 OPSP0017 htsid가 잘못되었습니다`. **C-1 해소**
- [ ] **⚠ 질의 (1분)** **오전 10시 30분경 이 PC에서 무엇을 하고 계셨는가.**
      세 프로세스가 6분 안에 동시 폴트했고 `crashing_frames`가 전부 비어 있어(네이티브 스레드)
      로그로는 여기까지다. **사용자 기억이 로그 열 개보다 크다** (C-5)
- [ ] **알림 기능 마무리 (2분, 언제든)** `.env`의 `KIS_HTS_ID`를 **실제 HTS 로그인 ID**로 교체 →
      `python scripts/probe_order_notice.py` 재실행. 성공 시 22필드 실응답 →
      **F-51(capability_matrix)을 그 결과로 한 번에 갱신**
- [ ] **장후 산출물 3거래일치 커밋 (3분, 언제든)** 08-24·25·26. **판정에 영향 없음**(그 판정은 `src`·`scripts`만 본다).
      자동화는 G-35
- [ ] **재시동 — 불필요.** 프로세스는 15:35 정상 종료. 내일 08:20 기동이 `5a09ac0`을 싣는다.
      지금 띄우면 기동 창 밖이라 자진 거절된다
- [ ] **저녁 Fix 목록** F-49 · F-50 · F-51 · F-52 · F-54②③ · F-55 · F-56 · F-57 · F-58 · F-59 · F-60 · F-61 ·
      F-62 · F-63 · **F-64 · F-65 · F-66 · F-67 · F-68 · F-69 · F-70**

## 2026-08-26 야간 — 장후 자동조치 예약 신설 ([MW0601])

- [x] 예약작업 `messiah-postmarket-autofix` 등록 — 평일 `10 18 * * 1-5`(실측 기동 18:15).
      리포트의 F·G·S 3절 + 「사용자 조치」 최종판을 읽어 A·B등급만 자동 구현·테스트·커밋·푸시
- [x] 리포트 규격에 **「수익률 향상방안 — 장후(S 시리즈)」** 6칸 신설 —
      관측 근거 · 변경 대상 · 기대 효과 · 회귀 위험 · 검증 · **표본 상태**.
      `references/report_template.md` · `SKILL.md` · `references/phases.md` C-6 · `references/schedule_prompts.md`
- [ ] **검증 (2026-08-27 18:15 첫 회차)** — 가드 7종 동작 · 등급 분류 일치 · `.git/index.lock` 부재 ·
      푸시 성공 · 리포트 제6부 append. **라이브 미검증 상태다**
- [ ] 첫 회차가 F-58·F-70을 규약 커밋으로 먼저 처리하는지 확인 — 의존 방향(규약 → 배선) 준수 여부
- [ ] **2026-08-27 장후 점검부터** S 시리즈 절을 실제로 쓰는지 확인.
      표본 상태가 「부족」이면 자동조치가 건너뛰는 것이 정상 동작이다
- [ ] G-35(장후 산출물 자동 커밋)는 이 예약의 §6에 흡수됨 — **선행조건 F-60은 여전히 유효**하며
      자동조치는 네이티브 PowerShell 규약으로 우회한다

## 2026-08-26 18:15 — 장후 자동조치 1회차 결과 ([MW0601] · 자동)

### 구현 완료 — 커밋 6건 + 산출물 1건

- [x] **F-58** 즉시 조치/재발방지 번호 분리 규칙 — `report_template.md` · `SKILL.md` §5 (`0f4b9be`)
- [x] **F-70** 장중 국면은 커밋을 요구하지 않는다 — `phases.md` B-5절 신설 (`0f4b9be`)
- [x] **F-60** 수집기가 HEAD sha·브랜치·커밋 제목을 `.git` 직접 읽기로 얻는다 + git 화이트리스트
      + 락 생성 자경 (`a169976`). **문언보다 좁게 구현** — 워킹트리 대 HEAD 비교는 `.git`만으론
      불가하므로 `status`·`diff`는 남기고 가뒀다(DECISION_LOG Why 4)
- [x] **F-61** 다이제스트 §1 마운트 착시 자경 · `--ignore-all-space` 기준 병기 (`a169976`)
- [x] **F-64** 크래시 두 계기 병기 + 덤프 1건 이상이면 finding 1건 (`d1d7be6`)
- [x] **G-32** `_paired_axes()` — 짝 축 블록(크래시 · 발행 · 소급 불가 손실) (`d1d7be6`)
- [x] **F-65** `ui-crash-isolation` 지표를 `native_crashes_or_dumps`로 조임 (`2040a57`).
      실측 재채점: `검증 완료`(16일) → **`재발`(0일)**. 그것이 목적이다
- [x] **F-66** `PublishGraceBreached` WARNING 등록(`7587d0b`) + 배선(`991b191`)
- [x] **사용자 조치 5번 대행** — 08-24·25·26 장후 산출물 3거래일치 커밋

### 이월 — 시간 상한(6시간)과 사람 결정

- [ ] **F-49** `stale=false`가 미커밋 소스를 가리지 못하게. **두 가지 이유로 보류** —
      ㉠ 착수 순서상 여기서 6시간 상한을 넘긴다 ㉡ 리포트가 「경고를 어느 강도로 낼 것인가」를
      **결정 필요 사항**으로 남겼다(㉠요약 표기 / ㉡자가점검 WARN / ㉢live 기동 거부).
      **권고는 ㉠부터 20거래일 관측 후 ㉡.** 사람이 고르면 그대로 구현한다 **[다음 회차]**
- [ ] **F-56** 화면 생사를 장중에 판정 (하트비트 5분) — 착수 순서상 F-49 다음 **[다음 회차]**
- [ ] **F-63** 프로브 실측을 파일로 붙잡기 **[다음 회차]**
- [ ] **F-51** capability_matrix 갱신 — **사용자가 HTS ID를 고쳐 프로브를 다시 돌리면**
      그 결과로 한 번에 갱신한다. 지금 갱신하면 이틀 뒤 또 고친다 **[사용자 선행]**
- [ ] **F-67** `resolve_secret()` 회귀 3건 · **F-68** 손실 표기 정합 · **F-69** 장중 계측 시계열
      **[다음 회차]**
- [ ] **F-50** 「0줄」 가르기 — F-51 결정 선행 **[대기]**
- [ ] **F-62** 미커밋 관측 범위 확장 — R18 섀도 필드 선행 **[다음 회차]**
- [ ] **F-52** 승격 관문 3종 측정 — **주문이 나가야 측정된다. 사람 결정 대기(C등급)**
- [ ] **F-59** 등록부가 인덱스락 계열을 받게 — 15:10판이 「F-54·F-57·F-58 다음」으로 선행조건을
      걸었고 F-54·F-57이 아직이다 **[선행 대기]**
- [ ] **F-54②③** · **F-55** · **F-57** — 착수 순서 뒤쪽 **[다음 회차]**
- [ ] **G-33** 시간대 경계 중간 요약 · **G-34** 섀도 관문 누적표 · **G-35** 산출물 자동 커밋 —
      고도화 하루 3건 상한, 오늘은 G-32 한 건. **G-35는 오늘 손으로 대신 처리했다** **[다음 회차]**

### 자동조치 1회차 자체의 검증 결과 (전 세션이 건 5개 항목)

- [x] ① 가드 7종 전부 통과 — 리포트 존재·완결 · 상태파일 신규 생성 · 15:35 이후(18:13)이고
      MESSIAH 프로세스 0개 · 브랜치 `master` · `=== 장후 절차 요약 ===` 존재 · `src`/`configs` 미커밋 0
- [x] ② 등급 분류가 리포트 표식과 일치 — F-65는 **B**(조이는 방향), F-52는 **C**(사람 결정),
      F-49는 「결정 필요 사항」 + 상한 초과로 보류
- [x] ③ 종료 시 `.git/index.lock` 부재 — 쓰기는 전부 네이티브 PowerShell에서 실행
- [x] ④ `git push origin master` 성공
- [x] ⑤ 리포트 끝에 `## 제6부. 장후 자동조치 구현 결과` append (앞 본문 무수정)
- [ ] **㉠ 2026-08-27 장후에 확인** — `PublishGraceBreached`가 실제 세션에서 나오는가.
      N-20이 음수면 나와야 하고, 양수면 안 나와야 한다. **둘 다 이 계기의 검증이다**
- [ ] **㉡ 2026-08-27 장후** — 무결성 리포트 본문에 「짝 축」 블록과 크래시 병기 줄이 찍히는가
- [ ] **㉢ 2026-08-27 장후** — 등록부 `ui-crash-isolation`이 `재발`로 뜨는가 (F-65 실전 확인)
- [ ] **㉣ 2026-08-27 장전** — 수집기가 락을 안 만드는가 (F-60 첫 실전). **사흘 연속 재발의 종결점**

### F-58 후속 — 사람이 확정할 일 (자동 부여 금지)

`grep -nE "^\s*- \[ \].*(하위|미해소)" dev_memory/NEXT_TODO.md` 후보 **4건**(8085 · 8258 · 8755 · 8757행).
전부 F-34 재발방지 계열이고 **이미 F-54로 번호가 부여됐다** — 즉 「ID 없는 미해소 항목」은
실질 **0건**이다(F-58 측정 지표의 목표값). 8755·8757행은 F-58 자신의 계획 문장이므로 후보가 아니다.

- [ ] 위 판정을 사람이 한 번 확인 — 맞으면 「ID 없는 미해소 항목 0건」을 기준선으로 고정한다

### 사용자 몫으로 남은 손 작업 (2026-08-26 최종판에서 이월)

- [ ] **⚠ 질의 (1분)** 오전 10시 30분경 이 PC에서 무엇을 하고 계셨는가 — **오늘 고친 것들은
      「다음에 같은 일이 나면 즉시 보이게」 만든 것이지 오늘 무슨 일이었는지는 못 알려준다**
- [ ] **알림 기능 마무리 (2분)** `.env`의 `KIS_HTS_ID`를 실제 HTS 로그인 ID로 교체 →
      `python scripts/probe_order_notice.py` 재실행. 성공하면 F-51이 따라 닫힌다
- [x] ~~장후 산출물 3거래일치 커밋~~ — **자동조치가 대신 처리함**

## 2026-08-27 장전 점검 ([MW0601])

보고서 `logs/dailycheck/2026-08-27_report.md` 제1부 · 증거 `logs/dailycheck/evidence_20260827_pre.md`.
**코드 변경 없음** — 장전 국면(R11 · 금지계명 3·4). 아래 전부 **장후 18:15 이후 적용**.

### 🆕 F-71 — 검증 등록부의 기한을 지표 변경과 함께 재산정 `P1` · 약 90분 · 대응 1-1

- [ ] **F-71①** `configs/pending_verifications.yaml` 71행 `ui-crash-isolation`의 `deadline: 2026-08-14`
      → **`2026-09-02`**(지표 리셋일 08-26 + `consecutive_days` 3 + 여유 2거래일). **사유를 54~65행
      F-65 사유문 바로 아래 주석에 남긴다** — 근거 없이 민 기한은 다음 사람이 되돌린다
- [ ] **F-71②** `src/messiah/ops/fix_verification.py` — `metric` 변경 시 `metric_changed_at` 기록,
      기한 판정을 `max(deadline, metric_changed_at + consecutive_days + 여유)`로
- [ ] **F-71③** 자동 연장 시 **`VerificationDeadlineExtended`를 WARNING으로** — 조용히 미는 기한은
      금지계명 12와 같은 성질이다. **연장은 `metric_changed_at`이 있는 항목에만** (지표 그대로인데
      기한만 지난 항목은 진짜 미이행이므로 대상 아님)
- [ ] **F-71④** `scripts/self_check.py` `deadlines` 줄 — 도달 불가 1건 이상이면 줄 머리 `[WARN]` (F-72와 같은 자리)
- [ ] **F-71 결정 필요** 연장 여유를 **2거래일**로 둘지 0일로 둘지. **권고 2거래일** — 리셋 직후 하루만
      계측이 빠져도 다시 도달 불가로 재진입한다
- [ ] **F-71 검증** `pytest tests/ops/test_fix_verification.py` 신설 4건 · 2026-08-28 장전에
      `기한 도달 불가 0건` 복귀 · 2026-09-02 이후 `ui-crash-isolation`이 실질 판정(㉢가 그때 답을 얻는다)
- [ ] **F-71 선행** **C-2 판정**(아래) — 자가점검이 충족 완료 항목을 도달 불가 집계에서 빼는가

### 🆕 F-72 — 자가점검 줄의 심각도를 본문 최대 심각도로 승격 `P1` · 약 60분 · 대응 1-2

- [ ] **F-72①** `scripts/self_check.py` 조립부 — 본문에 `[WARN]`/`[FAIL]` 토큰이 있으면 줄 머리 승격
- [ ] **F-72②** 조립 직후 `assert "[WARN]" not in body or head != "[OK ]"` **성질 단언**
      (등호·위치 단언 금지 — 1-18 규율)
- [ ] **F-72③** 요약을 `self-check: PASS(경고 N건) — 기동 허용`으로. 경고가 있는데 요약이 침묵하면 기계가 못 읽는다
- [ ] **F-72④** `.claude/skills/messiah-daily-check/scripts/collect_evidence.py` 자가점검 파싱 —
      "비-OK N행"을 줄 머리만이 아니라 **본문 토큰까지** 보고 센다. **오늘 이 수집기가 경고 2건을
      「비-OK 0행」으로 집계했다 — 점검 도구 자신의 결함이다.** 같은 회차에 고친다
- [ ] **F-72 회귀 확인** `[OK ]`→`[WARN]` 전이가 live 기동 거부 조건에 걸리는가.
      **최종 판정(PASS/FAIL)은 건드리지 않는다** — 게이트 불변이므로 R18 비해당
- [ ] **F-72 검증** `pytest tests/scripts/test_self_check.py` 신설 3건 · 다음 장전에 `[WARN] bundle …` ·
      수집기 재실행 시 `비-OK 2행`

### 🆕 F-73 — 옵션 체인 기준가의 신선도를 발행에 실어 보낸다 `P1` · 약 3시간 · 대응 1-3

- [ ] **F-73①** `src/messiah/core/messages.py` `OptionChainSnapshot`(275행 인근) —
      **`spot_as_of: datetime`**(tz-aware · R3) · **`spot_stale: bool`** 추가. **기본값을 주어 하위 호환**
- [ ] **F-73②** `src/messiah/data/option_chain_poller.py:274` 발행부 — 두 필드를 페이로드·로그에 싣고,
      스테일 사이클은 **DEBUG → WARNING** · 전용 태그 **`OptionChainStaleSpot`**(R6 — 기존 태그 겸용 금지).
      사이클당 1건 요약 원칙(2026-08-14 F-6) 유지 — 다리마다 울리지 않는다
- [ ] **F-73③** `src/messiah/core/logging.py:435` 인근 레벨표에 `"OptionChainStaleSpot": logging.WARNING`
- [ ] **F-73④** `src/messiah/ops/integrity_report.py:2667` 인근 —
      `stale_spot_cycles` · `stale_spot_first_kst` · `stale_spot_last_kst` · `max_spot_age_seconds` 축 추가.
      오늘 값이면 10건 · 08:22:24~08:44:02 · 최대 약 62,000초
- [ ] **F-73⑤ 경보 피로 대책** 장전(08:15~08:45)은 **구간당 1건**으로 접어 첫 발생·해소 시각만,
      그 외 시간대는 사이클마다. **시간대 차등 사유를 함수 주석과 DECISION_LOG 양쪽에**
- [ ] **F-73 결정 필요** 스테일 임계 **60초** vs 1분봉 유예 2,000ms. **권고 60초** —
      2초 임계는 정상 폴링 간격(98~300초)에서 상시 참이라 무의미하다
- [ ] **F-73 선행** **K-5 판정**(장후) — 무결성 리포트가 이 구간을 이미 다른 축으로 잡고 있으면
      축이 둘로 갈라진다(어제 F-64·G-32가 그 문제였다)
- [ ] **F-73 검증** `pytest tests/data/test_option_chain_poller.py` 신설 4건 ·
      `scripts/run_replay.py --symbol A05609 --start 2026-08-27`로 오늘 10건 재현 ·
      2026-08-28 장전 `OptionChainStaleSpot` 첫 발생 08:22 전후·해소 08:45~08:50 ·
      `daily_integrity_20260828.json`에 `stale_spot_cycles > 0`
- [ ] **F-73 R18** 비해당(발행을 막지 않고 표시만). **훗날 이 값으로 차단하면 그때는 섀도 20거래일 대상**

### 🆕 F-74 — 시리즈별 폴링 주기 결정을 로그에 남긴다 `P2` · 약 40분 · 대응 1-4

- [ ] **F-74①** `scripts/run_l1_daily.py` 폴러 등록 루프 직후 **`OptionChainScheduleResolved`**(INFO) —
      `{series, interval_seconds, phase_offset_seconds, reason}`. **`reason`이 없으면 반쪽 fix다**
- [ ] **F-74②** `src/messiah/core/logging.py` 레벨표 등록
- [ ] **F-74 선행** **C-1 판정** — 만기 요일 분기가 코드에 **있으면** 로깅만으로 끝나고,
      **없으면 F-75(설정 드리프트 추적)로 분화**한다
- [ ] **F-74 검증** `pytest tests/scripts/test_run_l1_daily.py` 신설 1건 · 2026-08-31(월)에
      `weekly_mon`이 최다 주기를 받는지로 C-1 가설 동시 검증

### 🆕 고도화 G-36 ~ G-38 (당일 관측 근거 · 하루 3건 상한)

- [ ] **G-36** 장전 구간을 `not_yet` / `wrong` / `unmeasurable` 세 라벨로 가른다 (`readiness_phase`).
      **근거**: 오늘 성격이 다른 셋이 같은 모양으로 기록됐다 — 화면 4토픽 `NO_DATA`(차례 전) ·
      기준가 스테일(사실과 어긋남) · 번들 릴리스 미측정(구조적 판정 불가).
      **효과**: 거짓 경보 4건 감소 + 진짜 1건 신규 포착. **선행 F-72.** 약 3시간. **이번 주**
- [ ] **G-37** `deadlines` 줄을 `계측 중 N · 기한 경과 N · 도달 불가 N · 임박 N`으로 확장.
      **근거**: 오늘 「도달 불가 1건」만 보고했으나 실제 `deadline` 22건 중 **20건이 이미 경과**.
      **위험**: 20건이 드러나면 하루에 처분하고 싶어진다 — 그 압박이 근거 없는 기한 밀기로 이어지면
      F-71 취지가 무너진다. **이 줄은 숫자만 내고 조치를 지시하지 않는다.**
      **선행 C-2 · F-71.** 약 90분. **이번 주 · F-71과 같은 회차**
- [ ] **G-38** `spot_refresh_lag_seconds`(첫 틱 → 첫 기준가 갱신) 지표 신설.
      **근거**: 오늘 08:45:00 첫 틱 → 08:49:02 첫 갱신 = **242초**. 어제는 344초.
      그런데 Feature 발행은 08:46:00에 이미 시작 — **약 3분간 Feature는 새 시세, 옵션은 옛 기준가.**
      **효과**: 60초 이하면 개장 직후 옵션 피처 투입 가능, 4~6분이면 그 구간에서 빼는 편이 낫다는 근거.
      **지금 임계를 정하지 않는다 — 표본 2일뿐.** 20거래일 분포 후 결정. **선행 F-73.**
      약 2시간. **다음 단계**

### 확인 필요 (2026-08-27 장전 → 장중·장후 판정)

- [ ] **C-1** 시리즈별 폴링 주기가 만기 요일 기준 동적 결정인가, 설정 드리프트인가.
      **볼 것**: `data/option_chain_poller.py` 주기 결정부 · `scripts/run_l1_daily.py`의
      `FixedTickScheduler` 등록부(독스트링 28행이 여기를 가리킨다). **장후**
- [ ] **C-2** 등록부 `deadline` 22건 중 **20건 기한 경과**인데 자가점검은 「도달 불가 1건」만 보고.
      충족 완료 항목을 집계에서 빼는가. **볼 것**: `ops/fix_verification.py` 기한 판정 함수 ·
      각 항목 현재 `clean_days`. **F-71 구현이 이 판정에 의존한다.** **장후**
- [ ] **C-3** 화면(Command Center) 장중 생사 — `ui_20260827.log`가 08:20:55의 2행이 전부.
      첫 렌더 시점 4토픽 `NO_DATA`는 **첫 틱(08:45) 이전이라 결함 아님.** 문제는 이후 31분 무기록.
      **기존 F-56(하트비트 5분)에 귀속 — 중복 등재하지 않는다.** **장중(K-4)**

### 관측 항목 K-1 ~ K-6 (2026-08-27 장중·장후가 판정)

- [ ] **K-1** `publish_offset_ms` 시간대별 분포. 어제 08시대 p90 1,772.3ms · p99 3,536.3ms ·
      1,000ms 초과 7/21건(33%). 오늘 08시대 8건 최대 **1,069.8ms**(3m · `local_only` 축).
      **표본 100건 이상에서 재본다.** **장중**
- [ ] **K-2** `PublishGraceBreached`(어제 F-66 · 커밋 `7587d0b`·`991b191`) — 유예 여유 음수면 나오고
      양수면 안 나와야 한다. **둘 다 이 계기의 검증이다** (어제 ㉠). **장후**
- [ ] **K-3** `RegimeWarmStart.bars_by_source` 전월물 비중 — 오늘 **A05608 84/200 = 42.0%**.
      추이 08-24 72.0% → 08-25 64.5% → 08-27 42.0%. **0%가 되는 날 C-11이 자연 종결.**
      오늘 그 200봉으로 `RegimeSeeded RANGE`(0.93)가 나갔다. **장후**
- [ ] **K-4** 화면 생사 — `ui_20260827.log`에 08:20:55 이후 기록이 붙는가 · 4토픽이 신선 배지로 전이하는가 (C-3). **장중**
- [ ] **K-5** 1-3 스테일 구간이 무결성 리포트에 잡히는가 — `daily_integrity_20260827.json`의
      `series_findings`·`horizon_findings`에 08:22~08:44이 나타나는가. **어제는 둘 다 `[]`였다.**
      **안 잡히는 것이 정상 동작임을 확인하면 F-73 필요성이 확정된다.** **장후**
- [ ] **K-6** 승격 관문 미판정 번들 `real-20260820-2053-30m` — 오늘 **5거래일째**.
      **기존 N-23 · 새 번호 없음.** 해소 경로는 F-52(사람 결정 C등급). **장후**

### ✅ 해소 (2026-08-27 장전 확인)

- [x] **어제 1-1 미커밋 소스 반입** — `worktree_dirty: false` · `src`+`scripts` 0파일.
      어제 장후 자동조치 커밋 8건(`0f4b9be`~`5755804`) 전부 반입됨
- [x] **㉣ F-60 첫 실전 — 점검 수집기가 저장소 잠금을 만드는가** — 08:50:53 실행 후 `.git/index.lock` **부재**.
      **사흘 연속 재발하던 항목의 종결점.** `run_git()` 화이트리스트가 `add -n` 계열을 실행에서 거절한 결과

### ⚠ 예측 대비 실제가 갈린 것 — ㉢

어제 검증 항목 ㉢는 *"장후에 `ui-crash-isolation`이 `재발`로 뜨는가"*였으나, 실제로는 **장전 자가점검에서
`기한 도달 불가`**로 떴다. `기한 도달 불가`는 `재발`과 달리 **원인 신호를 담지 않는다.**
- [ ] **㉢ 판정 연기** — F-71 적용 후 **2026-09-02 이후**로 미룬다. 그 전까지 이 항목은 판정 불가다

### 사용자 조치 (2026-08-27 장전판)

- [ ] **개장 전에는 할 일 없음.** 오늘 발견한 셋은 전부 기록·판정 계층이고 거래를 막지 않는다
- [ ] **장 마감 후 (3분)** F-71① — 채점 마감일 재산정. **안 하면** 화면 급사 감시 자리가 매일
      같은 경고만 반복하고 실제 개선 여부를 못 알려준다. 며칠이면 아무도 그 경고를 안 본다
- [ ] **언제든 (2분)** `.env`의 `KIS_HTS_ID`를 실제 HTS 로그인 ID로 교체 →
      `python scripts/probe_order_notice.py` 재실행. 성공하면 F-51이 따라 닫힌다 **[어제에서 이월]**
- [ ] **⚠ 질의 (1분)** 2026-08-26 오전 10시 30분경 이 PC에서 무엇을 하고 계셨는가 **[어제에서 이월]**

## 2026-08-31 장전 점검 ([MW0601])

> 직전 점검 2026-08-27 장전 — **그 사이 4회 점검 공백**(08-27 장후 · 08-28 3국면).
> 아래 F-71~F-74 · G-36~G-38은 그 공백으로 미적용된 이월분이다. 새 번호를 붙이지 않는다.

### 적용 순서 (의존 관계 · 전부 오늘 15:35 마감 이후)

- [ ] **순서 정본** F-75(오늘 밤 필수) → F-72 → F-76 → F-77 → F-73 → F-74 → F-71(범위 축소)

### 🆕 F-75 — 장중에 생긴 미커밋 소스 5개를 안전한 순서로 반입 `P1` · 약 40분 · 대응 1-1

- [ ] **F-75①** `git add src/messiah/features/sets.py` **를 먼저** 실행 →
      `git status --porcelain -- src`에 `??` 0행 확인. `.gitignore`가 이 경로를 배제하지 않는지도 본다
- [ ] **F-75 위험 핵심** `git commit -am`은 추적 4파일만 담는다 → `features/spec.py:45`가 없는 모듈을
      임포트하는 **깨진 HEAD**. `spec` → `features/engine` → `run_l1_daily` 전 경로 ImportError
- [ ] **F-75②** 회귀 검증(금지계명 2): `pytest tests/features/ -k "spec or sets"` ·
      `pytest tests/core/test_config.py` · `pytest tests/strategy/ -k decision` ·
      `python scripts/run_replay.py --symbol A05609 --start 2026-08-28`
- [ ] **F-75② 호출부 전수** `grep -rn "registered_names\|FEATURE_SETS" src scripts`
- [ ] **F-75③** `tests/features/test_sets.py` 신설 — (a) 4개 키 문자열 동일성
      (`v2026.07`·`v2026.08-fl`·`v2026.08-ev`·`v2026.08-fl-ev` — 모델 번들이 이 문자열로 입력 모양을 주장)
      (b) `import messiah.features.sets` 후 `sys.modules`에 `polars` 부재.
      **(b)가 이번 변경의 목적 그 자체다 — 없으면 임포트 한 줄로 되돌아간다**
- [ ] **F-75④** 커밋 첫 단어 `[MW0601]` · 본문에 "2026-08-27 장중 12:33~12:39 수정분 · 나흘 지연 반입 ·
      R11 위반 사후 정리" 명기. **위반이 있었다는 사실이 이력에 남아야 한다**
- [ ] **F-75⑤** 반입 후 `status_snapshot.json` `worktree_dirty_files == 0` ·
      다음 기동 `SessionStart.source_mtime_max`가 커밋 시각 이후
- [ ] **F-75 회귀 위험** `spec.registered_names = sets.registered_names`는 **모듈 속성 대입**이라
      그것을 몽키패치하던 테스트가 있으면 깨진다(R5가 금지하나 확인)
- [ ] **F-75 R18** 비해당 · **검증 기한 2026-09-01 장전**

### 🆕 F-76 — 코드 동일성 판정에 작업트리 상태를 반영 `P1` · 약 30분 · 대응 1-2

- [ ] **F-76①** `src/messiah/ops/status_board.py` `code_version` — `stale`을
      `sha_mismatch or worktree_dirty`로 확장 + `stale_reason`(`sha_mismatch|worktree_dirty|both|null`) 신설.
      **`stale`의 뜻을 「커밋과 다르다」→「저장소 상태와 다르다」로 바꾸는 의미 변경 — 주석에 남긴다**
- [ ] **F-76②** 같은 함수 `summary` — `"코드 5755804 — 전 프로세스 동일"` →
      `"코드 5755804 + 미커밋 5파일 — 저장소와 다름"`. **화면·장중 점검이 이 문자열을 그대로 읽는다**
- [ ] **F-76③** `verdict.reasons`에 `worktree_dirty` 추가하되 **dev에서 `ok`를 false로 내리지 않는다**
      (dev dirty는 설계상 허용) — 「모르는 것을 정상이라 말하지 않되, 알려진 허용을 결함으로 만들지도 않는다」
- [ ] **F-76④** `src/messiah/ops/record_vs_commit.py` `session_git_shas`에 `source_mtime_max` 병기 —
      사후에 "그날 무슨 바이트로 돌았는가" 복원
- [ ] **F-76 검증** `tests/ops/test_status_board.py` 신설 3건(dirty×sha 조합) + 오늘 상태 재실행 시 `stale: true`
- [ ] **F-76 R18** 비해당(표시 계층 · 발행·주문 차단 없음)

### 🆕 F-77 — 점검 미실행을 다음 점검이 자동 검출 `P1` · 약 90분 · 대응 1-5

- [ ] **F-77①** `.claude/skills/messiah-daily-check/scripts/collect_evidence.py` `§10 점검 연속성` 신설 —
      직전 5거래일(`configs/krx_holidays.yaml`로 휴장 제외)의 `<날짜>_report.md` 존재와
      `## 제1·2·3부` 헤딩 유무를 표로. **빠진 국면을 §9 자동 적신호로 올린다**
- [ ] **F-77②** 같은 파일 — dev_memory 두 파일 마지막 갱신이 직전 거래일 마감 이후인지, 아니면 적신호
- [ ] **F-77③** `src/messiah/ops/task_schedule.py` + `scripts/install_scheduled_tasks.ps1` 정본 목록에
      **18:15 장후 자동조치 추가**(현재 5종: Messiah · Messiah-ClockResync · Messiah-G2 ·
      Messiah-Postmarket · Messiah-Shutdown → **6종**). **지금 18:15이 정본에 없어 안 돌아도 계기가 침묵한다**
- [ ] **F-77 선행 · K-7 분기** 오늘 18:15이 **돌면** ③은 정본 목록 누락만 고치고,
      **안 돌면** 스케줄 등록 자체가 없는 것이라 등록 작업이 추가된다
- [ ] **F-77 검증** `tests/ops/test_task_schedule.py` 정본 6종 일치 ·
      수집기에 08-27/08-28 넣어 **4건 공백이 적신호로 뜨는지**(오늘 데이터가 그대로 회귀 픽스처)

### 🔄 F-72 — 자가점검 줄의 심각도를 본문 최대 심각도로 승격 `P1` · 약 60분 · **08-27 이월 · 우선순위 상향** · 대응 1-3

- [ ] **F-72①** `src/messiah/ops/health.py` 렌더러 — 표시 심각도 =
      `max(반환 status, 본문 [WARN]/[ERROR] 토큰)`. `[OK ] git [WARN] ...` → `[WARN] git ...`
- [ ] **F-72②** `self-check: PASS — 기동 허용 (경고 2건: git, bundle)` 형태로 요약 병기.
      **PASS 판정 자체는 바꾸지 않는다**(dev dirty로 기동을 막으면 안 된다)
- [ ] **F-72③** `collect_evidence.py`의 `비-OK N행` 집계가 이 승격을 반영 —
      오늘 `비-OK 0행`이 수집기가 물려받은 오염이다
- [ ] **F-72 검증** `tests/ops/test_health.py` 신설 2건 + 오늘 로그 픽스처로 `git`·`bundle` 승격 확인
- [ ] **F-72 상향 사유** 오늘 실피해 실증 — 이 한 줄이 1-1의 유일한 장전 신호였고 나흘간 안 읽혔다

### 🔄 F-73 — 옵션 체인 기준가의 신선도를 발행에 실어 보낸다 `P1` · 약 3시간 · **08-27 이월 · 착수 가능** · 대응 1-4

- [ ] **F-73 선행 해제** **K-5 판정 완료 — 무결성 리포트는 이 구간을 잡지 않는다.**
      `daily_integrity_20260828.json` `series_findings: []` · `horizon_findings: []` ·
      `flat_price_minutes: 0` · `pre_open_minutes: 15`. → **필요성 확정**
- [ ] **F-73 임계 결정 = 60초 확정** 정상 폴링 간격 300초/600초라 1분봉 유예 2,000ms는 상시 참이 되어 무의미.
      60초 = 1분봉 1개 분량
- [ ] **F-73①** `src/messiah/core/messages.py` `OptionChainSnapshot`(275행 인근) —
      `spot_as_of` · `spot_age_seconds` 추가
- [ ] **F-73②** `src/messiah/data/option_chain_poller.py:274` 발행부 — 두 필드를 페이로드·로그에 싣고
      60초 초과 시 `OptionChainStaleSpot`
- [ ] **F-73③** `src/messiah/core/logging.py:435` 인근 레벨표에 `"OptionChainStaleSpot": logging.WARNING`
- [ ] **F-73④** `src/messiah/ops/integrity_report.py:2667` 인근 — `stale_spot_cycles` 집계
- [ ] **F-73⑤ 경보 피로 대책** 장전(08:15~08:45)은 **구간당 1건**으로 접어 첫 발생·해소 시각만
- [ ] **F-73 검증** `pytest tests/data/test_option_chain_poller.py` 신설 4건 ·
      `scripts/run_replay.py --symbol A05609 --start 2026-08-31`로 오늘 10건 재현 ·
      다음 거래일 08:22 전후 첫 발생 · 08:45~08:50 해소 · `daily_integrity`에 `stale_spot_cycles > 0`
- [ ] **F-73 R18** 비해당(발행 차단 없이 표시만). **훗날 이 값으로 차단하면 그때는 섀도 20거래일 대상**
- [ ] **F-73 오늘 근거** 08-31 스테일 22분(10회 · 420다리 · +3.12% 과대 · 1069.44→1037.10) ·
      08-28 스테일 23분(11회 · 462다리 · 1089.56→1075.60)

### 🔄 F-74 — 시리즈별 폴링 주기 결정을 로그에 남긴다 `P2` · 약 40분 · **08-27 이월 · 필요성 확정** · 대응 C-1

- [ ] **F-74①** `scripts/run_l1_daily.py` 폴러 등록 루프 직후 **`OptionChainScheduleResolved`**(INFO ·
      `{series, interval_seconds, phase_offset_seconds, reason}`). **`reason`이 없으면 이 fix는 반쪽이다**
- [ ] **F-74②** `src/messiah/core/logging.py` 레벨표 등록
- [ ] **F-74 선행 C-1** 만기 요일 분기가 코드에 **있으면** 로깅만으로 끝나고, **없으면**(설정 드리프트)
      `configs/instance.yaml` 정본화가 따라붙는다. 볼 곳 `data/option_chain_poller.py` `FixedTickScheduler`
      등록부 · `broker/kis/symbol_master.py:93` `_WEEKLY_EXPIRY_WEEKDAY`. **오늘 장후**
- [ ] **F-74 검증** `pytest tests/scripts/test_run_l1_daily.py` 신설 1건 + 화·수 관측으로 요일 가설 완결

### 🔄 F-71 — 검증 등록부의 기한을 지표 변경과 함께 재산정 `P1` → **범위 축소** · 약 40분 · **08-27 이월**

- [ ] **F-71 전제 하나 소멸** C-2 판정 완료 — 집계 로직에 결함 없음(`clean 21 · overdue 1 · pending 1 = 23`).
      → 「집계 결함 수정」이 아니라 **「지표 정의가 바뀌면 그 항목 `deadline`을 함께 민다」**로 축소
- [ ] **F-71①** `src/messiah/ops/fix_verification.py` 기한 판정 함수 —
      지표 리셋 시 `deadline` 재산정. 대상 `configs/pending_verifications.yaml` `ui-crash-isolation`
      (`registered: 2026-08-03` · 남은 0일 < 필요 1일)
- [ ] **㉢ 판정 연기 유지** F-71 적용 후 **2026-09-02 이후** [08-27에서 이월]
...
- [ ] **G-39** `source_ahead_of_commit_seconds` 신설 — `SessionStart.source_mtime_max`(이미 나가고 있다)와
      ...
      `ops/status_board.py` · `ops/integrity_report.py` · 자가점검 `git` 줄.
      **약한 동일성(해시)과 강한 동일성(바이트)의 분리 계측** — dev 상시 양수 가능하므로
      **경보 아닌 계측으로 시작**, 5거래일 누적 후 임계 논의
- [ ] **G-40** `spot_refresh_lag_seconds` + `stale_spot_legs` 신설(08-27 G-38 승계).
      **08-28 344초 → 08-31 144초, 그 사이 커밋 0건 — 지연이 우연에 좌우되고 재는 계기가 없다.**
      `ops/integrity_report.py` · `data/option_chain_poller.py`(**F-73①②와 같은 자리 — 함께 하면 비용 절반**).
      실측 2점 · 20거래일 필요
- [ ] **G-41** 폴링 주기 계약 테스트 + 나흘 분포 픽스처
      `tests/data/fixtures/option_chain_cadence_20260826_20260831.json`.
      `tests/data/test_option_chain_poller.py`(F-73 검증과 같은 파일) · `FixedTickScheduler` 등록부.
      **F-74의 `reason`을 테스트로도 고정 — 설정 드리프트면 CI가 먼저 운다.**
      화·수 데이터 전까지 `xfail`로 두고 관측만(2026-09-02 완결 예정)

### 확인 필요 (2026-08-31 장전 → 장중·장후 판정)

- [ ] **C-1 부분 판정** 폴링 주기 요일 의존. 08-26(수) regular 6 / 08-27(목) weekly_thu 6 /
      08-28(금) regular 5 / 08-31(월) weekly_mon 6. 위상 나흘 고정 `:44`/`:24`/`:02` → **R9 위반 아님.**
      새 가설: *"그 요일 만기 위클리가 있으면 그것이 5분, 없으면 regular가 5분, 나머지 10분."*
      **미확정**: 코드 분기인가 설정 드리프트인가 · 사유가 로그에 없다. **장후**
- [x] **C-2 판정 완료 · 무해** 검증 등록부 집계 로직 결함 없음 —
      `postmarket_20260828.log` 15:46:24 `FixVerificationScoreboard` `clean 21 · overdue 1 · pending 1 = 23`.
      기한 경과 20건은 이미 검증 완료로 `clean`에 들어가 있다
- [ ] **C-3 신규** 08-27 12:33~12:39 소스 수정의 주체. 같은 창(12:36)에 장중 점검 증거 수집이 돌았다.
      **점검 세션이 고쳤다면 `phases.md` B-5 · R11 정면 위반**, 사람이면 절차 문제.
      2026-08-26 F-60은 `run_git()` 화이트리스트로 **git 실행**을 막았지 **파일 쓰기**는 안 막았다 —
      이 구분이 핵심일 수 있다. **사용자 질의 + 장후 판정**

### 관측 항목 K-1 ~ K-8 (2026-08-31 장중·장후가 판정)

- [ ] **K-1** `publish_offset` 시간대별 분포. 전일(08-28) 08시대 p50 683.1ms · p90 1,437.9ms ·
      p99 1,613.1ms · 1,000ms 초과 6/21건(28.6%) · 유예 초과 0. 하루 전체 p90 1,406.9ms ·
      p99 3,955.3ms · 최대 4,712.2ms(표본 708). **오늘 표본 100건 이상에서 재본다. 장중**
- [ ] **K-2** `PublishGraceBreached`(08-26 F-66 · 커밋 `7587d0b`·`991b191`) — 전일 10시대 `over_grace: 2`건.
      **그 2건에 태그가 실제로 나왔는지가 계기 검증이다.** 나왔으면 F-66 합격, 아니면 배선 결함. **장후**
- [ ] **K-3** `RegimeWarmStart.bars_by_source` 전월물(A05608) 비중.
      08-24 72.0% → 08-25 64.5% → 08-27 42.0% → **08-31 4.5%**(피처 엔진 54/1,200봉 ·
      국면 엔진 54/200봉=27.0%). **0%가 되는 날 C-11 자연 종결.** 장후
- [ ] **K-4** 화면 생사 — `ui_20260831.log`에 08:20:51 이후 기록이 붙는가 ·
      4토픽이 신선 배지로 전이하는가(1-7 · 기존 F-56 귀속). **장중**
- [x] **K-5 판정 완료** 스테일 구간은 무결성 리포트에 **잡히지 않는다** →
      **F-73 필요성 확정**(1-4에 반영)
- [ ] **K-6** 승격 관문 미판정 번들 `real-20260820-2053-30m` — **7거래일째.**
      `promotion_evidence_eligible: false` · `legacy_rows_without_countable: 17`
      (**시간이 지나도 승격에 가까워지지 않는다**). 기존 N-23 · 해소 경로 F-52(사람 결정 C등급). **장후**
- [ ] **K-7 신규** 오늘 18:15 장후 자동조치가 실제로 도는가. 08-27·08-28 이틀 연속 흔적 없음.
      **이것이 1-5가 일회성인지 구조적 정지인지를 가른다. F-77③의 분기 조건. 장후**
- [ ] **K-8 신규** `DecisionIntent.cadence_seconds`(1-1 미커밋 변경분)가 장중에 실제로 채워지는가.
      채워지면 화면에서 판단 토픽이 30분 주기 기준으로 신선 판정, `None`이면 종전대로 30초 하한.
      **이 관측이 1-1 변경분의 라이브 검증을 겸한다. 장중**

### ✅ 해소 / 판정 (2026-08-31 장전 확인)

- [x] **C-2** 검증 등록부 집계 로직 — 결함 없음으로 종결(위 참조). **F-71 범위 축소로 반영**
- [x] **K-5** 무결성 리포트가 개장 전 스테일 구간을 잡지 않음 확인 — **F-73 착수 가능**
- [x] **전일 무결성** `daily_integrity_20260828.json` `unmeasured: []` · 늦은 봉 폐기 0 · 재기동 0 ·
      비정상 종료 0 · 네이티브 크래시 0 · 거래량 일치율 99.980%(103,936/103,957 · 결손 0분)

### ⚠ 재발 — 어제 해소 선언이 4시간 만에 깨졌다

- [ ] **1-1 미커밋 소스 반입** 2026-08-27 08:51 장전이 `worktree_dirty: false` · `src+scripts 0파일`로
      ✅해소 선언 → **같은 날 12:33~12:39에 재발**. 신규 요소는 **미추적 파일 포함**
      (종전 재발분은 전부 추적 파일이었다). **F-75로 처분**

### 사용자 조치 (2026-08-31 장전판)

- [ ] **개장 전에는 할 일 없음.** 오늘 발견한 일곱은 전부 기록·판정 계층이고 거래를 막지 않는다(dev·simulator)
- [ ] **오늘 밤 (10분 · 최우선)** F-75 — 미커밋 소스 5개 반입. **`sets.py`를 먼저 `git add`할 것.**
      **안 하면** 그 파일 유실 시 다음 기동에서 전 경로 ImportError, 또는 `commit -am`으로 깨진 HEAD 생성
- [ ] **장 마감 후 (60분)** F-72 — 자가점검 심각도 승격. **안 하면** 오늘 같은 은폐가 반복된다. 이번엔 나흘이었다
- [ ] **장 마감 후 (30분)** F-76 — 코드 동일성 판정 확장. **안 하면** 저장소에 없는 코드가 돌아도 화면은 계속 정상
- [ ] **오늘 18:15 (확인 2분)** 장후 자동조치가 도는지 확인(K-7). **안 돌면** 매일 저녁 정리가 통째로
      멈춰 있다는 뜻이고 오늘 세운 계획들도 같은 이유로 실행되지 않는다
- [ ] **언제든 (2분)** `.env`의 `KIS_HTS_ID`를 실제 HTS 로그인 ID로 교체 →
      `python scripts/probe_order_notice.py` 재실행. 08-28 채점에서도 `order-path-live`가
      `FixVerificationUndiagnosed`(미착수)로 기록됐다 **[08-27에서 이월]**
- [ ] **⚠ 질의 (1분)** **2026-08-27(목) 12:33~12:39에 이 PC에서 소스를 직접 고치셨는가** (C-3).
      답에 따라 「점검 도구가 장중에 파일을 못 쓰게 막는」 작업이 추가된다
- [ ] **⚠ 질의 (1분)** 2026-08-26 오전 10시 30분경 이 PC에서 무엇을 하고 계셨는가 **[08-27에서 이월]**

---

## [MW0601] 2026-08-31 장중 (12:42) — 신규·갱신 체크박스

### 0순위 ⚡ 지금 실행 가능 (장중 허용 · 사람 실행 · 코드 변경 아님)

- [ ] **⚡ 잠금 회수 (30초 · P1 · 🆕 · 대응 1-8)** `.git/index.lock` 0바이트 · mtime
      `2026-08-31 08:51:49.401897400 KST` · 나이 3h51m · git 프로세스 0 → 스테일 3중 조건 충족
      - [ ] PowerShell에서 `cd C:\Users\82108\PycharmProjects\fuoption`
      - [ ] `python scripts\git_lock_guard.py --check` → rc=2 확인
      - [ ] `python scripts\git_lock_guard.py --reclaim`
      - [ ] `Test-Path .git\index.lock` → **False** 확인
      - [ ] **`scripts/git_lock_guard.py`는 futures 정본 바이트 동일 사본 — 직접 수정 금지**(9581·9603)
      - [ ] **안 하면 F-75 전체가 착수 불가** (`fatal: Unable to create '.git/index.lock': File exists`)
      - [ ] 선례 F-53(DECISION_LOG 11998) — 08-26에도 장중 회수. L1·G2는 저장소를 쓰지 않아 관측 영향 0

### 🆕 F-78 — 점검이 저장소에 잠금을 남기지 않는다 (F-60 범위 확장) `P1` · 약 90분 · 대응 1-8

- [ ] **F-78①** `.claude/skills/messiah-daily-check/SKILL.md` §1·§2 —
      **"점검 세션은 `git`을 직접 실행하지 않는다. 저장소 상태는 증거 수집기의 네이티브 관측
      결과(§1)만 인용한다"** 명문화. **지금 이 금지가 SKILL.md에 없다** — 장전 세션은 규칙을
      어긴 게 아니라 규칙이 없었다
- [ ] **F-78②** `scripts/collect_evidence.py` `run_git()` 화이트리스트에
      `diff --ignore-all-space --stat` · `ls-files --others --exclude-standard` **읽기 전용 추가**
      + **호출 전후 `.git/index.lock` 부재 단언**을 반드시 짝지을 것.
      **단언 없이 추가하면 이 변경은 F-60의 후퇴다**
- [ ] **F-78③** 같은 파일 §9 — 잠금 적신호에 「생성 시각이 점검 실행 창 ±5분 내면 점검 자신이 범인」
      판정 추가. 회귀 픽스처 = 오늘 08:51:10(수집) / 08:51:49(잠금) 39초 간격
- [ ] **F-78④** `self_check.py::_check_git()` — `git_lock_guard.inspect()` **임포트 재사용**
      (존재·크기·나이·git 프로세스 수 4가지) → 자가점검 `git` 줄에 표면화.
      **NEXT_TODO 8747 기등재 항목 — 신설이 아니라 착수**
- [ ] **F-78 검증** `pytest tests/ops/test_git_lock_guard_integration.py` 신설 2건 —
      (a) 수집기 실행 전후 잠금 부재 유지 (b) 인위 잠금 시 §9 적신호 + 「점검 창 내 생성」 판정
- [ ] **F-78 라이브 검증** 다음 거래일 **장전 점검 종료 직후** `.git/index.lock` 부재 확인 → K-9
- [ ] **F-78 R18 비해당** — 게이트·차단 신설 아님
- [ ] **F-78 적용 시점: 장후(15:35 이후).** 0순위 잠금 회수만 장중 허용

### 🆕 F-79 — 전일 델타에 표본 수를 강제한다 `P2` · 약 50분 · 대응 1-9

- [ ] **F-79①** `scripts/collect_evidence.py` §4 「항상 인용 태그」에 **`ClockSkewMeasured` 추가**
      (첫 1건 + 마지막 1건 + 최대·최소로 접는다). **지금 없어서 오늘 원본 로그를 직접 grep해야 했다**
- [ ] **F-79②** 같은 파일 — 전일 델타 후보의 `samples` 불일치 시
      `⚠ 표본 불일치(30 vs 600) — 비교 불가` 표시. **막지 말고 표시**(사람 판단 재료를 뺏지 않는다)
- [ ] **F-79③** `references/report_template.md` 「작성 시 주의」 —
      "전일 델타는 같은 표본 조건끼리 비교한다. 다르면 델타를 쓰지 않고 두 값을 표본 수와 함께 나란히 적는다"
- [ ] **F-79④** `src/messiah/ops/integrity_report.py` — `clock_skew_converged_seconds`(n≥600 중앙값) ·
      `clock_skew_first_seconds`(첫 측정 · 표본 병기) **분리 기록**.
      현재 어느 쪽도 산출물에 안 남아 **사후 재구성 불가**
- [ ] **F-79 결정 필요** 수렴 기준을 `samples >= 600`으로 못 박을 것인가 →
      **권고: 병기 + `>=600`을 「수렴」으로 명명.** 임계를 숨기면 표본 정책이 바뀔 때 같은 오판이 난다
- [ ] **F-79 검증** `pytest tests/ops/test_evidence_clock_skew.py` 신설 2건.
      **오늘 로그(08-31 8건 · 08-28 14건)가 그대로 회귀 픽스처**
- [ ] **F-79 적용 시점: 장후.** 우선순위 8순위 (실피해 0 — 앞을 밀지 않는다)

### 🆕 G-42 — 완성봉 유예 초과에 장중 계기를 세운다 (고도화 · 다음 주)

- [ ] **G-42 근거** 오늘 유예 초과 2건(`08:53:01.960` 1m 2,017.3ms · `11:41:03.241` 1m 3,061.1ms)이
      **장중 로그에 경보 0건**. 검출 지연 최대 **6시간 35분**(08:53 사건 → 15:35 마감). 08-26엔 14건
- [ ] **G-42 내용** `features/engine.py` `_note_publish_offset`의 `_publish_offsets.append(...)` 직후 —
      태그 `PublishGraceCrossed`(WARNING · `core/logging.py` 레벨표 등록) ·
      조건 `offset_ms > _grace_ms(horizon)` ·
      **억제: Horizon당 세션 최초 1건 + 이후 30분당 최대 1건**
- [ ] **G-42 페이로드** `offset_ms` · `grace_ms` · `headroom_ms` · `bar_to_publish_ms` ·
      `publish_offset_skew_ms` → 「대기인가 계산인가」가 한 줄에서 갈린다
- [ ] **G-42 R18 비해당** — 발행을 막지 않음(F-66 주석 논리 승계).
      **훗날 이 값으로 차단하면 그때는 섀도 20거래일 대상**
- [ ] **G-42 비용** 60분 + 테스트 30분. `_grace_headroom()`을 F-66과 공유 → 함께 구현 시 비용 절반

### 🔄 G-40 보강 (새 번호 없음)

- [ ] **G-40 범위 축소** 개장 후 정규 구간은 기준가가 **폴링마다 즉시 추종**(12:32:24 `1049.72` →
      12:42:24 `1051.02`, 지연 0) → 지연은 **「첫 틱 이전」 구간의 성질**. 종일 지표가 아니라
      **개장 전 구간 전용**으로 정의하면 잡음이 준다
- [ ] **G-40 형제 지표** `spot_stale_window_minutes`(첫 폴링→첫 갱신 분) 추가 — 오늘 22분 · 08-28 23분.
      다리 수(420 vs 462)보다 분 단위 창이 추세 관측에 안정적

### 관측 항목 — 장후가 판정 (2026-08-31 장중 등록)

- [ ] **K-2 ⬆️예측치 확정** `PublishGraceBreached`는 **마감 1회**만 돈다(함수 주석). 12:42 0건은 정상.
      **오늘 실측 유예 여유 `1m −1,061.1ms`(음수) → 마감에 반드시 떠야 한다.**
      예상 본문: 최악 `1m` −1,061ms · 유예 초과 2건(08시 1 · 11시 1).
      **안 뜨면 F-66 배선 결함 확정(P1)**
- [ ] **K-8 ⚠판정 불가 이월** `cadence_seconds` 오늘 전 로그 1회(`ui` 08:20:51 첫 렌더 · 전부 `null`) ·
      `g2_daily` **0회**(`DecisionEmitted`가 이 필드를 안 싣는다 · 8회 전건 확인).
      **결함 아님 · 관측 수단 부재(1-7).** 화면이 한 줄이라도 더 남기면 판정, 아니면 F-56 전까지 영구 불가
- [ ] **K-4 →장후** 화면 로그에 장후에 한 줄이라도 붙는가. 안 붙으면 종일 2행 확정 → F-56 착수 확정
- [ ] **K-9 🆕** `.git/index.lock` — 사용자 회수 여부 + **장후 점검 세션 종료 직후 재생성 여부.**
      장후가 또 만들면 **1-8이 같은 날 2회 재발**
- [ ] **K-10 🆕** KIS 500 재시도 종일 건수. 12:42까지 **8건**(옵션체인 6 · 수급 2 · 전건 `attempts:2`
      복구 · 결손 0 · 09:20~10:40 집중). 기준선 N-12: "12:36까지 4건 · 종일 8건 이하 정상".
      **반일에 이미 8건 — 종일 12건 이상이면 브로커 회선 별건 등록**
- [ ] **K-11 🆕** 발행 오프셋 종일 분포. 반일 p90 1,058.7 · p99 1,791.7 · 최대 3,061.1(n=404).
      **종일 p90이 전일 1,406.9ms를 밑돌면 개선 확정**
- [ ] **K-3 · K-6 · K-7 🔄장후** (K-7은 18:15 이전 — **부재를 결함으로 적지 않는다**)
- [ ] **C-1 🔄장후 · 새 미지수** 09:00 이후 실측 `weekly_mon` 300초 29/45 · `regular` 600초 13/23 ·
      `weekly_thu` 600초 15/22 → 요일-만기 가설 지지. **새 미지수**: 개장 전 위상 고정(`:24`/`:44`/`:02`)이
      09:00 이후 `:23`~`:42`로 흩어짐(간격 282~317초). 재시도 시각과 겹치므로 **완료 시각 기록 탓으로
      보이나 미확정** → F-74가 함께 판정
- [ ] **C-3 ⬆️격상** 1-8이 「점검이 git을 실행한다」를 실증. 그러나 C-3은 「소스 파일을 고쳐 쓴다」를
      묻는다 — **다른 행위. 실증을 확정으로 승격하지 않는다.** 사용자 답변 유지

### ✅ 해소 / 판정 (2026-08-31 장중 확인)

- [x] **1-5 해소** 일일 점검 4회 공백 → **장전 08:51 + 장중 12:42 2회 연속 실행**.
      부수 효과: 점검이 돌았기에 1-8이 발생 3h51m 만에 잡혔다. **단 F-77 미적용 — 재발 방지책 없음**
- [x] **K-1 판정 완료 · 개선** 표본 404건 p50 579.4 / p90 1,058.7 / p99 1,791.7 / 최대 3,061.1ms ·
      >1,000ms 43건(10.6%). 전일 종일 p90 1,406.9 / p99 3,955.3 / 최대 4,712.2(n=708) 대비 전 분위 개선
- [x] **K-4 판정 완료(부정)** `ui_20260831.log` 08:20:51.976 이후 **4시간 21분 무기록** · 1,361B 불변
- [x] **장중 파이프라인 전수 통과** 1분봉 234개 결손 0 · `bar_to_publish_ms` 최대 188ms(예산 1,000ms의
      18.8%) · 합성봉 173개 유실 0 · `nan_ratio` 최대 0.0073 · `OptionChainPolled` 104회 전부 42/42다리 ·
      `DecisionEmitted` 8건 30분 격자 준수 전부 NO_TRADE · `UnmatchedFill` 0 · 주문 0 ·
      `WARNING`/`ERROR`/`CRITICAL` 각 0건 · 로그 공백 0 · 장중 학습·배포 흔적 0

### ⚠ 재발 — 지난주 해소 선언이 오늘 아침 다시 깨졌다

- [ ] **1-8 저장소 잠금** 08-26 1-6과 **형태 동일 · 시각대 동일(08:50~08:52) · 원인 경로 상이.**
      F-60(`a169976`)은 **수집기의** git만 묶었고 **세션 자신의 git**은 열려 있었다.
      08-27 "㉣ F-60 첫 실전 통과"(12770)는 **참이었다 — 그날은 수집기만 돌았기 때문**.
      **F-60을 되돌리지 않고 범위를 넓힌다** → F-78

### 사용자 조치 (2026-08-31 12:42 갱신판 — 장전판을 대체하지 않고 이어받는다)

- [ ] **⚡ 지금 (30초 · 신규)** 저장소 잠금 회수 (위 0순위). **안 하면 오늘 밤 전량 착수 불가**
- [ ] **오늘 밤 (10분)** F-75 미커밋 소스 5개 반입. **단 위 30초 작업 다음.** `sets.py`를 먼저 `git add`
- [ ] **장 마감 후 (60분)** F-72 자가점검 심각도 승격
- [ ] **장 마감 후 (30분)** F-76 코드 동일성 판정 확장
- [ ] **장 마감 후 (90분 · 신규)** F-78 점검이 저장소를 못 건드리게. **안 하면 매일 아침 반복 가능**
- [ ] **장 마감 후 (⬆️순서 상향)** F-56 화면 하트비트. **안 하면 K-8이 영구 판정 불가**
- [ ] **오늘 18:15 (확인 2분)** 장후 자동조치가 도는가 (K-7)
- [ ] **언제든 (2분)** `.env`의 `KIS_HTS_ID` 교체 → `python scripts/probe_order_notice.py` 재실행
- [ ] **⚠ 질의 (1분 · 무게 상승)** 2026-08-27(목) 12:33~12:39 소스 직접 수정 여부 (C-3)
- [ ] **⚠ 질의 (1분)** 2026-08-26 오전 10시 30분경 무엇을 하고 계셨는가 **[08-27에서 이월]**

## [MW0601] 2026-08-31 장후 (15:58) — 신규·갱신 체크박스

### 0순위 ⚡ 사람 실행 (코드 변경 아님) — 아래 전부의 선행

- [ ] **저장소 잠금 회수 (30초 · 오늘 세 번째 요청 · 미이행 7시간 6분)**
      `python scripts\git_lock_guard.py --check` → rc=2면 `--reclaim`.
      **안 하면 밤 작업 8건(F-75·F-78·F-72·F-76·F-56·F-80·F-73·F-79) 전량 착수 불가**

### 🆕 F-80 — 유예 초과를 「Horizon별 추세」로 기록한다 `P1` · 약 70분 · 대응 1-10

- [ ] **F-80-① 대상** `src/messiah/features/engine.py` `_warn_if_grace_breached()` —
      현재 조건 `worst_headroom_ms < 0` 하나(최악 Horizon만 본다).
      **`breached_horizons: ["1m","3m"]` 필드 추가** + msg에 `"음수 Horizon 2개(1m −2926ms · 3m −160ms)"` 병기.
      오늘 `by_horizon`에 값이 이미 다 있으므로 **집계 로직 추가 없이 필드 하나**
- [ ] **F-80-② 대상** `src/messiah/ops/daily_integrity_report.py` `publish_grace` 조립부 —
      전일 대비 `headroom_delta_ms: {"1m": -214.1, "3m": -1319.7, ...}`.
      **전일 파일 부재 시 `null`**(표본 없는 비교 금지 — F-79 원칙 선적용)
- [ ] **F-80-③** `breached_horizons` 길이가 전일보다 늘면 msg 접두를 `"유예 초과 Horizon이 늘었다(1개 → 2개)"`로
- [ ] **F-80 R18 비해당** — 발행을 막지 않는 순수 관측(F-66 주석 논리 승계).
      기존 `worst_headroom_ms` 유지 → `composer-bucket-completeness` 등 검증항목 무영향
- [ ] **F-80 검증** `pytest tests/features/test_publish_grace.py` 신설 3건 +
      replay(08-28→08-31 순서 · `headroom_delta_ms["3m"] == -1319.7` ±0.1). **기한 2026-09-02**
- [ ] **F-80 비용** 구현 50분 + 테스트 20분. **G-42와 같은 `by_horizon`을 읽으므로 함께 하면 160분 → 110분**

### 🆕 G-43 — 반일에서 종일을 외삽할 때 「꼬리 사건」과 「분포 통계」를 구분한다 (고도화 · 30분)

- [ ] **G-43 근거** 오늘 장중 예고 2건 vs 실제 10건(5배). 같은 관측에서 **p90은 오차 2.9%로 잘 맞았다**
- [ ] **G-43 대상** `.claude/skills/messiah-daily-check/references/phases.md` B절 ·
      `references/report_template.md` 「장후가 판정할 항목」 표 규격
- [ ] **G-43 내용** 표에 `추정 종류` 열 추가 — `분위수(외삽 가능)` / `건수(외삽 불가)`.
      분위수·비율은 표본 200건 이상이면 예상치 허용. **건수·발생 횟수는 「반일 실측 N건 · 종일 미확정」으로만**
      (하한 표기만 허용). **기준 완화 아님 — 형식을 정확히 하는 것**
- [ ] **G-43 검증** 다음 거래일 장중 보고서 표에 열이 있고 건수형에 예상치가 없는지

### 🔄 G-42 우선순위 상향 (다음 주 → 이번 주 · 새 번호 없음)

- [ ] **G-42 상향 사유** ① 검출 지연 **2h35m 실측**(13:00 사건 → 15:35 첫 보고) ② **이틀 연속 관측**
      (08-26 14건·장중 0건 / 08-31 10건·장중 0건) ③ **축 확산**(1m → 1m·3m)
- [ ] **G-42 억제 설계 검산 완료** 「Horizon당 세션 최초 1건 + 이후 30분당 1건」을 오늘 10건에 적용 →
      **발화 7건**(08:1·11:1·12:1·13:2·14:1·15:1), 08-26 14건엔 약 4건.
      **하루 10건 이하로 억제되며 13시대 집중을 놓치지 않는다**
- [ ] **G-42 조건 확인** Horizon 무관 `offset_ms > _grace_ms(horizon)` — 원안이 이미 그러하나 3m 합류로 재확인

### 🔄 G-40 근거 보강 (새 번호 없음) — 리포트 쪽에서 필요성 확정

- [ ] **G-40 확정 근거** `daily_integrity_20260831.json` `pre_open_minutes: 15`인데 실제 스테일 22분 →
      **7분이 계측 밖**. `flat_price_minutes: 0`(정규장만 본다).
      **`spot_stale_window_minutes`는 기존 두 계기로 대체 불가** — 실측 3점째(오늘 22분 · 08-28 23분)

### 🆕 수익률 향상방안 S-1 ~ S-3

- [ ] **S-1 착수 보류 (표본 부족)** 30m 축 기준선(IC +0.174) 초과 4개 — `ev_tod_cos` +0.565(t +4.2) ·
      `ev_close_remain` −0.318(t −2.0) · `px_high_dist_5` +0.325(t +2.1) · `px_ema_dev_5` −0.326(t −2.1).
      **표본 117건/16거래일 — 최소 30거래일(220건) 축적 후 재판정. 그 전에 착수 금지**
- [ ] **S-1 사전 검증 2건** ① `ev_tod_cos` 제외 3개만으로 기준선 초과 유지되는가
      ② 15m·30m 두 축에서 동일 특징이 동시에 사는가(오늘 30m만 4개인 것이 우연일 수 있다)
- [ ] **S-1 R18 해당** 판단 경로 반입 → 섀도 20거래일. 대상 `features/spec.py`(**1-1 미커밋 파일 — 반입 후에 손댈 것**)
- [ ] **S-2 오늘 착수 가능 (40분 · 표본 충분)** `ops/daily_integrity_report.py` `meta_gate`에
      `passed_shadow` 종일 집계(`shadow_passes`·`shadow_blocks`·`blocked_by_regime`).
      회귀 위험 없음 · **R18 비해당**(리포트 필드만)
- [ ] **S-2 검증** `pytest tests/ops/test_integrity_meta_gate.py` 신설 2건 + replay 08-28·08-31 검산
- [ ] **⚠ S-3 사용자 결정 필요** 승격 관문 3항목이 전부 손익 유도값 → **주문 0인 한 영원히 미측정**(8거래일째).
      **「주문을 내야 관문 통과, 관문 통과해야 주문」 순환.**
      ① 신호 관문(섀도 승격) / 손익 관문(실전 승격) **2단 분리** ② 현 구조 유지하고 주문 경로부터 개통 —
      **어느 쪽인지 답 전에는 착수하지 않는다**

### 관측 항목 — 다음 거래일이 판정 (2026-08-31 장후 등록)

- [ ] **K-7 🔄이월** 2026-08-31 **18:15** 장후 자동조치 흔적. 이 점검은 15:58에 돌았다 — **부재를 결함으로 적지 않았다.**
      자가점검 `schedule_drift` 정본에 18:15 작업이 없는 것이 방증이나 **정본 미등재 가능성으로 확정 안 함.**
      **판정 전까지 F-77 보류**(③항 범위 미정)
- [ ] **K-9-b 🆕** 잠금 회수 여부 + **09-01 아침 점검 후 재생성 여부**
- [ ] **K-12 🆕** 3m 유예 초과 재발. `grace_headroom.by_horizon["3m"].headroom_ms` —
      **다시 양수면 단발**(F-80 우선순위 하향) · **연속 음수면 추세 확정**(P1 유지 · F-80 착수 확정).
      **오늘이 1점째 — 1점으로 추세를 말하지 않는다**
- [ ] **K-13 🆕** 브로커 재시도 종일 건수. 오늘 13건(옵션체인 9 + 수급 4 · 전건 `attempts:2` 복구 · 결손 0).
      **3거래일 연속 12건 이상이면 P2 → P1 승격 및 브로커 회선 별건 등록**
- [ ] **K-14 🆕** 유예 초과 시간대 편중. 오늘 오후 8/10 · 08-26도 오후 편중.
      **3일째도 오후 편중이면 「오후에 무엇이 늘어나는가」를 별건으로 판다**
- [ ] **K-15 🆕** 개장 전 기준가 스테일 구간(G-40 4점째). 08-28 23분 · 08-31 22분.
      **F-73 미적용이므로 재발 예정** — 재발하면 G-40 지표 정의 확정
- [ ] **K-3 🔄** 웜스타트 전월물 27.0% → 0%면 C-11 자연 종결. **다음 롤 2026-09-11까지 지속**
- [ ] **K-6 🔄** 승격 관문 번들 9거래일째. **S-3 결정 전에는 움직이지 않는다**
- [ ] **C-1 🔄** 폴링 주기 — 로그 근거는 완결(위클리 5분 · 나머지 10분 종일 유지 ·
      `option_chain/regular` 44행 카덴스 10분 커버리지 99.1%). **F-74의 `OptionChainScheduleResolved`가 있어야 종결**
- [ ] **C-3 🔄 3일째** 08-27 12:33~12:39 소스 수정 주체 — **오늘 추가 실증 없음**(두 세션 모두 파일 미기록 →
      반례도 증거도 못 만듦). **사용자 답변 대기**

### ✅ 해소 / 판정 (2026-08-31 장후 확인)

- [x] **1-5 완전 해소** 3국면 전부 실행(08:51 · 12:42 · 15:58) · 증거 다이제스트 3종 존재.
      **단 F-77 미적용 — 구조는 그대로**
- [x] **1-9 해소** 종일 `clock_skew_seconds: -0.184` · range 0.227 · n=14. 전일 -0.178 대비 -0.006초 — 사실상 동일
- [x] **K-2 F-66 첫 실전 합격** `15:35:05.753 PublishGraceBreached` `headroom_ms: -2926.3`.
      장중이 못 박은 조건 충족 후 실제 발화. **「예고하고 확인한다」가 계기 검증 방식으로 작동**
- [x] **K-4 부정 확정** ui 종일 JSON 2행 · 1.3KB · **7시간 19분 무기록**. 15:40:01 watchdog가 3PID 종료 —
      **죽은 게 아니라 말이 없었다**
- [x] **K-8 영구 판정 불가 확정** `cadence_seconds` 전 로그 1회(ui 첫 렌더 · 전부 null) · g2 0회.
      **F-56 완료 전까지 재시도하지 않는다.** → 1-1은 「닷새째 방치」가 아니라 **「닷새째 미검증 반입」**
- [x] **K-11 판정** 종일 n=708 p50 578.8 · p90 1,089.4 · p99 3,061.1 · 최대 5,160.1ms.
      전일 대비 **p90·p99 −22.6% 개선 · 최대치 +9.5% 악화** · 유예 초과 16 → 10건 개선
- [x] **K-3 판정** `bars_by_source {A05609:146, A05608:54}` = 국면 엔진 27.0% 불변
- [x] **장후 배치·산출물 전수 통과** 6/6 완주 · findings 0 · `unmeasured: []` ·
      거래량 비율 0.999985(차 2계약) · 1분봉 410개 결손 0 · `late_bar_drops: 0` ·
      findings 계열 7종 전부 `[]` · `DecisionEmitted` 14건 격자 이탈 0(최대 편차 2.6초) ·
      `SessionEnd` 3건 「정상 종료」 · `task_exit_codes` 4건 code 0 · `abnormal_exits: []` · 크래시 0
- [x] **`FixVerificationRecurred` 0건 · `FixVerificationFailed` 0건 · Passed 22건.**
      등록부 23건 — 위반 0 · 검증완료 22 · 기한 0 · 대기 1. **`ui-crash-isolation`이 기한 → 검증완료 이동**
- [x] **`DailyCloseBarHandedOff` 이상 아님 확정** 08-25~08-31 5거래일 전부 하루 1건 — 마감 시퀀스 상시 동작.
      불변원칙 2의 명문 예외
- [x] **재시동 불필요 판정** `code_version.stale: false` · 당일 커밋 0건 · 프로세스 이미 15:35 종료.
      **단 밤 커밋 후 수동 기동 시 반드시 커밋 이후에** — 안 그러면 `git_sha`가 옛 값으로 박힌다

### ⚠ 신규 이상점 (2026-08-31 장후)

- [ ] **1-10 `P1`** 유예 초과가 1m 단독 → **3m 합류**. 3m headroom 전일 +1,159.6 → 오늘 **−160.1ms**(1,319.7ms 악화).
      `bar_to_publish_ms` 3m 최대 110ms(예산의 11%) → **원인은 계산이 아니라 대기.** 실피해 0(`late_bar_drops: 0`)
- [ ] **1-11 `P2`** 장중 예고 2건 vs 실제 10건. 검출 지연 **2h35m 실측**. 13시대 p50 571.3·p90 992.2인데 **p99만 4,926.3ms**
- [ ] **1-12 `P2`** 브로커 재시도 종일 13건 — N-12 기준선 12건 초과 **1일째**. 전건 복구·결손 0

### 사용자 조치 (2026-08-31 15:58 최종판 — 아침·낮판을 대체하지 않고 이어받는다)

- [ ] **⚡ 지금 (30초 · 세 번째 요청)** 저장소 잠금 회수. **안 하면 밤 작업 8건 전량 착수 불가**
- [ ] **오늘 밤 (10분)** F-75 미커밋 소스 5개 반입. **`sets.py`를 먼저 `git add`**
- [ ] **오늘 밤 (90분)** F-78 점검이 저장소를 못 건드리게. **오늘 K-9로 방향이 옳다는 것이 실증됨**
- [ ] **오늘 밤 (60분)** F-72 자가점검 심각도 승격
- [ ] **오늘 밤 (30분 · ⬇️60→30 하향)** F-76 코드 동일성 판정 확장 — `record_vs_commit` 판정 참조로 축소
- [ ] **오늘 밤 (120분)** F-56 화면 하트비트. **안 하면 K-8이 영구 판정 불가로 고정**
- [ ] **이번 주 (70분 · 신규)** F-80 유예 초과 Horizon별 추세
- [ ] **이번 주 (110분 · 함께하면 −31%)** G-42 장중 계기 + F-80
- [ ] **다음 거래일 아침 (확인 2분)** 08-31 18:15 자동조치 흔적 (K-7)
- [ ] **언제든 (2분)** `.env`의 `KIS_HTS_ID` 교체 → `python scripts/probe_order_notice.py` 재실행
- [ ] **⚠ 결정 (10분 · 신규)** S-3 승격 관문 2단 분리 여부 — **기준을 손대는 일이라 임의로 하지 않는다**
- [ ] **⚠ 질의 (1분 · 3일째)** 2026-08-27(목) 12:33~12:39 소스 직접 수정 여부 (C-3)
- [ ] **⚠ 질의 (1분 · 5일째)** 2026-08-26 오전 10시 30분경 무엇을 하고 계셨는가 **[08-27에서 이월]**

### [MW0601] 2026-08-31 16:2x 추가 — 사용자 조치 중 발견

- [ ] **1-13 `P1` 🆕** `scripts/git_lock_guard.py`가 cp949 콘솔에서 `UnicodeEncodeError`로 죽는다.
      `main()` 249행 `print(_fmt(info))` · `—`(긴 줄표) · `_fmt()`는 191행 · 스트림 재구성 없음.
      **판정 로직은 230~243행에서 이미 끝난 뒤** — 답을 계산해 놓고 말하다 죽는다.
      `--json` 경로(246행)도 `ensure_ascii=False`라 같은 자리에서 같은 이유로 죽는다(우회 불가).
      **종료 코드가 rc=1로 오염** → 0/2/3 분기가 정의되지 않는다(F-78이 이 rc로 분기할 예정)
- [ ] **즉시 우회(검증됨)** `python -X utf8 scripts\git_lock_guard.py --check` · 세션 전체는 `$env:PYTHONIOENCODING="utf-8"`
- [ ] **🆕 F-81 `P1` · 45분 · 오늘 밤 첫 코드 변경으로** `scripts/git_lock_guard.py` `main()` 진입부에
      `for _s in (sys.stdout, sys.stderr): if hasattr(_s,"reconfigure"): _s.reconfigure(encoding="utf-8", errors="replace")`.
      `errors="replace"`는 **재구성 실패 환경에서도 판정이 나오게** — 깨진 글자가 무응답보다 낫다
- [ ] **F-81-②** `_fmt()` `verdict` 본문의 `—` → `-` (2차 방어 · 의미를 나르지 않는 장식)
- [ ] **F-81-③ 회귀 방지** `tests/ops/test_console_encoding.py` 신설 —
      `scripts/` 하위에서 `print(`를 쓰면서 스트림 재구성이 없는 파일 전수 스캔
- [ ] **F-81 실환경 검증(본질)** 한글 윈도우 파워셸에서 **`-X utf8` 없이** rc가 0/2/3 중 하나로 나와야 닫힌다
- [ ] **F-81 R18 비해당** 출력 인코딩만 변경 · 판정 로직·종료 코드 무변경.
      파일 리다이렉트 경로(`run_postmarket.bat`의 `Out-File -Encoding utf8`)는 이미 UTF-8 — 이중 변환 없음
- [ ] **적용 순서 정정** F-81을 1순위로(F-75 앞). **단 오늘 밤에 한해 `-X utf8` 우회로 잠금을 먼저 풀 수 있으므로
      F-81은 「오늘 밤 첫 코드 변경」이지 「오늘 밤을 막는 것」이 아니다**
- [ ] **범위 확인 필요** 같은 형태가 다른 운영 스크립트에도 있는가.
      배치는 `Out-File -Encoding utf8`로 살아남았고 **콘솔 직접 출력 경로만 노출** —
      오늘 이 스크립트가 처음으로 사람 손에 콘솔에서 실행됐다

### [MW0601] 2026-08-31 18:10 — F-81 구현 완료 · 커밋 대기

- [x] **F-81-① 완료** `src/messiah/core/console.py` `ensure_utf8_console()` 신설
- [x] **F-81-② 완료** `scripts/git_lock_guard.py` 자체 `_ensure_utf8_console()` + `main()` 첫 줄 호출 +
      출력 `—`→`-` 6곳 · `⚠`→`[!]` 1곳. **공용 헬퍼 미사용은 의도** (의존성 없는 단일 파일 제약)
- [x] **F-81-③ 완료** `tests/ops/test_console_encoding.py` 18건 — **18 passed**
- [x] **F-81-④ 완료(예상 밖)** 회귀 스캔이 잡은 3파일 동시 수정 —
      `probe_order_notice.py`(**오늘 사용자 조치 목록에 있던 것**) · `backfill_threshold_source.py` ·
      `suggest_fix_commits.py`. **노출 면적 4개 → 0개**
- [x] **F-81 재현 검증 완료** `-X utf8` 없이 cp949 강제 4경로 — rc 0/2/3 정상 · `UnicodeEncodeError` 0건 ·
      회수 시 락 실제 제거 확인 · `--json` 정상
- [ ] **⚠ F-81 라이브 검증 (기한 2026-09-01)** 사용자 PC **실제 파워셸**에서 `-X utf8` **없이**
      `python scripts\git_lock_guard.py --check` → `fuoption OK 정상 - 락 없음` · rc 0.
      **안 되면 재구성이 안 먹는 환경 — 2차 방어만으로 버티는지 재판정**
- [ ] **⚠ F-81 커밋 (사용자 PC에서 · 1분)** 점검 환경엔 pre-commit·ruff가 없어
      **훅 실패로 index.lock이 남을 수 있다(1-8 재현 위험) → 여기서 커밋하지 않는다.**
      `git add` 6파일 + dev_memory 2파일 명시. **`git add .` 금지**(08-27 5파일이 섞인다).
      메시지 `[MW0601] F-81 콘솔 인코딩 방어 — 판정이 출력 단계에서 죽지 않는다 (1-13)`
- [ ] **F-75는 별개 커밋으로** `features/sets.py` 먼저 `git add`
- [ ] **규약** 새 `scripts/` 스크립트는 `main()` 첫 실행문으로 `ensure_utf8_console()`.
      `_KNOWN_UNPROTECTED`에 이름 추가는 **답이 아니다**
### [MW0601] 2026-08-31 18:25 — F-81 커밋 완료 · 1-14 신규

- [x] **F-81 커밋 완료** `3322164` · 8파일 · 훅 9종 통과 · index.lock 잔존 없음 · HEAD `5755804`→`3322164`
- [ ] **⚠ F-81 라이브 확인 (1분 · 기한 2026-09-01)** 사용자 PC 실제 파워셸에서 **`-X utf8` 없이**
      `python scripts\git_lock_guard.py --check` → `fuoption OK 정상 - 락 없음` · rc 0. **이게 마지막 관문**
- [ ] **1-14 `P2` 🆕** pre-commit stash/restore 가 08-27 미커밋 4파일 mtime 을 `2026-08-31 18:21:06` 으로 덮었다.
      `features/sets.py` 만 보존(untracked → stash 대상 아님). **내용 무손상**(py_compile 통과 · 크기 정상).
      **F 항목 만들지 않는다 — 근본 해소는 F-75(미커밋을 남기지 않는 것)**
- [ ] **K-16 🆕** 내일 08:20 기동의 `SessionStart.source_mtime_max` 가 `2026-08-31T09:21Z` 로 찍히는가.
      찍히면 1-14 진단 확정. **다음 점검은 그 값을 「코드 변경」으로 읽지 않는다**
- [ ] **규약 🆕** mtime 기반 판정 전에 `.git/logs/HEAD` 마지막 커밋 시각을 먼저 본다.
      **같은 분대면 그 mtime 은 증거가 아니다**
- [ ] **C-3 판정 재료 주의** 「5파일 6분 순차 수정」 시간 간격 패턴은 **파일 시스템에서 소실**.
      증거는 `l1_daily_20260831.log` 의 `source_mtime_max` 와 장전 보고서 시각표에만 남았다
- [ ] **교훈 (F-81 1차 시도)** 환경에 없는 도구(ruff)의 결과를 예단하지 않는다.
      사전 확인 가능한 것과 아닌 것을 구분해 적을 것

### [MW0601] 2026-08-31 18:35 — 장후 자동조치(예약) 결과 · 이월 항목

- [x] **F-78 완료** — 점검 세션이 `git`을 직접 안 부른다. 커밋 대기.
- [x] **F-72 완료** — 자가점검 심각도 승격(줄머리가 본문 경고를 반영).
- [x] **F-76 완료** — `code_version.stale`이 워킹트리 dirty를 반영(뜻 확장).
- [x] **F-80 부분 완료** — `breached_horizons` 필드만. 아래 잔여 참조.
- [x] **S-2 완료** — `meta_gate.shadow_*` 국면별 종일 집계.
- [ ] **F-80 잔여 (②③)** `src/messiah/ops/integrity_report.py`의 `_publish_grace_axis()`
      (또는 별도 조립부)에 전일 대비 `headroom_delta_ms` · 초과 Horizon 증가 시 msg 접두 변경.
      **막힌 이유**: 전일 리포트 파일 조회(`logs/daily_integrity_<전일>.json`) + 휴장일 스킵
      로직이 추가 설계를 요구해 이번 세션 예산 밖. 08-28·08-31 두 날치로 검산
      (`headroom_delta_ms["3m"] == -1319.7 ±0.1`)이 원래 검증 방법.
- [ ] **F-56** 화면 라이브 하트비트(120분) — 순서표 5순위, 이번 세션 미착수.
- [ ] **F-73** 옵션 체인 기준가 신선도 발행 탑재(90분, 임계 60초 확정) — 순서표 7순위, 미착수.
- [ ] **F-74** 시리즈별 폴링 주기 결정 로그(40분) — 순서표 9순위, 미착수.
- [ ] **F-79** 전일 대비 델타 표본수 불일치 표시(50분) — 순서표 8순위, 미착수.
- [ ] **F-75** 미커밋 소스 5개 반입 — **사람 몫**(병행 세션 18:25 절이 이미 사용자 조치로 지정,
      `features/sets.py` 먼저 `git add` · F-81과 별개 커밋). 이 세션은 §1 가드 7항에 따라 손대지 않았다.
- [ ] **F-77 · F-71** 리포트가 보류 유지(K-7 판정 대기 · 범위 축소) — 다음 판정 뒤 재검토.
- [ ] **⚠ 사람 결정 필요** S-3(승격 관문 순환 구조 — 신호/손익 2단 분리 여부). 이 세션은 착수하지 않았다.
- [ ] **표본 대기** S-1(30분 축 특징 4개 판단 입력 승격) — 30거래일(표본 220건 이상)까지 착수 금지.

### [MW0601] 2026-09-01 08:55 — 장전 점검 결과 · 신규 G-44

- [ ] **⚠ F-81 라이브 확인 — 기한 오늘(2026-09-01) 도달, 아직 미이행 확인됨.** 1분 ·
      `python scripts\git_lock_guard.py --check`(`-X utf8` 없이) · 기대 `fuoption OK 정상 - 락 없음`.
      **오늘 아무 때나 즉시 실행 가능(코드 변경 아님).**
- [ ] **⚠ F-75 반입 — 기한 오늘(2026-09-01) 장전 도달, 아직 미이행 확인됨.** git 상태가 08-31
      저녁과 동일(5파일). **오늘 장후(15:35 이후) 반입 권고** — 개장 직전 강행은 실수 대응 여유가
      없어 손익상 불리(상세 손익 비교는 `logs/dailycheck/2026-09-01_report.md` F-75 절 참조).
- [ ] **🆕 G-44 `P2` · 약 20분** `.claude/skills/messiah-daily-check/scripts/collect_evidence.py`에
      `NEXT_TODO.md`·`DECISION_LOG.md`를 정규식 `검증 기한\s+(\d{4}-\d{2}-\d{2})`로 스캔해 실행일
      이전·당일 기한 항목을 §9 자동 적신호 아래 새 절로 목록화하는 기능 추가.
      **관측 근거**: 오늘 F-75·F-81 기한 도달을 사람이 802.9KB 파일을 수동 grep해서 찾았다 — 자동
      노출 계기가 없으면 다음 기한부터도 우연에 의존한다. R18 비해당(판정 로직 무관, 순수 보고 기능).
- [x] **참고 확인** `.git/refs/remotes/origin/master` = `dfb835f`(로컬 HEAD와 일치) · 파일 mtime
      08-31 18:41 — 08-31 18:45 사용자 조치("`git push` 사람이 직접 실행")가 이행됐을 강한 정황.
      **확정 아님**(이 세션은 `git fetch`/`push`를 직접 실행하지 않음, F-78 준수) — 장후 점검이
      실제 `git status`(원격 대비)로 재확인 권고.


### [MW0601] 2026-09-01 08:53 장전 점검 — 대시보드 감사 결과 (F-82 ~ F-86 · G-44 ~ G-46)

**정본 노트 (사용자 지시로 기록) — 첫 봉은 08:46에 그려진다**

- [x] **첫 봉 시각 정본화** `first_bar_kst`(= 봉이 **여는** 시각, 08:45)와 봉이 **그려지는** 시각을
      섞지 않는다. 2026-09-01 실측(파티션 디렉터리 생성): **1m 08:46 · 3m 08:48 · 5m 08:50**.
      화면 기본 Horizon은 **5m**(`ui/app.py:1455` `index=2`) → 기본 화면 기준 오경보 창 **약 5분**.
      전일 대조: `daily_integrity_20260831.json` `bar_continuity[1m].first_bar_kst="08:45"` · rows 410 · 결측 0분.

**🆕 신규 이상점 (2026-09-01 장전)**

- [ ] **1-1 `P1` 🆕 → F-83** 화면 코드 버전 줄이 F-76 수정을 안 탔다.
      화면 `코드 dfb835f — 전 프로세스 동일`(회색) vs 같은 분 `status_snapshot`
      `stale:true / stale_reason:"worktree_dirty" / "코드 dfb835f + 미커밋 5파일 — 저장소와 다름"`.
      **재발 아님 — F-76(`2ea7072`)이 두 소비자 중 하나만 배선한 부분수정 잔여**
- [ ] **1-2 `P1` 🆕 → F-82** `🛑 08:45이 지났는데 오늘 봉이 없다` 적색 경보가 **매 거래일 확정 발생**.
      임계=첫 틱(08:45), 판정 대상=봉 파일 존재. 2026-08-11 F-3의 잔여 구간
- [ ] **1-3 `P1` 🆕 → F-84** 패널 ①이 전일 15:29 판단을 현재값처럼 표시.
      추가로 `st.metric(label, value, delta)`의 delta 칸에 절대값 문자열을 넣어 **초록 상승 화살표**가
      붙는다(`delta_color="off"`도 없음). **L18의 새 변종 — 위젯 인자 오용으로 없는 방향성이 생김**
- [ ] **1-4 `P2` 🆕 → F-85** 야간 정상 침묵이 `죽음(1036분 침묵) · 프로세스 확인`.
      `TopicSnapshot.dead`에 세션 경계 개념 없음. 매일 08:20~09:00 약 40분 확정 발생
- [ ] **1-5 `P2` 🆕 → F-86** `Self-Evaluation 미니보드: Phase 5 미구현 — 자리만` 하드코딩.
      실제로는 `logs/self_eval_<날짜>.json`이 매일 15:34 생성(`SelfEvalReportGenerated`).
      **F-5가 고친 줄 바로 아래 줄**
- [ ] **1-6 `P2` 🔄** 미커밋 소스 5파일 6거래일째 — **새 F 번호 만들지 않는다. 기존 F-75**

**🆕 Fix 항목 (전부 장후 적용 — 장전은 계획만, R11 · 금지계명 3·4)**

- [ ] **F-82 `P1` · 40분 · 1순위** `ui/app.py:1157 _live_date_notice()`에 `horizon` 인자 추가.
      `first_expected = first_tick_time + Horizon 길이 + 발행 유예`. 유예는 **새 상수를 만들지 말고**
      `bar_close` 자가점검이 쓰는 값(1분봉 2000ms)을 단일 소스로 가져온다.
      `expected` 문구 → `ℹ 오늘 첫 {horizon}봉은 {HH:MM} 예정 — 그때까지 전일 {날짜} 차트`.
      **`alert` 문구는 그대로 유지**(월물 롤/수집기 2후보 — 2026-08-14 F-3 판단 훼손 금지).
      회귀 위험: 진짜 적재 정지 발견이 최대 5분 늦어진다 — `l1.collector` 배지·`irrecoverable_loss`
      스트립이 더 빠르므로 실질 손실 없음. 검증 5건(5m·08:47 expected / 5m·08:52 alert /
      1m·08:47 alert / 휴장일 불변 / 09:30 ok). **R18 비해당**
- [ ] **F-83 `P1` · 30분 · 2순위** `ui/app.py:1021 _render_version_strip()`이
      `ops/status_board.py:393 code_version_axis()`를 쓴다. `dirty_files`는
      **`status_snapshot.json`의 `code_version.worktree_dirty_files`에서 읽는다 — 화면에서 git 금지**(F-78).
      `stale_reason=="worktree_dirty"`면 캡션을 `저장 안 된 소스 {n}파일이 섞여 돈다 — 재기동해도
      커밋 전엔 안 바뀐다`로 교체. **기존 캡션 `재기동해야 최신 코드가 적재된다`는 `sha_mismatch`
      전용 — 틀린 처방을 지우는 것이 이 항목의 절반이다.**
      검증 3건(dirty 5 앰버 / dirty 0 회색 / None 미측정). **R18 비해당**
- [ ] **F-84 `P1` · 35분 · 3순위** `ui/app.py:1120 render_ai_decision_panel()`.
      ① delta 인자 제거 → `st.metric("의도", side)` + `st.caption(f"확신도 … · 불확실성 …")`
      ② `intent_snap.badge==STALE` 또는 `.dead`면 패널 맨 위 `st.warning(f"이 판단은 {age}분 전({시각})
      것이다 — 지금 시장의 판단이 아니다")`. `intent_snap` 이미 보유 — 시그니처 변경 불필요.
      검증 4건. **R18 비해당**
- [ ] **F-85 `P2` · 50분 · 4순위** `ui/data_source.py:54 TopicSnapshot.dead` + `ui/app.py:829 _badge_caption()`.
      `dead` 판정에서 비-세션 구간 제외(`SessionHours` 사용 · 새 상수 금지).
      **⚠ 제약: 세션 내 침묵 판정은 한 톨도 건드리지 않는다**(2026-08-21 F-11 ㉠ 규율).
      **테스트로 그 불변을 먼저 못 박고 시작할 것.** 검증 6건 —
      08:30·1036분→dead=False / **10:00·95분→dead=True 유지** / 10:00·31분→False /
      15:50·20분→False / 휴장일 / `cadence_seconds=None` 불변.
      **R18 비해당이나 탐지 감도 하향 방향 — 사람 확인 권장**
- [ ] **F-86 `P2` · 45분 · 5순위** `ui/app.py:1313 render_bottom_zone()`의 하드코딩 캡션 제거.
      `_self_eval_lines(today) -> list[str]` 신설(`_event_calendar_lines()`와 같은 꼴).
      `wiring_stage` · `n_return_samples` · `pnl_measurable` 3줄. 파일 부재 →
      `오늘 자기평가 산출 전 — 15:34 예정`, 파손 → 사유. **빈칸으로 두지 않는다**(F-5 규율).
      TTL 캐시 태우고 예외는 삼켜 한 줄 보고. 검증 4건. **R18 비해당**

**🆕 고도화 (G)**

- [ ] **G-44 `30분`** `tests/ui/test_no_stale_claims.py` 신설 — `ui/app.py`의 **조건 없이 출력되는
      단정 문자열**(`미구현`·`미배선`·`알려진 갭`·`자리만`)을 정적 수집해, 반증 산출물이 `logs/`에
      있으면 실패. 화이트리스트에 **왜 아직 참인지** 주석 강제.
      근거: F-5가 고친 줄 바로 아래 줄이 오늘까지 살아남았다 — 사람 리뷰로 못 잡는다는 증거 2회
- [ ] **G-45 `40분`** `ops/integrity_report.py`에 `_ui_notice_axis()` 신설 —
      화면이 `st.error`/`죽음`을 낸 구간을 UI 로그에서 세어 `ui_alert_minutes`로 적재.
      **N거래일 연속 같은 시간대 경보 → 「오경보 후보」 자동 승격.**
      근거: 1-2·1-4를 사람이 화면 캡처로 발견했다. 캡처 안 한 날은 못 본다
- [ ] **G-46 `50분`** `ops/integrity_report.py` `bar_continuity`에 `first_bar_written_kst` 추가
      (현재 `first_bar_kst`는 **여는 시각**만). Horizon별 「첫 봉이 디스크에 내려앉은 시각」 매일 적재
      → F-82 임계가 상수 추정이 아니라 **전일 실측의 함수**가 된다

**관측 항목 — 오늘 장중이 판정**

- [ ] **K-17** 08:50 이후 화면 ② 적색 경보 소멸 확인 → 1-2 「5분 창」 확정
- [ ] **K-18** 09:00 첫 사이클에서 `decision.intent` STALE→OK, 「죽음」 소멸 → 1-4 「매일 40분」 확정
- [ ] **K-19 (C-1)** `intel.futures` 09:20 시점. 값 도착=Pub/Sub 특성으로 정상 /
      NO_DATA 유지=**발행 경로 미배선 신규 이상점**.
      근거: 08-31 g2 로그에 `FuturesView` 발행 **0건**, `decision.intent`는 Streams(`bus.py:31`)라 재생됨
- [x] **K-20 판정 완료** `l1.composer` 08:47:52 UNKNOWN → 08:51:38 OK(합성봉 4개 · 유실 0). 웜업 회색 정상 해소

**전일 이월 처분**

- [x] **F-81 라이브 검증 통과 — 닫음** `$env:PYTHONIOENCODING="cp949"` · `cmd /c "chcp 949 …"`
      두 조건 모두 rc=0 · 한글 정상. ⚠ **첫 시도는 무효였다** — 세션 기본 콘솔이 `chcp 65001` ·
      `PYTHONIOENCODING=utf-8:surrogateescape`라 결함 조건 미재현.
      **교훈 규약 🆕: 검증 전에 검증 환경이 결함 조건을 갖는지부터 확인한다**
- [x] **K-16 판정 불가 — 항구 종결** 실측 `source_mtime_max = 2026-08-31T09:37:40.542736+00:00`
      (= 08-31 18:37 KST). 18:30~18:37 장후 자동조치 커밋 5건이 18:21:06 stash/restore mtime을 덮었다.
      **1-14를 반증하는 것이 아니라 이 값으로는 확정도 반증도 못 한다**
- [ ] **1-14 (08-31) 🔄 무해 지속** 근본 해소는 F-75
- [ ] **F-75 🔄 사람 몫 · 6거래일째** 미커밋 소스 5파일 반입. `features/sets.py`(untracked) 먼저 `git add`
- [ ] **F-80 잔여(②③) 🔄** 검산 재료 확보 — 08-31 `publish_grace` 1m p90 949.6ms(409표본) ·
      3m p90 1332.6ms · 30m p90 1075.8ms
- [ ] **F-56 · F-73 · F-74 · F-79 🔄** 08-31 미착수 4건 — 순서표 F-82~F-86 뒤
- [ ] **F-77 · F-71 🔄** 보류 유지(K-7 판정 대기)
- [ ] **S-1 🔄 표본 대기**(30거래일) · **S-3 🔄 사람 결정 필요**
- [x] **S-2 배선 확인** 08-31 `meta_gate {evaluations:14, passes:14, p50:0.0537, p90:0.4324, max:0.541}`

**확인 필요 (판정 보류)**

- [ ] **C-2 사람 결정** 화면 최상단 `모드 LIVE`는 `DataSourceMode`(실시간 구독)이지 매매 모드가 아니다
      (`app.py:1092`). 실제 인스턴스는 `messiah-dev-01 / mode=dev / secrets dev·simulator`.
      **실전 전환 시점 화면 정책과 함께 결정** — 이상점으로 올릴지 표기 개선(G)으로 볼지
- [ ] **C-3 장후 판정** `command_center_ui.json` `pid:21380` vs UI 로그 `SessionStart.pid:22708`.
      전자는 `streamlit.exe` 래퍼, 후자는 자식 파이썬(`core/ui_launcher.py:284`).
      `shutdown_watchdog`은 명령줄 매칭이라 무해할 가능성 높음 →
      **오늘 15:40 `shutdown_watchdog.log`에 UI 3프로세스가 다 잡히는지로 닫는다**

**장중 점검 처분 (2026-09-01 12:37)** — 위 K-17·K-18·K-19(C-1)를 판정 완료로 닫는다. 원본 항목 줄은 그대로 두고(이력 보존) 아래에 처분만 덧붙인다.

- [x] **K-17 ✅ 판정 완료(2026-09-01 장중)** 08:50부터 화면 적색 경보 소멸 확정 — 1-2 「5분 창」 확정.
      코드 근거: `_live_date_notice()`(`app.py:1183`)는 `chosen==today`면 무조건 `"ok"`. 5m 파티션 생성이 08:50이라 그 시각부터 전환.
      **원인(코드)은 미수정 — 내일도 같은 5분 창 재현. F-82 장후 적용 대상 그대로.**
- [x] **K-18 ✅ 판정 완료(2026-09-01 장중)** 09:00 첫 판단(`DecisionEmitted` 09:00:01.394474)과 함께 「죽음」 배지 소멸 확정 — 1-4 「매일 40분」 확정.
      08:20(UI 기동)~09:00(첫 판단)=40분, 계산 일치. **원인(코드) 미수정 — F-85 장후 적용 대상 그대로.**
- [x] **K-19 (C-1) ✅ 판정 완료(2026-09-01 장중) — 정상, Pub/Sub 특성 확정.** 「발행 경로 미배선」 가설 **기각**.
      근거: 오늘 `DecisionEmitted` 8건 전부 `n_experts:1`(0 아님) · `④ 우위 부족` 분기(`n_experts==0` 분기 `①′`가 아님) —
      `FuturesAIService`가 09:00부터 30분마다 정상 발행 중. 08:45 `NO_DATA`는 첫 30분봉 이전 워밍업일 뿐.
- [ ] **F-82 구현 시 주의(2026-09-01 장중 발견)** `chosen == today` 분기가 시간 판정보다 **먼저** 실행되므로,
      `first_expected` 계산 로직을 그 뒤에(오늘 봉이 아직 없을 때만) 넣을 것 — 순서를 지키지 않으면 이중 분기로 회귀 위험.
- [x] **B-2 완성봉 규율 오늘 검증(2026-09-01 장중)** 1m `bar_to_publish_ms` p90 78ms · max 172ms(표본 237, 09:00~12:41) — 유예 500ms 초과 0건. F-80 잔여 검산 재료로 적재.
- [ ] **1-1(F-83)·1-3(F-84)·1-5(F-86)·1-6(F-75)·C-2·C-3** 🔄 장중 무변화 지속 — 장후 적용/처리 대상 그대로(2026-09-01 장중 확인).

### [MW0601] 2026-09-01 16:04 — 장후 점검 결과 · 신규 F-87

- [x] **C-3 ✅ 해소(2026-09-01 장후)** `shutdown_watchdog.log` 15:40:01 — UI 3프로세스(PID 21380·7276·22708)
      전부 명령줄 매칭으로 정상 종료 확인. PID 불일치는 종료 절차에 영향 없음, 무해 확정. 새 번호 없음.
- [ ] **C-2 🔄 지속 — 사람 결정 보류** 실전 전환 시점까지 판정 유보. 오늘 장후로 닫히지 않는다.
- [ ] **🆕 1-7 `P2` → F-87** 종가 처리(15:35:05) 마지막 1분봉 `PublishGraceBreached` 1m headroom_ms
      -6,730.9ms — 최근 4거래일 최대치(08-31 -2,926.3ms)의 2.3배. 데이터 유실 없음(`late_bar_drops: 0`).
      다음 거래일 -8,000ms 이하로 추가 확대 시 P1 격상 검토.
- [ ] **F-87 `P2` · 30분 · 6순위(F-82~86 뒤)** `features/engine.py:1373 _grace_headroom()` —
      `PublishGraceBreached`에 전일 대비 `headroom_delta_ms` 추가(F-80 잔여②의 재확정).
      전일 파일/필드 없으면 `null`+"비교 불가"(L18). `abs(delta) > 전일 50%`면 `⚠ 급변` 표시.
- [ ] **관측 특기사항(신규 이상점 아님)** 15:00 `SizerZeroQty`(raw=0.931, 2026-08-03 이후 최초) +
      15:30 `RiskReject`(R6, 마감임박거부) — 둘 다 설계대로 작동(F-23·Holding Policy §2.2 A).
      `order-path-live`(4거래일째 판정 불가 → 5일째) 근접 신호로 다음 거래일 관측 대상 등재.
- [ ] **재시동 불필요(2026-09-01 장후 권고)** `code_version.sha_stale: false`(오늘 신규 커밋 없음).
      `stale: true`는 F-75(미커밋 5파일) 때문 — 재시동으로 해소 안 됨.
- [ ] **다음 거래일 관측 예정 (2026-09-01 장후 등록)**
      ① 1-7 후속 — `PublishGraceBreached` 1m headroom_ms 추이(다음 거래일 15:35)
      ② F-82~F-87 적용 확인 — `status_snapshot.code_version.stale`·`sha_stale`(다음 거래일 장전)
      ③ order-path-live — `orders_submitted`(다음 거래일, 도달률 93.1% 근접 상태에서 관찰)
      ④ K-17·K-18 원인 수정 재현 여부 — F-82·F-85 미적용 시 오늘과 동일 패턴 재현이 정상(다음 거래일 장전)
- [ ] **F-75 🔄 사람 몫 · 7거래일째** 미커밋 소스 5파일 반입, 변경 없음(2026-09-01 장후 확인).
- [ ] **1-1(F-83)·1-3(F-84)·1-5(F-86)** 🔄 장후 무변화 지속 — 오늘 밤 자동조치 또는 사용자 지시 시 적용 대상.
- [ ] **종가 손익(2026-09-01)** 실현/평가 손익 0원 · 종가 포지션 0계약 · 주문 시도 2건 체결 0건(사이징 미달 1·R6거부 1).
      `g2_daily_returns.jsonl`: `{"date": "2026-09-01", "return": 0.0, "n_orders": 0, "countable": false}`.

### [MW0601] 2026-09-02 08:56 — 장전 점검 결과 · 신규 F-88 · G-47

- [ ] **🆕 1-1 `P1` → F-88** F-82~F-87(2026-09-01 저녁 구현분)이 커밋·기록 없이 오늘 아침부터
      실행 중. `worktree_dirty_files` 5→10(어제→오늘). 대상: `src/messiah/data/bar_composer.py` ·
      `src/messiah/data/close_grace.py`(신규) · `src/messiah/ui/app.py` · `src/messiah/ui/data_source.py` ·
      `src/messiah/features/engine.py` + 신규 테스트 4건(`tests/ui/test_first_bar_expected.py` ·
      `test_off_session_silence.py` · `test_self_eval_board.py` · `test_version_strip_and_decision_panel.py`)
      + 기존 테스트 수정 2건(`tests/test_ui_symbol_and_freshness.py` · `tests/ui/test_bar_reader.py`).
- [ ] **F-88 `P1` · 25분 · 1순위** pytest 통과 확인 후 위 파일들만 정식 커밋
      (`[MW0601] 2026-09-02 장후 커밋 — F-82~F-87 (2026-09-01 저녁 구현분 소급 커밋)`).
      F-75 잔여 4파일은 별도 커밋으로 분리 — 섞지 않는다.
- [ ] **G-47 `1시간` · 이번 주** 자가점검이 dirty 파일 중 "새 구현"과 "NEXT_TODO 기등록 부채"를
      자동 구분 — `scripts/self_check.py` 등에 파일별 mtime과 NEXT_TODO 대상 파일 대조 추가.
- [ ] **C-1 확인 필요** 오늘 08:45~08:50 화면에 적색 경보·「죽음」 배지가 실제로 안 떴는지 —
      로그로 재구성 불가, 사용자 확인/캡처로만 판정. K-21로 등록(내일 같은 창 재관측 대안).
- [ ] **C-2 확인 필요** 신규/수정 테스트 6건의 pytest 실통과 여부 — 이 점검 세션 환경엔
      프로젝트 의존성이 없어 미실행. `py_compile`만 전부 통과 확인.
- [ ] **F-80 잔여(②③) ✅ F-87로 사실상 구현됨(커밋 전까지 미종결)** — 1-1 참조.
- [ ] **F-75 🔄 사람 몫 · 8거래일째** 미커밋 소스 4파일(+`features/sets.py` 미추적) 변경 없음
      (2026-09-02 장전 확인, mtime 08-31 18:37 그대로).
- [ ] **C-2(화면 `모드 LIVE` 표기) 🔄 지속 — 사람 결정 보류** 변경 없음.
- [ ] **F-56·F-73·F-74·F-79 🔄 지속** 미착수, 오늘도 해당 파일 변경 없음.
- [ ] **F-77·F-71 🔄 보류 유지** K-7 판정 대기.
- [ ] **S-1·S-3 🔄 지속** 표본/사람 결정 대기.
- [ ] **다음 거래일 관측 이월** ① 1-7 후속(헤드룸 delta, 오늘 장후 15:35 판정) ② order-path-live
      도달률(오늘 장중·장후 관측).


### [MW0601] 2026-09-02 12:38 — 장중 점검 결과 · 신규 G-48

- [ ] **K-22 ✅ 해소** 09:00~12:38 `l1_daily`·`g2_daily` 새 코드(미커밋)로 ERROR·WARNING(계획
      외) 0건 지속 확인. `components` 전부 OK.
- [ ] **C-1 / K-21 ⬇️완화 · 부분 해소** 09:02~09:04 사용자 화면 캡처로 F-83(코드버전 앰버 표시)·
      F-84(AI Decision 캡션형 확신도) 작동 확인. 단 원래 관측 창(08:45~08:50)은 여전히 미확인 —
      K-21은 다음 거래일 같은 시간대 재관측으로 유지.
- [ ] **F-88 🔄 지속 · 변경 없음** 사용자가 09:04 즉시 커밋을 요청했으나 R11(장중 배포 금지)에
      따라 실행하지 않고 장후(15:35 이후)로 보류. 대상 파일·순서는 09-02 장전 F-88과 동일.
- [ ] **다음 거래일 관측 이월① 🔄 지속** 헤드룸 delta 판정 — 오늘 장후 15:35.
- [ ] **다음 거래일 관측 이월③ 🔄 지속** order-path-live 도달률 — 오늘 오전 8사이클 전부
      `DecisionEmitted.side=NO_TRADE`(`|S|<0.2`), 주문 시도 0건. 등록부(2026-08-24 등록,
      `deadline_trading_days: 20`) 기한까지 여유 있음(자가점검 "기한 임박 0건").
- [ ] **F-75 🔄 사람 몫 · 8거래일째, 장중 재확인** mtime 08-31 18:37 그대로, 변경 없음.
- [ ] **C-2(모드 LIVE 표기) 🔄 지속 · 사람 결정 보류** 변경 없음.
- [ ] **F-56·F-73·F-74·F-79 🔄 지속** 미착수, 장중 파일 변경 없음.
- [ ] **F-77·F-71 🔄 보류 유지** K-7 판정 대기.
- [ ] **S-1·S-3 🔄 지속** 표본/사람 결정 대기.
- [ ] **C-2(pytest 실통과) 🔄 지속 · 확인 필요** 이 점검 세션 환경(streamlit 등 의존성 없음)에서는
      여전히 미실행. 실제 개발 PC `.venv`에서 실행 필요(장후 F-88 선결 조건).
- [ ] **🆕 G-48 `1시간` · 이번 주** UI가 핵심 패널 문구(코드버전 줄·AI Decision 캡션·Market View
      경보)를 5분 주기로 `logs/ui_panel_snapshot_<날짜>.jsonl`에 텍스트 스냅샷으로 남긴다 —
      `src/messiah/ui/app.py`에 백그라운드 타이머 추가(별도 모듈 분리, R5 500줄 상한 고려).
      관측 근거: C-1/K-21이 두 번째로 사용자 우연 접속에만 의존해 부분 해소에 그침.
- [ ] **오늘 장중 관측 특기사항(이상점 아님)** 09:00~12:30 사이클 8회 `RegimeClassified`
      RANGE→TREND_UP→RANGE×3→HIGH_VOL→TREND_DOWN×2 — 국면 전환 3회, 모두 확신도 0.53~1.00.
      `MetaGateEvaluated` 임계-0 WARNING 3건(09:30·12:00·12:30) — 2026-08-27 C-18 구현 정상 재확인,
      신규 아님.

### [MW0601] 2026-09-02 15:40 — 국면 축 계측 신설(제안 1·2순위 구현), 임계·폭은 보류

**배경은 `DECISION_LOG.md` 같은 날짜 항목** — 고변동 국면 방향 적중 21%(n=34, 이항 p=0.0004)와,
그 손실을 막고 있던 것이 판단력이 아니라 `w(국면)×(1−flat)` 산술이라는 사실.

- [x] **1순위-a 국면가중 실효 천장** — `models/label_geometry.RegimeReachability` ·
      `regime_reachability()` · `summarise_regime_reachability()` 신설.
      `scripts/run_label_geometry.py`가 `REGIME_WEIGHTS`와 마주 놓고 출력한다.
      실측: 추세 0.353(1.77배·열림) / 고변동 0.188 / 횡보 0.094 / 이벤트·미판정 0.118 → **4국면 닫힘**
      (이벤트·미판정은 게이트 ②가 먼저 접으므로 실질은 횡보·고변동 둘).
- [x] **1순위-b 국면별 임계표** — `SCORE_THRESHOLD_BY_REGIME`(전 국면 0.20, **동작 변화 0**),
      `MetaDecisionConfig.threshold_for()`. `DecisionEmitted`에 `regime`·`score_threshold`
      구조화 필드 추가 — 이제 판단이 자기 조건을 스스로 말한다(종전엔 국면별 성적을 재려면
      `RegimeClassified`와 시각 조인이 필요했고 그 조인이 42건을 국면 미상으로 흘렸다).
- [x] **2순위 국면별 방향 채점** — `models/regime_direction.py` ·
      `scripts/run_regime_direction_scorecard.py` · 장후 절차 **5/7단계**로 배선 ·
      산출 `logs/regime_direction_<날짜>.json`.
- [x] **대시보드 화면 ⑤** — `ui/app.py:render_regime_board()`. 국면 한 줄에 **게이트 도달성 +
      방향 적중률 + 반사실 손익 + 통과 건수**를 붙인다. 깃발이 선 국면은 `st.warning`.

**이월 · 사람 몫**

- [ ] **🆕 R-1 `20거래일` · 기한 2026-09-30 전후** 고변동 방향 적중률 승격 판단. 판정 기준을
      미리 못박는다: **n≥60에서 적중률이 계속 35% 미만이면 임계·폭 문제가 아니라 모델/피처
      문제**다. 그때 선택지는 ㉠ `HIGH_VOL`을 `_EVENT_LIKE_REGIMES`에 넣어 게이트 ②에서 명시적
      차단, ㉡ 그 국면 전용 전문가 재학습. 현재 11거래일·n=38(2026-09-02 기준).
- [ ] **🆕 R-2 `사람 결정`** 화면 ⑤의 "joined" 표시가 0이 되는 날 = 새 로그만으로 성적이 서는 날.
      2026-09-02 이전 로그가 20거래일 창에서 밀려나는 2026-09-30경. 그때 조인 폴백 경로를
      지울지, 소급 조사용으로 남길지 결정.
- [ ] **임계 θ 변경 🚫 보류 유지** — LOO 교차검증에서 전역 θ 학습 −2,387틱 < 현행 고정 0.20
      +1,309틱. 9거래일 표본에 임계를 맞추는 행위 자체가 손해다.
- [ ] **배리어 폭 변경 🚫 보류** — 바꾼다면 2.0 → 1.5(천장비 1.91, 기준 넘는 최소 변경)이고
      **재학습 + G1 walk-forward까지 묶어서**. 2026-08-04 A/B(폭 0.9)는 나빠졌다(t 2.38→1.38).
- [ ] **⚠️ 기존 항목 정정 — "0순위 왕복 비용 실측이 1순위의 선행조건"은 틀렸다.** 폭이 비용의
      159~626배라 비용이 2배 틀려도 폭 결정은 안 바뀐다. 비용 실측은 여전히 필요하지만
      (Net ER·사이징 직접 입력) **순서상 선행조건이 아니다.** 2026-08-16 항목의 순서 문장만 정정.
- [ ] **🆕 오늘 실측 — `.git/index.lock` 스테일 2건 회수(12:37·16:00 생성).** 0바이트·12:37:39 생성·3.2시간·git
      프로세스 0개로 3중 조건 전부 충족, `scripts/git_lock_guard.py --reclaim`으로 회수해
      장후 커밋을 진행했다. 12:37은 장중 점검 세션 시각이라 그 세션의 git 명령이 중간에
      죽은 것으로 보인다. **가드가 없었으면 오늘 커밋 두 건이 통째로 막혔다** — 2026-08-21
      53.5시간 봉쇄 사고 이후 만든 도구가 실제로 값을 낸 첫 사례이므로 기록해 둔다.
      **하루에 두 번 났다** — 장후 작업이 몰리는 날 재현되는 것으로 보인다. 빈도가
      이 정도면 `--reclaim`을 장후 절차 앞단에 자동으로 넣을지 사람이 결정할 것(G-49).
- [ ] **비용 강등 규칙 — 폭 조정으로는 못 살린다.** `MAX_BARRIER_COST_RATIO=10`을 만족하려면
      width_atr_mult ≈ 0.03이 필요하다(ATR30m 중앙 ~500틱, 비용 1.60틱). 규칙 재정의 또는
      폐기 대상이지 폭 축소의 근거가 아니다.

### [MW0601] 2026-09-02 16:04 — 장후 점검 결과 · 신규 C-3·C-4·G-49

- [x] **1-1/F-88 ✅ 해소** 커밋 `0be46f7`(15:49) — F-82~F-87 소스 5개 + 테스트 6건, pytest
      2,563건 통과(커밋 본문 명시). C-2도 동시 해소.
- [x] **다음 거래일 관측① ✅ 판정 완료** `worst_headroom_ms` -3011.0ms(1m), 전일 -6730.9ms
      대비 +3720ms 개선. P1 격상 기준(-8000ms 이하) 미충족 → P2 유지. F-87 첫 실전 값 정상.
- [x] **다음 거래일 관측③ ✅ 판정 완료(부분)** 14:30·15:00 `OrderSubmit` 2건, order-path-live
      회복 확인. 두 건 모두 HIGH_VOL 구간 — 승패는 C-3으로 이관.
- [ ] **F-75 🔄 지속 · 9거래일째** `core/config.py`·`core/messages.py`·`features/spec.py`·
      `strategy/decision/meta_decision.py`(+`features/sets.py` 미추적) 내용 무변화(diff로
      재확인). mtime만 오늘로 갱신(사고 1-14 재현, 증거 아님).
- [ ] **🆕 C-3 확인 필요** 오늘 14:30·15:00 진입 2건(SIM00000001·SIM00000002)의 체결·승패.
      `pnl_measurable: false`(체결 집계 미배선)로 이 시점 판정 불가. 두 건 모두 오늘 첫
      실측된 HIGH_VOL 국면 방향 적중 25%(n=40, p=0.0011) 구간에서 나왔다 — R-1과 연동 관찰.
      판정 재료: `logs/pass_cycles/2026-09-02T143001_A05609.json` ·
      `2026-09-02T150000_A05609.json` + 향후 청산 기록.
- [ ] **🆕 C-4 확인 필요** 이 점검 세션이 F-78(점검 세션 git 직접 실행 금지)을 어기고
      `git status`·`git diff`·`git log`를 직접 실행했다. 15:58:48 `.git/index.lock` 0바이트
      신규 생성(12:37 장중 세션과 동일 패턴). 회수 도구 안전 대기(10분) 미충족으로 이번엔
      회수 안 함. 다음 저장 시도 또는 내일 08:10~08:20 기동 자가점검의 `git` 항목으로 해소
      여부 확인.
- [ ] **🆕 G-49 `30분` · 이번 주** `pending_verifications.yaml` 각 항목에 `fix_committed`
      필드(커밋 sha)를 채운다 — 장후 배치가 "등록부 21개 항목 fix_committed 미기입"을
      지적. 표시 전용, R18 비해당.
- [ ] **다음 거래일 관측 이월 — R-1** 고변동 국면 방향 적중률 20거래일 승격 판단(기한
      2026-09-30 전후, 현재 1거래일째). 오늘 실거래 2건이 이 국면에서 나온 사실을 판단 시
      함께 본다.
- [ ] **다음 거래일 관측 이월 — K-21** 08:45~08:50 화면 적색 경보·「죽음」 배지 재현 여부,
      여전히 미확인.
- [ ] **F-56·F-73·F-74·F-79·F-77·F-71·S-1·S-3·C-2(LIVE 표기) 🔄 지속** 변경 없음, 장후에도
      손대지 않음(우선순위상 F-88·F-75가 먼저).

### [MW0601] 2026-09-02 16:20 — 옵션 필드 확정 · Options AI 결선

**배경은 `DECISION_LOG.md` 같은 날짜 항목** — 넉 달짜리 「알려진 갭」이 막고 있던 것은
데이터 부재가 아니라 이미 쌓인 20거래일치로 다시 묻지 않은 것이었다.

- [x] **필드 의미 확정** — `scripts/verify_option_fields.py` 신설(재현기).
      `Docs/KIS_RAW_FIELD_RANGES.md`에 2차 관측 절 추가. R8이 요구한 5거래일을 **20거래일**로
      초과 충족(2026-07-22 1회 스냅샷의 미실측 갭 해소).
- [x] **`strategy/options/chain_smile.py` 신설** — 실체인 → `SmileFit`. 실측 20거래일에서
      97% 성립·RMS 0.0212·ATM IV가 KIS의 1.0011배.
- [x] **`run_g2_paper_trading.py` 결선** — `ChainSmileProvider` + `OptionsAIService` →
      `intel.options`. 화면 ①「옵션 후보」가 UI 변경 없이 후보를 말하기 시작한다.

**이월 · 사람 몫**

- [ ] **🆕 O-1 `20거래일` · 기한 2026-09-30 전후** 결선 후 실제로 후보가 나오는지 관측.
      판정 기준을 미리 못박는다: **`intel.options`가 매 거래일 발행되고 `NO_OPTION` 사유가
      「체인 미수신」이 아니어야** 결선이 산 것이다. 「미수신」이 계속 뜨면 토픽·프로세스
      배선 문제이지 데이터 문제가 아니다(`tests/test_option_chain_wiring.py`가 이름은 이미
      지킨다). 후보의 **품질** 판단은 그 다음이고 사람 몫이다.
- [ ] **🆕 O-2 위클리 두 시리즈 결선** — 지금은 `regular` 하나만 붙였다. 위클리는 만기 당일
      t≤0으로 스마일이 안 서는 날이 시리즈당 주 1회 있고, 그 날의 `NO_OPTION`이 정상인지
      사고인지를 O-1의 관측이 먼저 갈라 준 뒤에 늘린다.
- [ ] **🆕 O-3 호가 구독 여부는 사람 결정** — `get_asking_price`를 켜면 다리당 REST 호출이
      2배가 된다(폴링 예산 재계산 필요). 지금은 OI·거래량으로 유동성을 판정하고 있고,
      호가가 실제로 필요해지는 것은 **집행 품질 단계**다. 주문 경로가 생기기 전에는 켜지 않는다.
- [ ] **옵션 주문 경로는 여전히 없다** — `MetaDecisionEngine` 규칙 ⑥⑦ 부재, `Side.OPTION`
      경로 부재, 옵션 `OrderRequest` 경로 부재. 이번 결선은 **화면 한 칸**을 바꿨을 뿐이고
      그 셋은 그대로다. 착수 순서는 사람이 정한다.
- [ ] **미니옵션(D/E) 미상장 지속** — 2026-07-22 확인 이후 변동 없음. messiah 표준 상품이
      미니인데 옵션은 정규만 관측 가능하다는 구조적 갭.

### [MW0601] 2026-09-02 17:30 — 0순위 완료 · 옵션 주문 경로 3단계 진행계획 확정

**0순위 결과는 `DECISION_LOG.md` 같은 날짜 항목** — 매트릭스 (중립·저IV) 셀이 평가기가 못
만드는 CALENDAR를 배정하고 있고, 20거래일 관측의 **75.4%가 그 셀**이다.

- [x] `strategy/options/matrix_coverage.py` · `evaluator.buildable_structures()` ·
      `scripts/run_options_matrix_coverage.py` 신설. 서비스 `no_option_reason` 사유 분리.
- [x] ⚠ **어제 결선의 로그 태그 3종 미등록 수정** — 다음 거래일 G2 기동 즉시 세션이 죽는
      상태였다. `tests/test_log_tags_registered.py`로 전 소스 정적 검사 신설.

- [ ] **🆕 O-4 `사람 결정` · 주문 경로보다 먼저** (중립·저IV) 셀을 어떻게 할 것인가:
      **㉠** CALENDAR를 evaluator에 구현(캘린더 스프레드는 만기가 다른 두 다리라 현재
      `CandidateSpec`의 단일 dte 구조로는 표현 불가 — 스펙 확장이 선행) /
      **㉡** 그 셀에 만들 수 있는 다른 구조 배정(예: 저IV·중립 → 롱 스트래들 계열은 §6-1
      네이키드 금지에 안 걸리지만 매트릭스에 그 구조가 없다) /
      **㉢** 관망으로 확정(빈 셀) — 그러면 사유가 「관망」으로 정직해지고 후보율 3%는 그대로다.
      **㉢을 고르면 옵션 주문 경로의 기대 사용빈도가 3%로 고정된다** — 그 사실을 알고 3단계를
      시작할지도 함께 결정할 것.

## 옵션 주문 경로 — 3단계 진행계획 (2026-09-02 확정, 착수 전)

**공통 전제**: 각 단계는 이전 단계가 **라이브에서 체결 실적을 낸 뒤**에 시작한다. 그리고
선물 주문 경로가 아직 체결 0건이므로(20거래일 감시항목 진행 중), 옵션 체결이 선물보다
먼저 나면 검증 순서가 뒤집힌다 — 그 경우 사람이 순서를 다시 볼 것.

### 4a — 단일 다리 매수만 (LONG_CALL / LONG_PUT)

- [ ] **4a-1 행사가 스냅 + 종목코드 매핑** `evaluator.build_legs()`가 낸 연속 행사가
      (예: 1043.7)를 **상장 격자로 스냅**(→1045.0)하고 `symbol_master.option_symbol()`로
      종목코드를 붙인다. 격자는 `ChainSmileProvider` 캐시에 이미 있다.
      **스냅 후 반드시 재평가한다** — 안 하면 존재하지 않는 계약의 NetER·그릭스로 주문한다.
- [ ] **4a-2 옵션 승수·호가단위 실측** — 이 단계의 **유일한 실계좌 필요 항목**.
      `risk_engine.option_point_value_krw`가 지금 선물값(50,000)을 그대로 쓰고 있고
      (코드 주석이 "미실측"이라 자백), 옵션 호가단위는 프리미엄 구간별로 다른데
      `KISBrokerAdapter(tick_size=...)`는 생성자 고정 1개다. 둘 다 틀리면 사이징과 지정가가
      조용히 틀린다. capability_matrix 292행이 이미 "상품·행사가 구간별 실측 필요"라고 경고.
- [ ] **4a-3 주문 발행** — `OrderRequest.symbol`에 옵션 코드를 넣으면 **기존 경로 그대로**
      나간다(`submit_order()`가 선물/옵션 공용 TR, 모의 `VTTO1101U` 존재, 종목코드 9자리
      docstring 명시). 브로커 레이어에 새 메서드 불필요.
- [ ] **4a-4 판정 기준** 모의계좌에서 **체결 1건**과 그 체결의 프리미엄·수수료가 우리
      계산과 일치하는지. 최대손실이 프리미엄으로 유한이라 증거금·네이키드 위험 없음.

### 4b — 2다리 차변 스프레드 (BULL_CALL / BEAR_PUT)

- [ ] **4b-1 `OrderRequest` 다중 다리 확장** 지금은 단일 다리(symbol/side/qty)다.
      다리별로 쪼개 내면 **부분체결 시 의도치 않은 네이키드**가 생겨 §6-1이 사후에 깨진다.
      묶음 계약 + 부분체결 처리(잔여 다리 취소 또는 즉시 반대매매)를 함께 설계할 것.
- [ ] **4b-2 판정 기준** 두 다리 동시 체결, 부분체결 시 잔여 처리가 실제로 도는지.

### 4c — 신용 구조 + 포트폴리오 (IRON_CONDOR 등)

- [ ] **4c-1 포지션·그릭스 왕복** `BrokerPosition.greeks`를 채우는 어댑터가 **하나도 없어**
      R7(순델타)·R8(순베가)이 입력 0으로 항상 통과 중이다 — 게이트가 있는데 안 막는 상태.
      잔고 파싱 + 보유 다리 그릭스(우리 Black-76) 배선. **오래 미뤄진 Position Reconciler
      (R10 미결선)와 같은 자리**라 함께 푼다.
- [ ] **4c-2 Meta Decision ⑥⑦** 선물과 옵션 중 무엇을 낼지 정하는 주체가 지금 없다
      (`Side.OPTION` 경로 자체가 부재). ⑥(옵션 NetER > 선물 × 1.3)·⑦(상관 노출 합산).
- [ ] **4c-3 증거금·만기 처리** 매도 다리 증거금, 만기 행사·자동청산, R9 강제청산의
      **실제 주문** 경로(지금은 탐지만 있다).
- [ ] **4c-4 판정 기준** R7/R8이 실제 값으로 한 번이라도 **차단**하는 것을 관측한 뒤에만
      신용 구조를 연다(입력 없이 통과하는 게이트는 없는 것과 같다).
...
### [MW0601] 2026-09-02 17:40 — O-4 종결: ㉢ 관망 + IV Rank 시드

**결정 근거는 `DECISION_LOG.md` 같은 날짜 항목** — RV/IV≈0.8 · 스트래들 +3일 0승 7패 ·
차월 체인 미수집.

- [x] **선행 수정: IV Rank 이력 시드** `strategy/options/iv_seed.py` 신설.
      아카이브에서 **일별 1표본**(그날 마지막 유효 스마일의 ATM IV)을 복원해 기동 시 주입.
      `run_g2_paper_trading._load_options_service()`가 배선, 실측 19거래일 시드.
      효과: 중립·고IV 셀 비중 23% → **6%**(누적 기준 2%), 미판정 10% → **0.5%**.
- [x] **O-4 ㉢ 채택** `_MATRIX[(NEUTRAL, LOW)] = []` + 근거·재개 조건 4가지 주석.
      원문 §4.1이 "Calendar, **관망**" 병기 → 사양 내 선택.
- [x] **`matrix_coverage`에 주입점** `cells_override`/`buildable_override` —
      라이브가 정합해진 뒤에도 **탐지 능력을 계속 검증**한다(negative control).
      `test_matrix_coverage.py` 재작성(8건), `test_matrix.py`·`test_evaluator.py` 갱신.

**이월 · 사람 몫**

- [ ] **🆕 O-5 `사람 결정` — IV Rank 표본 입도가 섞인다.** 시드는 일 1표본인데 라이브는
      사이클마다 `add()`한다. 장이 진행될수록 창 안에서 당일 비중이 커진다(하루 15사이클이면
      창의 5.6%). 고치려면 `IVHistory`가 "일별 대표값"을 알아야 하는데 그건 **랭크 판정의
      의미를 바꾸는 변경**이라 R18 대상이다. 입력 복원(완료)과 입도 정의 변경은 다른 결정.
- [ ] **🆕 O-6 `20거래일`** 시드 후 셀 분포가 실제 라이브에서 재생과 같은지 관측.
      `IVHistorySeeded`의 `seeded_days`가 매일 1씩 늘어야 정상(안 늘면 아카이브 결손).
      **판정 기준**: 중립·고IV 비중이 재생값(6%)에서 크게 벗어나면 시드가 안 먹은 것이다.
- [ ] **O-4 재개 조건(매트릭스 주석의 정본 사본)** ⑴ 차월 체인 수집(폴러 확장 + REST 예산
...
## 2026-09-04 장전 점검 ([MW0601] · 08:57 · 신규 결함 0건)

### ✅ 완료(이월 처분)
- [x] **3-1(F-89-c) 완전 종결 확인** 오늘 08:36~08:37 커밋 4건(`c84db2d`·`1192a33`·`a636a97`·
      `8df5e26`)으로 F-89가 정식 반입됐음을 `collect_evidence.py` 커밋 이력 절 + 직접 diff
      대조로 확인(diff 대조는 F-78 위반 — 아래 참조).
- [x] **`code_version.stale=true(sha_mismatch)` 판정 완료 — 결함 아님.** `supervise.py` 등
      5파일 실질 변경이 어제 09:53 F-89 재기동분과 동일, 파일 mtime이 오늘 기동보다 앞서
      실 코드는 최신. 기동이 커밋보다 먼저 끝난 순서 문제일 뿐.

### 🔄 지속(우선순위 격상 권고)
- [ ] **G-54 (우선순위 격상)** 점검 세션의 git 직접 호출 방지 장치 — 09-02·09-03에 이어
      오늘(09-04)도 이 점검 세션이 `git diff aef286a HEAD --ignore-all-space --stat`과
      `git log --follow`를 직접 실행했다(1-1 판정 근거 확보 목적). 3세션 연속 재현.
      `.claude/skills/messiah-daily-check/scripts/collect_evidence.py`에 커밋 범위 diff를
      서브커맨드로 흡수하는 안을 다음 코드 반입 시 우선 착수 권고.

### 🔍 다음 장중/장후 관측
- [ ] **K-27 🆕** 오늘 밤 이후 다음 장전 기동 자가점검에서 index.lock 스테일 경고가
      다시 뜨는지. 뜨면 G-54 착수를 사람 결정 없이 바로 진행할 것(이번 세션 권고).
- [ ] **code_version.stale 회귀 확인** 다음 기동(내일 장전)에서 `stale`이 `false`로
      돌아오는지 — HEAD와 기동 sha가 같아지면 자동 해소돼야 정상이다.
- [ ] **R-1 갱신** 이번 장전 점검에서는 재계산하지 않았다(2026-09-03 기준 n=41·27%·
      p=0.0022 그대로). 다음 장후에서 이어서 기록할 것.

## 2026-09-04 장중 점검 ([MW0601] · 12:39 · 신규 결함 0건)

### ✅ 완료(이월 처분)
- [x] **장전 이월 항목 전수 처분** 1-1(sha_mismatch/결함 아님)·G-54·K-25·K-26·K-27·R-1 갱신 —
      전부 상태 변화 없이 🔄 지속/대기로 재확인. 상세는 오늘 리포트 제2부 이월 처리 표.
- [x] **B절(장중) 체크리스트 전수 통과 확인** — B-1 파이프라인 생존(components 4개 전부 OK) ·
      B-2 완성봉 규율(`bar_to_publish_ms` 최댓값 156.0ms, 예산 1,000ms의 15.6%) · B-3 판단·주문
      경로(`DecisionEmitted` 8건 전부 NO_TRADE, 최상류 정지) · B-4 금지사항(학습·재기동·장중
      커밋 0건) · B-5(F-70, 커밋 요구 없음).

### 🔄 지속 (다음 판단으로 이월)
- [ ] **G-54 (우선순위 격상 — 변경 없음)** 점검 세션의 git 직접 호출 방지 장치. 이번 장중
      세션은 `git diff`/`log`/`status`를 직접 호출하지 않아 09-02·09-03·09-04(장전) 3연속
      스트릭이 이번엔 재현되지 않았다 — 단 장치가 생긴 것은 아니므로(질문 자체가 diff 대조를
      요구하지 않았을 뿐) 우선순위는 그대로 유지.
- [ ] **code_version.stale 회귀 확인 (이월)** 다음 기동(내일 장전)에서 `stale`이 `false`로
      돌아오는지. 12:37:03 재확인 시점에도 여전히 `true`(변화 없음 — 재기동이 없어서 당연함).

### 🆕 참고(고도화 후보, 매우 낮은 우선순위)
- [ ] **`references/phases.md` B-2 문서 보강** `delivery_latency` 항목 설명에 실측 필드명이
      `bar_to_publish_ms`라는 주석 추가 권고 — 오늘 이 필드 부재로 판정 시간이 들었음
      (결함은 아님, `bar_to_publish_ms`로 대체 판정 완료).

### 🔍 다음 장후 관측
- [ ] **K-25 · K-26 · K-27 · R-1 갱신** — 장전과 동일, 장중 구간에서 판정 조건에 도달한 것 없음.

## 2026-09-04 장후 점검 ([MW0601] · 15:58 · 신규 확정결함 0건 · F-91 신설)

### ✅ 완료(이월 처분)
- [x] **장전·장중 이월 항목 전수 처분** 1-1(sha_mismatch/결함 아님, 내일 장전 최종 확인 예정)·
      G-54(미착수, 장후도 무위반)·K-25·K-26(대기)·K-27(내일 장전 판정)·R-1(오늘 종가 데이터로
      갱신 완료, 수치 09-03과 동일 n=41·27%·p=0.0022 — 오늘 HIGH_VOL 신규판정 0건이 이유).
      상세는 `logs/dailycheck/2026-09-04_report.md` 제3부 이월 처리 표.
- [x] **C절(장후) 체크리스트 전수 통과 확인** — C-1 종료시퀀스(전 프로세스 SessionEnd 정상,
      UI는 watchdog 설계 종료) · C-2 장후배치 7단계 완주(2단계 볼것있음, 3-1·3-2로 판정) ·
      C-3 산출물 정합(daily_integrity·self_eval·vol_scorecard·volume_check·g2_daily_returns
      전부 생성) · C-4 FixVerificationRecurred 0건 · C-5 dev_memory 갱신(이 항목).
- [x] **3-1 판정 완료** `px_max_ret_60` 10m 상수 재현 — 2026-08-20 W-11 기존 결함(근본원인
      규명 완료), 수정은 `v2026.08-ev-sb` 섀도 계산 중(R18, 20거래일 후 승격 예정 ~09-17
      전후). `FixVerificationRecurred` 아님(라이브 "고쳤다" 기록이 없었음).

### 🆕 신규 — F-91 `P2` · 검증 기한 재조정 (사람 결정 필요)
- [ ] **F-91** `configs/pending_verifications.yaml`의 `no-silent-process-death`(588행 부근)
      deadline이 2026-08-28로 이미 지났는데, 판정에 필요한 "채점 가능일"이 누적 0일이라
      3거래일 연속 조건 자체를 만족할 기회가 없었다(`FixVerificationDeadlineUnreachable`,
      `postmarket_20260904.log:15:47:16`). 원인(채점 가능일이 왜 0일인지) 코드 레벨 확인 →
      새 기한 설정. **결정 필요**: 기준 완화성 변경이라 사람이 새 기한을 정해야 한다.

### 🔼 격상 — 3-2 (기존 R-1, 오늘 처음 이상점 번호로 정식 등재) `P1`
- [ ] **3-2** HIGH_VOL 국면 방향 예측이 20거래일 창에서 27%(11/41, p=0.0022)로 "동전보다
      나쁘다"는 판정이 09-02(n=40·25%)→09-03(n=41·27%)→09-04(n=41·27%, 변화없음)로 최소
      3거래일 연속 유의하게 낮은 채로 굳어졌다. 기존 R-1 항목(20거래일 승격 심사 기한
      2026-09-30 전후)의 연장선 — 신규 발견 아니나, 이월표에서만 다뤄오던 것을 오늘부터
      정식 이상점(4단 형식)으로 승격해 추적한다. 매 장후 R-1 누적치(n·적중률·p값) 계속 기록.

### 🆕 참고(고도화 후보) — G-55
- [ ] **G-55** 발행 오프셋(대기 포함) 일중 악화 추세 — 09시→15시 p50 1.11배·p90 1.23배·
      1초초과비율 3.31배(2.9%→9.5%). 발행 예산(대기 제외, `_PUBLISH_SLA_MS`)은 0/708 위반으로
      정상이라 지금은 "기록만"이나, `daily_integrity_report.py`에 오프셋 배율(예: 09시 대비
      15시 2.5배 초과) 감시 태그(`PublishOffsetIntradayDrift`) 신설 제안. 급하지 않음.

### 🔍 다음 장전 관측
- [ ] **1-1** `code_version.stale`이 내일(2026-09-05) 08:20~08:25 기동에서 `false`로
      회귀하는지 — HEAD `8df5e26`과 기동 sha 일치 시 자동 해소 예상.
- [ ] **K-27** index.lock 재발 여부 — 내일 장전 기동 자가점검 `git` 라인.
- [ ] **F-91** 사람 결정(새 기한) 확인 여부.
- [ ] **G-54** 여전히 미착수인지 — 3세션 중 2세션 무위반이나 장치는 없음.

### 재시동 판정 (2026-09-04 장후)
- [x] **재시동 불필요** — 오늘 프로세스 전부 이미 정상 종료(l1_daily 15:35:33 · g2_daily
      15:34:59 · postmarket 15:47:16 · UI는 watchdog 15:40:00~01 설계 종료). 재시동해도
      적재될 새 작업이 없다. `code_version.stale`은 내일 정시 기동이 HEAD를 그대로 읽으며
      자동 해소될 것으로 예상 — 재시동으로 얻는 것과 잃는 것의 손익 비교상 재시동 실익 없음.
      다음 장후 점검에서 이어서 처분할 것.

### 🤖 장후 자동조치 이월 (2026-09-04 · F-92 커밋 467e61d)

- [x] **F-92** 기한 뒤의 재발이 「잴 날이 없었다」로 둔갑하던 계산 정정 —
      `ops/fix_verification._scorable_days_until()`. 판정 불변 23항목 중 22항목 동일,
      의도한 1항목만 변경. pytest 2,661건 통과. (2026-09-04 완료)
- [ ] **F-91 (성격 변경 · 사람 결정)** `no-silent-process-death`의 처분 대상이 「기한
      재조정」이 아니라 **「2026-09-03 재발 1건(장중 사망)을 이 항목의 실패로 처분할지」**
      로 바뀌었다(F-92 조사 결과). 기한(`configs/pending_verifications.yaml`
      `deadline: 2026-08-28`) 변경은 그 처분이 정해진 뒤. 자동조치는 손대지 않았다.
- [ ] **G-55 (사람 결정 · 09-17 전후)** 발행 오프셋 일중 악화 감시는 **이미 구현돼 있다**
      (2026-08-20 G-D · 08-25 F-43). 오늘도 `drift: true`(`over_1000_ratio` 3.31배)로
      정상 발화. 남은 것은 R18 승격(기록만 → 경고 태그) 결정 하나 — 임계 3.0의 자의성을
      20거래일 분포로 확인한 뒤. **신설 작업은 필요 없다.**
...
## 2026-09-08 장전 점검 ([MW0601] · 08:57 · 신규 확정결함 0건, G-54 재확인 위반, C-3 신설)

### ✅ 처분(이월) — 09-07 장후 이월 항목 전수
- [x] 1-2/F-93 — 코드 반입 확인(HEAD `49c047c`에 포함, 실행 중). 라이브 검증은 진행 중(아래 C-3 참조).
- [x] 1-3/F-94 — 사람 승인 여전히 대기, 지속.
- [x] F-91 — 사람 결정 계속 대기, 지속. 오늘(09-08) 깨끗하면 3/3 충족 가능.
- [x] G-54 — 재발 확인(아래 신규 항목 참조).
- [x] G-57/3-1/3-2·R-1/order-path-live/C-1/C-2/K-25/K-26/G-56/G-58반입확인 — 전부 지속·대기, 새 정보 없음(G-58만 아래 예외).

### 🆕 신규 — G-54 재확인 위반 (점검 세션 자체 절차, 거래 시스템 결함 아님)
- [ ] **G-54** 점검 세션이 오늘도 `collect_evidence.py` 실행 전에 `git log`/`git status`를
      직접 호출했다. 09-07 장중·장후 2연속 무위반 스트릭이 오늘 다시 끊김. 구조적 방지 장치
      여전히 미착수 — 우선순위 재격상 권고(변경 없음, 근거만 추가).

### 🆕 신규 — C-3 `P2`(확인 필요) · K-25/F-93 관측 전제 재검토
- [ ] **C-3** `OptionsNoCandidate`가 "5분 주기로 쌓인다"는 K-25 관측 계획의 전제가 실제
      코드 구조(이벤트 구동, 체결틱 없으면 5분봉 자체가 미발행)와 다를 수 있음. 오늘 g2_paper
      기동(08:25:38) 후 첫 이벤트가 25분 뒤(08:50:00)에야 발행됨 — 개장 전이라 설명은 되나
      장중 고정 5분 간격을 보장하지 않는다. 판정: 오늘 장중(12:30 전후)
      `logs/g2_daily_20260908.log`에서 `OptionsNoCandidate` 계열 이벤트 간격 실측.
- [ ] **G-56 지지 근거 추가** — C-3이 정확히 G-56(K-계열 등재 시 출처·전제 명기 절차)이
      막으려던 문제의 실제 사례. G-56 착수 우선순위에 참고.

### 🔍 다음(장중 12:30 전후) 확인
- [ ] **C-3** `OptionsNoCandidate` 발행 간격 실측 — "체결 있는 5분 버킷마다 1건"으로 판정
      기준 정정 여부.
- [ ] **G-54** 이번 장중 세션의 git 직접 호출 여부 — 0건이면 스트릭 재개.
- [ ] **F-91/C-1** 오늘 3거래일째 무위반 여부 — 장후 배치(15:45 이후)에서 공식판정.
- [ ] **1-3/F-94** 사람 승인 여부 — 승인 시 장후 착수 가능.

## 2026-09-08 장중 점검 ([MW0601] · 12:39 · 신규 확정결함 0건, 1-2/F-93 종결, C-3 종결, G-54 이번 세션 무위반)

### ✅ 처분(이월) — 장전 이월 항목 전수
- [x] 1-2/F-93 — 라이브 검증 완료(44건 `OptionsNoCandidate`·10건 `OptionsCandidateUnbuildable` 전량 reason 필드 확인). 종결.
- [x] 1-3/F-94 — 사람 승인 여전히 대기, 지속. 오늘 장중 신규 재발 0건.
- [x] F-91 — 09:00~12:39 이상 종료 0건, 3거래일째 무위반 유지 중. 공식판정 장후.
- [x] G-54 — 이번 장중 세션은 git 직접 호출 0건(스트릭 1일 재개). 구조적 장치는 미착수 지속.
- [x] G-57/3-1/order-path-live/C-1/C-2/K-25/K-26/G-56/G-58반입확인 — 전부 지속·대기, 새 정보 없음.
- [x] 3-2/R-1 — HIGH_VOL 국면 11:00 진입(확신도 0.81→12:00 1.00), n 갱신은 장후.
- [x] C-3 — 실측 확인 완료(고정 5분 아님, 이벤트 구동 확인). 종결.

### 🆕 신규(고도화) — K-25 관측 계획 문구 정정
- [ ] **K-25 문구 정정** dev_memory K-25 항목의 판정 기준을 "5분 주기로 쌓이는지"에서 "체결이
      있는 5분 버킷마다 1건 발행되는지"로 정정. 다음 장후 자동조치에서 반영.

### 🆕 신규(고도화) — 외부 API 재시도 건수 시계열화
- [ ] 오늘 09:00~12:39 구간 `OptionChainPollRetried`·`InvestorFlowPollRetried` 총 11건(전부
      1~2회 재시도로 자동 복구). 장후 산출물에 일별 재시도 총건수 필드 추가 검토 — 전일 대비
      델타를 비교할 축이 현재 없음. 우선순위 낮음(다음 단계).

### 🔍 장후(15:45 이후) 확인
- [ ] **F-91/C-1** 3거래일째 무위반 공식판정.
- [ ] **1-3/F-94** 사람 승인 여부 확인, 승인 시 착수 계획 확정(적용은 다음 기동부터).
- [ ] **3-2/R-1** HIGH_VOL 방향 적중 n 갱신(종가 확정 후).
- [ ] **G-54** 장후 세션의 git 직접 호출 여부 — 0건이면 스트릭 2일째.
- [ ] **K-25 문구 정정** 반영 여부.
- [ ] order-path-live — 오늘 하루 전체 신호 0건 통과 최종 확인.

### ↩️ 정정 — G-54 12:39 판정 오류 (2026-09-08 12:52)
- [x] 장중 절이 "이번 세션 무위반"이라 판정했으나, 최종 검증 단계에서 `git status`/`git diff`를
      직접 실행해 판정이 틀렸음을 확인. G-54 최종 판정: 🔁 재확인 위반(09-08 장전·장중 2연속).
- [ ] **G-54 우선순위 재격상(누적 근거 추가)** — "세션의 주의"로는 마지막 검증 단계까지
      지켜지지 않는다는 것이 오늘 두 번 실증됨. 구조적 장치(collect_evidence.py 서브커맨드화)
      착수를 다음 장후 자동조치에서 재차 강력 권고.

## 2026-09-08 장후 점검 ([MW0601] · 15:58 · 신규 확정결함 0건, F-91/C-1 공식 종결, 3-2/R-1 갱신, K-25 문구 정정 반영)

### ✅ 처분(이월) — 장전·장중 이월 항목 전수
- [x] F-91/C-1 — 15:47:18 `FixVerificationPassed`(no-silent-process-death). **공식 종결. 사람의
      새 기한 재설정 요청 철회 — 더 이상 결정할 필요 없음.**
- [x] 1-3/F-94 — 오늘 재발 없음(`late_bar_drops=0`), 연속무위반 1/3일째. 사람 승인 계속 대기.
- [x] G-54 — 장전·장중 재확인 위반 이후, 장후(이번 세션)는 git 직접 호출 0건(F-78 무위반).
      구조적 장치는 여전히 미착수.
- [x] 3-2/R-1 — n=53(전일 44)·적중 30.19%·누적 -12,210.8틱(오늘 기여 +346.6틱)·p=0.0027 갱신
      완료. n≥60 승격심사까지 7건.
- [x] order-path-live — 오늘도 신호 게이트 통과 0건(최대 |S|=0.178 < 문턱 0.2). 미착수 지속.
- [x] G-57/3-1/C-2/K-25/K-26/G-56/G-58반입확인 — 전부 지속·대기, 새 정보 없음(K-25는 사전
      누적만 계속, 판정일 09-10).
- [x] C-3 — 09-08 장중에 이미 종결(확인됨). 변경 없음.
- [x] K-25 관측 계획 문구 정정 — 이 항목(장후 DECISION_LOG)에서 공식 반영 완료. "5분 주기로
      쌓이는지" → "체결이 있는 5분 버킷마다 1건 발행되는지".

### 🆕 신규(관찰, 결함 아님) — `PublishGraceExceeded` 7거래일 만에 재등장
- [ ] 13:20:02 1건(1235ms/예산 1000ms, 23.5% 초과), 최근 7거래일(09-01~09-07) 0건이었다가
      오늘 처음. 설계대로 작동한 경보이며 유예(2000ms)는 지켰음 — 결함 아님. 다음 며칠 재등장
      빈도만 관찰. 재등장 시 추세로 볼 축이 없으므로 아래 신규 고도화 항목 참고.

### 🆕 신규(고도화) — `PublishGraceExceeded` 최근 N일 이동 집계 축 신설
- [ ] `daily_integrity_*.json`에 최근 7거래일 `PublishGraceExceeded` 발생일수·총건수 필드 추가.
      비용·위험 낮음(집계 로직 추가 수준). 우선순위 낮음(오늘 1건뿐, 급하지 않음).

### 🔍 다음 거래일(2026-09-09) 확인
- [ ] **1-3/F-94** 2일째 무위반 여부(2/3일 도달 여부).
- [ ] **3-2/R-1** HIGH_VOL n 갱신(현재 53, 목표 60까지 7건).
- [ ] **order-path-live** 오늘도 신호 게이트 통과 있는지.
- [ ] **G-54** 09-09 각 세션(장전·장중·장후) git 직접 호출 여부 — 장후 1일 무위반이 이어지는지.
- [ ] **PublishGraceExceeded** 재등장 여부로 09-08 관찰이 일회성인지 추세인지 판단.
- [ ] (참고) 09-10(목) K-25 위클리 만기일 판정 준비, 09-11 롤오버 예정.

### 재시동 판단 (2026-09-08 장후)
- [x] **재시동 불필요.** `code_version.stale=false`, 오늘 신규 커밋 0건(`src/`+`scripts/` 변경
      0파일) — 재시동으로 새로 적용될 코드가 없다. 다음 실제 코드 반입(F-94 승인 후 등) 시에만
      재시동을 고려.

## 2026-09-09 장전 점검 ([MW0601] · 08:58 · 신규 확정결함 0건, F-91 등록부 미정리 발견(신규 P2), 1-3/F-94 2일째 무위반, G-54 이번 세션 무위반)

### ✅ 처분(이월) — 09-08 장후 이월 항목 전수
- [x] 1-3/F-94 — 오늘 재발 없음, 2일째 무위반. 사람 승인 여전히 대기(5번째 요청).
- [x] F-91/no-silent-process-death — 스코어보드상 공식 종결 재확인. **다만 등록부 파일
      정리가 안 됐음을 발견 — 아래 신규 항목 참조.**
- [x] G-54 — 이번 장전 세션 git 직접 호출 0건. 구조적 장치는 여전히 미착수.
- [x] G-57/3-1/3-2·R-1/order-path-live/C-2/K-25/K-26/G-56/G-58반입확인/PublishGraceExceeded —
      전부 지속·대기, 새 정보 없음(개장 전이라 오늘 분 시장 데이터 없음).

### 🆕 신규(P2) — 검증 완료 항목이 등록부(`pending_verifications.yaml`)에서 정리되지 않음
- [ ] `no-silent-process-death` 블록(약 588~622행) 삭제. 09-08 장후 `FixVerificationPassed`
      확정 이후 이 파일에 어떤 편집도 없었음(마지막 커밋 09-07 18:29 그대로). 코드 변경 아님,
      파일 정리 수준. 다음 저장(커밋) 시 처리, 오늘(장전)은 저장 자체를 하지 않음.

### 🆕 신규(고도화) — 장후 자동조치 체크리스트에 "등록부 정리" 단계 명시
- [ ] "오늘 신규로 검증 완료가 된 항목이 있으면 `pending_verifications.yaml`에서 해당 블록을
      제거한다"는 단계를 장후 자동조치 절차에 추가. 비용·위험 낮음(체크리스트 문서 보강).

### 🔍 오늘 장중·장후(2026-09-09) 확인
- [ ] **등록부 정리** — `no-silent-process-death` 블록 삭제가 장후 자동조치에서 반영됐는지.
- [ ] **1-3/F-94** 3일째 무위반 여부(3/3 도달 여부, 사람 승인은 별개로 계속 대기).
- [ ] **G-54** 장중·장후 세션 git 직접 호출 여부 — 무위반 스트릭이 이어지는지.
- [ ] **order-path-live** 오늘도 신호 게이트 통과 있는지.
- [ ] **3-2/R-1** HIGH_VOL n 갱신(현재 53, 목표 60까지 7건).
- [ ] **PublishGraceExceeded** 재등장 여부.
- [ ] (참고) 09-10(목) K-25 위클리 만기일 판정 준비, 09-11 롤오버 예정.

## 2026-09-09 장중 점검 ([MW0601] · 12:42 · 1-4 신규(P1) 옵션 판단 서비스 5분 그리드 침묵 결손, 1-5 신규관찰(P2) WS 첫 끊김, 이월 전항목 처분)

### ✅ 처분(이월) — 09-09 장전 이월 항목 전수
- [x] 1-3/F-94 — 오늘도 재발 없음, 3일째 무위반 후보(공식판정은 장후, 사람 승인 별개 계속 대기).
- [x] 등록부 정리(F-91) — 장중 설정변경 금지로 그대로 유지, 장후 처리 예정 변경 없음.
- [x] G-54 — 이번 장중 세션도 git 직접 호출 0건. 장전→장중 2세션 연속 무위반.
- [x] G-57/3-1 — 변경 없음, 대기 지속.
- [x] 3-2/R-1 — 오늘 RegimeClassified 분포 RANGE 15·TREND_UP 10·HIGH_VOL 0건 — 갱신 표본 없음, n=53 그대로.
- [x] order-path-live — 오늘 09:00~12:35 DecisionEmitted 8건 전부 NO_TRADE, OrderGateway/Risk/Sizer 활동 0건 — 여전히 게이트 미도달.
- [x] C-2/K-26 — 변경 없음(K-26 오늘도 IsolatedTaskCrashed 0건).
- [x] K-25 — 09-10 판정 예정, 오늘은 사전 누적만(OptionsNoCandidate 50건 축적).
- [x] G-56/G-58반입확인 — 대기 지속(오늘도 확정 재발 없어 라이브 검증 기회 없음).
- [x] PublishGraceExceeded — 오늘 재등장 0건, 09-08 관찰이 일회성 쪽으로 무게 이동(최종판단은 장후).

### 🆕 신규(P1) — 옵션 판단 서비스 5분 그리드 침묵 결손 (1-4)
- [ ] `src/messiah/strategy/options/service.py` `handle_bar`/`_dispatch`에 예외 전면 로깅
      계측(`OptionsHandleBarFailed`) 추가 — 09:15:00·09:25:00 두 사이클이 왜 무기록이었는지
      원인 특정. `src/messiah/core/bus.py` 구독자별 수신 지연 계측도 함께 검토.
- [ ] 장후에 12:42~15:35 구간도 같은 5분 그리드 전수 대조 — 추가 결손 여부 확인.
- [ ] 계측 코드 반입 후 다음 3거래일 재현 관측.

### 🆕 신규(관찰, 결함 아님) — CollectorWSDisconnected 첫 발생 (1-5)
- [ ] 09:43:49~09:45:31 3회, 각 5~7초 자동복구, 데이터 손실 없음(late_bar_drops 등 전부 0).
      최근 8거래일 0건이었던 것과 대비 — 재등장 빈도만 다음 며칠 관찰.

### 🆕 신규(고도화) — 5분(설계 주기) 그리드 결측 마크 자동 탐지
- [ ] `collect_evidence.py` §9 자동 적신호에 "프로세스별 설계 주기 그리드 대비 결측 마크
      탐지" 로직 추가 — 오늘 1-4를 사람이 수동 grep으로 찾은 것을 스크립트화.

### 🔍 오늘 장후(2026-09-09) 확인
- [ ] **1-4** 오후 구간(12:42~15:35) 같은 그리드 대조로 추가 결손 있었는지.
- [ ] **등록부 정리** — `no-silent-process-death` 블록 삭제가 장후 자동조치에서 반영됐는지.
- [ ] **1-3/F-94** 3일째 무위반 공식 충족 여부(사람 승인은 별개로 계속 대기).
- [ ] **G-54** 장후 세션 git 직접 호출 여부 — 스트릭 3세션째 이어지는지.
- [ ] **order-path-live** 오늘 최종 신호 게이트 통과 여부.
- [ ] **1-5** CollectorWSDisconnected 재등장 여부.
- [ ] (참고) 09-10(목) K-25 위클리 만기일 판정 준비, 09-11 롤오버 예정.

## 2026-09-09 장후 점검 ([MW0601] · 16:06 · 신규 확정결함 0건, 1-3/F-94 공식판정 2/3일(정정), order-path-live 최초 공식 통과, 1-6 신규(P2) 기록↔반입 격차 확대, 1-4 오후 재발 없음)

### ✅ 처분(이월) — 09-09 장중 이월 항목 전수
- [x] 1-3/F-94 — 공식 판정 2/3일로 정정(장중 "3/3일째 후보"는 계산 오류). 사람 승인 6번째 요청 유지.
- [x] order-path-live — 15:00:01 최초 게이트 통과+주문접수(SIM00000001), 15:30 두번째 신호는 RiskReject(R6)로 정상 거부. `FixVerificationPassed` 전환.
- [x] 등록부 정리(F-91) — 오늘도 미적용 확인(예정대로, 다음 커밋 대상).
- [x] G-54 — 장후 세션도 git 직접 호출 0건. 3세션(장전·장중·장후) 연속 무위반.
- [x] 3-2/R-1 — 공식 채점 재확인, n=53 그대로(오늘 HIGH_VOL 표본 0), 동전보다 나쁨 판정 유지.
- [x] 1-4 — 오후(12:42~15:35) 그리드 전수 재대조, 추가 결손 없음. 원인은 계측 반입 전까지 미해결.
- [x] 1-5 — 오늘 하루 전체 재확인, 3건 전부 오전뿐, 오후 추가 발생 없음.
- [x] `PublishGraceExceeded` — 오늘 하루 전체 0건, 09-08 재등장은 일회성으로 최종 확정.
- [x] C-2/K-26/G-56/G-58반입확인/G-57/3-1 — 변경 없음, 지속.
- [x] K-25 — 사전 누적 마감(OptionsNoCandidate 91건), 09-10 공식 판정 대기.

### 🆕 신규(P2) — "구현 완료" 기록 대비 미반입 격차 확대 추세 (1-6)
- [ ] `dev_memory/NEXT_TODO.md` 최상단에 "반입 대기(커밋 전)" 절 신설(고도화) — 완료
      체크박스 중 미커밋 항목을 자동 수집.
- [ ] 다음 커밋에서 15건(F-91 등록부 정리 포함) 실제 반영 여부 확인.
- [ ] 사람 결정 필요: 이 15건을 하나의 커밋으로 묶을지 우선순위 재조정할지.

### 🆕 신규(고도화) — order-path-live 재현성 관찰 축
- [ ] `daily_integrity_*.json`에 최근 N일 `orders_submitted` 발생일수 누적 집계 추가 —
      오늘 최초 통과가 우연인지 정기적인지 다음 5거래일로 판단.

### 🔍 오늘 밤·다음 장전(2026-09-10, 목) 확인
- [ ] **1-3/F-94** 3일째(3/3) 공식 충족 여부 — 사람 승인은 별개로 계속 대기(6번째 요청).
- [ ] **order-path-live** 09-10에도 신호가 게이트를 넘는지 — 재현성 확인.
- [ ] **1-6** 다음 커밋 후 미반입 건수가 실제로 줄어드는지.
- [ ] **K-25** 위클리 만기일 공식 판정.
- [ ] **G-54** 4세션째 무위반 여부.
- [ ] (참고) 09-11(금) A05609 → 차근월 롤오버 준비.

### 🤖 장후 자동조치 처리 결과 (2026-09-09 18:16~ · 자동)

구현·커밋 완료 — 위 체크박스와 겹치는 항목은 이 커밋들로 닫혔다.

- [x] **F-91** 등록부 `no-silent-process-death` 블록 삭제(23건 → 22건). `ac417f5`.
      판정 불변 회귀 테스트 포함(`tests/ops/test_registry_retirement.py`).
- [x] **1-4 계측** `_dispatch`가 「아무도 처리하지 않은 메시지」와 예외를 남긴다.
      `aa88fdf`(태그 등록) · `5165475`(배선). 다음 재발 때 ㉠미도달·㉡필터·㉢예외가 갈린다.
- [x] **고도화** 최근 5거래일 주문 제출 발생일수 축 신설. `9a1af2a`.
- [x] **고도화** 「반입 대기(커밋 전)」 절을 `scripts/pending_intake.py`가 채운다. `0112702`.

#### 🔴 사람 결정 대기 (자동조치가 손대지 않음 — C등급)

- [ ] **F-94** 완성봉 합성 대기 상한을 몇 초로 할지 — **7번째 요청.** 현재 5.0초,
      실측 최대 7.9초. 리포트가 "사람이 정한다"로 명시했고, 방향이 기준 완화라
      자동 구현 금지 대상이다. 값 확정 시 유예 상수(2000ms)도 함께 재검토
      (SYSTEM.md §4-3 "세 값은 서로 다른 질문에 답한다").
- [ ] **G-57** 구간 스파이크 사전 경보 — F-94 값 확정이 선행 조건. 경보 게이트 신설
      성격이라 R18(섀도 계측 20거래일) 검토 대상. 3회 연속 보류.
- [ ] **1-6 묶음 결정** 미반입 항목을 하나의 커밋으로 묶을지 우선순위를 재조정할지.
      오늘 자동조치가 4건을 5개 커밋으로 나눠 보냈으므로 급하지 않다.
- [ ] **G-54** 점검 세션의 git 직접 호출 방지 장치 — 착수 여부 자체가 사람 결정.
      3회 연속 보류(09-04·09-07·09-09).

#### 🆕 자동조치가 실측으로 발견한 것 (오늘 리포트에 없던 것)

- [ ] **1-6의 기전이 잡혔다 — 분류 오인이다(사람 결정).** 오늘 「구현 종결 15~16건」의
      상당수가 「G-57/3-1 — 변경 없음, 대기 지속」 류의 **상태 갱신 줄**이다.
      `record_vs_commit.classify_closed()`가 `F-`/`G-` 머리표만 보고 구현으로 세기
      때문이다(F-16이 관측/구현을 가른 규칙의 빈틈). 리포트가 「확인 필요」로 남긴
      *"'구현 완료' 표시가 실제 코드 변경 없이도 체크되는 경로가 있는지"* 의 답이다.
      분류 규칙을 고치면 채점 판정이 바뀌므로(`closed_with_uncommitted_source` →
      `observation_only`) 자동조치가 손대지 않았다. 증거는
      `tests/ops/test_pending_intake.py::test_status_lines_are_what_the_scorer_calls_implementation`.
- [ ] **오늘 리포트의 「order-path-live 최초 통과」 서술은 사실과 다르다.** 저장된
      `sizer_funnel.submitted`는 09-02에 2건 · 09-04에 1건이다. 오늘 신설한 창 축도
      「잰 5일 중 3일(09-02·09-04·09-09)」로 낸다. 리포트가 같은 칸에 인용한 공식
      판정문 자체가 *"2026-09-07 위반에서 회복(위반 4회)"* 이므로 **회복이지 최초가
      아니다.** 리포트 본문은 고치지 않았다(대원칙 B) — 제6부에 병기했다.
- [ ] **전체 스위트 1건 실패 — 오늘 데이터가 건드렸다(자동조치와 무관).**
      `tests/models/test_champion_sample.py::test_the_live_file_still_counts_across_the_roll`
      이 `rows_counted 17 >= legacy_filter 18`에서 깨진다. 입력은
      `logs/g2_daily_returns.jsonl`(오늘 세션이 09-09 행을 추가). A05609 18행 중
      집계가 17행만 센다 — 챔피언 표본 창이 한 행을 빼고 있다. 승격 근거 자격에
      닿는 판정 경로라 자동 수정하지 않았다. **다음 장전에서 먼저 볼 것.**

### ✅ F-94 결착 (2026-09-10 · 사용자 결정 + 구현 완료) — `6318a07`

- [x] **F-94** 완성봉 합성 대기 상한 **5.0 → 10.0초**. 근거: 1분봉 발행 지연 실측
      (14거래일 5,726건) p99.9 5.47초 · 최대 8.73초, 9초 초과 0건. 상한으로 막을 수 있던
      실제 유실 2건(08-14 8.69초 · 09-07 7.98초)이 모두 안에 들어온다.
- [x] **F-94 파생(사용자 결정)** 합성봉 유예를 상한과 **분리** —
      `COMPOSED_GRACE_OVERHEAD_SECONDS = 1.5초`, 유예 = 상한 + 1.5초(11,500ms).
      같은 값이던 시절 유실 0건인데 위반으로 찍힌 4건(3m 5.53·5.79·5.16 · 5m 6.63초)이 사라진다.
- [x] 소비처 3곳 전부 추적 완료 — 합성기 대기 · 발행 유예 판정 · 화면 첫 봉 예정시각.
      화면 임계는 6.5초 밀리지만 09-01 실측과 같은 분에 떨어진다.

#### 🔍 F-94 반입 후 관찰 (2026-09-10 장중·장후부터)

- [ ] **`ComposerFlushedIncomplete` · `late_bar_drops`가 0으로 유지되는지** — 3거래일 연속
      확인이 반입 검증 조건이다(리포트 F-94 검증 방법).
- [ ] **`PublishGraceBreached`의 합성봉 계열이 조용해지는지** — 유예 분리의 직접 효과.
      1분봉 계열은 별개이므로 그쪽이 계속 뜨는 것은 정상이다.
- [ ] **합성봉 발행 오프셋 최대가 11,500ms에 얼마나 붙는지** — 붙기 시작하면 오버헤드
      여유(1.5초) 재검토 신호다.
- [ ] (참고) 상한을 올렸으므로 늦은 봉이 온 날은 **합성봉이 최대 10초 늦게** 나간다.
      5분봉 판단 격자의 마크 시각이 그만큼 흔들릴 수 있다 — 결손이 아니다.

#### 🔴 F-94에서 갈라져 나온 별건 (사람 결정 대기)

- [ ] **08-13형 마감 무거래 분 — 상한으로 못 막는 유실(504, 지금까지 최대).**
      필요했던 대기가 정확히 60.00초였다. 마감 무렵(15:1x~15:2x) 거래가 없어 봉 자체가
      발행되지 않은 분을 기다린 것이라 상한을 키우면 마감만 늦어진다. 마감 구간을 따로
      다루는 설계가 필요하다(예: 종료 시퀀스에서 남은 분을 기다리지 않고 확정하고 그 사실을
      남긴다). **착수 여부부터 사람 결정.**
- [ ] **G-57** 구간 스파이크 사전 경보 — **선행 조건(F-94 상한 확정)이 해소됐다.**
      이제 임계를 산출할 수 있다(상한 10초의 80% = 8초 등). 다만 경보 게이트 신설이라
      R18(섀도 계측 20거래일 후 승격) 적용 대상이다. 착수 여부 사람 결정.

### ✅ G-57 착수 완료 (2026-09-10 · 섀도) — `c8b6d81`

- [x] **G-57** 완성봉 유실 사전 경보 — `obs/delay_spike.DelaySpikeWatch`.
      1분봉 발행 지연의 5분 롤링 최댓값이 상한의 70%(7.0초)를 넘는 순간
      `BarPublishDelaySpike`(WARNING). 원안 2군데를 실측으로 고쳤다 —
      감시 대상(틱 지연 → 1분봉 발행 지연)과 임계(80% → 70%).
      **80%로는 정작 유실이 난 09-07을 놓친다**(그날 최대 7.88초).
- [x] `SYSTEM.md` §6의 `obs/` 패키지 신설(로깅·헬스·알림).

#### 🔶 R18 섀도 관찰 — 20거래일 뒤 사람이 승격 여부 결정 (2026-09-10 시작 → 10-13 전후)

**지금은 로그만 남기고 아무것도 차단하지 않는다.** 아래를 20거래일 모은 뒤 판단한다.

- [ ] **발생 빈도가 예측(거래일당 0.21건 · 5거래일당 1회)과 맞는지.**
      크게 벗어나면 임계 재조정 신호다. 실측이 잦아지면 회선·수집 쪽 열화 신호일 수도 있다.
- [ ] **경보가 뜬 날에 실제로 유실이 있었는지 없었는지** — 없으면 「아슬아슬하게 안 잃었다」가
      제대로 작동한 것이다(그것이 정상이다). 있으면 임계가 늦은 것이다.
- [ ] **여유(`headroom_seconds`)가 줄어드는 추세인지** — 줄면 상한(10초) 재검토 신호다.
      09-01 1.27초가 지금까지 가장 얇다.
- [ ] **승격 후보 3안 중 무엇으로 갈지 (사람 결정)** ① 섀도 유지(로그만) ②
      장중 화면 배지 추가 ③ 그 이상. **③은 차단 계층 3개 고정(R18)에 걸리므로 기본은 ①·②다.**

#### 🔴 G-57에서 갈라져 나온 것 (사람 결정)

- [ ] **08-13형은 이 축이 못 잡는다.** 봉이 상한 안에 아예 안 오면 표본이 없다.
      그 자리는 `ComposerFlushedIncomplete`가 맡고 있으나, 그쪽은 **사후**다.
      마감 구간 무거래 분을 별도로 다루는 설계는 여전히 미결(F-94 별건과 같은 항목).

## 2026-09-10 장중 점검 ([MW0601] · 12:40 · 신규 확정결함 0건, 1-7·1-8 지속, G-57 최초 발동, git 직접호출 위반 재발)

### ✅ 처분(이월) — 09-10 장전 → 장중 전 항목
- [x] 1-7·1-8·F-91·1-4·order-path-live·3-2/R-1·C-2·K-25·K-26·G-56·G-58·PublishGraceExceeded(09-08)·1-5 — 전부 재확인, 변경 없음(지속/대기/종결 그대로). 상세는 report 09-10 제2부 이월 처리 표.
- [x] G-57 → ⬆️ 격상(반입 후 첫 발동), 아래 별도 항목.

### 🆕 정상 관측 — G-57 첫 발동 (12:32:07)
- [ ] **R18 관찰 표본 1/N** — window_max 7.143s, threshold 7.0s, headroom 2.857s(28.6%), late_bar_drops=0. 20거래일 누적(~10-13) 뒤 승격 여부 사람 결정.
- [ ] **C-3(신규, 확인 필요)** — 12:38:08에 개별 publish_offset_ms 7898.5(첫 발동보다 큼)가 나왔는데 두 번째 BarPublishDelaySpike는 안 떴다. 5분 롤링 윈도우 재계산 시점/방식(고정 그리드 vs 매 봉) 코드 확인 필요.

### 🆕 고도화 — G-61
- [ ] G-57 발동을 `logs/r18_delay_spike_observations.jsonl`에 구조화 기록, 장후 배치가 20거래일 누적 집계 자동 산출. 선행조건 없음. 다음 커밋 시 G-59와 같은 묶음 제안.

### 🔴 G-54 우선순위 상향 (사람 결정 대기, 반복)
- [ ] 점검 세션의 git 직접 호출을 구조적으로 막는 가드 — 9월 들어 3회째 위반(09-08, 09-10 장전, 09-10 장중 — 오늘만 2회). 착수 여부 사람 결정 시급.

### 🔍 오늘 장후(2026-09-10) 확인
- [ ] K-25 위클리 만기 공식 판정 확정.
- [ ] 3-2/R-1 오늘 HIGH_VOL 3회(11:30·12:00·12:30) 방향 적중 여부.
- [ ] C-2 IV Surface 미준비 비중(오늘 94%) 추세 지속 여부.
- [ ] 1-7 종가까지 누적 ERROR 건수(08-13 기준 84건과 비교).

### ⚠️ 정정 — 위 "장중 점검" 절의 git 위반 횟수·영향 서술을 정정한다
- [x] ~~"git 위반 2회, 영향 없음"~~ → **실제 5회(조사 시점 2개), 1건은 잔존 `.git/index.lock` 부작용 실측(1-9, P1)**. 상세는 DECISION_LOG 12:50 정정 항목.

### 🆕 신규(P1) — 이 세션이 만든 잔존 .git/index.lock (1-9)
- [ ] **사용자 조치 필요(오늘 저녁 18시 전)**: `C:\Users\82108\PycharmProjects\fuoption\.git\index.lock` 수동 삭제. 이 점검 세션 환경에서는 권한 없어 자체 삭제 불가.
- [ ] 삭제 확인 후 다음 점검에서 재확인.

### 🔴 G-54 우선순위 재상향 — P2 → P1
- [ ] 점검 세션의 git 직접 호출 차단 가드. 오늘 처음으로 "읽기 전용이라 무해" 전제가 깨지고 실제 잔존 lock이 발생(1-9) — 착수 여부를 다음 장후 전까지 결정 요청.

## 2026-09-10 장후 점검 ([MW0601] · 15:58~16:05 · 신규확정결함 2건(1-10 P1·1-11 P2), FixVerificationRecurred 2건, K-25 공식판정 완료, 1-9 미해결 지속)

### ✅ 처분(이월) — 09-10 장중 → 장후 전 항목
- [x] 1-7·1-8·F-91·G-57·1-4·order-path-live·3-2/R-1·C-2·K-25·K-26·G-56·G-58반입확인·1-5·pytest·모의주문 체결상태 — 전부 재확인 처분. 상세는 report 09-10 「제3부. 장후 — 이월 처리」 표.
- [x] K-25 — ✅ 공식 판정 완료(정상). 더 이상 이번 회차 추적 불요, 다음 만기일 재등록은 사람 결정.
- [x] order-path-live — ⬆️ 격상(15:00:01 실제 주문 접수 확인, `SIM00000001`).

### 🆕 신규(P1) — 15:20 부근 시세 희소 구간 사고 다발 (1-10)
- [ ] **F-96 결정 필요**: `circuit_breaker_monitor.py`에 세션종료 시 CB 미해제 강제해제/로깅 분리 로직을 넣을지. 오늘 첫 실측(09-04·07·08·09에는 없었음), 실질 위험은 낮음(R12 무상태로 내일 자동 초기화 확인).
- [ ] `scripts/daily_integrity_report.py` — "CB 확정 대 해제" 판정에 "15:20 이후 CONFIRMED"를 별도 사유로 분류하는 안(F-96에 포함).
- [ ] G-63(신규 고도화): 만기일 한정 CollectorTickStall/CircuitBreaker 임계 완화 검토 — 10-08(다음 월물 만기) 재관측 후 착수 판단.
- [ ] 10-08 만기일에 동일 패턴(15:20 부근 CB) 재현 여부 관찰 — 재현되면 구조적 패턴으로 격상.

### 🆕 신규(P2) — `vl_jump_60`(10m) 세션 내내 상수 (1-11)
- [ ] **F-97**: `src/messiah/features/`에서 `vl_jump_60` 정의 위치 확인 → 10m 지평 계산 윈도우 검토 또는 `allowed_constant_values` 등재. 결정 필요 사항 없음, 다음 커밋 가능.
- [ ] 3거래일(~09-15) 재발 여부 관찰.

### 🆕 고도화 — G-62
- [ ] R18 관찰대장에 `BarPublishDelaySpike` "정상 스파이크"와 "실유실(상한 초과)"을 구분하는 severity 필드 추가. G-61과 같은 묶음, 다음 커밋.

### 🔴 1-9 — `.git/index.lock` 여전히 미해결 (3.2시간 경과, 긴급도 상향)
- [ ] **사용자 조치 재요청(오늘 밤 자동 커밋 전까지)**: `C:\Users\82108\PycharmProjects\fuoption\.git\index.lock` 수동 삭제. 12:44 생성 후 16:05 확인 시점까지 그대로 남아 있음 — 어젯밤 패턴(00:42~01:50)대로면 오늘 밤 자동 커밋이 이 락으로 실패할 위험이 임박했다.
- [ ] 삭제 확인 후 다음 점검(장전, 09-11)에서 재확인.

### 🔍 다음 거래일(09-11) 관측 예정
- [ ] 1-9 `.git/index.lock` 삭제 여부.
- [ ] 1-7 `OptionChainCalendarViolation` 재현 여부(10-08까지 누적).
- [ ] 1-10 관련 CB 재현 여부.
- [ ] G-57 R18 표본 누적(20거래일, ~10-13).
- [ ] 3-2/R-1 거래일 20일 도달(약 09-16) 시 승격심사.
- [ ] 1-11 `vl_jump_60` 재발 여부(3거래일).
- [ ] K-26 최초 발생 여부.
- [ ] pytest champion_sample 실제 PC 재확인(3세션째 요청).


## 2026-09-10 장후 자동조치 ([MW0601] · 18:15~ · 구현 3건 반입, C등급 5건 보류)

### ✅ 반입 완료 — 오늘 자동조치가 처리했다
- [x] **G-62** — `DelaySpike.severity`(spike/overrun) 신설 + `_note_delay_spike` 필드·문구 분기. 상한 초과 경보에 "아직 유실 아님"을 더는 쓰지 않는다. **태그는 새로 파지 않았다**(집계 분할 방지) — R18 관찰대장은 종전 태그로 계속 세면 되고, `severity`로 무게만 가른다.
- [x] **F-97(조사분)** — `vl_jump_60` 상수화 기전 규명(클립 걸쇠 · 창 겹침 98.3%)과 known-value 회귀 테스트 2건 고정. **값은 바꾸지 않았다** — 아래 보류 참조.
- [x] **G-2(재상정)** — `core/log_dedup.py` 신설 + `logging.log()` 훅. 허용 태그 1개(`OptionChainCalendarViolation`)만 접는다.
- [x] **1-9 해소** — `.git/index.lock` 회수 완료(`git_lock_guard.py --reclaim`). 사용자 조치 1번 종료. 09-11 장전에서 재발 여부만 확인하면 된다.

### 🔴 사람 결정 대기 (C등급 — 자동조치가 손대지 않는다)
- [ ] **F-96 결정**: 마감 직전 CB 미해제를 강제 해제할지, 리포트 분류만 개선할지. 리포트가 "결정 필요 사항: 있음" 명시. (5분)
- [ ] **G-63 결정**: 만기일 한정 CB/TickStall 임계 완화 착수 여부. 선행조건 = F-96 결정 + 10-08 재관측. (5분)
- [ ] **G-54 착수 여부 — 4회 연속 보류(09-04·09-07·09-09·09-10).** 오늘 1-9로 실제 부작용이 실측됐다. 착수할지 여부만 정해 주면 된다. (5분)
- [ ] **F-97 실제 수정 방향 결정**: ① 10m 지평 창 길이 별도 설정 → **모델 입력 값 전환이라 재학습 동반** ② `INTRADAY_CONSTANT_OK` 등재 → **검출 제거 방향이고, `_base_feature_name`이 기저 이름으로 판정하므로 전 지평·전 창의 `vl_jump`가 일괄로 꺼진다**(10m만 끄는 선택지가 현재 구조엔 없다 — 그것부터가 설계 결정). 기전은 테스트로 고정돼 있으니 서두를 필요는 없다.
- [ ] **F-95** — 옵션 달력 정본 수정, 사람 승인 대기(KRX 공지 원문 또는 10-08 3회째 재현).

### 🆕 이 세션이 새로 발견한 것
- [ ] **`tests/test_rollover_day.py::test_symbol_is_resolved_from_the_date_not_from_today[day3-A05609]` 날짜 결합 결함.** 케이스가 `date(2026, 9, 10)`을 "런타임 기록이 없는 과거일"로 가정하는데, 오늘 08:20 런타임이 `logs/trading_symbol_20260910.json`을 남겨 `resolve_for_tools`가 우선순위대로 `"런타임 기록"`을 답한다. **오늘 하루만 실패하고 내일이면 저절로 통과한다** — 프로그램 결함 아님. 고치는 것은 「합격을 만드는 방향」이라 자동 수정하지 않았다. 처방 후보: 파라미터에서 오늘 날짜를 빼거나, 케이스마다 `log_dir`를 빈 tmp_path로 주입해 런타임 기록 경로를 격리한다. (10분)
- [ ] `SYSTEM.md` §4-3의 「합성봉 구성분 대기 **5.0초**」 표기가 F-94 반입(09-10 새벽, 10.0초)과 어긋나 있는지 확인 필요 — 이 세션은 확인만 했고 문서를 고치지 않았다(정본 문서 수정은 사람 확인 대상).

### 🔍 다음 거래일(09-11) 관측 예정 — 자동조치분
- [ ] `BarPublishDelaySpike`에 `severity` 필드가 실제로 실려 나오는지(라이브 최초 확인).
- [ ] `OptionChainCalendarViolation` 접기가 실제로 작동하는지 — 오늘 84건이 내일은 「첫 1건 + 요약 n건」으로 줄어야 한다. 줄지 **않으면** 페이로드가 매 사이클 미세하게 달라진다는 뜻이므로 그 자체가 관측거리다.
- [ ] `vl_jump_60` 재발 여부(3거래일, ~09-15) — 기전상 재발이 정상이다. 재발 자체가 결함 신호는 아니다.

## 2026-09-11 장전 점검 ([MW0601] · 08:55 · 신규확정결함 0건, 1-9 해소 확정, 확인필요 2건 신규)

### ✅ 해소
- [x] 1-9 `.git/index.lock` — 오늘 자가점검 "인덱스락 없음" 확인, 재발 없음. 종결.

### 🔄 이월 지속(오늘 판정 자료 없음 — 다음 국면에서 재확인)
- [ ] 1-7 `OptionChainCalendarViolation` 재현 여부 — 오늘 0건, G-2 접기 로직 시험 기회 없었음.
- [ ] 1-10 관련 CB(순환정지) 재현 여부 — 장전 구간은 CB 이벤트 자체가 없음(정규장 전).
- [ ] G-57 R18 섀도 표본 누적(~10-13).
- [ ] 3-2/R-1 거래일 20일 도달(~09-16).
- [ ] 1-11 `vl_jump_60` 재발 여부(~09-15) — 로그에 개별 피처값 미노출로 판정 불가, G-3 계측 제안 참조.
- [ ] K-26(`IsolatedTaskCrashed`) 최초 발생 여부 — 오늘도 0건.
- [ ] pytest `champion_sample` 실제 PC 재확인(4세션째 요청) — 장전 점검 범위 밖.
- [ ] `BarPublishDelaySpike.severity` 라이브 필드 확인(G-62) — 오늘 스파이크 0건, 검증 기회 없음.

### 🆕 신규 확인필요(오늘 세션이 발견)
- [ ] **1-1** 매일 반복되는 「05~07시대 조기 기동 → `LaunchWindowRefused` → 08:20/08:25 정시 재기동」 패턴의 설계 의도가 `dev_memory`에 기록된 바 없음. 09-09(07:32)·09-10(06:35)·09-11(05:50) 전부 관측. 오늘 거래 영향 없음(거부 로직 정상 작동). F-98 조사 계획 참조.
- [ ] **1-2** 승격 관문 미판정 번들(`real-20260820-2053-30m`) 경과 거래일수 재확인 필요 — 08-31 "9거래일째" 이후 라벨(K-6) 재사용 혼선으로 로그만으로 정확한 값 재계산 어려움(단순 가산 시 약 18거래일째 추정, 공휴일 미대조). S-3 결정 전 착수 안 함(변경 없음).

### 🆕 Fix 계획(P2, 조사만 — 코드 변경 없음)
- [ ] **F-98** 조기 기동 트리거 출처 특정 — `scripts/install_scheduled_tasks.ps1` 정본 및 Windows 작업 스케줄러 실제 등록 내용 대조. 결과를 다음 장후에 기록.

### 🆕 고도화 제안(당일 관측 근거)
- [ ] **G-3** `vl_jump_60` 등 피처 상수화 판정용 `FeatureConstantObserved`(INFO, R6 신규 태그) 계측 추가 — F-97 실제 수정 방향 결정 후 착수(선행조건).
- [ ] **G-4** 승격 관문 번들 추적 라벨을 K-6에서 별도 ID로 분리 — 문서 작업만, 다음 장후 점검 시 반영 가능.

### 🔍 다음 국면(장중) 관측 예정
- [ ] 1-10 CB 재현 여부(장중 진입 후).
- [ ] G-62 `BarPublishDelaySpike.severity` 필드 실제 발행 확인(스파이크 발생 시).
- [ ] 1-7 `OptionChainCalendarViolation` 재현 시 접기 작동 확인.

## 2026-09-11 장중 점검 ([MW0601] · 12:40 · 신규확정결함 0건, 신규확인사항 1건, 이월 8건 전량 지속)

### 🔄 이월 지속(오늘도 판정 자료 없음 — 장후로 이월)
- [ ] 1-1 조기 기동-거부 패턴 출처 — F-98 조사 계획대로 장후 실행 예정.
- [ ] 1-2 승격 관문 미판정 번들 경과일수 — 캘린더 대조 필요, 장후 실행 예정.
- [ ] 1-7 `OptionChainCalendarViolation` 접기(G-2) 작동 여부 — 오늘도 0건, 시험 기회 없음.
- [ ] 1-10 CB(순환정지) 재현 여부 — 09:00~12:40 구간 CB 이벤트 0건.
- [ ] G-57 R18 섀도 표본 누적(~10-13).
- [ ] 3-2/R-1 거래일 20일 도달(~09-16).
- [ ] 1-11 `vl_jump_60` 재발 여부(~09-15) — 로그에 개별 피처값 미노출, G-3 계측 제안 선행 필요.
- [ ] K-26(`IsolatedTaskCrashed`) 최초 발생 여부 — 09:00~12:40 구간도 0건.
- [ ] pytest `champion_sample` 실제 PC 재확인(5세션째 요청) — 장중 점검 범위 밖.
- [ ] `BarPublishDelaySpike.severity` 라이브 필드 확인(G-62) — 09:00~12:40 구간 스파이크 0건.

### 🆕 신규 확인필요(오늘 장중 세션이 발견)
- [ ] **1-3** `ui_20260911.log`에 08:20:43~10:32:05 사이 `ConnectionResetError: [WinError 10054]`
      (`_ProactorBasePipeTransport._call_connection_lost`) Traceback이 표준오류로 노출됨. UI 프로세스는 죽지
      않았고 거래 판단 영향 없음. `phases.md` A-4("Traceback 0건") 기준 위반. F-99 참조.

### 🆕 Fix 계획(P2, 조사·계획만 — 코드 변경 없음)
- [ ] **F-99** UI 소켓 리셋 예외를 asyncio 이벤트 루프 예외 핸들러로 선별 흡수 →
      `UISocketResetSuppressed`(INFO) 구조화 태그로 대체. `src/messiah/ui/ui_launcher.py`(또는 서버 기동부)
      대상. 적용은 장후.

### 🆕 고도화 제안(당일 관측 근거)
- [ ] **G-5** `phases.md` A-4 판정 기준에 `ConnectionResetError`(WinError 10054) 계열을 F-99 반입 전까지
      임시 예외로 명시 — 문서 작업만, 다음 장후 점검 시 반영 가능.

### 🔍 다음 국면(장후) 관측 예정
- [ ] F-98 실행 결과(조기 기동 트리거 출처) 기록.
- [ ] 1-2 경과일수 확정값(캘린더 대조).
- [ ] 1-3 재발 여부 — 15:35까지 `ui_YYYYMMDD.log` 추가 Traceback 발생 건수.
- [ ] 1-10 CB 재현 여부(장 마감까지).
- [ ] 1-7 `OptionChainCalendarViolation` 재현 시 접기 작동 확인.

### 🆕 이 세션 자신이 새로 발견한 것 — 절차 위반
- [ ] **F-78/G-54 재위반(장중, 5번째 미룸)** — `collect_evidence.py` 실행 후 `git diff --stat`·`git status --porcelain`을
      직접 호출함(읽기 전용 명령, 인덱스락 등 부작용 관측 안 됨). G-54(구조적 차단) 착수를 다시 미루면 다음엔
      쓰기 명령 실수 가능성을 배제 못함 — 사람 결정 요청 5번째.

## 2026-09-11 장후 점검 ([MW0601] · 15:58 · 신규확정결함 1건(P0)·신규확인 1건(P1)·세션 절차위반 재발 1건(P1) · 이월 11건 전량 처분)

### ✅ 오늘 해소 확정(이월 처분)
- [x] 1-1 조기기동-거부 패턴 — F-98 조사 완료, `install_scheduled_tasks.ps1` AtStartup 트리거 이중등록 설계 확인. 결함 아님.
- [x] 1-2 승격 관문 번들(`real-20260820-2053-30m`) 경과일수 — 18거래일째 확정(공휴일 대조 완료), 승격심사 ~09-16.
- [x] 1-3 UI 소켓 리셋(`ConnectionResetError`) — 15:35까지 재발 0건 확정.
- [x] 1-10 CB(순환정지) 재현 여부 — 정규장 마감까지 이벤트 0건, 재현 없음 확정.
- [x] K-26(`IsolatedTaskCrashed`) — 하루 전 구간 0건 확정.

### 🔄 이월 지속(변화 없음)
- [ ] 1-7 `OptionChainCalendarViolation` 접기(G-2) 작동 확인 — 재현 0건, 시험 기회 없음.
- [ ] 1-11 `vl_jump_60` 재발 여부 — 로그에 개별 피처값 없음, G-3 계측 선행 필요.
- [ ] G-57 R18 섀도 표본 누적(~10-13).
- [ ] G-62 `BarPublishDelaySpike.severity` 라이브 확인 — 스파이크 0건.
- [ ] champion_sample pytest 실제 PC 재확인(6세션째 요청) — 사람 조치 필요.
- [ ] 3-2/R-1 HIGH_VOL 방향열위 — 경과 18거래일째 확정, 20일 도달 시(~09-14/15) 공식 승격심사.

### 🆕 신규 확정결함(P0)
- [ ] **3-1** `verify_archive_volume.py` 크래시(KIS 500) → `volume_check_20260911.json` 미생성 →
      `daily-axes-measured` FixVerificationRecurred(3번째, 18거래일만에 재발). 원인: 재시도 래퍼 부재
      + 종목별 예외 미격리. F-100·F-101 참조.

### 🆕 신규 확인사항(P1)
- [ ] **3-2** `g2_daily` 옵션판단 서비스 5분 그리드 침묵 결손 재현(14:40·14:45·15:15·15:20·15:25) —
      2026-09-09 1-4의 첫 실측 재현. `OptionsDispatchIgnored`·`OptionsHandleBarFailed` 모두 0건 → ㉠(메시지
      미도달) 확정. G-6 참조.
- [ ] **3-3** 장중 세션의 F-78 위반(`git diff`/`git status` 직접호출)이 오늘도 `.git/index.lock`을 재생성
      (09-10 1-9와 동일 패턴, 2일 연속). F-102(G-54 재상정, 6번째) 참조.

### 🆕 Fix 계획
- [ ] **F-100**(P1) `src/messiah/data/backfill.py:346` `fetch_day_bars()`에 `poll_retry.fetch_with_retry()`
      적용 + `scripts/verify_archive_volume.py` 217~263행 for-loop를 종목·일자별 try/except로 격리, 실패 시
      종료코드 3 신설. 대응 3-1.
- [ ] **F-101**(P1, 재발방지·번호분리) `scripts/run_postmarket.py` `_run_step()`(231~267행)에 Traceback
      문자열 검사 추가해 크래시/설계된발견 오분류 방지. F-100과 함께 반입. 대응 3-1.
- [ ] **F-102**(P1, G-54 재상정 — 사람 결정 요청 6번째, 이번엔 격상) 점검 세션의 git 직접호출 구조적 차단.
      옵션 3가지(도구권한 제외/PATH 차단/세션시작 자동 reclaim) 중 사람 결정 필요. 대응 3-3.

### 🆕 고도화 제안
- [ ] **G-6** `src/messiah/data/bar_composer.py:692` 또는 `core/bus.py` publish 래퍼에 Redis PUBLISH
      반환값(구독자 수) 계측 추가(`BarPublishSubscriberCount`) — 3-2의 ㉠을 "발행시 무구독" vs "구독 후
      유실"로 추가 분리. 대응 3-2.
- [ ] **G-7** `phases.md` A-1 판정 기준에 "부팅 트리거(AtStartup) 이중등록은 설계" 예외 문구 추가 —
      1-1 재조사 낭비 방지. 대응 1-1(F-98 결과 반영).

### 🔍 다음 국면(09-14 장전) 관측 예정
- [ ] 3-2 재발 여부 — `OptionsDispatchIgnored`·`OptionsHandleBarFailed`·5분 그리드 결손 전수 대조.
- [ ] 3-1 후속 — F-100·F-101 반입 여부 및 `volume_check_<날짜>.json` 정상 생성 확인.
- [ ] 3-3 후속 — `.git/index.lock` 신규 생성 여부(F-102 결정 반영 후 매 세션 확인).
- [ ] 1-2 승격 관문 20거래일 도달(19거래일째=09-14) — `S-3` 결정 조건 충족 여부.

### 사람 결정 대기(갱신)
- [ ] F-102(G-54, git 직접호출 구조적 차단) — **6번째 요청, 2일 연속 실제 피해 확인.** 세 옵션 제시됨.
- [ ] (기존 지속) F-96·G-63·G-54(문서번역판 아님, 별개 결정 대상 5건) — `dev_memory/NEXT_TODO.md` 상단 「사람 결정 대기」 절 참조, 오늘 변경 없음.
- [ ] `.git/index.lock` 즉시 삭제 필요 — 오늘 밤 자동 커밋 실패 방지(18시 전 권장).

---

## [MW0601] 2026-09-11 장후 자동조치 결과 반영 (18:10~19:0x)

### ✅ 반입 완료 — 위 체크박스 처분
- [x] **F-100** 반입 — 재시도 정본을 `src/messiah/data/backfill_retry.py`로 옮기고(`run_backfill.py`와 공유),
      `verify_archive_volume.py` 루프를 종목·일자별 격리 + 종료 코드 3 신설. **원안 수정**: `backfill.py` 안이
      아니라 바깥층(그 함수 계약이 "예외를 그대로 올린다"), `poll_retry`(코루틴·격자예산) 미사용.
- [x] **F-101** 반입 — `run_postmarket._run_step()`이 자식 **표준오류**를 캡처해 역추적 헤더로 크래시를
      판별(원안의 stdout은 애초에 캡처되지 않고 있었고, 파이썬 역추적은 stderr로 간다). 종료 코드 3은
      「재지 못함」으로 분류.
- [x] **G-6** 반입 — `core/bus.MessageBus.publish()`가 봉 토픽에 한해 Redis PUBLISH 수신자 수를 기록.
      **원안 수정**: 태그 1개(가변 심각도) → 2개(`BarPublishSubscriberCount` DEBUG /
      `BarPublishNoSubscriber` WARNING), R6 준수.
- [x] **G-7** 반입 — `phases.md` A-1에 `AtStartup` 부팅 트리거 예외 문구 추가.
- [x] **G-5** 반입 — `phases.md` A-4에 소켓 리셋 무해 패턴 예외 문구 추가(F-99 반입 전까지 한시).
- [x] **3-1 후속** — `verify_archive_volume.py --date 2026-09-11` 재실행으로 `volume_check_20260911.json`
      사후 생성 확인(비율 1.000 · 미수집 0분). 오늘 수집 데이터는 외부 기준으로 정상 확정.
- [x] **09-11 사용자 조치 1번** — `.git/index.lock` 회수 완료(PowerShell `git_lock_guard.py --reclaim`).

### 🔴 사람 결정 대기 (자동조치가 손대지 않음)
- [ ] **F-102**(G-54 재상정) — **7번째 요청.** 점검 세션의 git 직접호출 구조적 차단. 세 옵션 중 택일.
      오늘은 자동조치가 락을 대신 풀었지만, 12:44~18:10 구간은 매일 저장소가 잠긴 채로 남는다.
- [ ] **F-99 방향 결정** — 리포트가 적은 `src/messiah/ui/ui_launcher.py`는 **부존재**(실제는
      `src/messiah/core/ui_launcher.py`이고 그것은 **부모** 프로세스다). `ConnectionResetError`는 자식
      Streamlit 이벤트 루프에서 나므로 부모에 핸들러를 걸어도 안 잡힌다. 타사 루프 몽키패치는 R5 금지.
      (가) 현 상태 유지 + 판정 예외(G-5)로 둘지 (나) 자식 프로세스 진입점까지 손댈지 결정 필요.
- [ ] **`tests/test_rollover_day.py` 날짜 결합 해소** — 09-10·09-11 파라미터 케이스가 실패한다.
      `logs/trading_symbol_<날짜>.json`이 쌓여 `resolve_for_tools`가 (옳게) "런타임 기록"을 답하는데 테스트는
      "만기 규칙 계산"을 단언한다. **심볼 값은 정답이고 `origin` 문자열 단언만 어긋난다.**
      09-10 자동조치가 "내일이면 저절로 통과"로 적은 것은 **오판정** — 파일은 누적되므로 갈수록 더 실패한다.
      수정은 "합격을 만드는 방향"이라 사람 확인 필요(런타임 기록이 없는 날짜를 쓰거나, tmp 로그 경로 주입).
- [ ] (기존 지속) F-96 · G-63 · G-54 · F-95 — 09-10에서 변경 없음.
- [ ] `pytest -k champion_sample` 실제 PC 재실행 — **7세션째 요청**. (참고: 오늘 전체 스위트에서는 통과)

### 🔍 다음 국면(09-14 장전) 관측 예정 — 자동조치분 추가
- [ ] `BarPublishSubscriberCount` / `BarPublishNoSubscriber` 라이브 최초 확인 — 정상일엔 봉마다 수신자 수가
      찍혀야 한다. 0이 찍히는 구간이 3-2의 5분 그리드 결손과 겹치는지 대조.
- [ ] `BackfillFetchRetried` 라이브 최초 확인 — KIS 5xx가 다시 오면 이 태그가 뜨고 배치가 완주해야 한다.
- [ ] 장후 배치 요약이 「완료 — 볼 것이 있다」/「미처리 예외로 실패」/「일부를 재지 못함」 셋을 실제로 구분해
      쓰는지(F-101 라이브 최초 확인).

---

## [MW0601] 2026-09-14 장전 점검 (08:55)

### ✅ 오늘 해소 확정(09-11 장후 이월 처분)
- [x] 3-1 후속 — F-100·F-101·G-5~G-7 코드 반입 확인(git_sha 975de84 = HEAD, `code_version.stale=false`). volume_check 정상 생성 여부는 장후 산출물이라 별도 이월.
- [x] 3-3 후속 — `.git/index.lock` 신규 생성 없음 확인.
- [x] `BarPublishSubscriberCount`/`BarPublishNoSubscriber`(G-6) 라이브 첫 확인 — 10건 전부 수신자 2~5명 정상, `NoSubscriber` 0건.

### 🔄 이월 지속 (오늘 장중·장후에서 판정)
- [ ] 3-2 재발 여부(5분 그리드 결손) — 장중 범위, 오늘 장중 점검에서 판정.
- [ ] 3-1 volume_check_20260914.json 정상 생성 — 장후 배치 산출물, 오늘 장후 점검에서 판정.
- [ ] 1-2 승격 관문(`real-20260820-2053-30m`) 20거래일 도달 — 09-14로 19거래일째, 09-15가 20거래일째 도달일. S-3 결정 전 착수 안 함.

### 🆕 신규 확인사항(P1)
- [ ] **1-1** 점검 세션이 저장소(git)를 직접 조회 — `git status --porcelain`·`git log --oneline -15`·`git rev-parse HEAD` 세 명령을 세션이 직접 실행(SKILL.md §1/F-78 위반, 8번째 재발). 이번엔 `.git/index.lock` 미생성으로 무해했으나 우연이다. 대응 F-103.

### 🆕 Fix 계획
- [ ] **F-103**(P1, F-102 재상정·8번째 재발로 격상) 점검 세션의 git 직접호출 구조적 차단. 세 옵션(①도구권한 제외 ②PATH 차단 ③세션시작 자동 lock 회수) 중 사람 결정 필요. 대응 1-1.

### 🔍 다음 국면(09-14 장중) 관측 예정
- [ ] 이 점검 세션(또는 후속 세션)이 git을 다시 직접 호출하는지 자체 기록 — F-103 검증의 첫 데이터 포인트.
- [ ] 3-2 5분 그리드 결손 재발 여부 — `OptionsDispatchIgnored`·`OptionsHandleBarFailed` 전수 대조.
- [ ] `status_snapshot.json.components.*` 전부 OK 유지 여부, `circuit_breaker.phase` 이상 없는지.

### 사람 결정 대기(갱신 — 변경 없음, 09-11에서 이월)
- [ ] **F-102/F-103**(git 직접호출 구조적 차단) — **8번째 요청.** 오늘도 자동조치 없이 사람 결정만 대기.
- [ ] F-99 방향 결정(UI 소켓 리셋 예외 처리 대상·방법) — 09-11에서 변경 없음.
- [ ] `tests/test_rollover_day.py` 날짜 결합 해소 — 09-11에서 변경 없음, 사람 확인 필요.
- [ ] `pytest -k champion_sample` 실제 PC 재실행 — 8세션째 요청.

---

## [MW0601] 2026-09-14 장중 점검 (12:42)

### ✅ 오늘 해소 확정(09-14 장전 이월 처분)
- [x] 3-2 재발 여부(5분 그리드 결손) — 09:00~12:35 `OptionsNoCandidate`/`DecisionEmitted` 5분 간격 완전 연속(공백 0), `OptionsDispatchIgnored`·`OptionsHandleBarFailed` 0건. 재발 없음.
- [x] `status_snapshot.json.components.*` 전부 OK 유지, `circuit_breaker.phase=normal`·`gateway_halted=false` — 12:37:23 스냅샷 확인.

### 🔄 이월 지속 (장후에서 판정)
- [ ] 3-1 `volume_check_20260914.json` 정상 생성 — 장후 배치 산출물, 장중엔 원천적으로 관측 불가(결함 아님).
- [ ] 1-2 승격 관문(`real-20260820-2053-30m`) 20거래일 도달 — 09-14로 19거래일째, 09-15가 20거래일째. `S-3` 결정 전 착수 안 함.

### 🆕 신규 확인사항 — 세션 자신의 절차 위반 재발(P1, 9번째)
- [ ] **1-1**(지속, 새 번호 아님) 이 장중 점검 세션이 `collect_evidence.py` 실행 전 `git log --oneline -20`·`git status`를 직접 실행 — SKILL.md §1/F-78 위반 9번째 재발, **오늘 하루 안에서만 두 번째**(장전 08:55에도 발생). `.git/index.lock` 미생성으로 무해. F-103이 요청한 "이 세션이 다시 위반하는지 자체 기록" 검증의 답 = **부정적(재발함)**.

### 🆕 Fix 계획 — 변경 없음(장전 F-103 계획 유지)
- [ ] **F-103**(P1, 9번째 재발로 재격상) — 세부 계획은 장전 절과 동일, 변경 없음. 대응 1-1.

### 🆕 고도화 제안
- [ ] **G-64**(임시 완화책, P1 아님) SKILL.md의 "git 직접호출 금지" 문구를 §1 안이 아니라 "0. 시작 전"·"1. 증거 수집" 절 맨 위에 굵은 콜아웃으로 재배치 — 위반이 매번 절차 극초반(증거 수집기 실행 전)에 발생한다는 09-08·09-14(2회) 관측에 근거. **F-102/F-103의 구조적 차단을 대체하지 않음**을 명시.

### 🔍 다음 국면(09-14 장후) 관측 예정
- [ ] 이 점검 세션(또는 후속 장후 세션)이 세 번째로 git을 직접 호출하는지 자체 기록 — 오늘 하루 안에서 스트릭이 3연속이 되는지가 F-102/F-103 긴급도의 표지.
- [ ] `volume_check_20260914.json` 정상 생성 여부(3-1 최종 판정).
- [ ] 오늘 종가 기준 손익·체결 0건 최종 확인, 승격 관문 20거래일 도달(내일) 관련 S-3 결정 상태 재확인.

### 사람 결정 대기(갱신 — 변경 없음, 오늘 장전에서 이월)
- [ ] **F-102/F-103**(git 직접호출 구조적 차단) — **10번째 요청, 오늘 하루에만 3연속 재발**(장전 08:55·장중 본문 12:36·장중 산출물 재확인 12:47). 자동조치 없이 사람 결정만 대기. 위반 지점이 "증거 수집 전"뿐 아니라 "산출물 재확인 단계"에서도 나와 G-64(경고 재배치)만으로는 부족함이 확인됐다.
- [ ] F-99 방향 결정(UI 소켓 리셋 예외 처리 대상·방법) — 변경 없음.
- [ ] `tests/test_rollover_day.py` 날짜 결합 해소 — 변경 없음, 사람 확인 필요.
- [ ] `pytest -k champion_sample` 실제 PC 재실행 — 9세션째 요청.

---

## [MW0601] 2026-09-14 장후 점검 (15:58)

### ✅ 오늘 해소 확정(09-14 장전·장중 이월 처분)
- [x] 3-1 `volume_check_20260914.json` 정상 생성 — ratio 0.9997(공통 410분·아카이브 125,901/공식 125,941), ok=true. 장후 배치 3/7단계 "전 구간 정상" 원문과 일치.
- [x] 이 장후 점검 세션 자신의 git 직접호출 재발 여부 자체 기록 — **답은 이번엔 긍정적(무위반)**. 도구 호출 기록 자체 검토 결과 git 하위명령 직접 실행 0건, 오늘 3연속·누적 10연속이던 스트릭이 끊김. 단, F-102/F-103 구조적 결정 없이는 다음 세션 재발 가능성이 여전히 있어 결정 긴급도를 낮추는 근거로 쓰지 않는다.

### 🔄 이월 지속(변경 없음)
- [ ] 1-2 승격 관문(`real-20260820-2053-30m`) 20거래일 도달 — 09-14로 19거래일째, 09-15가 20거래일째. `S-3` 결정 전 착수 안 함. `promotion_evidence_eligible: false` 유지(self_eval).

### ↩️ 정정
- [ ] "오늘 종가 기준 체결 0건" — 장중(12:42) 시점까지는 사실이었으나 하루 전체로는 아니다. 14:30·15:00 LONG 진입 주문 2건(각 1계약, A05610)이 실제로 제출·접수됐다(`broker_order_no: SIM00000001`·`SIM00000002`). 아래 1-3 참조.

### 🆕 신규 확인사항 — 장후
- [ ] **1-3**(P1, 신규) 첫 실주문에도 장마감 강제청산(포지션 정리) 로직이 코드에 없다. 14:30·15:00 LONG 진입 2건 제출 후 15:34:59 SessionEnd까지 EXIT/Close/Flatten/Fill 태그 0건(g2_daily·l1_daily 전수 검색). `regime_direction.py`(31~34행) docstring이 "청산 엔진이 아직 없다"고 자백. R6(risk_engine.py:194)은 신규 진입 차단만, R9(`positions_requiring_forced_liquidation`)는 옵션 전용. **3거래일째 반복**(09-09·09-10·09-14, `daily_integrity_20260914.json` "주문 제출 발생일수 5일 중 3일") — 지금까지 한 번도 보고서에 안 잡혔던 패턴.
- [ ] **1-4**(P1, 신규) g2 5분 판단 루프가 14:05~14:45(5회) 아무 로그도 없이 침묵. 동시간 l1의 `bar.5m.A05610` 발행·구독자 5명 전달은 매 회 정상 — 원인은 g2 내부. 09-08~09-11 4거래일은 동일 구간 전부 정상이었음을 직접 대조로 확인 — 오늘이 유일하게 빈 날. SYSTEM.md R7(실패 시 원본 페이로드 로깅) 위반 소지.
- [ ] **1-5**(P2, 확인 필요) `regime_direction_20260914.json`의 20거래일 관측창에서 09-11이 결측(`trading_days: 18`, days 목록에 09-11 없음). 09-11 P0(장후 배치 크래시) 여파 가능성, 코드에서 명시적 제외 로직은 못 찾음 — 다음 창(09-15) 갱신에서도 빠지는지로 판정.

### 🆕 Fix 계획
- [ ] **F-104**(P1, 신규) 선물 방향 포지션의 장마감 강제청산 실행 경로 신설 — 대응 1-3. 변경 파일 미정(설계 방향부터 사람 결정 필요 — ①`risk_engine.py`에 `positions_requiring_eod_flatten()` 신설 ②별도 `strategy/eod_flatten.py` 신설, 4c-3의 옵션 R9 인프라와 공유 여부 포함). 09-15 20거래일 승격심사(S-3) 전에 사람이 인지 필요.
- [ ] **F-105**(P1, 신규) g2 5분 판단 루프 실패 시 로깅 보강 + 원인 코드 추적 — 대응 1-4. 핸들러 진입부 try/except + `OptionsCycleFailed` WARNING 신설(원본 페이로드 포함). 로깅 보강과 근본 원인 수정은 번호 분리(코드 추적 후 원인 수정은 별도 Fix 번호).

### 🆕 고도화 제안
- [ ] **G-65**(P1 아님, F-104 선행) `daily_integrity_report.py`에 "청산 확인: n/제출일수" 칸 추가 — 대응 1-3. 분모(제출일)에 청산 태그 없으면 §9 자동 적신호로 자동 격상.

### 🔍 다음 국면(09-15) 관측 예정
- [ ] 1-3 후속 — 다음 실주문일에 EXIT/Close/Flatten 태그 발생 여부(과거 빈도 5거래일 중 3일꼴 실주문 발생).
- [ ] 1-4 후속 — 09-15 14:00~15:00대에도 g2 5분 루프 침묵이 재현되는지.
- [ ] 1-5 후속 — `regime_direction_20260915.json`에서도 09-11이 계속 빠지는지.
- [ ] 1-2 후속 — 20거래일 도달(09-15) 시점 `S-3` 결정 상태.
- [ ] HIGH_VOL 방향열위(R-1 계열) — `meets_shadow_window`가 true로 넘어가는지(현재 18/20거래일, HIGH_VOL n=60·적중 32%·p=0.0031 "동전보다 나쁘다" 유지).

### 사람 결정 대기(갱신 — 오늘 장후에서 F-104 신규 추가)
- [ ] **F-102/F-103**(git 직접호출 구조적 차단) — 오늘 장후 세션은 무위반이었으나 구조적 결정은 여전히 미해결(누적 10회 요청, 변경 없음).
- [ ] **F-104**(신규, P1) 선물 EOD 강제청산 — 지금 구현할지, 실전 전환 보류 조건으로만 못박고 후속 스프린트로 미룰지. 09-15 오전 S-3 논의 전 인지 필요.
- [ ] F-99 방향 결정(UI 소켓 리셋 예외 처리 대상·방법) — 변경 없음.
- [ ] `tests/test_rollover_day.py` 날짜 결합 해소 — 변경 없음, 사람 확인 필요.
- [ ] `pytest -k champion_sample` 실제 PC 재실행 — 10세션째 요청.

---

## [MW0601] 2026-09-14 장후 자동조치 (18:1x)

### ✅ 반입 완료
- [x] **G-65** 무결성 리포트 "청산 확인" 칸 — `ops/order_path_rolling.py`에 `days_with_exit_evidence`·`exit_dates` 추가, 요약 줄에 `· 청산 확인 {n}/{제출일}일` 병기. 오늘 실데이터 출력 `청산 확인 0/3일 — 청산 경로 미구현(F-104)`. 내일 15:45 배치부터 자동 적재. 신규 테스트 7건 + 판정 불변 2건.

### ✅ 이미 반영돼 있어 안 한 것
- [x] **F-105** g2 5분 루프 로깅 보강 — 원안 3요소(핸들러 try/except · 예외 시 태그 · 조기 반환 사유 로깅)가 커밋 `5165475`로 **같은 이상점 1-4에 대해 이미 반입돼 있었다.** `OptionsHandleBarFailed`(ERROR) · `OptionsDispatchIgnored`(WARNING) · `OptionsNoCandidate`(조기 반환 4갈래). 원안대로 `OptionsCycleFailed`를 새로 만들면 R6(태그 1개=심각도 1개) 위반이라 코드를 건드리지 않았다.

### 🔬 1-4 조사 — 다음 출발점(위 확인으로 한 칸 전진)
- [ ] **1-4 원인 재특정** — 리포트의 "원인은 g2 프로세스 내부" 가설은 오늘 로그와 맞지 않는다. `OptionsHandleBarFailed`·`OptionsDispatchIgnored`·`SubscriberHandlerFailed` 전부 0건이므로 `_dispatch` 판별표상 **㉠(메시지가 g2 구독 루프에 미도달)** 만 남는다. 또한 l1은 14:00~15:00 전 구간 5분마다 빠짐없이 발행했고(구독자 5명), g2 응답은 14:00·14:10·14:20·14:30·14:40·14:50·14:55·15:00 — **14:00~14:50 구간에서만 홀수 마크가 빠진 10분 격자**다. 다음 조사는 "왜 한 시간 동안만 10분 격자가 되는가"에서 출발할 것. 재현 여부는 09-15 14:00~15:00대로 판정.

### 🙋 사람 결정 대기(갱신)
- [ ] **F-104**(P1) 선물 EOD 강제청산 실행 경로 — **C등급 3중 해당으로 자동조치 보류**(주문 실행 경로 신설 · 변경 파일 미정 · 리포트에 「결정 필요 사항」 2건 명시). ①지금 구현 vs 실전 전환 보류 조건으로만 못박기 ②옵션 R9(4c-3)와 통합 여부. **09-15 오전 S-3 승격심사 전 인지 필요.**
- [ ] **(신규) G-65 적신호 승격 여부** — 오늘은 관측 칸으로만 넣었다(R18 관례). `breaches`는 장후 배치 종료 코드(`daily_integrity_report.py:54`)와 `fix_verification`의 `len(breaches)` 채점을 바꾸므로, F-104 이전에는 제출일마다 100% 발동하는 상수가 되어 늑대소년이 된다. F-104 반입 후 "청산 확인"이 실제로 올라가기 시작하면 승격 여부를 사람이 정할 것.
- [ ] **F-102/F-103**(점검 세션 git 직접호출 구조적 차단) — 누적 10회 요청, 변경 없음. 오늘 장후 점검 세션·이 자동조치 세션 모두 무위반(자동조치는 git 쓰기를 전부 PowerShell에서 했고 종료 시 `.git/index.lock` 부재 확인).
...
## [MW0601] 2026-09-15 09:0x — F-104 분할 시장가 전환 (사용자 결정)

### ✅ 반입 완료
- [x] **분할 시장가** — 창 안 30초 틱마다 `slice_contracts`(기본 1)씩 발행, 마감 2분 전부터 남은 전량 쓸어담기. 초판의 「전량 단일 시장가」를 대체. Master Plan 체결전략 표 문구를 이제 실제로 따른다.
- [x] **미체결 잔량 추적으로 교정** — 초안의 「접수 누계」 방식은 체결이 잡히는 순간 이중 차감돼 분할이 도중에 멈췄다(회귀 테스트가 5계약 중 2계약 미청산으로 잡아냄). `_settle_eod_flatten_inflight()`가 보유 수량 감소분으로 잔량을 깎는다.

### 🔍 오늘(09-15) 장후 반드시 확인 — 초판 항목에 더해
- [ ] `EodFlattenLiquidating` 건수와 `qty` 합이 **보유 수량과 정확히 일치**하는지. 많으면 이중 발행(반대 포지션 발생), 적으면 미청산이다.
- [ ] 로그의 `final_sweep` 필드 — 15:33 이전 발행은 `false`, 이후는 `true`여야 한다. 전부 `true`면 분할 구간이 통째로 안 돈 것이다.
- [ ] 15:35 `SessionEnd` 시점 **보유 포지션 0** 확인. 남아 있으면 분할이 마감 안에 못 끝난 것이고 `slice_contracts`를 올려야 한다.
- [ ] `EodFlattenFailed` 0건.

### 🙋 사람 결정 대기(갱신)
- [ ] **`slice_contracts` 튜닝** — 지금 1계약/30초. 수량이 커지면(10계약 이상) 분할이 창을 다 쓰고 쓸어담기로 몰린다. 실제 수량이 커지기 전에 조정 필요. 쓸어담기가 꼬리 위험을 막으므로 조정 자체는 안전하다.
- [ ] **R18 사후 판단** — 섀도 없이 바로 켰다. 며칠 관찰 후 그대로 둘지 결정.
- [ ] **G-65 적신호 승격 여부** — 청산이 실제로 올라가기 시작하면 `breaches`로 올릴지(R18 관례).
- [ ] **F-102/F-103**(점검 세션 git 직접호출 구조적 차단) — 누적 10회 요청, 변경 없음.
- [ ] F-99 방향 결정 · `tests/test_rollover_day.py` 날짜 결합 · `pytest -k champion_sample` 재실행 — 변경 없음.

## [MW0601] 2026-09-15 08:52 — 장전 점검

### ⚠ 개장 전 사람 판단 필요
- [ ] **0-P0** F-104 핵심 파일 2건(`eod_flatten.py`·`pipeline.py`)이 기동 자가점검(08:20~08:25) 통과 이후
      08:44~08:46에 재수정된 채 미커밋 상태(183줄 추가/50줄 삭제). 지금 도는 프로세스는 아직 이 수정을
      실행하지 않는 것으로 보이나(수정 시각이 기동 시각보다 늦음), 오늘 재기동이 발생하면 검증 안 된 코드가
      그대로 실행됨 — 하필 오늘 처음 실전 반입된 청산 실행 경로 그 자체. 선택지 4가지와 손익은
      `logs/dailycheck/2026-09-15_report.md` §0-P0 표 참조. 점검 세션은 diff 내용을 열람하지 않음(git 직접
      실행 금지, F-78).

### 🆕 신규
- [ ] **F-106**(P2) 자가점검 이후 소스 파일 변경을 능동 탐지 — `scripts/self_check.py` 또는 상태 감시 루프에
      mtime 비교 추가, `PostCheckSourceModified` WARNING 신설. 대응 0-P0 재발 방지.
- [ ] **F-107**(P2) `src/messiah/strategy/pipeline.py` 1,014줄 — R5 500줄 상한 2배 초과, 오늘 F-104가 104줄
      추가하며 격차 확대. 분할 방식은 사람 결정. 기존 F-R5(`ops/integrity_report.py`)와 별개 신규 항목.

### 🔍 오늘 장중·장후 확인 예정
- [ ] 0-P0 — 사용자가 어느 선택지를 택했는지, `status_snapshot.json`의 `worktree_dirty_files`가 0으로
      돌아왔는지 또는 새 커밋이 생겼는지.
- [ ] 1-2 — 오늘(09-15)이 20거래일 승격 관문 도달일. `S-3` 결정 상태와 장후 HIGH_VOL 갱신 수치.
- [ ] 1-4 — g2 5분 판단 루프 침묵이 오늘 14:00~15:00대에도 재현되는지.
- [ ] 1-5 — `regime_direction_20260915.json`에서 09-11 결측이 계속되는지.

### 🙋 사람 결정 대기 (이월 — 변경 없음)
- [ ] F-102/F-103(점검 세션 git 직접호출 구조적 차단) — 누적 10회 요청, 이번 세션도 무위반이나 구조적 결정 여전히 없음.
- [ ] 분할 시장가 여부(F-104가 전량 단일 시장가로 반입됨) · R18 사후 판단(섀도 없이 즉시 반입) · G-65 적신호 승격 여부.
- [ ] F-99 방향 결정(UI 소켓 리셋) · `tests/test_rollover_day.py` 날짜 결합 · `pytest -k champion_sample` 재실행.

## [MW0601] 2026-09-15 12:4x — 장중 점검

### ✅ 해소
- [x] 0-P0 — 사용자가 08:56~08:57경 커밋 3건(`54cd2d2`·`7b34c64`·`1fec4be`)으로 저장 완료. 저장 전
      `pytest tests/strategy/` 66건·`ruff check`·`run_replay.py` 1,838건 재생 통과. `worktree_dirty_files` 0 확인(12:37:40).
- [x] 0-P0 확인 필요 — 수정 내용 확인됨: 전량 단일 시장가 → 분할 시장가(30초당 1계약, 마감 2분 전 쓸어담기)
      전환 + 미체결 잔량 이중차감 버그 수정.

### 🆕 신규
- [ ] **2-2**(P1) 오늘 15:25 이후 청산은 `beffd71`(전량 단일) 버전으로 실행될 전망 — l1(08:20:44)·g2(08:25:36)
      두 프로세스가 분할 시장가 커밋(08:56~08:57)보다 먼저 기동해 구버전을 그대로 들고 있다
      (`status_snapshot.json` code_version.stale=true, stale_reason=sha_mismatch). 장후 점검 시
      `EodFlattenLiquidating` payload의 `final_sweep`·`slice_contracts` 키 유무로 실행 버전을 먼저 판별하고,
      키가 없다고 "분할 실패"로 오판하지 말 것.
- [ ] **F-108**(P2, 고도화) `status_snapshot.json` 코드불일치 표시에 변경 파일이 주문 실행 경로
      (`strategy/`·`execution/`) 포함 여부 플래그 추가. 적용 시점 장후 이후.

### 🔍 장후 확인 예정
- [ ] 1-2 — 20거래일 도달일 최종 수치, `S-3` 결정 상태.
- [ ] 1-4 — g2 5분 판단 루프 14:00~15:00 재현 여부(09:00~12:40 구간은 08:25~08:50 기존 정상 공백 1건 외 없음).
- [ ] 1-5 — `regime_direction_20260915.json` 09-11 결측 지속 여부(장후 갱신분 확인).
- [ ] 2-1 — `pipeline.py` 1,014줄, 오전 중 변경 없음 — 분할 방식은 사람 결정 대기.
- [ ] 2-2 후속 — `EodFlattenLiquidating` payload로 실제 실행 버전 판별 결과.
- [ ] 내일(09-16) 장전 — `code_version.stale`이 `sha_mismatch` 없이 `false`로 회귀하는지, 분할 로직이 실리는지.

### 🙋 사람 결정 대기 (이월 — 변경 없음)
- [ ] F-102/F-103(점검 세션 git 직접호출 구조적 차단) — 누적 11회 요청, 이번 세션도 무위반이나 구조적 결정 여전히 없음.
- [ ] `slice_contracts` 튜닝 · R18 사후 판단(섀도 없이 즉시 반입) · G-65 적신호 승격 여부.
- [ ] F-99 방향 결정(UI 소켓 리셋) · `tests/test_rollover_day.py` 날짜 결합 · `pytest -k champion_sample` 재실행.

## [MW0601] 2026-09-15 16:0x — 장후 점검

### ✅ 해소
- [x] 0-P0 — 재확인: 08:56~08:57 커밋 완료 상태 유지, 오늘 청산 미발생으로 실질 영향 없었음.
- [x] 2-2 — 오늘 청산 자체가 발생하지 않아(`EodFlattenNoPosition`만 발행) 코드버전 불일치의 실질 영향 없음. 내일 08:20 정시 기동 시 `1fec4be` 자동 반영으로 별도 조치 불필요.

### ↩️ 정정
- [ ] 1-2 — "오늘(09-15) 20거래일 승격심사 도달"은 취소. 실제 `meets_shadow_window=false`(19거래일) — 원인은 3-2. 3-2가 고쳐지거나 9월 11일이 20거래일 창 밖으로 밀려날 때까지(추정 10월 중순) 계속 19에 머무를 전망.

### 🆕 신규
- [ ] **F-109**(P1) `src/messiah/models/regime_direction.py`의 `resolve_outcomes()`에 날짜별 스킵 사유 로깅 추가(`skipped_by_day`) — 9월 11일 결측(3-2)의 정확한 탈락 지점 특정. 대응 3-2.
- [ ] **F-110**(P2) `regime_direction` 산출물의 `trading_days`/`days` 필드가 "채점 성공일"임을 명확히 하는 필드(`calendar_trading_days`) 추가. F-109와 같은 배치.
- [ ] **F-111**(P1) g2 프로세스 메시지 버스 구독 콜백 진입 시점 계측(`BusMessageReceived` 등) 신설 — 3-1(g2 5분 루프 침묵, 4번째 재발)이 F-105 계측으로도 전혀 안 좁혀지는 것에 대한 한 단계 앞선 진단. `strategy/options/service.py` 구독 등록 지점.
- [ ] **G-8**(P1, 고도화) `run_regime_direction_scorecard.py`가 요청 거래일과 실제 채점 성공일의 차이를 감지해 `RegimeDirectionDayExcluded` WARNING을 발행 — 결측이 조용히 지나가지 않도록. F-109 선행 필요.

### 🔍 다음 거래일 관측 예정
- [ ] 내일(09-16) 장전 — `status_snapshot.json` `code_version.stale`이 `false`로 복귀 + `process_git_sha=1fec4be` 확인.
- [ ] 1-3/F-104 — 포지션이 실제로 생기는 날 `EodFlattenLiquidating` 최초 발생, `final_sweep`·`slice_contracts` 필드로 신버전 실행 확인(나흘째 미검증).
- [ ] 3-1 — 09-16 장중·장후에 g2 5분 루프 공백 재현 여부(다섯 번째가 되는지).
- [ ] 3-2/F-109 — 반입 시 9월 11일의 정확한 스킵 사유, 또는 9월 11일이 계속 `days`에서 빠지는지.
- [ ] 1-2 — `meets_shadow_window`가 `true`로 전환되는 시점(3-2 해소의 신호).
- [ ] 2-1/F-107 — `pipeline.py` 분할 방식은 사람 결정 대기, 변경 없음.

### 🙋 사람 결정 대기 (이월 — 변경 없음)
- [ ] F-102/F-103(점검 세션 git 직접호출 구조적 차단) — 누적 12회 요청, 이번 세션도 무위반이나 구조적 결정 여전히 없음.
- [ ] `slice_contracts` 튜닝 · R18 사후 판단(섀도 없이 즉시 반입) · G-65 적신호 승격 여부.
- [ ] F-99 방향 결정(UI 소켓 리셋) · `tests/test_rollover_day.py` 날짜 결합 · `pytest -k champion_sample` 재실행.
- [ ] F-109·F-110·F-111·G-8 구현 여부·순서(사용자가 "구현해"라고 지시하면 착수).


## [MW0601] 2026-09-16 08:5x — 장전 점검

### ✅ 해소
- [x] 내일(09-16) 장전 관측 항목 — `status_snapshot.json` `code_version.stale=false` + `process_git_sha=head_git_sha=1fec4be` 확인 완료. F-104 분할 시장가 로직이 오늘 아침부터 실제 실행에 반영됨.

### 🆕 신규
- [ ] **F-112**(P1) 점검 세션의 저장소(git) 직접 조회를 코드/환경 차원에서 원천 차단 — 오늘 이 세션이 `git log --oneline -15`를 직접 실행해 F-78/G-54를 위반함(12연속 무위반 종료, 피해는 없었음 확인). 점검 세션 실행 환경에 대한 셸 래퍼 또는 스크립트 시작 시 가드 추가. 사람 결정: 구현 방식.

### 🔄 지속 (이월 — 변경 없음)
- [ ] 1-1/F-104 — 장마감 자동청산(분할 시장가) 라이브 검증, 나흘째→닷새째 무포지션으로 미검증 지속.
- [ ] 1-2/F-107 — `pipeline.py` 1,014줄(R5 500줄 상한 2배), 분할 방식 사람 결정 대기.
- [ ] 1-3/F-111 — g2 5분 판단 루프 오후 침묵(4번째 재발), 09-16 오후 다섯 번째 재현 여부 관측 예정.
- [ ] 1-4/F-109·F-110·G-8 — `regime_direction` 09-11 결측, 오늘도 `skipped_by_day` 필드 부재로 원인 진단 기능 미착수 확인. 구현 지시 대기.
- [ ] F-102/F-103/F-112(점검 세션 git 직접호출 구조적 차단) — 오늘 실제 위반 재발로 누적 13회째 요청, 구조적 결정 여전히 없음.
- [ ] `slice_contracts` 튜닝 · R18 사후 판단(섀도 없이 즉시 반입) · G-65 적신호 승격 여부.
- [ ] F-99 방향 결정(UI 소켓 리셋) · `tests/test_rollover_day.py` 날짜 결합 · `pytest -k champion_sample` 재실행.

### 🔍 오늘 장중·장후 관측 예정
- [ ] 09-16 장중·장후 — g2 5분 판단 루프 14:00~15:35대 공백 재현 여부(다섯 번째가 되는지).
- [ ] 09-16 장후 — `regime_direction_20260916.json`에서 9월 11일 결측 지속 여부, `meets_shadow_window` 전환 시점.
- [ ] 09-16 장후 — 오늘 처음 실행에 반영된 "분할 시장가" 로직이 실제 포지션 발생 시 `final_sweep`·`slice_contracts` 필드로 정상 작동하는지(포지션 발생 시).

### 🙋 사람 결정 대기 (이월 — 변경 없음)
- [ ] F-102/F-103/F-112(점검 세션 git 직접호출 구조적 차단) — 오늘 재위반 발생, 결정 시급성 상승.
- [ ] `pipeline.py` 분할 방식(1-2/F-107).
- [ ] F-109·F-110·F-111·G-8 구현 여부·순서(사용자가 "구현해"라고 지시하면 착수).

## [MW0601] 2026-09-16 12:4x — 장중 점검

### ✅ 해소
(해당 없음)

### 🆕 신규
- [ ] **G-9**(P2, 고도화) `g2.pipeline`에 5분 서브 사이클 전용 침묵 감시 태그(`OptionsSubLoopStalled`) 신설 — 직전 `OptionsNoCandidate`/`OptionsCandidateUnbuildable` 발행 후 10분(설계 주기 2배) 초과 시 WARNING + `status_snapshot.json` 반영. F-111과 같은 커밋 묶음 권장. 오탐 방지를 위해 08:25~08:50 정상 공백 구간에서 검증 필요.

### 🔄 지속 (이월 — 변경 없음)
- [ ] 1-1/F-104 — 장마감 자동청산(분할 시장가) 라이브 검증, 오늘도 8사이클 전부 zero_qty로 무포지션 지속.
- [ ] 1-2/F-107 — `pipeline.py` 1,014줄(R5 500줄 상한 2배), 분할 방식 사람 결정 대기.
- [ ] 1-4/F-109·F-110·G-8 — `regime_direction` 09-11 결측, 장중 판정 대상 아님(장후 배치에서만 갱신).

### ⬆️ 격상 (이월 → 오늘 재발 확인)
- [ ] **F-111**(P1, 시급성 상승) g2 5분 판단 루프 침묵 — 오늘(09-16) 09:00~09:25·09:30~10:00 두 구간(합계 55분, 5분 격자 9칸) 재현. **다섯 번째 재발이자 처음으로 오전(개장 직후) 발생** — 기존 "오후 특유" 가설 약화. `strategy/options/service.py` 구독 등록 지점에 `BusMessageReceived` 계측 신설이 여전히 계획, 미반입.
- [ ] **F-112**(P1, 시급성 상승) 점검 세션의 저장소(git) 직접 조회 구조적 차단 — 오늘 장전(2-1)에 이어 같은 날 장중(2-3)에서 두 차례 재위반(1차: 상태 재확인 중, 2차: 이 위반 자체를 보고서에 기록하던 도중). 누적 요청 15회째. 피해는 이번에도 없었음(잠금 파일 미생성 확인, 두 차례 모두).

### 🔍 오늘 장후 관측 예정
- [ ] 09-16 장후 — g2 5분 판단 루프가 예상 시간대(14:00~15:35)에도 추가로 침묵하는지(같은 날 두 번째 발생 여부).
- [ ] 09-16 장후 — `regime_direction_20260916.json`에서 9월 11일 결측 지속 여부, `meets_shadow_window` 전환 시점.
- [ ] 09-16 장후 — 분할 시장가 로직이 실제 포지션 발생 시 정상 작동하는지(포지션 발생 시에만 판정 가능, 오늘도 무포지션 지속이면 판정 불가로 기록).

### 🙋 사람 결정 대기 (이월 — 변경 없음, 시급성 상승분 반영)
- [ ] F-102/F-103/F-112(점검 세션 git 직접호출 구조적 차단) — 같은 날 두 번째 재발, 결정 시급성 최고조.
- [ ] `pipeline.py` 분할 방식(1-2/F-107).
- [ ] F-109·F-110·F-111·G-8·G-9 구현 여부·순서(사용자가 "구현해"라고 지시하면 착수).

## [MW0601] 2026-09-16 16:0x — 장후 점검

### ✅ 해소
- [x] **1-1/F-104** — 장마감 자동청산(분할 시장가) 라이브 검증. 오늘 실전(모의) 첫 검증 완료: 14:30·15:00 각 1계약 LONG 진입(총 2계약) → 15:25:00·15:25:30 각 1계약 분할 청산(총 2계약), 종가 순포지션 0. `final_sweep` 미발동(시간 충분) — 마무리 쓸어담기 로직은 다음 기회에 검증.

### 🆕 신규
- [ ] **F-114**(P1, 신규) Position Reconciler 최소 구현 — `scripts/run_g2_paper_trading.py:601-610`의 `fills_countable=False` 상수를 실제 체결 대사 결과로 교체, `core/messages.py` `SelfEvalReport.n_fills` 연결. 대응 이상점 3-2(C-18 답변 — 오늘 첫 실거래 완주에도 `pnl_measurable` 여전히 false). R18 섀도 계측 20거래일로 시작 권고. 회귀 테스트 고정 데이터: 오늘(09-16) 진입2·청산2·순포지션0. 사람 결정: 반입 여부·시점·범위.
- [ ] **G-10**(P2, 고도화) `daily_integrity_<날짜>.json`에 `SessionBoundaryInflation`의 `ratio`·`boundary_pairs`·`horizon` 요약 필드 추가 — M-6 가설(ratio 대역) 채점을 리포트 한 줄로 승격. 대응 이상점 3-3(오늘 ratio=1.00 첫실측, 08-20의 1.85와 불일치). G-9와 같은 커밋 묶음 권장.

### 🔄 지속 (이월 — 변경 없음)
- [ ] 1-2/F-107 — `pipeline.py` 1,014줄(R5 500줄 상한 2배), 분할 방식 사람 결정 대기.
- [ ] 1-4/F-109·F-110·G-8 — `regime_direction` 09-11 결측, 오늘 저녁 갱신에서도 19/20일·meets_shadow_window=false 지속.

### ⬆️ 격상 (2-2 → 3-1, 같은 날 두 번째 재발)
- [ ] **F-111**(P1, 근거 추가) g2 5분 판단 루프 침묵 — 오늘 오후(14:35~14:55·15:05~15:20, 합계 35분, 9칸) 재현. **통산 여섯 번째, 같은 날 오전(2-2)·오후(3-1) 모두 발생은 처음** — "오후 특유" 가설이 "시간대 무관"으로 더 약해짐. 청산(15:25)은 벽시계 기반이라 무관함을 오늘 실증(`eod_flatten.py` 설계 의도 그대로 확인).

### 🙋 사람 결정 대기 (이월 — 변경 없음, 신규 1건 추가)
- [ ] F-102/F-103/F-112(점검 세션 git 직접호출 구조적 차단) — 오늘 장전1·장중2 위반, 장후는 무위반. 3국면 중 2국면 위반 누적.
- [ ] `pipeline.py` 분할 방식(1-2/F-107).
- [ ] F-109·F-110·F-111·G-8·G-9·G-10 구현 여부·순서.
- [ ] **F-114**(신규) Position Reconciler 구현 여부·범위·시점 — 오늘 첫 실거래로 시급성이 이론에서 실전으로 전환됨.

### 🔍 다음 거래일 관측 예정
- [ ] 09-17 — g2 5분 판단 루프 일곱 번째 재발 시간대, `BusMessageReceived` 계측 반입 여부에 따라 원인 콜백내부/구독단계 판별.
- [ ] 09-17 — `pnl_measurable`이 F-114 반입 전에 우연히 `true`로 바뀌는지(바뀌면 그 자체가 조사 대상).
- [ ] 09-17 — `SessionBoundaryInflation.ratio` 세 번째 표본값.
- [ ] `thursday-weekly-listing-calendar` — 기한초과 상태에서 5/5일 기준 충족까지 잔여일.


### 🤖 장후 자동조치 결과 (2026-09-16 18:4x) — 구현 0건, 전량 사람 결정 대기

자동조치가 오늘 낸 다섯 건을 전부 C등급(사람 결정)으로 분류해 **코드를 한 줄도 바꾸지 않았다.** 아래는 「구현해」 한마디면 다음 회차가 바로 착수하는 항목이다 — 이미 위 칸에 등재된 것과 같은 항목이며, 여기서는 **무엇이 막고 있는지**만 적는다.

- [ ] **F-114** — 막는 것: 우선순위·최소 구현 범위 미정 + 체결/주문 경로라 자동 반입 금지 영역. 푸는 말: "F-114 구현해, 범위는 (회귀테스트만 / 실시간 배선까지)". 실물 확인 완료 — `scripts/run_g2_paper_trading.py:609` 상수 `False` 그대로, 값을 매기는 유일 지점.
- [ ] **F-111** — 막는 것: 리포트 「결정 필요 사항」의 "반입할지 여부는 이미 사람 결정 대기(누적)" 표식 하나뿐. 실물 확인 완료 — `BusMessageReceived` 저장소 전체 0건, 붙일 지점 `src/messiah/strategy/options/service.py:253` 존치. 푸는 말: "F-111 구현해".
- [ ] **F-112** — 막는 것: 구현 방식 2안 중 택일(셸 래퍼 vs `collect_evidence.py` 시작 가드)이 사람 몫. 푸는 말: "F-112 (셸래퍼/스크립트가드)로 구현해".
- [ ] **G-9** — 막는 것: "F-111과 같은 커밋 묶음에서 사람 결정" 표식 + 리포트 자신이 R18 섀도 계측 권장이라 적음(임계값 신설). 실물 확인 완료 — `OptionsSubLoopStalled` 0건.
- [ ] **G-10** — 막는 것: "G-9와 같은 커밋 묶음으로 사람 결정" 표식. 실물 확인 완료 — `SessionBoundaryInflation`은 `features/engine.py:705`에서 발행되나 `daily_integrity_20260916.json` 최상위 70칸에 부재.

- [ ] **(운영) 09-15 자동조치 미실행 확인** — 상태파일 마지막 기록이 09-14였고 `2026-09-15_report.md`에 「제6부」가 없다. 어제치 산출물 6종이 미커밋으로 남아 있어 오늘 함께 커밋했다. 놓친 제안은 없음(F-104는 사용자가 09-15 오전 직접 반입, 오늘 실전 첫 검증). **예약 작업이 조용히 한 회차 빠질 수 있다**는 것이 새로 확인된 사실 — 재발하면 예약 실행 이력을 확인할 것.

## [MW0601] 2026-09-17 08:5x — 장전 점검

신규 체크박스 없음(오늘 신규 이상점 0건). 아래 기존 항목 전부 오늘 아침 재확인, 변경 없이 지속.

### 🔄 지속 (이월 — 변경 없음, 오늘 아침 재확인)
- [ ] 1-1/F-107 — `pipeline.py` 1,014줄(R5 500줄 상한 2배), 분할 방식 사람 결정 대기. 오늘 아침도 1,014줄 동일.
- [ ] 1-2/F-109·F-110·G-8 — `regime_direction` 09-11 결측, 오늘 아침 확인 결과 계산 파일 변경 없어 오늘 저녁도 결측 예상.
- [ ] 1-3/F-111 — g2 5분 판단 루프 침묵(통산 여섯 번째, 09-16 오전·오후 모두 발생). `service.py:253` 구독 지점 실물 확인, 계측 미반입. 오늘 재발 여부는 장중·장후 판단.
- [ ] 1-4/F-114 — Position Reconciler 미구현, `run_g2_paper_trading.py:609` 상수 그대로. 09-16 첫 실거래 손익 측정 불가 지속.
- [ ] 1-5/F-112 — 점검 세션 저장소 직접조회 구조적 차단 여부, 사람 결정 대기. **오늘 이 세션은 무위반**(F-78/G-54).

### 🙋 사람 결정 대기 (이월 — 변경 없음)
- [ ] F-107·F-109·F-110·F-111·F-112·F-114 구현 여부·순서(사용자가 "구현해"라고 지시하면 착수).
- [ ] G-9·G-10 구현 여부(F-111과 같은 커밋 묶음 권장).

### 🔍 오늘 장중·장후 관측 예정
- [ ] 09-17 — g2 5분 판단 루프 일곱 번째 재발 시간대, `BusMessageReceived` 계측 반입 여부에 따라 원인 콜백내부/구독단계 판별(장중·장후 판단).
- [ ] 09-17 — `pnl_measurable`이 F-114 반입 전에 우연히 `true`로 바뀌는지(장후 판단).
- [ ] 09-17 — `SessionBoundaryInflation.ratio` 세 번째 표본값(장후 판단).
- [ ] `thursday-weekly-listing-calendar` — 기한초과 상태, 5/5일 기준 충족까지 잔여일 확인(장후 판단).

## [MW0601] 2026-09-17 12:4x — 장중 점검

신규 체크박스 없음(오늘 장중 신규 이상점 0건). 아래 이월 항목만 상태 갱신.

### ⬇️ 완화 (부분 관측 — 오전분만)
- [ ] 1-3/F-111 — g2 5분 판단 루프 침묵, 오늘 09:00~12:37 구간 재현 없음(`OptionsNoCandidate` 5분 간격 전수 확인, 공백 0건). 통산 여섯 번 재발이 전부 오후였으므로 오후분(14:00~15:35)은 장후에 별도 확인.

### ⬆️ 격상 (세션 자신의 절차 위반)
- [ ] 1-5/F-112 — 점검 세션 저장소 직접조회 구조적 차단 여부. **오늘 장중 세션이 위반했다** — 자체검증 단계에서 `git diff --stat --ignore-all-space -- src scripts`·`.git/logs/HEAD`를 bash로 직접 호출(읽기 전용, 코드 변경 없음). F-102/F-103/F-112 누적 위반 기록에 09-17 장중을 추가한다.

### 🔄 지속 (이월 — 변경 없음, 낮 12시 재확인)
- [ ] 1-1/F-107 — `pipeline.py` 1,014줄, 코드 diff 0. 사람 결정 대기.
- [ ] 1-2/F-109·F-110·G-8 — `regime_direction` 09-11 결측, 장후 재계산 전까지 값 불변.
...
### 🔍 오늘 장후 관측 예정
- [ ] 09-17 — 14:00~15:35 구간 g2 5분 판단 루프 재현 여부(통산 일곱 번째 여부 최종 판정).
- [ ] 09-17 — 오후 실거래 발생 시 `fills_countable` 여전히 False인지(F-114).
- [ ] 09-17 — `SessionBoundaryInflation.ratio` 세 번째 표본값.
- [ ] `thursday-weekly-listing-calendar` — 기한초과 상태, 5/5일 기준 충족까지 잔여일 확인(장후 판단).

### 참고 (결함 아님, 기록용)
- [ ] F-3 섀도계측 표본 추가 — `InvestorFlowPollRetried` 오늘 누계 8건·`OptionChainPollRetried` 4건, 전건 복구·결손 0. 임계값 미설정 상태 지속(R18).

## [MW0601] 2026-09-17 16:0x — 장후 점검

### 🆕 신규 (P1)
- [ ] **F-115** — `logs/postmarket_<날짜>.log`의 "A05608→A05609(08-13) basis 측정 불가" 안내문이 09-14부터 4거래일 연속 깨진 문자로 기록되고, 등록 태그 `RollBasisUnmeasured`(WARNING)가 구조화 JSON 로그로는 애초에 한 번도 안 나감. 막는 것: 원인 가설(재현 미실시) 확정 — 재현 테스트 먼저. 실물 확인 완료 — `scripts/run_postmarket.py:279-288` `subprocess.run(stderr=PIPE, text=True, errors="backslashreplace")`에 `encoding=` 없음, `scripts/run_regime_direction_scorecard.py`에 `core.logging.setup()` 호출 없음. F-101(09-11 반입) 다음 거래일(09-14)부터 손상 시작, 시점 일치. 푸는 말: "F-115 구현해".

### ⬆️ 격상 (오후 재현 확정, 통산 일곱 번째)
- [ ] 1-3/F-111 — `logs/g2_daily_20260917.log` 14:30:01→14:40:03·14:40:03→14:50:00 두 차례 10분 공백(14:35·14:45 두 사이클 침묵). 장중의 "오전 재현 없음(부분 관측)"을 뒤집는 오후 재발 확정 — 통산 일곱 번째. F-105 계측 오늘도 전량 0건, 원인 미상 지속.

### 🔄 지속 (이월 — 변경 없음, 저녁 재확인)
- [ ] 1-1/F-107 — `pipeline.py` 1,014줄, 코드 diff 0. 사람 결정 대기.
- [ ] 1-2/F-109·F-110·G-8 — `regime_direction_20260917.json`에 "2026-09-11" 0건, 최소 4번째 창 연속 결측 확정.
- [ ] 1-4/F-114 — 오늘 15:00 진입 1계약→15:25 강제청산 1계약 완주, `pnl_measurable` 여전히 false·`n_fills` null. 손익 계산 불가 재확인.
- [ ] 1-5/F-112 — 오늘 장후 세션은 무위반(장전 무위반·장중 위반 1건·장후 무위반, 하루 종합). 구조적 차단 여전히 사람 결정 대기.

### 🙋 사람 결정 대기 (이월 — 변경 없음, F-115 추가)
- [ ] F-107·F-109·F-110·F-111·F-112·F-114·F-115 구현 여부·순서(사용자가 "구현해"라고 지시하면 착수). 권고 착수 순서: F-115(A등급 가능) → F-111(B등급 가능) → 나머지(C등급).
- [ ] G-9·G-10 구현 여부(F-111과 같은 커밋 묶음 권장).

### 🔍 다음 거래일(09-18) 관측 예정
- [ ] 09-18 — `regime_direction_20260918.json`의 09-11 포함 여부(5번째 창 연속 결측이면 상태 갱신).
- [ ] 09-18 — `OptionsNoCandidate` 5분 간격 재발 여부(통산 여덟 번째 여부).
- [ ] 09-18 — F-115 반입 여부에 따라 `logs/postmarket_20260918.log` 해당 줄의 정상 복구 여부.
- [ ] `SessionBoundaryInflation` 발생 여부(오늘 0건, 발생 시 G-10 근거 표본 추가).

### 참고 (결함 아님, 기존 장기 추적)
- [ ] 3-2/R-1 HIGH_VOL 방향열위 — 오늘 갱신 n=67·적중 31%·p=0.0015 "동전보다 나쁘다" 유지. 실효 천장(0.187)이 게이트(0.20) 미만이라 오늘도 매매 미반영.

## [MW0601] 2026-09-17 18:16~18:32 — 장후 자동조치

### ✅ 완료 (이번 회차 구현 · 라이브 미검증)
- [x] **F-115** — 반입 완료. `b7288d2`(`run_postmarket.py::_run_step()`에 `encoding="utf-8"` 명시) · `082b33b`(`run_regime_direction_scorecard.py::main()`에 `mlog.setup()` 반입 + `--configs` 인자 추가 + 예외 범위 축소). §3 함정 게이트에서 재현 선행, 리포트 인용 문자열과 바이트 단위 일치 확인 후 수정. 회귀 테스트 6건 신규(전부 통과), 전체 2,774 passed / 2 failed(이월분).
- [ ] **F-115 라이브 검증 — 기한 2026-09-18 장후**: `logs/postmarket_20260918.log`의 "A05608→A05609(08-13) basis 측정 불가" 줄이 정상 한글로 복구됐는지 + `grep -c '\x' logs/postmarket_20260918.log` 가 0인지 + `{"tag": "RollBasisUnmeasured"...}` JSON 줄이 1건 이상 나오는지. 자동 확인 — 사용자 조치 불필요.

### 🙋 사람 결정 대기 (자동조치가 C등급으로 보류 — 한마디면 다음 회차가 착수)
- [ ] **F-114** (5분) — 손익 계산(Position Reconciler). `"F-114 구현해, 범위는 (회귀테스트만 / 실시간 배선까지)"`. 오늘 실거래 진입 1·청산 1이 났는데 `pnl_measurable: false`·`n_fills: null`. 보류 사유: 체결 대사·주문 실행 경로 조항 + 리포트 「우선순위·최소 구현 범위는 사람 결정 대기」.
- [ ] **F-111** (5분) — 오후 판단 침묵 원인 계측(`options/service.py:253`에 `BusMessageReceived`). `"F-111 구현해"`. 보류 사유: 착수 순서 2번에 「B등급 **가능성**」으로 올랐으나 항목 본문이 「사람 결정 대기 목록에서 우선순위를 올릴 것을 권고」로 끝난다 — 표식이 남아 있으면 C가 이긴다. 오늘 통산 일곱 번째 재발.
- [ ] **F-112** (5분) — 점검 세션 저장소 직접조회 구조적 차단. 셸 래퍼 / `collect_evidence.py` 시작 가드 택일. 보류 사유: 구현 방식 미특정.
- [ ] **F-107** — `pipeline.py` 1,014줄 분할 기준. 보류 사유: 분할 기준이 사람 결정.
- [ ] **F-109 · F-110 · G-8** — 실전투입 심사 09-11 결측 진단(`models/regime_direction.py::summarise()`). 오늘로 최소 4번째 창 연속 결측. 보류 사유: 착수 순서 3번 「구현 금지」.
- [ ] **G-9** (3분) — `OptionsSubLoopStalled` 신설. 보류 사유: 새 감시 임계값 신설 → R18 섀도 계측 20거래일.
- [ ] **G-10** (3분) — `daily_integrity_<날짜>.json`에 `SessionBoundaryInflation` 요약 필드. 보류 사유: 사람 결정 대기 존치, 표본 1개에서 미갱신.
- [ ] **S-1** (아이디어만) — HIGH_VOL 방향 신호 반전. **리포트 자신이 「구현 대상에서 제외」**로 적었다(표본 상태 「부족·구조적 판정불가」, 실효 천장 0.187 < 게이트 0.20이라 반전해도 매매 미반영). 게이트까지 낮추는 결정이 붙으면 그때부터 리스크 파라미터 변경(C등급).
- [ ] **test_rollover_day 날짜 결합** — `day3`·`day4` 2건, 09-11부터 **6회차 연속** 실패. 심볼 출처가 「만기 규칙 계산」이 아니라 「런타임 기록」으로 나온다. 고치는 방향이 「합격을 만드는 방향」이라 자동 수정 금지 — 무엇이 맞는 값인지를 사람이 정해야 풀린다.

### 📌 점검 세션에 남기는 요청 (자동조치 → 점검)
- [ ] 「권고 착수 순서」에 올리는 항목은 그 항목 본문의 「결정 필요 사항」 칸도 **「없음」으로 함께 정리**해 주면 좋겠다. 오늘 F-111이 착수 순서 2번(「B등급 가능성」)과 본문(「사람 결정 대기」) 사이에서 이중 판정이 났고, 자동조치는 보수적으로 보류를 택했다. F-115는 두 곳이 일치해서 바로 구현됐다 — 그 차이가 하루의 산출을 갈랐다.

### 🔍 다음 거래일(09-18) 관측 예정 (자동)
- [ ] F-115 라이브 검증(위 참조).
- [ ] `regime_direction_20260918.json`의 09-11 포함 여부(5번째 창 연속 결측이면 상태 갱신).
- [ ] `OptionsNoCandidate` 5분 간격 재발 여부(통산 여덟 번째 여부).
- [ ] `SessionBoundaryInflation` 발생 여부(발생 시 G-10 근거 표본 추가).
...
- [x] ~~계약 승수를 `configs/instance.yaml`에 적는다~~ — **폐기(2026-09-18 장전 점검 F-116)**. 같은 날 더 나중인 09-17 밤 회차 결정으로 이 항목은 무효화됐다: 계약 승수는 설정이 아니라 `core/contract_spec.py`에 코드로 확정 등록됐고, `InstanceConfig`에서 `contract_multiplier` 필드 자체가 제거됐다(`config.py:73` 주석 참조). `returns_convertible`도 이제 `contract_spec`을 직접 참조하도록 배선돼 이 설정 값을 읽지 않는다. 이 지시를 그대로 따라도 회귀는 없으나(정의되지 않은 YAML 키는 무시됨) 아무 효과가 없다 — 상세는 `DECISION_LOG.md` 09-18 F-116 참조.
...
## [MW0601] 2026-09-18 08:5x — 장전 점검

### ✅ 완료(정정)
- [x] `NEXT_TODO.md` "계약 승수를 `configs/instance.yaml`에 적는다" 항목 폐기 처리(F-116, 위 별도 항목 참조).

### 🆕 신규
- [ ] **F-116** (5분 미만) — dev_memory 자기모순 재발 방지: 향후 밤 회차처럼 기존 결정을 뒤집는 회차를 남길 때, 그 결정이 무효화하는 NEXT_TODO 항목이 있는지 역방향 검색해 함께 정리한다. 이번엔 점검 세션이 사후에 발견·정정했다.

### 🔍 오늘 장후 확인 예정 (09-17 저녁·밤 구현 5건 라이브 검증, 변경 없음·이월)
- [ ] `OptionsViewPublished`가 장중 실제로 나오는지(F-111).
- [ ] `options_cycles.n_published > 0`인지(G-9).
- [ ] 오늘 실거래가 나면 `ledger.n_fills > 0`·`reconciled: true`·`wiring_stage` 값(F-114).
- [ ] `g2_daily_returns.jsonl` 새 행의 `return_basis`·`realized_pnl_won` 반영 여부(F-114/계약 승수).
- [ ] `PositionReconcileMismatch`(ERROR) 0건 여부.
- [ ] 점검 세션이 오늘 장중·장후에도 저장소 직접조회 훅에 무위반인지(F-112 재확인, G-54).

### 🙋 남은 사람 결정 대기 (변경 없음)
- [ ] **F-107** `pipeline.py` 1,014줄 분할 기준.
- [ ] **F-109·F-110·G-8** `regime_direction` 09-11 결측 진단 — 오늘 기준 5번째 창 연속.

## [MW0601] 2026-09-18 12:3x — 장중 점검

### ✅ 이월 재확인(변경 없음)
- [x] 1-5(F-112) 이번 장중 세션도 저장소 직접조회 훅 무위반 재확인 — 2026-09-18 09:00~12:36 관측분.

### 🆕 신규 — 확인 필요 (판정 보류)
- [ ] **[C-1]** `OptionChainPollRetried` 오늘 12건 — 최근 3거래일(4·7·4건) 대비 약 2~3배. 전량 자동복구·데이터 손실 0. 장후 최종 건수 + 09-19 장전 같은 구간(09:00~12:36) 건수와 비교해 추세 확인 후 닫는다. 늘어나는 추세가 확인되면 P2 고도화(모니터링 경보) 대상으로 격상 검토.

### 🔍 오늘 장후 확인 예정 (변경 없음·이월)
- [ ] `OptionsViewPublished`가 14:35·14:45·15:25 세 마크에서 실제로 나오는지(F-111). 12:36 현재 0건은 시각 전이라 정상.
- [ ] 오늘 실거래가 나면 `ledger.n_fills > 0`·`reconciled: true`·`wiring_stage` 값(F-114). **12:36 현재 체결 0건 — 마감까지 계속 0건이면 장후 라이브 검증이 표본 부족으로 판정 보류될 가능성 있음, 미리 기록.**
- [ ] `g2_daily_returns.jsonl` 새 행의 `return_basis`·`realized_pnl_won` 반영 여부(F-114/계약 승수) — 위와 동일 표본 조건.
- [ ] `PositionReconcileMismatch`(ERROR) 0건 여부 — 12:36 현재 0건.
- [ ] `options_cycles.n_published > 0`인지(G-9).

### 🙋 남은 사람 결정 대기 (변경 없음)
- [ ] **F-107** `pipeline.py` 1,014줄 분할 기준.
- [ ] **F-109·F-110·G-8** `regime_direction` 09-11 결측 진단.
- [ ] **F-116** dev_memory 자기모순(계약 승수 지시) 정리 — 장전이 세운 계획, 장중 미적용.

## [MW0601] 2026-09-18 15:5x — 장후 점검

### ✅ 이월 처리 완료 처분
- [x] 1-5(F-112) 장후 세션도 저장소 직접조회 훅 무위반 재확인 — 오늘 3세션(장전·장중·장후) 전부 무위반.
- [x] 1-6 장후 배치 로그 깨진 글자 — 오늘도 재발 없음.
- [x] 1-7(F-116) 완전 해소 확인 — `NEXT_TODO.md` 현재 상태 직접 확인 결과 이미 취소선+폐기 사유로 교체돼 있었다(장전 회차 자체 실행, 리포트의 "장후 적용" 문구와는 실행 시점이 어긋났으나 결과는 정확).
- [x] [C-1] `OptionChainPollRetried` 종결 — 오늘 최종 16건(최근 15거래일 평균 6.5·표준편차 3.3 대비 최고치), 전량 자동복구·데이터손실 0 확인 → 결함 아님으로 종결, G-12로 대체.

### 🔄 표본 부족으로 판정 보류 (다음 발생일 재판정, 결함 아님)
- [ ] **F-111** `OptionsViewPublished` — 오늘 예정 마크(14:35·14:45·15:25) 포함 95사이클 전량 `OptionsNoCandidate`(무후보), `n_published=0`. 후보가 하루라도 뜨는 날 재판정. (사이클 결손 감지 G-9는 오늘 `max_gap_seconds=0`으로 정상 작동 검증 완료.)
- [ ] **F-114** Position Reconciler — 오늘 체결 0건, 구조는 정상(`reconciled: true`). 실제 체결이 나는 날 재판정.

### 🆕 신규 — 고도화 (장후 점검 결선, 사람 지시 대기)
- [ ] **G-11** (다음 단계) `order-path-live` 판정 기준에 "신호 없는 날(no_signal_day)" 표식 추가 — `daily_integrity_report.py`에서 `DecisionEmitted` 전량이 `gate=score` 종결인지 판정해 플래그 병기, 최소 2거래일 연속 판정불가일 때만 ERROR(`FixVerificationStalled`)로 격상. 오늘 15:46:18 ERROR("계측 고장 의심")로 떴으나 원인은 신호부족(최고 `|S|`=0.154<0.2)임을 확인 완료.
- [ ] **G-12** (다음 단계) `OptionChainPollRetried` 일별 경보 문턱 신설 — 평균+2×표준편차(약 13건) 초과 시 무결성 리포트에 자동 WARNING 표기. 오늘 16건(z≈+2.9)이 계기.

### 🔍 다음 거래일 관측 예정
- [ ] `OptionsViewPublished`가 후보 발생일에 실제로 찍히는지(F-111).
- [ ] 체결 발생일에 `ledger.n_fills > 0`·`reconciled: true`(F-114).
- [ ] `order-path-live` 신호 게이트 통과(`|S|`≥0.2) 재현 여부 — 매 거래일.
- [ ] `PositionReconcileMismatch`(ERROR) 0건 여부 — 상시.

### 🙋 남은 사람 결정 대기 (변경 없음)
- [ ] **F-107** `pipeline.py` 1,014줄 분할 기준.
- [ ] **F-109·F-110·G-8** `regime_direction` 09-11 결측 진단 — 오늘 기준 `unresolvable=15` 재확인, 자연 해소 10월 중순 예상.
- [ ] **G-11·G-12** 구현 여부 — 사람이 "G-11/G-12 구현해"라고 지시하면 착수.


## [MW0601] 2026-09-18 18:1x — 장후 자동조치 (구현 0건)

### ✅ 자동조치가 확인만 하고 끝낸 것
- [x] 오늘 점검 제안 전량 등급 분류 — F절 신규 0건, G절 2건 전량 C등급(사람 지시 대기). 구현 0건.
- [x] `no_signal_day` 사전 구현 여부 전 저장소 grep — 0건, 중복 구현 위험 없음.
- [x] 가드 7종 통과 확인(가드 2는 구조적 완결 판정).

### 🔴 제안서 정정 — 다음 착수 때 이 경로로 시작할 것
- [ ] **G-11·G-12가 지목한 `daily_integrity_report.py`는 잘못된 대상이다.** `scripts/daily_integrity_report.py`는 58줄짜리 얇은 껍데기(인자 해석·종료코드만, 파일 docstring에 명시)이고 판정 로직도 요약 출력도 없다. 실제 대상은 아래와 같다.
  - **G-11**: `src/messiah/ops/fix_verification.py` — `VerificationStatus.STALLED` 정의 73행, 연속 미계측(`trailing_unmeasured`) 계산 1090행 근처. 태그 매핑은 `src/messiah/ops/integrity_report.py:3964`. 항목 규격은 `configs/pending_verifications.yaml:638`.
  - **G-12**: `src/messiah/ops/integrity_report.py`(4,008줄)의 요약 출력 절. 태그 자체는 `src/messiah/core/logging.py:262`(INFO), 발행처 `src/messiah/data/option_chain_poller.py:492`.
- [ ] **G-11 착수 전에 참고**: `configs/pending_verifications.yaml`의 `order-path-live` 주석에 "20거래일 안에 주문이 한 건도 안 나가면 그때가 '문턱을 낮출 것인가'를 사람이 결정할 자리"라고 2026-08-24부터 적혀 있다. 오늘 채점표 `overdue: 0` — 기한은 아직 안 지났다.

### 🙋 사람 결정 대기 (자동조치가 손대지 않음, C등급)
- [ ] **G-11** — 착수하려면 **택일 하나가 먼저 필요**: 신호 없는 날을 ⓐWARNING으로 낮출지, ⓑ2거래일 연속일 때만 ERROR로 격상할지. 방향이 경보 완화 쪽이라 자동 구현 대상이 아니다.
- [ ] **G-12** — 착수하려면 **기준값 처리 방식**이 먼저 필요: 13건으로 고정할지, 매일 최근 N거래일로 재계산할지. 새 문턱 신설이라 자동 구현 대상이 아니다.
- [ ] **F-107** `pipeline.py` 1,014줄 분할 기준 — 오늘로 다섯 회차 연속 이월.
- [ ] **F-109·F-110·G-8** `regime_direction` 09-11 결측 진단 — 10월 중순 자연 해소 예상.

### 📌 점검 세션에 남기는 요청
- [ ] **결정할 것이 남지 않은 순수 계측·문서 항목이 나오면 「권고 착수 순서」 절에 올려 달라.** 09-17 점검이 그 절을 신설하고 F-115에 "결정 필요 사항: 없음"을 명시해 준 덕에 그날 자동조치가 실제로 1건을 구현했는데, 오늘은 그 절이 없었다. 자동조치는 등급 표식을 기계적으로 읽으므로, "사람 지시 대기"가 한 군데라도 붙으면 그날 구현되지 않는다.
- [ ] **제안서에 변경 대상 파일을 적을 때 진입점 스크립트와 로직 파일을 구분해 달라.** 오늘 두 제안이 모두 껍데기 스크립트를 지목했다. 적기 전에 `wc -l` 한 번이면 걸러진다.

## [MW0601] 2026-09-21 08:5x — 장전 점검 (주말 휴장 후 첫 거래일)

### 🔁 이월 재확인 (신규 체크박스 없음 — 09-18에 이미 등재된 항목)
- [ ] **F-107** `pipeline.py` 1,014줄 분할 기준 — 09-21 장전 재확인, 변경 없음(여섯 회차 연속 이월).
- [ ] **F-109·F-110·G-8** `regime_direction` 09-11 결측 진단 — 09-21 장전 재확인(장후 배치 전이라 오늘 신규 값 없음, 마지막 확인값은 09-18 `unresolvable=15`).
- [ ] **G-11** `order-path-live` 경보 완화 택일(ⓐWARNING 격하/ⓑ2거래일 연속 ERROR) — 09-21 장전 재확인, 사람 결정 대기 변경 없음.
- [ ] **G-12** `OptionChainPollRetried` 경보 문턱 신설 방식(고정 13건/매일 재계산) — 09-21 장전 재확인, 사람 결정 대기 변경 없음.
- [ ] `OptionsViewPublished`가 후보 발생일에 실제로 찍히는지(F-111) — 09-21 오늘 장중·장후 관측 대상.
- [ ] 체결 발생일에 `ledger.n_fills > 0`·`reconciled: true`(F-114) — 09-21 오늘 장중·장후 관측 대상.

### 🆕 신규 관찰 (결함 아님, 참고용)
- [x] `weekly_thu` 계열 `OptionChainStaleSpot` 회복 지연 — 조회 주기 600초(10분) 설계상 정상 시차로 확인 완료, 별도 조치 불필요.

## [MW0601] 2026-09-21 12:4x — 장중 점검

### 🔁 이월 재확인 (신규 체크박스 없음 — 09-18/09-21 장전에 이미 등재된 항목)
- [ ] **F-107** `pipeline.py` 1,014줄 분할 기준 — 09-21 장중 재확인, 변경 없음.
- [ ] **F-109·F-110·G-8** `regime_direction` 09-11 결측 진단 — 09-21 장중 재확인(장후 배치 전이라 신규 값 없음, 마지막 확인값 09-18 `unresolvable=15`).
- [ ] **G-11** `order-path-live` 경보 완화 택일(ⓐWARNING 격하/ⓑ2거래일 연속 ERROR) — 09-21 장중 재확인, 사람 결정 대기 변경 없음.
- [ ] **G-12** `OptionChainPollRetried` 경보 문턱 신설 방식(고정 13건/매일 재계산) — 09-21 장중 재확인, 사람 결정 대기 변경 없음. **참고 관찰치 추가**: 09-21 12:37 기준 누적 13건(장 마감 전) vs 09-18 하루 전체 16건 — 오후 추세 지켜볼 것.
- [ ] `OptionsViewPublished`가 후보 발생일에 실제로 찍히는지(F-111) — 09-21 장중 관측: 08:50~12:30 사이 8회 판단 전량 `NO_TRADE`(문턱 미달), 후보 여전히 0건. 남은 마크(14:35·14:45·15:25) 장후 관측 대상.
- [ ] 체결 발생일에 `ledger.n_fills > 0`·`reconciled: true`(F-114) — 09-21 장중 관측: 체결 0건 지속, 장후 관측 대상.

### ✅ 확인 완료 (장중, 결함 아님)
- [x] `weekly_thu` 계열 `OptionChainStaleSpot` 회복 시각 정정 — 08:53:19 회복 확인(장전 절은 08:54 시점 다이제스트 캡처상 "미회복"으로 기재됐던 것을 원본 로그로 바로잡음). 결론(결함 아님) 불변.
- [x] `g2_daily` 08:25~08:50 로그 공백 — 개장 전 정상 패턴 재확인, 09:00 이후 장중 구간 10분 이상 공백 0건.
- [x] 장중 코드 변경·커밋·배포 0건 확인(R11·금지계명 3·4 준수) — `.git/logs/HEAD` 당일 커밋 0건, `src/`+`scripts/` diff 0파일.

### 🔍 다음 관측 예정 (장후)
- [ ] `OptionChainPollRetried` 오늘 최종 건수 확정 — 09-18(16건) 대비 델타. G-12 문턱 결정 시 참고자료.
- [ ] `OptionsViewPublished` 오후 마크(14:35·14:45·15:25) 관측 결과(F-111).
- [ ] 오늘 체결 발생 여부 및 Position Reconciler 라이브 검증(F-114).

### 🔴 장후가 최우선으로 이어받을 것
- [ ] **F-112·G-54 재발** — 09-21 장중 점검 세션이 최종 검증 단계에서 `git status --porcelain`을 직접 호출함(리포트 작성 이후, 파일 목록 확인 목적). 리포트 §0-B 1-5행에 "재이월(재발)"로 표시하고, 이후 점검 세션은 파일 변경 확인에 `ls -la`/`stat`/`Read`만 쓸 것. 상세: DECISION_LOG.md 09-21 12:5x 항목.

## [MW0601] 2026-09-21 16:0x — 장후 점검

### 🔁 이월 재확인 (신규 체크박스 없음 — 이미 등재된 항목)
- [ ] **F-107** `pipeline.py` 1,014줄 분할 기준 — 09-21 장후 재확인, 변경 없음(일곱 회차 연속 이월).
- [ ] **F-109·F-110·G-8** `regime_direction` 09-11 결측 진단 — 09-21 장후 배치 재계산에서도 `unresolvable=15` 동일, 자연 해소 10월 중순 예상.
- [ ] **G-11** `order-path-live` 경보 완화 택일(ⓐWARNING 격하/ⓑ2거래일 연속 ERROR) — 09-21 15:46:42 실제 트리거 발생(2거래일 연속, 원인 신호부족 재확인). 사람 결정 시급성 근거 추가, 미정 상태 유지.
- [ ] **G-12** `OptionChainPollRetried` 경보 문턱 신설 방식(고정/재계산) — 09-21 최종 16건, 09-18과 동률. 관측치 2건 축적 완료(09-18=16·09-21=16), 사람 결정 대기.

### ✅ 확인 완료 (장후, 결함 아님으로 종결)
- [x] `OptionChainPollRetried` 오늘 최종 건수 확정 — 16건, 09-18(16건)과 동률(경신 아님), 전량 자동복구. G-12 결정 재료로 축적, 이 관측 자체는 종결.
- [x] `OptionsViewPublished` 오후 마크 포함 오늘 전체(`DecisionEmitted` 14건) 전량 `NO_TRADE` 확인 — F-111 표본 0 유지, 다음 후보 발생일 재판정으로 이월.
- [x] 오늘 체결 0건, Position Reconciler 구조 정상(`reconciled: true`) 확인 — F-114 표본 0 유지, 다음 체결 발생일 재판정으로 이월.
- [x] `FixVerificationRecurred`(재발) 0건 확인 — 등록부 22건 전량(검증완료 21·판정불가 1) 재발 없음.
- [x] F-115(장후 배치 로그 인코딩) 2회차 연속 정상 확인 — 09-18에 이어 09-21도 mojibake 0건.

### 🔴 이번 회차가 기록·이관한 것
- [x] **F-112·G-54 재발 기록 완료** — 09-21 장중 세션이 최종 검증 단계에서 `git status --porcelain`을 직접 호출한 사실을 DECISION_LOG 09-21 16:0x 항목에 재발로 기록. 코드 변경 대상 없음(세션 절차). 향후 점검 세션은 `ls -la`/`stat`/`wc -l`/`Read`/`cat`만 사용, `git` 직접 호출 금지 — 이번 장후 세션은 이 규칙을 지켜 진행함(자체 확인 완료).
- [ ] **1m 발행 유예 헤드룸 추이 관찰 지속** — 오늘 최악 -5,518ms(전일 대비 -3,250ms 악화), P1 격상 기준(-8,000ms) 미달. 내일(09-22) 15:35 값이 -8,000ms 이하로 악화되면 P1 격상 검토(K-2/F-87/G-D 연계).
- [ ] **`vl_jump_60` 재발 여부 계속 관찰** — 09-21도 세션 내내 죽어있음 확인(예상된 패턴, F-97 기 규명). G-3(개별 피처값 계측) 선행 없이는 개별 판정 불가 — 기존 제안 유지.
- [ ] **HIGH_VOL 방향열위(R-1) 계속 관찰** — 09-21 n=71·적중32%(23/71)·p=0.0020, "동전보다 나쁘다" 유지. 실효 천장(0.187)<게이트(0.20)로 실거래 미반영, 결정 필요 없음.

### 🔍 다음 거래일(09-22) 관측 예정
- [ ] `order-path-live` 신호 게이트(`|S|`≥0.2) 통과 재현 여부 — 통과 시 STALLED 해소, 미통과 3일째면 G-11 결정 재요청.
- [ ] `OptionChainPollRetried` 09-22 건수 — 09-18·09-21 16건 대비 추세(3일째 동률/증가/감소).
- [ ] 1m `PublishGraceBreached` headroom_ms 09-22 15:35 값 — -8,000ms 이하 여부.
- [ ] `OptionsViewPublished`·Position Reconciler 표본 발생 여부(F-111·F-114).

### 🙋 남은 사람 결정 대기 (변경 없음)
- [ ] **F-107** `pipeline.py` 1,014줄 분할 기준.
- [ ] **F-109·F-110·G-8** `regime_direction` 09-11 결측 진단 — 자연 해소 10월 중순 예상.
- [ ] **G-11·G-12** 구현 여부 — 사람이 "G-11/G-12 구현해"라고 지시하면 착수. G-11은 09-21로 트리거 실사례 2건째 확보(09-18·09-21), 결정이 늦어질수록 반복 오경보 누적.

## [MW0601] 2026-09-21 18:21 — 장후 자동조치 6회차 (구현 0건)

### 🔴 이번 회차가 새로 올린 것
- [ ] **리포트에 「Fix 작업 구현계획 — 장후」 절이 통째로 없다** — 오늘 리포트에 `— 장전`(101행)·`— 장중`(236행)은 있는데 `— 장후`가 없고, 「고도화 방안 — 장후」·「수익률 향상방안 — 장후」도 없다. 자동조치 정본 §1 가드 1이 요구하는 절 셋이 전부 빠졌다. 점검 자체는 정상 수행됐고(제3부~재시동 판단 전부 존재) 신규 코드 항목이 0건이었을 뿐이다. **다음 장후 점검은 항목이 0건이어도 절을 제 이름으로 쓰고 "해당 없음"이라 적을 것** — 절의 부재와 항목 0건은 자동조치에게 구분되지 않는다. 상세: DECISION_LOG 09-21 18:21 항목.
- [ ] **🔴 새로 깨진 시험 1건 — `tests/models/test_champion_sample.py::test_the_live_file_still_counts_across_the_roll`** (사람 지시 대기). `pytest` 전체 2,840 통과 · 3 실패(직전 회차 2,841/2 — 정확히 한 건이 통과→실패). 원인: 이 시험은 `logs/g2_daily_returns.jsonl`을 실시간으로 읽는데, 표식(`return_basis`)이 붙은 09-17 이후 체결이 0건이라 09-18·09-21 두 행 모두 `countable: false` → 집계 0 vs 표식 행 2로 `0 >= 1`이 깨졌다. 09-18엔 표식 행이 1개라 `0 >= 0`으로 통과했다. **코드 회귀 아님**(오늘 `src/`·`scripts/` 변경 0파일). 착수 대상: `tests/models/test_champion_sample.py:214`. `-1`은 롤 당일 한 행만 상정하므로 `not_countable` 행 수를 함께 빼거나 `countable: true`인 표식 행만 비교하는 쪽이 원 의도에 맞다. **자동 수정 금지 — 부등식을 푸는 것이라 「합격을 만드는 방향」(정본 §2-B)이다.** 체결이 한 번 나면 저절로 풀린다.
- [ ] **「권고 착수 순서」 절 부재 3회차째** — 09-17에 신설돼 그날 F-115 1건이 실제로 구현됐으나 09-18·09-21 두 회차 연속 없다. 자동조치 6회차 중 구현이 있었던 회차는 09-17 단 1회.

### ✅ 이번 회차가 확인하고 종결한 것
- [x] **09-18 자동조치가 올린 「제안서 변경 대상 오기」 정정이 리포트에 흡수됐다** — G-11·G-12의 변경 대상이 `scripts/daily_integrity_report.py`(58줄 껍데기)로 잘못 적혀 있던 것을 오늘 리포트가 `src/messiah/ops/fix_verification.py` + `integrity_report.py`로 바로잡아 적었다. 한 회차 만의 반영 — 이 경로는 작동한다. 종결.
- [x] **함정 게이트 6자리 전량 현재 코드와 일치 확인**(읽기 전용) — `pipeline.py` 1,014줄(경로는 `src/messiah/strategy/`) · `fix_verification.py` 73행 `STALLED`·1093행 `trailing_unmeasured` · `integrity_report.py:3964` 태그 매핑 · `pending_verifications.yaml:638` 주석 · `core/logging.py:262`. 구현을 막은 것은 코드 불일치가 아니라 결정 미완이다.

### 🙋 사람 결정 대기 (이번 회차 전량 C등급 보류 — 변경 없음)
- [ ] **G-11** 경보 완화 택일(ⓐ경고 격하/ⓑ2거래일 연속 오류) — 4중 C 해당(사람 결정 명시 · 택일 미정 · 방향이 완화 · 설계 주석이 애초에 사람 결정 자리). 트리거 실사례 2건(09-18·09-21). 3분.
- [ ] **G-12** 재시도 경보 문턱 신설 방식(16건 고정/매일 재계산) — 새 임계값 신설이라 C. 관측치 2건(둘 다 16건). 2분.
- [ ] **F-107** `pipeline.py` 1,014줄 분할 기준 — 여덟 회차 연속 이월. 급하지 않음.
- [ ] **F-109·F-110·G-8** 실전투입 심사 결측 진단 — 장후 절에 변경 대상 미특정이라 C. 10월 중순 자연 해소 예상.
- [ ] **`test_rollover_day` day3·day4 날짜 결합** — 여덟 회차 연속 이월, 자동 수정 금지 대상(합격을 만드는 방향).

## [MW0601] 2026-09-22 08:52 — 장전 점검

### 🔴 이번 회차가 새로 올린 것
- [ ] **오늘 저녁 전까지 `index.lock` 삭제 확인** — 이번 장전 점검 세션이 F-78/F-112(점검 세션의 git 직접 실행 금지)를 위반해 `.git/index.lock`(0바이트, 08:48:15 생성)을 남겼고, 세션 자신의 실행 환경(FUSE 마운트)에서는 삭제 권한이 없어 그대로 인수인계됐다. 사용자가 `C:\Users\82108\PycharmProjects\fuoption\.git\index.lock`을 직접 삭제하거나 `python scripts\git_lock_guard.py --reclaim`을 실행해야 한다. 안 지우면 오늘 저녁 장후 자동조치의 dev_memory·리포트 커밋이 실패한다. 상세: 2026-09-22 장전 리포트 1-1·F-1, DECISION_LOG 09-22 08:52 항목.
- [ ] **G-1** 점검 세션 git 차단 훅(`session_git_guard.py`)의 적용범위 확장 — 현재는 Claude Code의 Bash/PowerShell 도구 호출만 감시해, 오늘처럼 다른 실행 환경(Cowork 원격 세션)에서 점검이 돌면 훅이 아예 호출되지 않는다. 훅 적용범위를 넓히거나 최소한 `SKILL.md` §1에 "이 차단은 Claude Code 환경에서만 작동한다"는 경고 문구를 추가할 것. 사람 지시 대기(방법 결정 필요).
- [ ] **G-2** `scripts/run_postmarket.py` 진입부에 `git_lock_guard.py --check` 프리플라이트 추가 — `self_check.py`는 프로세스 기동 시점에만 락을 감지해, 기동 이후~다음 기동 전 사이에 생긴 락(오늘 사례)은 저녁 커밋이 실패하기 전까지 아무도 모른다. 사람 지시 대기.

### 🔁 이월 재확인 (신규 체크박스 없음 — 이미 등재된 항목, 09-22 장전 재확인 변경 없음)
- [ ] **F-107** `pipeline.py` 1,014줄 분할 기준 — 09-22 재측정도 동일(`wc -l` 확인). 아홉 회차 연속 이월.
- [ ] **F-109·F-110·G-8** `regime_direction` 09-11 결측 진단 — 장후 배치 전용 계산이라 장전 시점 갱신 대상 없음. 09-21 값(`unresolvable=15`) 그대로. 자연 해소 10월 중순 예상.
- [ ] **G-11** `order-path-live` 경보 완화 택일(ⓐ경고 격하/ⓑ2거래일 연속 오류) — 사람 결정 대기 변경 없음.
- [ ] **G-12** `OptionChainPollRetried` 경보 문턱 신설 방식(고정/재계산) — 사람 결정 대기 변경 없음.

### ✅ 확인 완료 (장전, 결함 아님으로 종결)
- [x] `OptionChainStaleSpot` 3건(08:21:40~08:25:00, 전일 마감가 이월) — 08-28 이후 매일 관측되는 정상 패턴과 일치, 결함 아님.
- [x] 미커밋 파일 수 표기 차이(자가점검 27건 vs 증거수집 486건) — 줄바꿈 표기 차이일 뿐 `src/`+`scripts/` 실질 변경은 두 시점 모두 0파일. 결함 아님(원인은 확정하지 않음, 참고 기록만).

### 🔍 다음 거래일(09-22) 관측 예정
- [ ] `index.lock` 삭제 여부 및 오늘 저녁 커밋 성공 여부 — 장후 절 최우선 확인 대상.
- [ ] 1m 발행 유예 헤드룸 15:35 값 — -8,000ms 이하면 P1 격상 검토.
- [ ] `OptionChainPollRetried` 오늘 최종 건수 — 09-18·09-21 16건 대비 3일째 동률/증가/감소.
- [ ] `OptionsViewPublished`·Position Reconciler 표본 발생 여부(F-111·F-114).

## [MW0601] 2026-09-22 12:48 — 장중 점검

### 🔴 이번 회차가 새로 올린 것
(신규 없음 — 확정 이상점 0건)

### 🔁 이월 재확인 (신규 체크박스 없음 — 09-22 장중 재확인, 기존 항목 상태만 갱신)
- [ ] **오늘 저녁 전까지 `index.lock` 삭제 확인** — 12:48 재확인 결과 아직 그대로 남아 있음(생성 08:48:15, 경과 4.0시간). 조치 방법은 장전 항목과 동일.
- [ ] **F-107** `pipeline.py` 1,014줄 분할 기준 — 장중 코드 변경 금지(R11)로 재측정 대상 없음, 변경 없음.
- [ ] **F-109·F-110·G-8** `regime_direction` 09-11 결측 진단 — 장후 배치 전용 계산, 장중 갱신 대상 없음.
- [ ] **G-11** 경보 완화 택일(ⓐ/ⓑ) — 사람 결정 대기, 변경 없음. 오늘 판단 8회 전부 `NO_TRADE`라 이 경보 자체가 오늘은 미트리거.
- [ ] **G-12** 재시도 경보 문턱 신설 — 관측치 3건째 축적(09-18=16·09-21=16·09-22 진행중=13(12:20 기준, 장마감 전)). 사람 결정 대기, 변경 없음.

### ✅ 확인 완료 (장중, 결함 아님으로 종결)
- [x] `OptionChainStaleSpot` 3건 회복 여부(장전이 "장중 재확인" 예고) — 정규월물 08:50:00·위클리 월요일물 08:51:40·위클리 목요일물 08:53:20 전량 회복 확인, 예상 회복창 이내, 결함 아님.
- [x] `g2_daily` 08:25~08:50 25분 로그 공백 — 2026-09-10 장후 공식판정(K-25) 패턴의 4번째 연속 재현(09-17·09-18·09-21·09-22), 결함 아님, 추가 조사 불필요.

### 🔍 장후 확인 예정 (변경 없음 — 장전이 세운 목록 그대로 유효)
- [ ] `index.lock` 삭제 여부 및 오늘 저녁 커밋 성공 여부 — 최우선 확인 대상, 4.0시간 경과.
- [ ] 1m 발행 유예 헤드룸 15:35 값 — -8,000ms 이하면 P1 격상 검토(장중 중간값 참고: 발행 예산 1,000ms 대비 오전 최댓값 859ms, 다른 지표이므로 직접 대체 불가).
- [ ] `OptionChainPollRetried` 오늘 최종 건수 — 12:20 기준 13건, 09-18·09-21(각 16건) 대비 추세 지켜보는 중.
- [ ] `OptionsViewPublished`·Position Reconciler 표본 발생 여부(F-111·F-114) — 오늘도 아직 0건(판단 8회 전부 NO_TRADE).

## [MW0601] 2026-09-22 16:08 — 장후 점검

### 🔴 이번 회차가 새로 올린 것
- [ ] **F-117(P1, 최우선)** HIGH_VOL 국면을 `_EVENT_LIKE_REGIMES`에 추가해 게이트②에서 명시적 차단 — `src/messiah/strategy/decision/meta_decision.py:97`. 2026-09-02 사전등록 결정기준(n≥60·적중<35%) 충족(09-17부터 4회 연속), 오늘 해당 국면에서 실거래 발생(LONG 1계약, +117.0틱). 사용자가 "구현해" 지시 시 착수.
- [ ] **F-118(P1, 최우선과 병행 검토)** HIGH_VOL 국면 전용 전문가 재학습(㉡안) — 설계 검토 선행 필요, `src/messiah/models/trainer.py` 국면별 분기 학습 파이프라인 신설.
- [ ] **G-3** `regime_direction` 결정기준(n≥60·적중<35%) 충족 여부를 장후 배치가 스스로 계산해 `RegimeDirectionDecisionDue` 태그로 경보 — `scripts/run_regime_direction_scorecard.py`. 사람이 매번 dev_memory 대조해야 했던 구조가 오늘 결정 지연의 원인.
- [ ] **G-4** 모델 번들 경과 거래일수 표기를 세션마다 +1 하는 방식에서 날짜(`configs/krx_holidays.yaml`) 기반 계산으로 고정 — 장전 "33일째"→장중 "34일째" 표기가 세션 카운트였음을 오늘 확인. F-107과 병행 권고.

### 🔁 이월 재확인 (신규 체크박스 없음 — 09-22 장후 재확인, 기존 항목 상태·관측치만 갱신)
- [ ] **오늘 저녁 전까지 `index.lock` 삭제 확인** — 16:01 재확인 결과 아직 그대로 남아 있음(생성 08:48:15, 경과 7.2~7.3시간, 장마감 이후 진입). 오늘 저녁 dev_memory·리포트 커밋 시도가 막힐 위험 직접.
- [ ] **F-107** `pipeline.py` 1,014줄 분할 기준 — 09-22 재측정도 동일. 열 회차 연속 이월.
- [ ] **F-109·F-110·G-8** `regime_direction` 09-11 결측 진단 — 09-22 장후 배치 재계산도 `unresolvable=15` 동일. 자연 해소 10월 중순 예상.
- [ ] **G-11** `order-path-live` 경보 완화 택일(ⓐ/ⓑ) — 사람 결정 대기 변경 없음. 오늘은 신호 없는 날이 아니라 실제 주문이 나간 날이라 이 경보 자체 미트리거.
- [ ] **G-12** `OptionChainPollRetried` 경보 문턱 신설 — 오늘 최종 17건(09-17 미해당·09-18=16·09-21=16), 4번째 관측치 확정. 사람 결정 대기, 변경 없음.
- [ ] **champion_sample 시험 해소 확인** — 오늘 체결 2건 발생(첫 실거래)으로 해소 조건(09-21 dev_memory 예상) 충족 가능성 높으나, 이 점검 세션의 원격 실행 환경에 프로젝트 `.venv`(Windows 전용)가 없어 `pytest tests/models/test_champion_sample.py` 직접 실행 불가. **다음 로컬(Claude Code, Windows PC) 세션에서 재확인 필요.**

### ✅ 확인 완료 (장후, 해소·라이브검증 완료로 종결)
- [x] **F-111·F-114 라이브 검증 완료** — 14:30 LONG 진입(`OrderSubmit`→`FillMatched`)·14:30~15:00 `OptionsViewPublished` 3건·15:25 EOD 강제청산(`OrderSubmit`→`FillMatched`)·15:34:59 `PositionReconciled`(포지션 0·체결 2건·실현손익 +117.0틱) 전 경로 실물 관통 확인. 신설 이래 첫 실거래.
- [x] **1m 발행 유예 헤드룸 15:35 확정치** — 최악 1m -3,000ms(전일 대비 +2,518ms 개선), 격상기준(-8,000ms) 미달. P1 격상 불필요로 종결.
- [x] `OptionChainStaleSpot`·`g2_daily` 25분 공백 — 장중 절에서 이미 해소 확인, 재확인 불필요.

### 🔍 다음 거래일(09-23) 관측 예정
- [ ] `index.lock` 해소 및 오늘 저녁 커밋 성공 여부 — 최우선 확인 대상.
- [ ] F-117/F-118 사용자 결정 여부.
- [ ] HIGH_VOL 국면 재진입 시 게이트 통과 여부(F-117 적용 전/후로 다름).
- [ ] `test_champion_sample` — 다음 로컬 세션에서 pytest 재확인.

## [MW0601] 2026-09-22 저녁 — 청산 전략 조사 + F-119/F-120/F-121 반입 (사용자 지시)

사용자 지시로 청산 전략 전수 조사 → 구현계획 수립 → 반입까지 완주. 상세는 `DECISION_LOG.md`
동일 날짜 항목.

### 반입 완료

- [x] **F-119** 선물 장중 청산 엔진 — `src/messiah/strategy/position_exit.py` 신설(판정 순수
      함수 + `ExitStateTracker`) + `pipeline.handle_bar()` 결선. 손절 1.0×ATR(M1,14) ·
      시간배리어 구동 Horizon × 3봉(`labeling.BARRIER_PARAMS`에서 읽음). 태그 4종 신설.
      **사용자 결정 2건**: 즉시 실청산(R18 섀도 대신) · 손절폭 1.0×ATR(사이저 전제와 동축).
- [x] **F-120** R10(연속손실 3회) 결선 — `position_math.closes_position()` 신설 →
      `PositionReconciler.drain_closed_trades()` → `pipeline._feed_consecutive_loss_streak()`.
      2026-07-27부터 호출자 0건이던 `record_trade_result()`가 이제 실제로 불린다.
- [x] **F-121** `configs/holding_policy.yaml` 신설 + `core/config.load_holding_policy()` —
      Holding Policy Ver1.0 §5-4가 2026-07-21에 지시한 파일이 두 달 만에 생겼다.

### 라이브 검증 — 기한 2026-09-23 장후

- [ ] **W-119-1** `PositionExitArmed`가 진입 발생 시 1건 이상 찍히는가 · 손절선 수치가
      그 시점 ATR(M1,14)과 일치하는가.
- [ ] **W-119-2** 손절·시간배리어 도달 시 `PositionExitLiquidating` 발생 + 포지션 실제 감소.
      (조건 미도달이면 「미발생」으로 기록 — 실패가 아니다. 판정 자체가 돌았다는 증거는
      `PositionExitArmed`가 댄다.)
- [ ] **W-119-3 (최우선)** `EodFlattenLiquidating`과 **같은 포지션에 대해 동시 발생하지
      않는가**. 겹치면 없던 방향의 포지션이 새로 생긴다 — 구현 중 발견해 막은 경로다
      (DECISION_LOG Why 5). 15:25~15:35 구간 두 태그의 symbol·시각 대조.
- [ ] **W-119-4** `PositionExitFailed` 0건.
- [ ] **W-119-5** 청산 발생일에 `order_path_rolling`의 「청산 확인 n/제출일」 칸이 올라가는가.
- [ ] **W-120-1** 청산 체결이 난 날 R10 스트릭이 실제로 갱신되는가(손실 청산 시 +1,
      이익 청산 시 0으로 리셋). `logs/g2_daily_*.log`에 R10 사유의 `RiskReject`가 나오면
      그것도 정상 동작이다 — 3회 연속 손실 후 신규 진입 차단이 R10의 일이다.

### 사람 결정 대기 (신규)

- [ ] **익절(상단 배리어) 도입 여부** — `configs/holding_policy.yaml`의
      `take_profit_atr_mult: null`을 채우면 켜진다. 손절과 달리 **이기고 있는 포지션을
      자르는 새 정책**이다. 09-22 실거래 +117틱(ATR의 약 2.4배)이 1.0×였다면 +49틱에서
      끝났을 것이다. **판단 근거가 쌓이는 곳**: W-119 검증이 몇 거래일 돈 뒤의 청산 사유
      분포(손절 n건 / 시간배리어 n건 / EOD n건)와 각 사유별 평균 손익.
- [ ] **재기동 시 시간배리어 시계 초기화** — 진입 시각을 브로커가 답해 주지 않아 재기동
      직후 시계가 다시 시작한다(손절은 영향 없음, 늦어진 시간배리어는 EOD가 받는다).
      없애려면 진입 시각 영속화가 필요한데 R12(무상태)와 맞바꾸는 결정이다.
- [ ] **옵션 Lifecycle Manager 배선** — `strategy/options/lifecycle.py`는 설계 4조건이
      유일하게 코드로 존재하는 곳이나 호출자가 테스트뿐이다. **옵션 주문 실행 경로 자체가
      없어** 지금은 배선할 대상이 없다(Options AI는 후보 산출까지). 그 경로가 생기는 날의 몫.

### 신규 결함 (조사 중 발견, 제 변경과 무관)

- [ ] **F-122 (P2)** 시간이 지나면 저절로 깨지는 테스트 — `tests/test_rollover_day.py` 2건 ·
      `tests/models/test_champion_sample.py` 1건이 현재 실패 중. 원인은
      `core/symbol_resolution.recorded()`가 실제 `logs/`를 읽는데 2026-09-10·09-11에 라이브가
      돌아 런타임 기록이 **생겨 버려서** 기대 출처 문구("만기 규칙 계산")가 "런타임 기록"으로
      바뀐 것이다(실측: `sr.recorded(date(2026,9,10))` → `A05609`). 심볼 자체는 맞고 출처
      문구만 다르다. 처방: 테스트가 `log_dir`를 tmp로 주입받게 해 실 `logs/`와 격리.
      **F-119/F-120/F-121 반입 전에도 같은 3건이 같은 사유로 실패하고 있었다.**
- [ ] **F-123 (P2, 신규)** 이 PC의 `ruff`가 저장소 고정본보다 11개 마이너 앞서 있다 —
      설치본 `0.15.22` vs `.pre-commit-config.yaml:23` `rev: v0.4.10`. 셸에서 `ruff format`을
      직접 돌리면 훅과 서로 다른 결과를 내며 싸운다(2026-09-22 실측: 무관한 파일 15개가
      바뀌었고 그중 13개를 0.4.10 훅이 HEAD로 되돌렸다). **포맷의 정본은 훅이 고정한
      버전이다** — 수동 실행은 `pre-commit run ruff-format --files <파일>`을 쓴다.
      결정 필요: 고정본을 승격할 것인가. 승격하면 저장소 전체가 한 번 재포맷되므로 반드시
      단독 커밋이어야 한다(다른 변경과 섞이면 그 뒤로 어떤 diff도 못 읽는다).

## [MW0601] 2026-09-23 장전 점검

### 🔍 오늘(09-23) 장후 확인 예정 (신규 등재 없음 — 09-22 저녁 반입분 그대로)

- [ ] W-119-1~5 · W-120-1 라이브 검증 (위 09-22 항목과 동일, 기한 오늘 장후).

### 🆕 이번 회차가 새로 올린 것

- [ ] **G-1 (P2, 신규)** 자가점검 `boot_recovery` 줄이 `Messiah`·`Messiah-G2` 2개 트리거만 세고 `Messiah-Shutdown`·`Messiah-Postmarket`은 집계에서 빠져 있다 — `ops/host_health.py`에 `all_tasks()` 신설해 4종 전부 대조, 비수집 계열은 finding만 남기고 PASS/FAIL 판정은 안 뒤집는다(R18). 기존 관측 공백(7734행)의 구체 해법.

### ✅ 확인 완료 (장전, 결함 아님으로 종결)

- [x] `index.lock` 해소 여부 — 08:47 점검 시점 `.git/index.lock` 없음, 09-22 18:40 커밋 정상 반영 확인됨. 최우선 확인 대상 항목 종결.

## [MW0601] 2026-09-23 장중 점검

### 🔍 장후 최우선 재확인

- [ ] W-119-1~5 · W-120-1 라이브 검증 — 12:30 기준 거래 0건으로 검증 대상(포지션) 자체가 아직 없음. 장후에 거래가 났는지부터 확인.

### 🆕 이번 회차가 새로 올린 것

- [ ] **G-2 (P2, 신규)** 라이브 검증 기한이 있는 NEXT_TODO 항목에 `requires_trade` 표식을 달고, 장후 배치가 당일 체결 0건이면 해당 항목을 "검증 불가(거래 없음)·기한 자동 익일 이월"로 자동 표시 — `daily_integrity_report.py` 계열에 표시 로직만 추가, 판정 로직은 불변(R18).

### 🔁 이월 재확인 (신규 체크박스 없음 — 장전 항목 상태·관측치만 갱신)

- [ ] bundle 미승격(K-6) — 변경 없음.
- [ ] F-122 시계열 테스트 2종 — 원격 세션이라 재확인 불가, 변경 없음.
- [ ] boot_recovery 4종 집계 공백 — 변경 없음, G-1로 추적 중.

## [MW0601] 2026-09-23 장후 점검

### ✅ 확인 완료 (장후, W-119 라이브 검증 종결)

- [x] **W-119-1** `PositionExitArmed` 발생·손절선 수치 검증 — 15:01:00 발생 확인, entry_price_ticks=55828·stop_ticks=41.79·time_barrier=90분.
- [x] **W-119-2** 손절·시간배리어 도달 시 청산 — 오늘 미도달, "미발생" 판정으로 종결(설계상 실패 아님).
- [x] **W-119-3 (최우선)** 두 청산 경로 동시발생 금지 — `PositionExitLiquidating` 자체가 미발생이라 형식적 해소. **단 실제 충돌 시나리오는 아직 실전 시험 안 됨 — 다음 손절/시간배리어 실발동일에 재확인 필수.**
- [x] **W-119-4** `PositionExitFailed` 0건 — 확인.
- [x] **W-119-5** `order_path_rolling` 청산확인 칸 갱신 — `exit_dates`에 2026-09-23 포함 확인.

### 🔍 판정 불가로 지속 (신규 로깅 없이는 안 닫힘)

- [ ] **W-120-1** R10 연속손실 스트릭 실갱신 여부 — `record_trade_result()`가 로깅을 안 해 확인 불가. F-4 적용 전까지 매 점검 회차 "판정 불가"로 반복될 것으로 예상(미리 알림).

### 🆕 이번 회차가 새로 올린 것

- [ ] **F-3 (P2, 신규)** `KillSwitch.liquidate()`(`src/messiah/risk/kill_switch.py:114-139`)에 `source` 파라미터 추가해 마감정리(`pipeline.py:1046`) 재사용 호출과 실제 킬스위치 발동을 로그에서 구분. 오늘 15:25:00 `KillSwitchLiquidating`·`EodFlattenLiquidating` 동시 발생이 실제로는 진짜 킬스위치 트리거(`KillSwitch` 태그 0건) 없이 일어난 것으로 확인됨 — 로그만 보면 오독하기 쉽다.
- [ ] **G-3 (P2, 신규, F-3 종속)** Command Center UI 이벤트 피드가 `source` 필드로 긴급정지/마감정리를 구분 표시.
- [ ] **F-4 (P2, 신규)** `RiskEngine.record_trade_result()`(`src/messiah/risk/risk_engine.py:137-143`)에 스트릭 갱신 로깅 추가. 오늘 이긴 거래로 스트릭이 리셋됐을 것으로 추정되나 로그로 영구히 확인 불가능한 상태.
- [ ] **G-4 (P2, 신규)** `status_snapshot.json`에 `risk_engine.consecutive_losses` 필드 노출(F-4 선행 불필요, 독립 착수 가능).

### 🔁 이월 재확인 (신규 체크박스 없음 — 상태만 갱신)

- [ ] bundle 미승격(K-6) — 변경 없음, `self_eval_2026-09-23.json` 기준 미달 관문 3종(max_drawdown·negative_window_ratio·sharpe) 동일.
- [ ] F-122 시계열 테스트 2종 — 원격 세션이라 재확인 불가, 변경 없음.
- [ ] boot_recovery 4종 집계 공백 — 변경 없음, G-1로 추적 중.

## [MW0601] 2026-09-23 장후 자동조치

### ✅ 완료
- [x] **G-1** boot_recovery 줄에 정본 at_boot=false 비대상 명시(판정 불변) — `bd3db13`. 위 장전·장중·장후의 "boot_recovery 4종 집계 공백" 이월 항목도 이것으로 종결(공백이 아니라 설계상 비대상이었음).

### ⏸ C등급 — 사람 결정 대기 (자동조치가 구현 안 함)
- [ ] **F-4** — "구현해" 한마디면 다음 회차 자동 구현 가능(로깅만, B등급 성격). 태그는 `core/logging.py` 등록 필요(R6).
- [ ] **F-3** — 킬스위치 청산 경로라 사람 결정 필수.
- [ ] **F-1 / F-2** — 위 장전 항목 그대로.
- [ ] **G-2** — 점검 리포트가 대상 파일을 `src/messiah/ops/integrity_report.py`의 특정 함수로 다시 적으면 A등급으로 자동 구현 가능.
- [ ] **G-4** — 전제 정정: 스트릭(G2 프로세스)과 스냅샷(L1 프로세스)이 다른 프로세스다. 버스 메시지 신설 설계가 먼저.

### 🔍 다음 점검이 볼 것
- [ ] `test_champion_sample::test_the_live_file_still_counts_across_the_roll` — 로컬 확정 실패 `rows_counted 0 >= 4-1`. return_basis 표식 행 4개(09-18~09-23 A05610)가 있는데 집계 표본이 0인 이유 조사(원인 조사만, 부등식 수정은 C).
- [ ] 리팩터 후보: `src/messiah/ops/host_health.py` 682줄 — R5 500줄 상한 초과(이번 G-1로 +30).
