...
- **현 단계**: Phase 3 (Ver 2.0 §9 로드맵 W22~23 완료) — 15m·30m Expert(VL 확장+
  FeatureEngine deque 버그 수정+파이프라인 검증) → Aggregator·Meta Decision·Risk
  Engine·Sizer·Kill Switch(W24~26, 전 경로 관통) 순. FL/OP/RG Feature 카테고리는
  데이터 파이프라인 부재로 여전히 갭(Docs/capability_matrix.md)
...
2. **모든 프로세스 간 통신은 Redis Bus로만** — 직접 함수 호출·파일 공유 금지. 스키마는 `core/messages.py` 단일 정의
   - **명문화된 예외 (2026-08-20 G-4)**: 동일 프로세스 안에서 **구독이 성립하기 전** 1회성으로 전달해야 하는
     기동 시드는 직접 호출을 허용한다. 조건 셋을 **모두** 만족해야 한다 — ① 구독 성립 이전 ② 그 세션 1회뿐
     ③ 버스 발행을 **병행**하고 어느 경로로 닿았는지 태그에 남긴다(`RegimeSeeded.delivery`).
     근거: 버스는 구독 전 발행을 버리므로(`core/bus.py` pub/sub 의미론), 매 세션 첫 판단이 국면 없이 내려갔다
     (2026-08-19 F-5 / commit `f8e0fde`). 예외가 **함수 주석에만** 있으면 다음 사람이 되돌리거나 반대로
     이것을 근거 삼아 다른 직접 호출을 늘린다 — 그래서 여기 적는다.
3. **완성봉 규율**: Feature 발행·전문가 판단은 해당 Horizon 완성봉 확정 시점에만. 예외는 안전장치(틱 단위)뿐
     **늦은 틱 유예** — 봉을 닫기 전에 얼마나 기다리는가. 실제 운영값은 실측으로 정한다:
     1분봉 **2.0초**(`data/normalizer.MINUTE_CLOSE_GRACE_SECONDS`, 2026-08-11 G-4 — 회선 실측
     3거래일 최대 1.396초 위로 43% 여유) · 합성봉 구성분 대기 **5.0초**
     (`data/bar_composer._MAX_CONSTITUENT_WAIT_SECONDS`).
     **발행 예산** — 봉을 받고 나서 내보내기까지의 상한. `features/engine._PUBLISH_SLA_MS`
     (현재 1,000ms). 발행 오프셋(봉 확정 → 발행)이 아니라 **대기를 걷어낸 값**에 건다.
     2026-08-24까지 이 자리에 「유예 500ms」 하나가 적혀 있었고, 그 숫자가
     **합성 스케줄러 위상이자 마감 시한**이었다 — 합성기가 경계+0.5초에 발사하므로
     3m~30m의 오프셋은 정의상 `500ms + 계산시간`이고, 2거래일 598건 중 500ms 미만이
     **0건**(최소 532ms)이었다. 값이 자기 자신을 더한 수와 비교되고 있었고, 그것이
     6거래일 연속 경고의 정체다(2026-08-24 F-21 · 사용자 조치 4).
     **세 값은 서로 다른 질문에 답한다** — 섞으면 어느 것도 못 지킨다.
